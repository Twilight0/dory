...
/* dory_undo_set_up_dory_entry_for_undo
 *
...
static void
dory_entry_user_changed_callback (DoryEntry *entry)
{
...
void
dory_undo_set_up_dory_entry_for_undo (DoryEntry *entry)
{
	EditableUndoObjectData *data;

	if (!DORY_IS_ENTRY (entry) ) {
		return;
	}
	...
	g_signal_connect (entry, "user_changed",
			  G_CALLBACK (dory_entry_user_changed_callback),
			  NULL);
}

void
dory_undo_tear_down_dory_entry_for_undo (DoryEntry *entry)
{
	if (!DORY_IS_ENTRY (entry) ) {
		return;
	}
	...
	g_signal_handlers_disconnect_by_func
		(entry, G_CALLBACK (dory_entry_user_changed_callback), NULL);
...
void
dory_undo_set_up_editable_for_undo (GtkEditable *editable)
{
	EditableUndoObjectData *data;
...
void
dory_undo_tear_down_editable_for_undo (GtkEditable *editable)
{
	if (!GTK_IS_EDITABLE (editable) ) {
		return;
	}
