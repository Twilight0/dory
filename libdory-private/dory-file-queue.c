...
#include <config.h>
#include "dory-file-queue.h"

#include <glib.h>

struct DoryFileQueue {
	GList *head;
	GList *tail;
...
DoryFileQueue *
dory_file_queue_new (void)
{
	DoryFileQueue *queue;
	
	queue = g_new0 (DoryFileQueue, 1);
...
void
dory_file_queue_destroy (DoryFileQueue *queue)
{
	g_hash_table_destroy (queue->item_to_link_map);
	dory_file_list_free (queue->head);
	g_free (queue);
}

void
dory_file_queue_enqueue (DoryFileQueue *queue,
			     DoryFile      *file)
{
...
DoryFile *
dory_file_queue_dequeue (DoryFileQueue *queue)
{
	DoryFile *file;

	file = dory_file_queue_head (queue);
	dory_file_queue_remove (queue, file);
...
void
dory_file_queue_remove (DoryFileQueue *queue,
			    DoryFile *file)
{
	GList *link;
...
DoryFile *
dory_file_queue_head (DoryFileQueue *queue)
{
	if (queue->head == NULL) {
		return NULL;
	}

	return DORY_FILE (queue->head->data);
}

gboolean
dory_file_queue_is_empty (DoryFileQueue *queue)
{
