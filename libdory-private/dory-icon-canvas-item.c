...
#include <config.h>
#include <math.h>
#include "dory-icon-canvas-item.h"
...
/* Private part of the DoryIconCanvasItem structure. */
struct DoryIconCanvasItemDetails {
	/* The image, text, font. */
	double x, y;
	GdkPixbuf *pixbuf;
...
static void dory_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface);
static GType dory_icon_canvas_item_accessible_factory_get_type (void);

G_DEFINE_TYPE_WITH_CODE (DoryIconCanvasItem, dory_icon_canvas_item, EEL_TYPE_CANVAS_ITEM,
			 G_IMPLEMENT_INTERFACE (EEL_TYPE_ACCESSIBLE_TEXT,
						dory_icon_canvas_item_text_interface_init));

/* private */
static void     draw_label_text                      (DoryIconCanvasItem        *item,
						      cairo_t                       *cr,
						      EelIRect                       icon_rect);
static void     measure_label_text                   (DoryIconCanvasItem        *item);
...
static PangoLayout *get_label_layout                 (PangoLayout                  **layout,
						      ...
						      const char                    *text);
static gboolean hit_test_stretch_handle              (DoryIconCanvasItem        *item,
						      EelIRect                       canvas_rect,
						      GtkCornerType *corner);

static void       dory_icon_canvas_item_ensure_bounds_up_to_date (DoryIconCanvasItem *icon_item);

/* Object initialization function for the icon item. */
static void
dory_icon_canvas_item_init (DoryIconCanvasItem *icon_item)
{
	icon_item->details = G_TYPE_INSTANCE_GET_PRIVATE ((icon_item), DORY_TYPE_ICON_CANVAS_ITEM, DoryIconCanvasItemDetails);
	dory_icon_canvas_item_invalidate_label_size (icon_item);
}

static void
dory_icon_canvas_item_finalize (GObject *object)
{
	DoryIconCanvasItemDetails *details;

	g_assert (DORY_IS_ICON_CANVAS_ITEM (object));

	details = DORY_ICON_CANVAS_ITEM (object)->details;
	...
	G_OBJECT_CLASS (dory_icon_canvas_item_parent_class)->finalize (object);
}
...
static void
dory_icon_canvas_item_invalidate_bounds_cache (DoryIconCanvasItem *item)
{
...
void
dory_icon_canvas_item_invalidate_label_size (DoryIconCanvasItem *item)
{
	...
	dory_icon_canvas_item_invalidate_bounds_cache (item);
...
static void
dory_icon_canvas_item_set_property (GObject        *object,
					guint           property_id,
					const GValue   *value,
					GParamSpec     *pspec)
{
	...
	item = DORY_ICON_CANVAS_ITEM (object);
	...
	case PROP_EDITABLE_TEXT:
		...
		dory_icon_canvas_item_invalidate_label_size (item);
    ...
    case PROP_PINNED:
        ...
        dory_icon_canvas_item_invalidate_label (item);
	...
	default:
		g_warning ("dory_icons_view_item_item_set_arg on unknown argument");
		return;
	}
...
static void
dory_icon_canvas_item_get_property (GObject        *object,
					guint           property_id,
					GValue         *value,
					GParamSpec     *pspec)
{
...
static void
get_scaled_icon_size (DoryIconCanvasItem *item,
             gint *width,
             gint *height)
{
   EelCanvas *canvas;
...
cairo_surface_t *
dory_icon_canvas_item_get_drag_surface (DoryIconCanvasItem *item)
{
	...
	g_return_val_if_fail (DORY_IS_ICON_CANVAS_ITEM (item), NULL);
	...
	gtk_style_context_save (context);
        ...
        gtk_style_context_add_class (context, "dory-canvas-item");
...
void
dory_icon_canvas_item_set_image (DoryIconCanvasItem *item,
				     GdkPixbuf *image)
{
	DoryIconCanvasItemDetails *details;

	g_return_if_fail (DORY_IS_ICON_CANVAS_ITEM (item));
 ...
 */
static void
recompute_bounding_box (DoryIconCanvasItem *icon_item,
...
static EelIRect
compute_text_rectangle (const DoryIconCanvasItem *item,
			...
			DoryIconCanvasItemBoundsUsage usage)
{
	...
	if (DORY_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)->details->label_position == DORY_ICON_LABEL_POSITION_BESIDE) {
		if (!dory_icon_container_is_layout_rtl (DORY_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas))) {
...
void
dory_icon_canvas_item_update_bounds (DoryIconCanvasItem *item,
					 double i2w_dx, double i2w_dy)
{
	...
	dory_icon_canvas_item_get_icon_canvas_rectangle (item, &item->details->canvas_rect);
...
static void
dory_icon_canvas_item_update (EelCanvasItem *item,
				  double i2w_dx, double i2w_dy,
				  gint flags)
{
	dory_icon_canvas_item_update_bounds (DORY_ICON_CANVAS_ITEM (item), i2w_dx, i2w_dy);

	eel_canvas_item_request_redraw (EEL_CANVAS_ITEM (item));

	EEL_CANVAS_ITEM_CLASS (dory_icon_canvas_item_parent_class)->update (item, i2w_dx, i2w_dy, flags);
}
...
in_single_click_mode (void)
{
	...
	return click_policy == DORY_CLICK_POLICY_SINGLE;
}
...
#define IS_COMPACT_VIEW(container) \
        ((container->details->layout_mode == DORY_ICON_LAYOUT_T_B_L_R || \
...
static void
prepare_pango_layout_width (DoryIconCanvasItem *item,
			    PangoLayout *layout)
{
	if (dory_icon_canvas_item_get_max_text_width (item) < 0) {
		pango_layout_set_width (layout, -1);
	} else {
		pango_layout_set_width (layout, floor (dory_icon_canvas_item_get_max_text_width (item)) * PANGO_SCALE);
		pango_layout_set_ellipsize (layout, PANGO_ELLIPSIZE_END);
	}
}

static void
prepare_pango_layout_for_measure_entire_text (DoryIconCanvasItem *item,
					      PangoLayout *layout)
{
	...
	container = DORY_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
...
static void
prepare_pango_layout_for_draw (DoryIconCanvasItem *item,
			       PangoLayout *layout)
{
	DoryIconCanvasItemDetails *details;
	DoryIconContainer *container;
	...
	if (IS_COMPACT_VIEW (container)) {
		...
		pango_layout_set_height (layout,
					 dory_icon_container_get_max_layout_lines_for_pango (container));
	}
}

static void
measure_label_text (DoryIconCanvasItem *item)
{
	DoryIconCanvasItemDetails *details;
	DoryIconContainer *container;
	...
	if (have_editable) {
		...
		layout_get_size_for_layout (editable_layout,
					    dory_icon_container_get_max_layout_lines (container),
					    editable_height_for_entire_text,
...
static void
draw_label_text (DoryIconCanvasItem *item,
                 cairo_t *cr,
		 EelIRect icon_rect)
{
	DoryIconCanvasItemDetails *details;
	...
	is_rtl_label_beside = dory_icon_container_is_layout_rtl (container) &&
			      container->details->label_position == DORY_ICON_LABEL_POSITION_BESIDE;
	...
	max_text_width = floor (dory_icon_canvas_item_get_max_text_width (item));
...
void
dory_icon_canvas_item_set_is_visible (DoryIconCanvasItem       *item,
					  gboolean                      visible)
{
...
void
dory_icon_canvas_item_invalidate_label (DoryIconCanvasItem     *item)
{
	dory_icon_canvas_item_invalidate_label_size (item);
...
get_knob_pixbuf (void)
{
	...
	knob_pixbuf = gtk_icon_theme_load_icon (gtk_icon_theme_get_default (),
						...
						8, 0, NULL);
	if (!knob_pixbuf) {
		GInputStream *stream = g_resources_open_stream ("/org/dory/icons/knob.png", 0, NULL);
		if (stream != NULL) {
...
static void
draw_stretch_handles (DoryIconCanvasItem *item,
                      cairo_t *cr,
		      const EelIRect *rect)
{
	GtkWidget *widget;
...
real_map_surface (DoryIconCanvasItem *icon_item)
{
	EelCanvas *canvas;
...
static cairo_surface_t *
map_surface (DoryIconCanvasItem *icon_item)
{
#if 0
...
static void
dory_icon_canvas_item_draw (EelCanvasItem *item,
                                cairo_t *cr,
                                cairo_region_t *region)
{
	DoryIconContainer *container;
	DoryIconCanvasItem *icon_item;
	DoryIconCanvasItemDetails *details;
	EelIRect icon_rect;
    cairo_surface_t *temp_surface;
	GtkStyleContext *context;

	container = DORY_ICON_CONTAINER (item->canvas);
	icon_item = DORY_ICON_CANVAS_ITEM (item);
                            ...
                            const char *filter_text)
{
     ...
     * no remapping needed. */
    match_attrs = dory_fzy_match_attrs (filter_text, zeroified_text);
    if (match_attrs == NULL) {
        return;
    }
...
static PangoLayout *
create_label_layout (DoryIconCanvasItem *item,
		     const char *text)
{
	PangoLayout *layout;
	...
	container = DORY_ICON_CONTAINER (canvas_item->canvas);
	...
	add_filter_highlight_attrs (attr_list, zeroified_text,
	                            dory_icon_container_get_filter_highlight (container));
...
static int
dory_icon_canvas_item_event (EelCanvasItem *item, GdkEvent *event)
{
	DoryIconCanvasItem *icon_item;
	GdkCursor *cursor;
	GdkWindow *cursor_window;
    ...
    if (event->type == GDK_ENTER_NOTIFY) {
        dory_icon_container_update_tooltip_text (DORY_ICON_CONTAINER (item->canvas), icon_item);
		if (!icon_item->details->is_prelit) {
			icon_item->details->is_prelit = TRUE;
			dory_icon_canvas_item_invalidate_label_size (icon_item);
			eel_canvas_item_request_update (item);
			eel_canvas_item_send_behind (item,
						     DORY_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle);
        ...
        dory_icon_container_update_tooltip_text (DORY_ICON_CONTAINER (item->canvas), NULL);
		if (icon_item->details->is_prelit
...
static gboolean
hit_test (DoryIconCanvasItem *icon_item, EelIRect canvas_rect)
{
	DoryIconCanvasItemDetails *details;
...
static double
dory_icon_canvas_item_point (EelCanvasItem *item, double x, double y, int cx, int cy,
				 EelCanvasItem **actual_item)
{
	...
	*actual_item = item;
	...
	canvas_rect.x1 = cx + 1;
	canvas_rect.y1 = cy + 1;
	if (hit_test (DORY_ICON_CANVAS_ITEM (item), canvas_rect)) {
		return 0.0;
	} else {
...
static void
dory_icon_canvas_item_translate (EelCanvasItem *item, double dx, double dy)
{
	DoryIconCanvasItem *icon_item;
	DoryIconCanvasItemDetails *details;
...
void
dory_icon_canvas_item_get_bounds_for_layout (DoryIconCanvasItem *icon_item,
						 double *x1, double *y1, double *x2, double *y2)
{
	...
	dory_icon_canvas_item_ensure_bounds_up_to_date (icon_item);
...
void
dory_icon_canvas_item_get_bounds_for_entire_item (DoryIconCanvasItem *icon_item,
						      double *x1, double *y1, double *x2, double *y2)
{
	DoryIconCanvasItemDetails *details;
	EelIRect *total_rect;
...
static void
dory_icon_canvas_item_bounds (EelCanvasItem *item,
				  double *x1, double *y1, double *x2, double *y2)
{
	DoryIconCanvasItem *icon_item;
	DoryIconCanvasItemDetails *details;
...
static void
dory_icon_canvas_item_ensure_bounds_up_to_date (DoryIconCanvasItem *icon_item)
{
	DoryIconCanvasItemDetails *details;
...
EelDRect
dory_icon_canvas_item_get_icon_rectangle (const DoryIconCanvasItem *item)
{
	EelDRect rectangle;
	double pixels_per_unit;
    gint width, height;

	g_return_val_if_fail (DORY_IS_ICON_CANVAS_ITEM (item), eel_drect_empty);

	rectangle.x0 = item->details->x;
	rectangle.y0 = item->details->y;

	pixels_per_unit = EEL_CANVAS_ITEM (item)->canvas->pixels_per_unit;
    get_scaled_icon_size (DORY_ICON_CANVAS_ITEM (item), &width, &height);
...
EelDRect
dory_icon_canvas_item_get_text_rectangle (DoryIconCanvasItem *item,
					      gboolean for_layout)
{
	/* FIXME */
	EelIRect icon_rectangle;
...
void
dory_icon_canvas_item_get_icon_canvas_rectangle (DoryIconCanvasItem *item,
                                                 EelIRect *rect)
{
    gint width, height;

	g_assert (DORY_IS_ICON_CANVAS_ITEM (item));
	g_assert (rect != NULL);
...
void
dory_icon_canvas_item_set_show_stretch_handles (DoryIconCanvasItem *item,
...
static gboolean
hit_test_stretch_handle (DoryIconCanvasItem *item,
...
gboolean
dory_icon_canvas_item_hit_test_stretch_handles (DoryIconCanvasItem *item,
						    gdouble world_x,
						    gdouble world_y,
						    GtkCornerType *corner)
{
	...
	g_return_val_if_fail (DORY_IS_ICON_CANVAS_ITEM (item), FALSE);
...
/* dory_icon_canvas_item_hit_test_rectangle
 *
 * Check and see if there is an intersection between the item and the
 * canvas rect.
 */
gboolean
dory_icon_canvas_item_hit_test_rectangle (DoryIconCanvasItem *item, EelIRect canvas_rect)
{
	g_return_val_if_fail (DORY_IS_ICON_CANVAS_ITEM (item), FALSE);
...
const char *
dory_icon_canvas_item_get_editable_text (DoryIconCanvasItem *icon_item)
{
	g_return_val_if_fail (DORY_IS_ICON_CANVAS_ITEM (icon_item), NULL);

	return icon_item->details->editable_text;
}

void
dory_icon_canvas_item_set_renaming (DoryIconCanvasItem *item, gboolean state)
{
	g_return_if_fail (DORY_IS_ICON_CANVAS_ITEM (item));
...
double
dory_icon_canvas_item_get_max_text_width (DoryIconCanvasItem *item)
{
    EelCanvasItem *canvas_item;
    DoryIconContainer *container;
    ...
    if (container->details->label_position == DORY_ICON_LABEL_POSITION_BESIDE) {
        if (container->details->layout_mode == DORY_ICON_LAYOUT_T_B_L_R ||
            container->details->layout_mode == DORY_ICON_LAYOUT_T_B_R_L) {
            /* compact view */
        ...
        if (container->details->is_desktop) {
            return dory_get_desktop_text_width_for_zoom_level (dory_icon_container_get_zoom_level (container));
        } else {
            return dory_get_icon_text_width_for_zoom_level (dory_icon_container_get_zoom_level (container));
        }
    }
}

void
dory_icon_canvas_item_set_entire_text (DoryIconCanvasItem       *item,
...
gint
dory_icon_canvas_item_get_fixed_text_height_for_layout (DoryIconCanvasItem *item)
{
    DoryIconContainer *container;
    PangoLayout *layout;
    ...
    if (dory_icon_container_get_zoom_level (container) == DORY_ZOOM_LEVEL_SMALLEST) {
        // No label/info, just a bit of padding.
        return 0;
    }

    lines = dory_icon_container_get_max_layout_lines (container);
    lines += dory_icon_container_get_additional_text_line_count (container);
...
static void
dory_icon_canvas_item_class_init (DoryIconCanvasItemClass *class)
{
	...
	object_class->finalize = dory_icon_canvas_item_finalize;
	object_class->set_property = dory_icon_canvas_item_set_property;
	object_class->get_property = dory_icon_canvas_item_get_property;
	...
	item_class->update = dory_icon_canvas_item_update;
	item_class->draw = dory_icon_canvas_item_draw;
	item_class->point = dory_icon_canvas_item_point;
	item_class->translate = dory_icon_canvas_item_translate;
	item_class->bounds = dory_icon_canvas_item_bounds;
	item_class->event = dory_icon_canvas_item_event;

	atk_registry_set_factory_type (atk_get_default_registry (),
				       DORY_TYPE_ICON_CANVAS_ITEM,
				       dory_icon_canvas_item_accessible_factory_get_type ());

	g_type_class_add_private (class, sizeof (DoryIconCanvasItemDetails));
}

static GailTextUtil *
dory_icon_canvas_item_get_text (GObject *text)
{
	return DORY_ICON_CANVAS_ITEM (text)->details->text_util;
}

static void
dory_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface)
{
	iface->get_text = dory_icon_canvas_item_get_text;
}

/* ============================= a11y interfaces =========================== */

static const char *dory_icon_canvas_item_accessible_action_names[] = {
        "open",
        "menu",
        NULL
};

static const char *dory_icon_canvas_item_accessible_action_descriptions[] = {
        "Open item",
        "Popup context menu",
        NULL
};
...
typedef struct {
	...
	char *description;
} DoryIconCanvasItemAccessiblePrivate;

typedef struct {
	DoryIconCanvasItem *item;
	gint action_number;
} DoryIconCanvasItemAccessibleActionContext;

static GType dory_icon_canvas_item_accessible_get_type (void);

#define GET_PRIV(o) \
	G_TYPE_INSTANCE_GET_PRIVATE(o,\
				    dory_icon_canvas_item_accessible_get_type (),\
				    DoryIconCanvasItemAccessiblePrivate);

/* accessible AtkAction interface */
static gboolean
dory_icon_canvas_item_accessible_idle_do_action (gpointer data)
{
	DoryIconCanvasItem *item;
	DoryIconCanvasItemAccessibleActionContext *ctx;
	DoryIcon *icon;
	DoryIconContainer *container;
	GList* selection;
	GList file_list;
...
static gboolean
dory_icon_canvas_item_accessible_do_action (AtkAction *accessible,
						int i)
{
	DoryIconCanvasItem *item;
	DoryIconCanvasItemAccessibleActionContext *ctx;
	...
	item = DORY_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (accessible)));
	if (!item) {
		return FALSE;
	}
	...
	case ACTION_MENU:
		...
		ctx = g_new (DoryIconCanvasItemAccessibleActionContext, 1);
		ctx->action_number = i;
		ctx->item = item;
		g_queue_push_head (container->details->a11y_item_action_queue, ctx);
		if (container->details->a11y_item_action_idle_handler == 0) {
			container->details->a11y_item_action_idle_handler = g_idle_add (dory_icon_canvas_item_accessible_idle_do_action, container);
		}
		break;
        default :
                g_warning ("Invalid action passed to DoryIconCanvasItemAccessible::do_action");
                return FALSE;
        }
...
static int
dory_icon_canvas_item_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
dory_icon_canvas_item_accessible_action_get_description (AtkAction *accessible,
                                                             int i)
{
	DoryIconCanvasItemAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return dory_icon_canvas_item_accessible_action_descriptions[i];
	}
}

static const char *
dory_icon_canvas_item_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return dory_icon_canvas_item_accessible_action_names[i];
}

static const char *
dory_icon_canvas_item_accessible_action_get_keybinding (AtkAction *accessible,
							    int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
dory_icon_canvas_item_accessible_action_set_description (AtkAction *accessible,
							     int i,
...
static void
dory_icon_canvas_item_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = dory_icon_canvas_item_accessible_do_action;
	iface->get_n_actions = dory_icon_canvas_item_accessible_get_n_actions;
	iface->get_description = dory_icon_canvas_item_accessible_action_get_description;
	iface->get_keybinding = dory_icon_canvas_item_accessible_action_get_keybinding;
	iface->get_name = dory_icon_canvas_item_accessible_action_get_name;
	iface->set_description = dory_icon_canvas_item_accessible_action_set_description;
}

static const gchar *
dory_icon_canvas_item_accessible_get_name (AtkObject *accessible)
{
	DoryIconCanvasItem *item;
...
static const gchar*
dory_icon_canvas_item_accessible_get_description (AtkObject *accessible)
{
	DoryIconCanvasItem *item;
...
static AtkObject *
dory_icon_canvas_item_accessible_get_parent (AtkObject *accessible)
{
	DoryIconCanvasItem *item;
...
static int
dory_icon_canvas_item_accessible_get_index_in_parent (AtkObject *accessible)
{
	DoryIconCanvasItem *item;
	DoryIconContainer *container;
	GList *l;
...
static const gchar *
dory_icon_canvas_item_accessible_get_image_description (AtkImage *image)
{
	DoryIconCanvasItemAccessiblePrivate *priv;
	DoryIconCanvasItem *item;
	DoryIcon *icon;
	...
	if (priv->image_description) {
		...
		item = DORY_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (image)));
		if (item == NULL) {
			return NULL;
		}
		icon = item->user_data;
		container = DORY_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
		description = dory_icon_container_get_icon_description (container, icon->data);
...
static void
dory_icon_canvas_item_accessible_get_image_size
	(AtkImage *image,
	 gint     *width,
	 gint     *height)
{
...
static void
dory_icon_canvas_item_accessible_get_image_position
	(AtkImage		 *image,
	 gint                    *x,
	 gint	                 *y,
...
static gboolean
dory_icon_canvas_item_accessible_set_image_description (AtkImage    *image,
...
static void
dory_icon_canvas_item_accessible_image_interface_init (AtkImageIface *iface)
{
	iface->get_image_description = dory_icon_canvas_item_accessible_get_image_description;
	iface->set_image_description = dory_icon_canvas_item_accessible_set_image_description;
	iface->get_image_size        = dory_icon_canvas_item_accessible_get_image_size;
	iface->get_image_position    = dory_icon_canvas_item_accessible_get_image_position;
}

/* accessible text interface */
static gint
dory_icon_canvas_item_accessible_get_offset_at_point (AtkText	 *text,
                                                          gint           x,
                                                          gint           y,
                                                          AtkCoordType coords)
{
	...
	item = DORY_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (text)));
...
static void
dory_icon_canvas_item_accessible_get_character_extents (AtkText	   *text,
                                                            gint	   offset,
                                                            gint	   *x,
                                                            gint	   *y,
                                                            gint	   *width,
...
static void
dory_icon_canvas_item_accessible_text_interface_init (AtkTextIface *iface)
{
	...
	iface->get_character_extents   = dory_icon_canvas_item_accessible_get_character_extents;
	iface->get_offset_at_point     = dory_icon_canvas_item_accessible_get_offset_at_point;
}

typedef struct {
	AtkGObjectAccessible parent;
} DoryIconCanvasItemAccessible;

typedef struct {
	AtkGObjectAccessibleClass parent_class;
} DoryIconCanvasItemAccessibleClass;

G_DEFINE_TYPE_WITH_CODE (DoryIconCanvasItemAccessible,
			 dory_icon_canvas_item_accessible,
			 ATK_TYPE_GOBJECT_ACCESSIBLE,
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_IMAGE,
						dory_icon_canvas_item_accessible_image_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_TEXT,
						dory_icon_canvas_item_accessible_text_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
						dory_icon_canvas_item_accessible_action_interface_init));

static AtkStateSet*
dory_icon_canvas_item_accessible_ref_state_set (AtkObject *accessible)
{
	...
	state_set = ATK_OBJECT_CLASS (dory_icon_canvas_item_accessible_parent_class)->ref_state_set (accessible);
...
static void
dory_icon_canvas_item_accessible_finalize (GObject *object)
{
	DoryIconCanvasItemAccessiblePrivate *priv;
	int i;
	...
	g_free (priv->description);
        ...
        G_OBJECT_CLASS (dory_icon_canvas_item_accessible_parent_class)->finalize (object);
}

static void
dory_icon_canvas_item_accessible_initialize (AtkObject *accessible,
						 gpointer widget)
{
	ATK_OBJECT_CLASS (dory_icon_canvas_item_accessible_parent_class)->initialize (accessible, widget);

	atk_object_set_role (accessible, ATK_ROLE_ICON);
}

static void
dory_icon_canvas_item_accessible_class_init (DoryIconCanvasItemAccessibleClass *klass)
{
	AtkObjectClass *aclass = ATK_OBJECT_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = dory_icon_canvas_item_accessible_finalize;

	aclass->initialize = dory_icon_canvas_item_accessible_initialize;

	aclass->get_name = dory_icon_canvas_item_accessible_get_name;
	aclass->get_description = dory_icon_canvas_item_accessible_get_description;
	aclass->get_parent = dory_icon_canvas_item_accessible_get_parent;
	aclass->get_index_in_parent = dory_icon_canvas_item_accessible_get_index_in_parent;
	aclass->ref_state_set = dory_icon_canvas_item_accessible_ref_state_set;

	g_type_class_add_private (klass, sizeof (DoryIconCanvasItemAccessiblePrivate));
}

static void
dory_icon_canvas_item_accessible_init (DoryIconCanvasItemAccessible *self)
{
}

/* dummy typedef */
typedef AtkObjectFactory      DoryIconCanvasItemAccessibleFactory;
typedef AtkObjectFactoryClass DoryIconCanvasItemAccessibleFactoryClass;

G_DEFINE_TYPE (DoryIconCanvasItemAccessibleFactory, dory_icon_canvas_item_accessible_factory,
	       ATK_TYPE_OBJECT_FACTORY);

static AtkObject *
dory_icon_canvas_item_accessible_factory_create_accessible (GObject *for_object)
{
	AtkObject *accessible;
	DoryIconCanvasItem *item;
	GString *item_text;

	item = DORY_ICON_CANVAS_ITEM (for_object);
	g_assert (item != NULL);
	...
	accessible = g_object_new (dory_icon_canvas_item_accessible_get_type (), NULL);
...
static GType
dory_icon_canvas_item_accessible_factory_get_accessible_type (void)
{
	return dory_icon_canvas_item_accessible_get_type ();
}

static void
dory_icon_canvas_item_accessible_factory_init (DoryIconCanvasItemAccessibleFactory *self)
{
}

static void
dory_icon_canvas_item_accessible_factory_class_init (DoryIconCanvasItemAccessibleFactoryClass *klass)
{
	klass->create_accessible = dory_icon_canvas_item_accessible_factory_create_accessible;
	klass->get_accessible_type = dory_icon_canvas_item_accessible_factory_get_accessible_type;
}
