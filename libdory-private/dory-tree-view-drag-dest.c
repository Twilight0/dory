...
/* dory-tree-view-drag-dest.c: Handles drag and drop for treeviews which
 *                                 contain a hierarchy of files
 */

#include <config.h>

#include "dory-tree-view-drag-dest.h"
...
struct _DoryTreeViewDragDestDetails {
...
G_DEFINE_TYPE (DoryTreeViewDragDest, dory_tree_view_drag_dest,
	       G_TYPE_OBJECT);
...
static void
remove_scroll_timeout (DoryTreeViewDragDest *dest)
{
...
expand_timeout (gpointer data)
{
    ...
    DoryTreeViewDragDest *dest = DORY_TREE_VIEW_DRAG_DEST (data);
...
static void
remove_expand_timeout (DoryTreeViewDragDest *dest)
{
...
static void
set_widget_highlight (DoryTreeViewDragDest *dest, gboolean highlight)
{
...
static void
set_drag_dest_row (DoryTreeViewDragDest *dest,
		   GtkTreePath *path)
{
	if (path) {
...
static void
clear_drag_dest_row (DoryTreeViewDragDest *dest)
{
...
static gboolean
get_drag_data (DoryTreeViewDragDest *dest,
	       ...
	       guint32 time)
{
	...
	if (target == gdk_atom_intern (DORY_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE) &&
	    !dest->details->drop_occurred) {
		dest->details->drag_type = DORY_ICON_DND_XDNDDIRECTSAVE;
...
static void
free_drag_data (DoryTreeViewDragDest *dest)
{
	...
	if (dest->details->drag_list) {
		dory_drag_destroy_selection_list (dest->details->drag_list);
...
static char *
get_root_uri (DoryTreeViewDragDest *dest)
{
	char *uri;
...
static DoryFile *
file_for_path (DoryTreeViewDragDest *dest, GtkTreePath *path)
{
	DoryFile *file;
	char *uri;
...
static GtkTreePath *
get_drop_path (DoryTreeViewDragDest *dest,
	       GtkTreePath *path)
{
	DoryFile *file;
	GtkTreePath *ret;
	...
	while (file == NULL /* dummy row */ ||
	       !dory_drag_can_accept_info (file,
...
static char *
get_drop_target_uri_for_path (DoryTreeViewDragDest *dest,
			      GtkTreePath *path)
{
	DoryFile *file;
	char *target;
	...
	target = dory_file_get_drop_target_uri (file);
	dory_file_unref (file);
...
static guint
get_drop_action (DoryTreeViewDragDest *dest,
		 GdkDragContext *context,
		 GtkTreePath *path)
{
	char *drop_target;
	...
	if (!dest->details->have_drag_data ||
	    (dest->details->drag_type == DORY_ICON_DND_GNOME_ICON_LIST &&
	     dest->details->drag_list == NULL)) {
		return 0;
	}
		      ...
		      gpointer data)
{
	...
	dest = DORY_TREE_VIEW_DRAG_DEST (data);
	...
	if (action) {
        ...
        if (g_settings_get_boolean (dory_list_view_preferences, DORY_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION) &&
            g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_EXPAND_ROW_ON_DND_DWELL)) {
...
static char *
get_drop_target_uri_at_pos (DoryTreeViewDragDest *dest, int x, int y)
{
	char *drop_target;
	GtkTreePath *path;
	GtkTreePath *drop_path;
...
static void
receive_uris (DoryTreeViewDragDest *dest,
	      GdkDragContext *context,
	      GList *source_uris,
	      int x, int y)
{
	...
	if (real_action == GDK_ACTION_ASK) {
		if (dory_drag_selection_includes_special_link (dest->details->drag_list)) {
			/* We only want to move the trash */
			action = GDK_ACTION_MOVE;
		} else {
			action = GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK;
		}
		real_action = dory_drag_drop_action_ask
	...
	if (real_action > 0) {
        if (g_strcmp0 (drop_target, "favorites:///") == 0) {
            ...
            if (!dory_drag_uris_local (drop_target, source_uris)
...
static void
receive_dropped_icons (DoryTreeViewDragDest *dest,
		       GdkDragContext *context,
		       int x, int y)
{
	GList *source_uris;
	...
	for (l = dest->details->drag_list; l != NULL; l = l->next) {
		source_uris = g_list_prepend (source_uris,
					      ((DoryDragSelectionItem *)l->data)->uri);
	}
...
static void
receive_dropped_uri_list (DoryTreeViewDragDest *dest,
			  GdkDragContext *context,
			  int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_text (DoryTreeViewDragDest *dest,
		      GdkDragContext *context,
		      int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_raw (DoryTreeViewDragDest *dest,
...
static void
receive_dropped_netscape_url (DoryTreeViewDragDest *dest,
			      GdkDragContext *context,
			      int x, int y)
{
	char *drop_target;
...
static gboolean
receive_xds (DoryTreeViewDragDest *dest,
	     GtkWidget *widget,
	     guint32 time,
	     GdkDragContext *context,
	     int x, int y)
			     ...
			     gpointer data)
{
	...
	if (!dest->details->have_drag_data) {
		...
		if (info == DORY_ICON_DND_GNOME_ICON_LIST) {
			dest->details->drag_list =
				dory_drag_build_selection_list (selection_data);
		}
	}
...
static gboolean
set_direct_save_uri (DoryTreeViewDragDest *dest,
		     GdkDragContext *context,
		     int x, int y)
{
	GFile *base, *child;
		       ...
		       GObject *object)
{
	...
	dest = DORY_TREE_VIEW_DRAG_DEST (user_data);
...
static void
dory_tree_view_drag_dest_dispose (GObject *object)
{
	DoryTreeViewDragDest *dest;

	dest = DORY_TREE_VIEW_DRAG_DEST (object);
	...
	G_OBJECT_CLASS (dory_tree_view_drag_dest_parent_class)->dispose (object);
}

static void
dory_tree_view_drag_dest_finalize (GObject *object)
{
	DoryTreeViewDragDest *dest;
	...
	G_OBJECT_CLASS (dory_tree_view_drag_dest_parent_class)->finalize (object);
}

static void
dory_tree_view_drag_dest_init (DoryTreeViewDragDest *dest)
{
	dest->details = G_TYPE_INSTANCE_GET_PRIVATE (dest, DORY_TYPE_TREE_VIEW_DRAG_DEST,
						     DoryTreeViewDragDestDetails);
}

static void
dory_tree_view_drag_dest_class_init (DoryTreeViewDragDestClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);

	gobject_class->dispose = dory_tree_view_drag_dest_dispose;
	gobject_class->finalize = dory_tree_view_drag_dest_finalize;

	g_type_class_add_private (class, sizeof (DoryTreeViewDragDestDetails));

	signals[GET_ROOT_URI] =
		g_signal_new ("get_root_uri",
			      G_TYPE_FROM_CLASS (class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (DoryTreeViewDragDestClass,
					       get_root_uri),
			      NULL, NULL,
...
DoryTreeViewDragDest *
dory_tree_view_drag_dest_new (GtkTreeView *tree_view, gboolean strict_drop)
{
	DoryTreeViewDragDest *dest;
	GtkTargetList *targets;

	dest = g_object_new (DORY_TYPE_TREE_VIEW_DRAG_DEST, NULL);
