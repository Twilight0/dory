/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-thumbnail-cache.h: Thumbnail code for icon factory.
...
#define DEBUG_FLAG DORY_DEBUG_THUMBNAILS
...
#define DORY_THUMBNAIL_FRAME_LEFT 3
#define DORY_THUMBNAIL_FRAME_TOP 3
#define DORY_THUMBNAIL_FRAME_RIGHT 3
#define DORY_THUMBNAIL_FRAME_BOTTOM 3
...
/* How it works:
 * 
 * When dory_create_thumbnail(), dory_thumbnail_remove_from_queue or dory_thumbnail_prioritize are called,
 * a new DoryThumbnailInfo is made, with info->cmd_type set based on the method called.
 *
 ...
 * - dory_create_thumbnail (THUMBNAIL_ADD): The info is looked up by uri in thumbnails_to_make_hash. If
 ...
 *
 * - dory_thumbnail_remove_from_queue (THUMBNAIL_REMOVE): The info is looked up by uri in thumbnails_to_make_hash.
 ...
 *
 * - dory_thumbnail_prioritize (THUMBNAIL_BUMP): The info is looked up by uri in thumbnails_to_make_hash. If found,
 ...
 * - DoryThumbnailInfos are garbage-collected in the threadpool worker only.
 */
...
/* Causes the feeder_task to end. Only called when Dory is shutting down. */
...
get_max_threads (void) {
    ...
    gint pref = g_settings_get_int (dory_preferences, DORY_PREFERENCES_MAX_THUMBNAIL_THREADS);
...
lifo_sorter (gconstpointer a,
             ...
             gpointer      data)
{
    gint64 ta = ((const DoryThumbnailInfo *) a)->add_time;
    gint64 tb = ((const DoryThumbnailInfo *) b)->add_time;
...
static void
free_thumbnail_info (DoryThumbnailInfo *info)
{
...
static GdkPixbuf *
dory_get_thumbnail_frame (void)
{
    static GdkPixbuf *thumbnail_frame = NULL;
    static gsize once_init = 0;

    if (g_once_init_enter (&once_init)) {
        GInputStream *stream = g_resources_open_stream ("/org/dory/icons/thumbnail_frame.png", 0, NULL);
        if (stream != NULL) {
...
/* This is a one-shot idle callback called from the main loop to call
   ...
   We do this in an idle callback as I don't think dory_file_changed() is
   thread-safe. */
static gboolean
thumbnail_thread_notify_file_changed (gpointer image_uri)
{
    ...
    file = dory_file_get_by_uri ((char *) image_uri);

    DEBUG ("(Thumbnail Thread) Notifying file changed file: %p uri: %s", file, (char*) image_uri);

    if (file != NULL) {
        dory_file_set_is_thumbnailing (file, FALSE);
...
static void
remove_from_hash_table (DoryThumbnailInfo *info)
{
...
thumbnail_thread (gpointer data,
                  gpointer user_data)
{
    DoryThumbnailInfo *info = (DoryThumbnailInfo *) data;
    GdkPixbuf *pixbuf;
    time_t current_time;
    ...
    /* We need to call dory_file_changed(), but I don't think that is
...
#endif
        DoryThumbnailInfo *feeder_info = (DoryThumbnailInfo *) data;
        DoryThumbnailInfo *existing_info = NULL;
...
finalize_thumbnailer (void)
{
    ...
    info = g_new0 (DoryThumbnailInfo, 1);
    ...
    while ((data = g_async_queue_try_pop (feeder_queue))) {
        ...
        DoryThumbnailInfo *existing_info = (DoryThumbnailInfo *) data;
...
/* Mainloop */
void
dory_create_thumbnail (DoryFile *file)
{
    time_t file_mtime = 0;
    static gsize once_init = 0;
    if (g_once_init_enter (&once_init)) {
    ...
    if (dory_file_is_in_favorites (file)) {
        ...
        real_file = dory_file_get_by_uri (uri);
        if (real_file != NULL) {
            dory_create_thumbnail (real_file);
    ...
    gchar *file_uri = dory_file_get_uri (file);

    /* Hopefully the DoryFile will already have the image file mtime,
...
/* Mainloop */
void
dory_thumbnail_remove_from_queue (const char *file_uri)
{
    if (feeder_queue == NULL)
        return;
...
/* Mainloop */
void
dory_thumbnail_prioritize (const char *file_uri)
{
    if (feeder_queue == NULL)
        return;
...
gboolean
dory_can_thumbnail_internally (DoryFile *file)
{
...
gboolean
dory_can_thumbnail (DoryFile *file)
{
    ...
    mime_type = dory_file_get_mime_type (file);
    mtime = dory_file_get_mtime (file);
...
void
dory_thumbnail_frame_image (GdkPixbuf **pixbuf)
{
    GdkPixbuf *pixbuf_with_frame, *frame;

    /* The pixbuf isn't already framed (i.e., it was not made by
     * an old Dory), so we must embed it in a frame.
     */

    frame = dory_get_thumbnail_frame ();
    if (frame == NULL) {
        return;
    }

    pixbuf_with_frame = eel_embed_image_in_frame (*pixbuf, frame,
                                                  DORY_THUMBNAIL_FRAME_LEFT,
                                                  DORY_THUMBNAIL_FRAME_TOP,
                                                  DORY_THUMBNAIL_FRAME_RIGHT,
                                                  DORY_THUMBNAIL_FRAME_BOTTOM);
    g_object_unref (*pixbuf);
    *pixbuf = pixbuf_with_frame;
}

void
dory_thumbnail_pad_top_and_bottom (GdkPixbuf **pixbuf,
                                   gint        extra_height)
{
...
gboolean
dory_thumbnail_factory_check_status (void)
{
