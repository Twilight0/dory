...
/* dory-file-undo-operations.c - Manages undo/redo of file operations
 *
...
G_DEFINE_TYPE (DoryFileUndoInfo, dory_file_undo_info, G_TYPE_OBJECT)
...
struct _DoryFileUndoInfoDetails {
	DoryFileUndoOp op_type;
...
/* description helpers */
static void
dory_file_undo_info_init (DoryFileUndoInfo *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, DORY_TYPE_FILE_UNDO_INFO,
						  DoryFileUndoInfoDetails);
	self->priv->apply_async_result = NULL;
}

static void
dory_file_undo_info_get_property (GObject *object,
				      guint property_id,
				      GValue *value,
				      GParamSpec *pspec)
{
	DoryFileUndoInfo *self = DORY_FILE_UNDO_INFO (object);
...
static void
dory_file_undo_info_set_property (GObject *object,
				      guint property_id,
				      const GValue *value,
				      GParamSpec *pspec)
{
...
static void
dory_file_redo_info_warn_redo (DoryFileUndoInfo *self,
...
static void
dory_file_undo_info_warn_undo (DoryFileUndoInfo *self,
				   GtkWindow *parent_window)
{
...
static void
dory_file_undo_info_strings_func (DoryFileUndoInfo *self,
				      gchar **undo_label,
				      gchar **undo_description,
				      gchar **redo_label,
				      gchar **redo_description)
...
static void
dory_file_undo_info_finalize (GObject *obj)
{
	DoryFileUndoInfo *self = DORY_FILE_UNDO_INFO (obj);

	g_clear_object (&self->priv->apply_async_result);

	G_OBJECT_CLASS (dory_file_undo_info_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_class_init (DoryFileUndoInfoClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = dory_file_undo_info_finalize;
	oclass->get_property = dory_file_undo_info_get_property;
	oclass->set_property = dory_file_undo_info_set_property;

	klass->undo_func = dory_file_undo_info_warn_undo;
	klass->redo_func = dory_file_redo_info_warn_redo;
	klass->strings_func = dory_file_undo_info_strings_func;

	properties[PROP_OP_TYPE] =
		g_param_spec_int ("op-type",
				  "Undo info op type",
				  "Type of undo operation",
				  0, DORY_FILE_UNDO_OP_NUM_TYPES - 1, 0,
				  G_PARAM_READWRITE |
	...
	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoDetails));
	g_object_class_install_properties (oclass, N_PROPERTIES, properties);
}

static DoryFileUndoOp
dory_file_undo_info_get_op_type (DoryFileUndoInfo *self)
{
	return self->priv->op_type;
}

static gint
dory_file_undo_info_get_item_count (DoryFileUndoInfo *self)
{
	return self->priv->count;
}

void
dory_file_undo_info_apply_async (DoryFileUndoInfo *self,
				     gboolean undo,
				     GtkWindow *parent_window,
				     ...
				     gpointer user_data)
{
	...
	self->priv->apply_async_result = 
		g_simple_async_result_new (G_OBJECT (self),
					   callback, user_data,
					   dory_file_undo_info_apply_async);

	if (undo) {
		DORY_FILE_UNDO_INFO_CLASS (G_OBJECT_GET_CLASS (self))->undo_func (self, parent_window);
	} else {
		DORY_FILE_UNDO_INFO_CLASS (G_OBJECT_GET_CLASS (self))->redo_func (self, parent_window);
	}
}
...
gboolean
dory_file_undo_info_apply_finish (DoryFileUndoInfo *self,
				      GAsyncResult *res,
				      gboolean *user_cancel,
				      GError **error)
{
...
void
dory_file_undo_info_get_strings (DoryFileUndoInfo *self,
				     gchar **undo_label,
				     gchar **undo_description,
				     gchar **redo_label,
				     gchar **redo_description)
{
	return DORY_FILE_UNDO_INFO_CLASS (G_OBJECT_GET_CLASS (self))->strings_func (self,
											undo_label, undo_description,
											redo_label, redo_description);
}

static void
file_undo_info_complete_apply (DoryFileUndoInfo *self,
			       gboolean success,
			       gboolean user_cancel)
{
...
file_undo_info_transfer_callback (GHashTable * debuting_uris,
				  gboolean success,
                                  gpointer user_data)
{
	DoryFileUndoInfo *self = user_data;
...
static void
file_undo_info_operation_callback (DoryFile * file,
...
G_DEFINE_TYPE (DoryFileUndoInfoExt, dory_file_undo_info_ext, DORY_TYPE_FILE_UNDO_INFO)

struct _DoryFileUndoInfoExtDetails {
	GFile *src_dir;
	GFile *dest_dir;
...
static char *
ext_get_first_target_short_name (DoryFileUndoInfoExt *self)
{
	GFile *first_file;
	char *file_name = NULL;
...
static void
ext_strings_func (DoryFileUndoInfo *info,
		  gchar **undo_label,
		  gchar **undo_description,
		  gchar **redo_label,
		  gchar **redo_description)
{
	DoryFileUndoInfoExt *self = DORY_FILE_UNDO_INFO_EXT (info);
	DoryFileUndoOp op_type = dory_file_undo_info_get_op_type (info);
	gint count = dory_file_undo_info_get_item_count (info);
	...
	if (op_type == DORY_FILE_UNDO_OP_MOVE) {
		if (count > 1) {
	...
	} else if (op_type == DORY_FILE_UNDO_OP_RESTORE_FROM_TRASH)  {
	...
	} else if (op_type == DORY_FILE_UNDO_OP_COPY) {
		if (count > 1) {
	...
	} else if (op_type == DORY_FILE_UNDO_OP_DUPLICATE) {
		if (count > 1) {
	...
	} else if (op_type == DORY_FILE_UNDO_OP_CREATE_LINK) {
		if (count > 1) {
...
static void
ext_create_link_redo_func (DoryFileUndoInfoExt *self,
			   GtkWindow *parent_window)
{
	dory_file_operations_link (HEAD (self->priv->sources), NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_duplicate_redo_func (DoryFileUndoInfoExt *self,
			 GtkWindow *parent_window)
{
	dory_file_operations_duplicate (HEAD (self->priv->sources), NULL, parent_window,
					    file_undo_info_transfer_callback, self);
}

static void
ext_copy_redo_func (DoryFileUndoInfoExt *self,
		    GtkWindow *parent_window)
{
	dory_file_operations_copy (HEAD (self->priv->sources), NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_move_restore_redo_func (DoryFileUndoInfoExt *self,
			    GtkWindow *parent_window)
{
	dory_file_operations_move (HEAD (self->priv->sources), NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_redo_func (DoryFileUndoInfo *info,
	       GtkWindow *parent_window)
{
	DoryFileUndoInfoExt *self = DORY_FILE_UNDO_INFO_EXT (info);
	DoryFileUndoOp op_type = dory_file_undo_info_get_op_type (info);
	...
	if (op_type == DORY_FILE_UNDO_OP_MOVE ||
	    op_type == DORY_FILE_UNDO_OP_RESTORE_FROM_TRASH)  {
...
static void
ext_restore_undo_func (DoryFileUndoInfoExt *self,
		       GtkWindow *parent_window)
{
	dory_file_operations_trash_or_delete (HEAD (self->priv->destinations), parent_window,
						  file_undo_info_delete_callback, self);
}


static void
ext_move_undo_func (DoryFileUndoInfoExt *self,
		    GtkWindow *parent_window)
{
	dory_file_operations_move (HEAD (self->priv->destinations), NULL,
				       self->priv->src_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_copy_duplicate_undo_func (DoryFileUndoInfoExt *self,
			      GtkWindow *parent_window)
{
	GList *files;
	...
	dory_file_operations_delete (files, parent_window,
					 file_undo_info_delete_callback, self);
...
static void
ext_undo_func (DoryFileUndoInfo *info,
	       GtkWindow *parent_window)
{
	DoryFileUndoInfoExt *self = DORY_FILE_UNDO_INFO_EXT (info);
	DoryFileUndoOp op_type = dory_file_undo_info_get_op_type (info);

	if (op_type == DORY_FILE_UNDO_OP_COPY ||
	    op_type == DORY_FILE_UNDO_OP_DUPLICATE ||
	    op_type == DORY_FILE_UNDO_OP_CREATE_LINK) {
		ext_copy_duplicate_undo_func (self, parent_window);
	} else if (op_type == DORY_FILE_UNDO_OP_MOVE) {
...
static void
dory_file_undo_info_ext_init (DoryFileUndoInfoExt *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, dory_file_undo_info_ext_get_type (),
						  DoryFileUndoInfoExtDetails);
}

static void
dory_file_undo_info_ext_finalize (GObject *obj)
{
	DoryFileUndoInfoExt *self = DORY_FILE_UNDO_INFO_EXT (obj);
	...
	G_OBJECT_CLASS (dory_file_undo_info_ext_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_ext_class_init (DoryFileUndoInfoExtClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	DoryFileUndoInfoClass *iclass = DORY_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = dory_file_undo_info_ext_finalize;

	iclass->undo_func = ext_undo_func;
	iclass->redo_func = ext_redo_func;
	iclass->strings_func = ext_strings_func;

	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoExtDetails));
}

DoryFileUndoInfo *
dory_file_undo_info_ext_new (DoryFileUndoOp op_type,
				 gint item_count,
				 GFile *src_dir,
				 GFile *target_dir)
{
	DoryFileUndoInfoExt *retval;

	retval = g_object_new (DORY_TYPE_FILE_UNDO_INFO_EXT,
			       "op-type", op_type,
	...
	return DORY_FILE_UNDO_INFO (retval);
}

void
dory_file_undo_info_ext_add_origin_target_pair (DoryFileUndoInfoExt *self,
						    GFile                   *origin,
						    GFile                   *target)
{
...
G_DEFINE_TYPE (DoryFileUndoInfoCreate, dory_file_undo_info_create, DORY_TYPE_FILE_UNDO_INFO)

struct _DoryFileUndoInfoCreateDetails {
	char *template;
	GFile *target_file;
	gint length;
};
...
static void
create_strings_func (DoryFileUndoInfo *info,
		     gchar **undo_label,
		     gchar **undo_description,
		     gchar **redo_label,
		     gchar **redo_description)
...
{
	DoryFileUndoInfoCreate *self = DORY_FILE_UNDO_INFO_CREATE (info);
	DoryFileUndoOp op_type = dory_file_undo_info_get_op_type (info);
	char *name;
	...
	*undo_description = g_strdup_printf (_("Delete '%s'"), name);
	...
	if (op_type == DORY_FILE_UNDO_OP_CREATE_EMPTY_FILE) {
	...
	} else if (op_type == DORY_FILE_UNDO_OP_CREATE_FOLDER) {
	...
	} else if (op_type == DORY_FILE_UNDO_OP_CREATE_FILE_FROM_TEMPLATE) {
...
static void
create_from_template_redo_func (DoryFileUndoInfoCreate *self,
				GtkWindow *parent_window)
{
	GFile *parent;
	...
	dory_file_operations_new_file_from_template (NULL, NULL,
							 parent_uri, new_name,
							 self->priv->template,
...
static void
create_folder_redo_func (DoryFileUndoInfoCreate *self,
			 GtkWindow *parent_window)
{
	GFile *parent;
	gchar *parent_uri;
	...
	parent = g_file_get_parent (self->priv->target_file);
	parent_uri = g_file_get_uri (parent);
	dory_file_operations_new_folder (NULL, NULL, parent_uri,
					     create_callback, self);
...
static void
create_empty_redo_func (DoryFileUndoInfoCreate *self,
			GtkWindow *parent_window)

{
	...
	parent_uri = g_file_get_uri (parent);
	new_name = g_file_get_parse_name (self->priv->target_file);
	dory_file_operations_new_file (NULL, NULL, parent_uri,
					   new_name,
					   self->priv->template,
					   self->priv->length,
					   create_callback, self);
...
static void
create_redo_func (DoryFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	DoryFileUndoInfoCreate *self = DORY_FILE_UNDO_INFO_CREATE (info);
	DoryFileUndoOp op_type = dory_file_undo_info_get_op_type (info);
...
static void
create_undo_func (DoryFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	DoryFileUndoInfoCreate *self = DORY_FILE_UNDO_INFO_CREATE (info);
	GList *files = NULL;
...
static void
dory_file_undo_info_create_init (DoryFileUndoInfoCreate *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, dory_file_undo_info_create_get_type (),
						  DoryFileUndoInfoCreateDetails);
}

static void
dory_file_undo_info_create_finalize (GObject *obj)
{
	DoryFileUndoInfoCreate *self = DORY_FILE_UNDO_INFO_CREATE (obj);
	g_clear_object (&self->priv->target_file);
	g_free (self->priv->template);	

	G_OBJECT_CLASS (dory_file_undo_info_create_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_create_class_init (DoryFileUndoInfoCreateClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	DoryFileUndoInfoClass *iclass = DORY_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = dory_file_undo_info_create_finalize;

	iclass->undo_func = create_undo_func;
	iclass->redo_func = create_redo_func;
	iclass->strings_func = create_strings_func;

	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoCreateDetails));
}

DoryFileUndoInfo *
dory_file_undo_info_create_new (DoryFileUndoOp op_type)
{
	return g_object_new (DORY_TYPE_FILE_UNDO_INFO_CREATE,
			     "op-type", op_type,
			     "item-count", 1,
			     NULL);
}
...
void
dory_file_undo_info_create_set_data (DoryFileUndoInfoCreate *self,
                                         GFile                      *file,
                                         const char                 *template,
					 gint                        length)
{
...
/* rename */
G_DEFINE_TYPE (DoryFileUndoInfoRename, dory_file_undo_info_rename, DORY_TYPE_FILE_UNDO_INFO)

struct _DoryFileUndoInfoRenameDetails {
	GFile *old_file;
	GFile *new_file;
...
static void
rename_strings_func (DoryFileUndoInfo *info,
		     gchar **undo_label,
		     gchar **undo_description,
		     gchar **redo_label,
		     gchar **redo_description)
{
	DoryFileUndoInfoRename *self = DORY_FILE_UNDO_INFO_RENAME (info);
...
static void
rename_redo_func (DoryFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	DoryFileUndoInfoRename *self = DORY_FILE_UNDO_INFO_RENAME (info);
	DoryFile *file;

	file = dory_file_get (self->priv->old_file);
	dory_file_rename (file, self->priv->new_display_name,
...
static void
rename_undo_func (DoryFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	DoryFileUndoInfoRename *self = DORY_FILE_UNDO_INFO_RENAME (info);
	DoryFile *file;

	file = dory_file_get (self->priv->new_file);
	dory_file_rename (file, self->priv->old_display_name,
			      file_undo_info_operation_callback, self);
...
static void
dory_file_undo_info_rename_init (DoryFileUndoInfoRename *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, dory_file_undo_info_rename_get_type (),
						  DoryFileUndoInfoRenameDetails);
}

static void
dory_file_undo_info_rename_finalize (GObject *obj)
{
	DoryFileUndoInfoRename *self = DORY_FILE_UNDO_INFO_RENAME (obj);
	...
	G_OBJECT_CLASS (dory_file_undo_info_rename_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_rename_class_init (DoryFileUndoInfoRenameClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	DoryFileUndoInfoClass *iclass = DORY_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = dory_file_undo_info_rename_finalize;

	iclass->undo_func = rename_undo_func;
	iclass->redo_func = rename_redo_func;
	iclass->strings_func = rename_strings_func;

	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoRenameDetails));
}

DoryFileUndoInfo *
dory_file_undo_info_rename_new (void)
{
	return g_object_new (DORY_TYPE_FILE_UNDO_INFO_RENAME,
			     "op-type", DORY_FILE_UNDO_OP_RENAME,
			     "item-count", 1,
			     NULL);
}

void
dory_file_undo_info_rename_set_data_pre (DoryFileUndoInfoRename *self,
					     GFile                      *old_file,
					     gchar                      *old_display_name,
					     gchar                      *new_display_name)
{
...
void
dory_file_undo_info_rename_set_data_post (DoryFileUndoInfoRename *self,
					      GFile                      *new_file)
{
...
/* trash */
G_DEFINE_TYPE (DoryFileUndoInfoTrash, dory_file_undo_info_trash, DORY_TYPE_FILE_UNDO_INFO)

struct _DoryFileUndoInfoTrashDetails {
	GHashTable *trashed;
};

static void
trash_strings_func (DoryFileUndoInfo *info,
		    gchar **undo_label,
		    gchar **undo_description,
		    gchar **redo_label,
		    gchar **redo_description)
{
	DoryFileUndoInfoTrash *self = DORY_FILE_UNDO_INFO_TRASH (info);
...
trash_redo_func_callback (GHashTable *debuting_uris,
			  ...
			  gpointer user_data)
{
	DoryFileUndoInfoTrash *self = user_data;
...
static void
trash_redo_func (DoryFileUndoInfo *info,
		 GtkWindow *parent_window)
{
	DoryFileUndoInfoTrash *self = DORY_FILE_UNDO_INFO_TRASH (info);
	...
	if (g_hash_table_size (self->priv->trashed) > 0) {
		...
		dory_file_operations_trash_or_delete (locations, parent_window,
...
static GHashTable *
trash_retrieve_files_to_restore_finish (DoryFileUndoInfoTrash *self,
					GAsyncResult *res,
					GError **error)
{
...
{
	DoryFileUndoInfoTrash *self = DORY_FILE_UNDO_INFO_TRASH (object);
...
static void
trash_retrieve_files_to_restore_async (DoryFileUndoInfoTrash *self,
				 GAsyncReadyCallback callback,
				 gpointer user_data)
{
...
trash_retrieve_files_ready (GObject *source,
			    ...
			    gpointer user_data)
{
	DoryFileUndoInfoTrash *self = DORY_FILE_UNDO_INFO_TRASH (source);
...
static void
trash_undo_func (DoryFileUndoInfo *info,
		 GtkWindow *parent_window)
{
	DoryFileUndoInfoTrash *self = DORY_FILE_UNDO_INFO_TRASH (info);

	/* Internally managed op, pop flag. */
	dory_file_undo_manager_pop_flag ();

	trash_retrieve_files_to_restore_async (self, trash_retrieve_files_ready, NULL);
}

static void
dory_file_undo_info_trash_init (DoryFileUndoInfoTrash *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, dory_file_undo_info_trash_get_type (),
						  DoryFileUndoInfoTrashDetails);
	self->priv->trashed =
		g_hash_table_new_full (g_file_hash, (GEqualFunc) g_file_equal, 
				       g_object_unref, NULL);
}
...
static void
dory_file_undo_info_trash_finalize (GObject *obj)
{
	DoryFileUndoInfoTrash *self = DORY_FILE_UNDO_INFO_TRASH (obj);
	g_hash_table_destroy (self->priv->trashed);

	G_OBJECT_CLASS (dory_file_undo_info_trash_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_trash_class_init (DoryFileUndoInfoTrashClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	DoryFileUndoInfoClass *iclass = DORY_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = dory_file_undo_info_trash_finalize;

	iclass->undo_func = trash_undo_func;
	iclass->redo_func = trash_redo_func;
	iclass->strings_func = trash_strings_func;

	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoTrashDetails));
}

DoryFileUndoInfo *
dory_file_undo_info_trash_new (gint item_count)
{
	return g_object_new (DORY_TYPE_FILE_UNDO_INFO_TRASH,
			     "op-type", DORY_FILE_UNDO_OP_MOVE_TO_TRASH,
			     "item-count", item_count,
			     NULL);
}

void
dory_file_undo_info_trash_add_file (DoryFileUndoInfoTrash *self,
					GFile                     *file)
{
	GTimeVal current_time;
...
G_DEFINE_TYPE (DoryFileUndoInfoRecPermissions, dory_file_undo_info_rec_permissions, DORY_TYPE_FILE_UNDO_INFO)

struct _DoryFileUndoInfoRecPermissionsDetails {
	GFile *dest_dir;
...
static void
rec_permissions_strings_func (DoryFileUndoInfo *info,
			      gchar **undo_label,
			      gchar **undo_description,
			      gchar **redo_label,
			      gchar **redo_description)
{
	DoryFileUndoInfoRecPermissions *self = DORY_FILE_UNDO_INFO_REC_PERMISSIONS (info);
	char *name;
...
static void
rec_permissions_redo_func (DoryFileUndoInfo *info,
			   GtkWindow *parent_window)
{
	DoryFileUndoInfoRecPermissions *self = DORY_FILE_UNDO_INFO_REC_PERMISSIONS (info);
	gchar *parent_uri;
	...
	dory_file_set_permissions_recursive (parent_uri,
...
static void
rec_permissions_undo_func (DoryFileUndoInfo *info,
			   GtkWindow *parent_window)
{
	DoryFileUndoInfoRecPermissions *self = DORY_FILE_UNDO_INFO_REC_PERMISSIONS (info);
...
static void
dory_file_undo_info_rec_permissions_init (DoryFileUndoInfoRecPermissions *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, dory_file_undo_info_rec_permissions_get_type (),
						  DoryFileUndoInfoRecPermissionsDetails);
...
static void
dory_file_undo_info_rec_permissions_finalize (GObject *obj)
{
	DoryFileUndoInfoRecPermissions *self = DORY_FILE_UNDO_INFO_REC_PERMISSIONS (obj);

	g_hash_table_destroy (self->priv->original_permissions);
	g_clear_object (&self->priv->dest_dir);

	G_OBJECT_CLASS (dory_file_undo_info_rec_permissions_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_rec_permissions_class_init (DoryFileUndoInfoRecPermissionsClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	DoryFileUndoInfoClass *iclass = DORY_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = dory_file_undo_info_rec_permissions_finalize;
	...
	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoRecPermissionsDetails));
}

DoryFileUndoInfo *
dory_file_undo_info_rec_permissions_new (GFile   *dest,
					     guint32 file_permissions,
					     guint32 file_mask,
					     guint32 dir_permissions,
					     guint32 dir_mask)
{
	DoryFileUndoInfoRecPermissions *retval;

	retval = g_object_new (DORY_TYPE_FILE_UNDO_INFO_REC_PERMISSIONS,
			       "op-type", DORY_FILE_UNDO_OP_RECURSIVE_SET_PERMISSIONS,
			       "item-count", 1,
			       NULL);
...
void
dory_file_undo_info_rec_permissions_add_file (DoryFileUndoInfoRecPermissions *self,
						  GFile                              *file,
						  guint32                             permission)
{
...
G_DEFINE_TYPE (DoryFileUndoInfoPermissions, dory_file_undo_info_permissions, DORY_TYPE_FILE_UNDO_INFO)

struct _DoryFileUndoInfoPermissionsDetails {
	GFile *target_file;
...
static void
permissions_strings_func (DoryFileUndoInfo *info,
			  gchar **undo_label,
			  gchar **undo_description,
			  gchar **redo_label,
			  gchar **redo_description)
{
	DoryFileUndoInfoPermissions *self = DORY_FILE_UNDO_INFO_PERMISSIONS (info);
	gchar *name;
...
static void
permissions_real_func (DoryFileUndoInfoPermissions *self,
		       guint32 permissions)
{
	DoryFile *file;

	file = dory_file_get (self->priv->target_file);
	dory_file_set_permissions (file, permissions,
				       file_undo_info_operation_callback, self);
...
static void
permissions_redo_func (DoryFileUndoInfo *info, 
		       GtkWindow *parent_window)
{
	DoryFileUndoInfoPermissions *self = DORY_FILE_UNDO_INFO_PERMISSIONS (info);
...
static void
permissions_undo_func (DoryFileUndoInfo *info, 
		       GtkWindow *parent_window)
{
	DoryFileUndoInfoPermissions *self = DORY_FILE_UNDO_INFO_PERMISSIONS (info);
...
static void
dory_file_undo_info_permissions_init (DoryFileUndoInfoPermissions *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, dory_file_undo_info_permissions_get_type (),
						  DoryFileUndoInfoPermissionsDetails);
}

static void
dory_file_undo_info_permissions_finalize (GObject *obj)
{
	DoryFileUndoInfoPermissions *self = DORY_FILE_UNDO_INFO_PERMISSIONS (obj);
	g_clear_object (&self->priv->target_file);

	G_OBJECT_CLASS (dory_file_undo_info_permissions_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_permissions_class_init (DoryFileUndoInfoPermissionsClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	DoryFileUndoInfoClass *iclass = DORY_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = dory_file_undo_info_permissions_finalize;

	iclass->undo_func = permissions_undo_func;
	iclass->redo_func = permissions_redo_func;
	iclass->strings_func = permissions_strings_func;

	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoPermissionsDetails));
}

DoryFileUndoInfo *
dory_file_undo_info_permissions_new (GFile   *file,
					 guint32  current_permissions,
					 guint32  new_permissions)
{
	DoryFileUndoInfoPermissions *retval;

	retval = g_object_new (DORY_TYPE_FILE_UNDO_INFO_PERMISSIONS,
			     "op-type", DORY_FILE_UNDO_OP_SET_PERMISSIONS,
			     "item-count", 1,
			     NULL);
...
G_DEFINE_TYPE (DoryFileUndoInfoOwnership, dory_file_undo_info_ownership, DORY_TYPE_FILE_UNDO_INFO)

struct _DoryFileUndoInfoOwnershipDetails {
	GFile *target_file;
...
static void
ownership_strings_func (DoryFileUndoInfo *info,
			gchar **undo_label,
			gchar **undo_description,
			gchar **redo_label,
			gchar **redo_description)
{
	DoryFileUndoInfoOwnership *self = DORY_FILE_UNDO_INFO_OWNERSHIP (info);
	DoryFileUndoOp op_type = dory_file_undo_info_get_op_type (info);
	gchar *name;

	name = g_file_get_parse_name (self->priv->target_file);

	if (op_type == DORY_FILE_UNDO_OP_CHANGE_GROUP) {
	...
	} else if (op_type == DORY_FILE_UNDO_OP_CHANGE_OWNER) {
...
static void
ownership_real_func (DoryFileUndoInfoOwnership *self,
		     const gchar *ownership)
{
	DoryFileUndoOp op_type = dory_file_undo_info_get_op_type (DORY_FILE_UNDO_INFO (self));
	DoryFile *file;

	file = dory_file_get (self->priv->target_file);

	if (op_type == DORY_FILE_UNDO_OP_CHANGE_OWNER) {
		dory_file_set_owner (file,
					 ownership,
					 file_undo_info_operation_callback, self);
	} else if (op_type == DORY_FILE_UNDO_OP_CHANGE_GROUP) {
		dory_file_set_group (file,
					 ownership,
					 file_undo_info_operation_callback, self);
	}
...
static void
ownership_redo_func (DoryFileUndoInfo *info,
		     GtkWindow *parent_window)
{
	DoryFileUndoInfoOwnership *self = DORY_FILE_UNDO_INFO_OWNERSHIP (info);
...
static void
ownership_undo_func (DoryFileUndoInfo *info,
		     GtkWindow *parent_window)
{
	DoryFileUndoInfoOwnership *self = DORY_FILE_UNDO_INFO_OWNERSHIP (info);
...
static void
dory_file_undo_info_ownership_init (DoryFileUndoInfoOwnership *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, dory_file_undo_info_ownership_get_type (),
						  DoryFileUndoInfoOwnershipDetails);
}

static void
dory_file_undo_info_ownership_finalize (GObject *obj)
{
	DoryFileUndoInfoOwnership *self = DORY_FILE_UNDO_INFO_OWNERSHIP (obj);

	g_clear_object (&self->priv->target_file);
	g_free (self->priv->original_ownership);
	g_free (self->priv->new_ownership);

	G_OBJECT_CLASS (dory_file_undo_info_ownership_parent_class)->finalize (obj);
}

static void
dory_file_undo_info_ownership_class_init (DoryFileUndoInfoOwnershipClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	DoryFileUndoInfoClass *iclass = DORY_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = dory_file_undo_info_ownership_finalize;

	iclass->undo_func = ownership_undo_func;
	iclass->redo_func = ownership_redo_func;
	iclass->strings_func = ownership_strings_func;

	g_type_class_add_private (klass, sizeof (DoryFileUndoInfoOwnershipDetails));
}

DoryFileUndoInfo *
dory_file_undo_info_ownership_new (DoryFileUndoOp  op_type,
				       GFile              *file,
				       const char         *current_data,
				       const char         *new_data)
{
	DoryFileUndoInfoOwnership *retval;

	retval = g_object_new (DORY_TYPE_FILE_UNDO_INFO_OWNERSHIP,
			       "item-count", 1,
			       "op-type", op_type,
			       NULL);
