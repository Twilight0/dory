...
#include "dory-program-choosing.h"
#include "dory-ui-utilities.h"

#define DEBUG_FLAG DORY_DEBUG_ACTIONS
...
G_DEFINE_TYPE_WITH_PRIVATE (DoryAction, dory_action, GTK_TYPE_ACTION)

static void     dory_action_get_property  (GObject                    *object,
                                           guint                       param_id,
                                           GValue                     *value,
                                           GParamSpec                 *pspec);
static void     dory_action_set_property  (GObject                    *object,
                                           guint                       param_id,
                                           const GValue               *value,
                                           GParamSpec                 *pspec);
static void     dory_action_constructed (GObject *object);
static void     dory_action_finalize (GObject *gobject);
...
static void
dory_action_init (DoryAction *action)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static void
dory_action_class_init (DoryActionClass *klass)
{
    GObjectClass         *object_class = G_OBJECT_CLASS(klass);
    object_class->finalize = dory_action_finalize;
    object_class->set_property = dory_action_set_property;
    object_class->get_property = dory_action_get_property;
    object_class->constructed = dory_action_constructed;
...
static gboolean
recalc_dbus_conditions (DoryAction *action)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static void
queue_recalc_dbus_conditions (DoryAction *action)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static void
setup_dbus_condition (DoryAction *action, const gchar *condition)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static gboolean
recalc_gsettings_conditions (DoryAction *action)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
        ...
        if (len == 3) {
            ...
            if (g_variant_type_equal (setting_type, target_type)) {
                ...
                if (target_var != NULL) {
                    ...
                    g_warning ("Dory Action: gsettings value could not be parsed into a valid GVariant");
                }
            }
...
static void
queue_recalc_gsettings_conditions (DoryAction *action)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static void
setup_gsettings_condition (DoryAction *action,
                           const gchar *condition)
{
    ...
    if (len == 6 &&
        ...
        g_warning ("Dory Action: Either gsettings variant type (%s) or operator (%s) is invalid.",
...
static void
populate_patterns_and_filenames (DoryAction   *action,
                                 gchar       **array,
                                 GList       **allowed_patterns,
                                 GList       **forbidden_patterns,
                                 GList       **allowed_filenames,
...
void
dory_action_constructed (GObject *object)
{
    DoryAction *action = DORY_ACTION (object);
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    GIcon *gicon;

    G_OBJECT_CLASS (dory_action_parent_class)->constructed (object);
    ...
    if (conditions && condition_count > 0) {
        ...
        for (j = 0; j < condition_count; j++) {
            ...
            if (g_str_has_prefix (condition, "exec")) {
                /* handled in dory_action_get_visibility */
            }
            else
            ...
            if (g_strcmp0 (condition, "removable") == 0) {
                /* this is handled in dory_action_get_visibility() */
            }
            else
            ...
            if (g_strcmp0 (condition, "sidebar-only") == 0) {
                ...
                g_warning ("Ignoring invalid condition: %s."
                           " See sample action at /usr/share/dory/actions/sample.dory_action", condition);
            }
        }
    }
...
DoryAction *
dory_action_new (const gchar *name, 
                 const gchar *path)
{
    GKeyFile *key_file = g_key_file_new();
    ...
    if (deps != NULL) {
        ...
        for (i = 0; i < g_strv_length (deps); i++) {
            ...
            if (g_path_is_absolute (prg_name)) {
                if ((!dory_path_is_network_safe (prg_name)) && g_file_test (prg_name, G_FILE_TEST_EXISTS)) {
                    found = TRUE;
                }
            } else {
    ...
    return finish ? g_object_new (DORY_TYPE_ACTION,
                                  "name", name,
                                  "key-file-path", path,
                                  NULL): NULL;
}
...
static void
dory_action_finalize (GObject *object)
{
    DoryAction *action = DORY_ACTION (object);
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    ...
    G_OBJECT_CLASS (dory_action_parent_class)->finalize (object);
}

static void
dory_action_set_property (GObject         *object,
                          guint            prop_id,
                          const GValue    *value,
                          GParamSpec      *pspec)
{
    ...
    action = DORY_ACTION (object);

    switch (prop_id)
    {
    case PROP_KEY_FILE_PATH:
      action->key_file_path = g_strdup (g_value_get_string (value));
      action->uuid = dory_make_action_uuid_for_path (action->key_file_path);
      break;
    default:
...
static void
dory_action_get_property (GObject    *object,
             guint       prop_id,
             GValue     *value,
             GParamSpec *pspec)
{
...
static gchar *
get_path (DoryAction *action, DoryFile *file)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    gchar *ret, *orig;
    GFile *location;
...
static GString *
score_append (DoryAction *action, GString *str, const gchar *c)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static GString *
insert_separator (DoryAction *action, GString *str)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static GString *
insert_quote (DoryAction *action, GString *str)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
static gchar *
get_device_path (DoryAction *action, DoryFile *file)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    GMount *mount = dory_file_get_mount (file);
...
static gchar *
get_insertion_string (DoryAction *action,
                      TokenType   token_type,
                      GList      *selection,
                      DoryFile   *parent,
                      GtkWindow  *window)
        ...
        case TOKEN_PATH_LIST:
            if (g_list_length (selection) > 0) {
                for (l = selection; l != NULL; l = l->next) {
                    ...
                    gchar *path = get_path (action, DORY_FILE (l->data));
                    if (path)
        ...
        case TOKEN_URI_LIST:
            if (g_list_length (selection) > 0) {
                for (l = selection; l != NULL; l = l->next) {
                    ...
                    if (dory_file_is_in_recent (DORY_FILE (l->data))) {
                        uri = dory_file_get_activation_uri (DORY_FILE (l->data));
                    } else {
                        uri = dory_file_get_uri (DORY_FILE (l->data));
                    }
...
default_parent_path:
            ...
            if (parent != NULL) {
                ...
                if (path == NULL) {
                    gchar *name = dory_file_get_display_name (parent);
                    if (g_strcmp0 (name, "x-dory-desktop") == 0)
                ...
                if (g_list_length (selection) > 0 && dory_file_is_in_recent (DORY_FILE (selection->data))) {
...
default_parent_uri:
            ...
            if (parent != NULL) {
                ...
                gchar *name = dory_file_get_display_name (parent);
                if (g_strcmp0 (name, "x-dory-desktop") == 0) {
                    gchar *real_desktop_path = dory_get_desktop_directory ();
        ...
        case TOKEN_FILE_DISPLAY_NAME:
            if (g_list_length (selection) > 0) {
                gchar *file_display_name = dory_file_get_display_name (DORY_FILE (selection->data));
...
default_parent_display_name:
            ...
            if (parent != NULL) {
                ...
                gchar *real_display_name = dory_file_get_display_name (parent);
                if (g_strcmp0 (real_display_name, "x-dory-desktop") == 0)
                    parent_display_name = g_strdup_printf (_("Desktop"));
                else
                    parent_display_name = dory_file_get_display_name (parent);
            ...
            if (g_list_length (selection) > 0) {
                for (l = selection; l != NULL; l = l->next) {
                    if (!first)
                        str = insert_separator (action, str);
                    str = insert_quote (action, str);
                    gchar *dev = get_device_path (action, DORY_FILE (l->data));
                    if (dev)
...
static GString *
expand_action_string (DoryAction *action,
                      GList      *selection,
                      DoryFile   *parent,
                      GString    *str,
                      GtkWindow  *window)
{
    ...
    while (ptr != NULL) {
         ...
         * tokens (%U, %F, etc...).
         *
         * See: https://github.com/linuxmint/dory/issues/1956
         */
...
void
dory_action_activate (DoryAction *action,
                      GList      *selection,
                      DoryFile   *parent,
                      GtkWindow  *window)
{
    ...
    if (priv->run_in_terminal) {
        ...
        if (g_shell_parse_argv (exec->str, &argcp, &argvp, &error)) {
            dory_launch_application_from_command_array (gdk_screen_get_default (),
                                                        argvp[0],
                                                        TRUE,
...
const gchar *
dory_action_get_orig_label (DoryAction *action)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
...
void
dory_action_override_label (DoryAction  *action,
                            const gchar *label)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    g_free (priv->orig_label);
...
void
dory_action_override_icon (DoryAction  *action,
...
const gchar *
dory_action_get_orig_tt (DoryAction *action)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    return priv->orig_tt;
}
...
static gchar *
get_final_label (DoryAction *action,
                 GList      *selection,
                 DoryFile   *parent,
                 GtkWindow  *window)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    const gchar *orig_label = dory_action_get_orig_label (action);
...
static gchar *
get_final_tt (DoryAction *action,
              GList      *selection,
              DoryFile   *parent,
              GtkWindow  *window)
{
    DoryActionPrivate *priv = dory_action_get_instance_private (action);
    const gchar *orig_tt = dory_action_get_orig_tt (action);
...
static void
finalize_strings (DoryAction *action,
                  GList      *selection,
                  DoryFile   *parent,
                  GtkWindow  *window)
{
...
static gboolean
check_exec_condition (DoryAction  *action,
                      const gchar *condition,
                      GList       *selection,
                      DoryFile    *parent,
                      GtkWindow   *window)
...
static gboolean
get_is_dir (DoryFile *file)
{
    gboolean ret = FALSE;

    GFile *f = dory_file_get_location (file);

    if (g_file_is_native (f) && (!dory_location_is_network_safe (f))) {
        gchar *path;
        ...
        ret = dory_file_is_directory (file);
    }
...
static gboolean
check_is_allowed (DoryAction *action,
                  DoryFile   *parent,
                  GList      *selection,
                  GList      *allowed_patterns,
                  GList      *forbidden_patterns,
                  ...
                  GList      *forbidden_names)
{
    ...
    if (parent != NULL) {
        DEBUG ("Checking pattern/name matching for location: %s", dory_file_peek_name (parent));
    ...
    for (l = files; l != NULL; l = l->next) {
        ...
        g_autofree gchar *path = dory_file_get_path (file);
        ...
        const gchar *name = dory_file_peek_name (file);
...
static gboolean
get_visibility (DoryAction *action,
                GList      *selection,
                DoryFile   *parent,
                gboolean    for_places,
                GtkWindow  *window)
{
    ...
    if ((priv->uri_scheme != NULL) && !dory_file_has_uri_scheme (parent, priv->uri_scheme)) {
        return FALSE;
    }
    ...
    if (ext_count == 1 && g_strcmp0 (extensions[0], "any") == 0) {
      ...
      for (iter = selection; iter != NULL && found_match; iter = iter->next) {
          found_match = FALSE;
          gboolean is_dir;
          gchar *raw_fn = dory_file_get_name (DORY_FILE (iter->data));
          ...
          if (mime_count > 0) {
              for (i = 0; i < mime_count; i++) {
                  if (dory_file_is_mime_type (DORY_FILE (iter->data), mimetypes[i])) {
                      found_match = TRUE;
                      break;
                  }
              }
          ...
          if (dory_file_is_mime_type (DORY_FILE (iter->data), "application/x-dory-link")) {
              found_match = FALSE;
          }
      }
    ...
    if (condition_count > 0) {
        ...
        for (j = 0; j < condition_count; j++) {
            ...
            if (g_strcmp0 (condition, "desktop") == 0) {
                gchar *name = dory_file_get_display_name (parent);
                if (g_strcmp0 (name, "x-dory-desktop") != 0 && !(parent == NULL && priv->show_in_blank_desktop))
                ...
                if (g_list_length (selection) > 0) {
                    ...
                    if (!mount && for_places) {
                        ...
                        f = dory_file_get_location (file);

                        if (g_file_is_native (f)) {
                            mount = dory_get_mount_for_location_safe (f);
...
void
dory_action_update_display_state (DoryAction *action,
                                  GList      *selection,
                                  DoryFile   *parent,
                                  gboolean    for_places,
                                  GtkWindow  *window)
