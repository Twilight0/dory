...
/* 
   dory-trash-monitor.c: Dory trash state watcher.
...
struct DoryTrashMonitorDetails {
	gboolean empty;
	GIcon *icon;
...
static DoryTrashMonitor *dory_trash_monitor = NULL;

G_DEFINE_TYPE(DoryTrashMonitor, dory_trash_monitor, G_TYPE_OBJECT)

static void
dory_trash_monitor_finalize (GObject *object)
{
	DoryTrashMonitor *trash_monitor;

	trash_monitor = DORY_TRASH_MONITOR (object);
	...
	G_OBJECT_CLASS (dory_trash_monitor_parent_class)->finalize (object);
}

static void
dory_trash_monitor_class_init (DoryTrashMonitorClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = dory_trash_monitor_finalize;

	signals[TRASH_STATE_CHANGED] = g_signal_new
		("trash_state_changed",
		 G_TYPE_FROM_CLASS (object_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (DoryTrashMonitorClass, trash_state_changed),
		 NULL, NULL,
		 g_cclosure_marshal_VOID__BOOLEAN,
		 G_TYPE_NONE, 1,
		 G_TYPE_BOOLEAN);

	g_type_class_add_private (object_class, sizeof(DoryTrashMonitorDetails));
}

static void
update_icon (DoryTrashMonitor *trash_monitor)
{
	g_clear_object (&trash_monitor->details->icon);

	if (trash_monitor->details->empty) {
		trash_monitor->details->icon = g_themed_icon_new (DORY_ICON_TRASH);
        trash_monitor->details->symbolic_icon_name = DORY_ICON_SYMBOLIC_TRASH;
	} else {
		trash_monitor->details->icon = g_themed_icon_new (DORY_ICON_TRASH_FULL);
        trash_monitor->details->symbolic_icon_name = DORY_ICON_SYMBOLIC_TRASH_FULL;
	}
}

static void
update_empty_info (DoryTrashMonitor *trash_monitor,
		   gboolean is_empty)
{
...
trash_query_info_cb (GObject *source,
                     ...
                     gpointer user_data)
{
        DoryTrashMonitor *trash_monitor = user_data;
        GFileInfo *info;
        guint32 item_count;
...
static void
schedule_update_info (DoryTrashMonitor *trash_monitor)
{
	GFile *location;
	      ...
	      gpointer user_data)
{
	...
	trash_monitor = DORY_TRASH_MONITOR (user_data);

	schedule_update_info (trash_monitor);
}

static void
dory_trash_monitor_init (DoryTrashMonitor *trash_monitor)
{
	GFile *location;

	trash_monitor->details = G_TYPE_INSTANCE_GET_PRIVATE (trash_monitor,
							      DORY_TYPE_TRASH_MONITOR,
							      DoryTrashMonitorDetails);
...
{
	g_object_unref (dory_trash_monitor);
}

DoryTrashMonitor *
dory_trash_monitor_get (void)
{
	if (dory_trash_monitor == NULL) {
		/* not running yet, start it up */

		dory_trash_monitor = DORY_TRASH_MONITOR
			(g_object_new (DORY_TYPE_TRASH_MONITOR, NULL));
		eel_debug_call_at_shutdown (unref_trash_monitor);
	}

	return dory_trash_monitor;
}

gboolean
dory_trash_monitor_is_empty (void)
{
...
GIcon *
dory_trash_monitor_get_icon (void)
{
	DoryTrashMonitor *monitor;
...
gchar *
dory_trash_monitor_get_symbolic_icon_name (void)
{
    DoryTrashMonitor *monitor;
...
void
dory_trash_monitor_add_new_trash_directories (void)
{
