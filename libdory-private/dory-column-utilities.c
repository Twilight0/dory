...
get_extension_columns (void)
{
	...
	providers = dory_module_get_extensions_for_type (DORY_TYPE_COLUMN_PROVIDER);
	
	columns = NULL;
	
	for (l = providers; l != NULL; l = l->next) {
		DoryColumnProvider *provider;
		GList *provider_columns;
		
		provider = DORY_COLUMN_PROVIDER (l->data);
		provider_columns = dory_column_provider_get_columns (provider);
...
get_trash_columns (void)
{
	...
	return dory_column_list_copy (columns);
}
...
GList *
dory_get_all_columns (void)
{
    GList *columns = NULL;

	columns = g_list_concat (dory_get_common_columns (),
	                         get_trash_columns ());
...
GList *
dory_get_columns_for_file (DoryFile *file)
{
    GList *columns;
    gchar *uri;
...
GList *
dory_column_list_copy (GList *columns) 
{
	GList *ret;
	GList *l;
...
void
dory_column_list_free (GList *columns)
{
	GList *l;
...
static int
column_compare (DoryColumn *a, DoryColumn *b, char **column_order)
{
	int index_a;
	int index_b;
	char *name;
...
GList *
dory_sort_columns (GList  *columns, 
		       char  **column_order)
{
	if (!column_order) {
		return NULL;
