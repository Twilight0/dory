/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-desktop-directory-file.c: Subclass of DoryFile to help implement the
   virtual desktop.
...
struct DoryDesktopDirectoryFileDetails {
	DoryDesktopDirectory *desktop_directory;

  	DoryFile *real_dir_file;
...
typedef struct {
	DoryDesktopDirectoryFile *desktop_file;
	DoryFileCallback callback;
	gpointer callback_data;

	DoryFileAttributes delegated_attributes;
	DoryFileAttributes non_delegated_attributes;
...
G_DEFINE_TYPE (DoryDesktopDirectoryFile, dory_desktop_directory_file,
	       DORY_TYPE_FILE);

static gchar *
get_indexed_key (DoryFile *file)
{
    DoryDesktopDirectory *desktop_directory;
    gchar *indexed_key;

    desktop_directory = DORY_DESKTOP_DIRECTORY_FILE (file)->details->desktop_directory;

    indexed_key = g_strdup_printf ("desktop-monitor-%d",
                                   DORY_DESKTOP_DIRECTORY (desktop_directory)->display_number);
...
static void
real_file_changed_callback (DoryFile *real_file,
			    gpointer callback_data)
{
	DoryDesktopDirectoryFile *desktop_file;
	
	desktop_file = DORY_DESKTOP_DIRECTORY_FILE (callback_data);
	dory_file_changed (DORY_FILE (desktop_file));
}

static DoryFileAttributes 
get_delegated_attributes_mask (void)
{
	return DORY_FILE_ATTRIBUTE_DEEP_COUNTS |
		DORY_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		DORY_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;
}

static void
partition_attributes (DoryFileAttributes attributes,
		      DoryFileAttributes *delegated_attributes,
		      DoryFileAttributes *non_delegated_attributes)
{
	DoryFileAttributes mask;
...
static void
desktop_directory_file_monitor_add (DoryFile *file,
				    ...
				    DoryFileAttributes attributes)
{
	...
	desktop_file = DORY_DESKTOP_DIRECTORY_FILE (file);
	...
	dory_file_monitor_add (desktop_file->details->real_dir_file,
...
static void
desktop_directory_file_monitor_remove (DoryFile *file,
				       gconstpointer client)
{
	DoryDesktopDirectoryFile *desktop_file;
	DesktopMonitor *monitor;
...
desktop_callback_destroy (DesktopCallback *desktop_callback)
{
	...
	g_assert (DORY_IS_DESKTOP_DIRECTORY_FILE (desktop_callback->desktop_file));

	dory_file_unref (DORY_FILE (desktop_callback->desktop_file));
...
desktop_callback_check_done (DesktopCallback *desktop_callback)
{
    ...
    file = DORY_FILE (desktop_callback->desktop_file);
    name = get_indexed_key (file);

    dory_desktop_update_metadata_from_keyfile (file, name);
	...
	(* desktop_callback->callback) (DORY_FILE (desktop_callback->desktop_file),
...
static void
desktop_directory_file_call_when_ready (DoryFile *file,
					DoryFileAttributes attributes,
					DoryFileCallback callback,
					gpointer callback_data)
...
static void
desktop_directory_file_cancel_call_when_ready (DoryFile *file,
					       DoryFileCallback callback,
					       gpointer callback_data)
{
	DoryDesktopDirectoryFile *desktop_file;
...
static gboolean
real_check_if_ready (DoryFile *file,
		     DoryFileAttributes attributes)
{
...
static gboolean
desktop_directory_file_check_if_ready (DoryFile *file,
				       DoryFileAttributes attributes)
{
	DoryFileAttributes delegated_attributes, non_delegated_attributes;
	DoryDesktopDirectoryFile *desktop_file;
	...
	return real_check_if_ready (file, non_delegated_attributes) &&
		dory_file_check_if_ready (desktop_file->details->real_dir_file,
					      delegated_attributes);
}

static gboolean
desktop_directory_file_get_item_count (DoryFile *file, 
				       guint *count,
				       gboolean *count_unreadable)
{
	...
	got_count = dory_file_get_directory_item_count (desktop_file->details->real_dir_file,
							    count,
							    count_unreadable);
...
desktop_directory_file_get_deep_counts (DoryFile *file,
					...
					goffset *total_size)
{
	...
	status = dory_file_get_deep_counts (desktop_file->details->real_dir_file,
						directory_count,
						file_count,
...
static gboolean
desktop_directory_file_get_date (DoryFile *file,
				 ...
				 time_t *date)
{
	...
	return dory_file_get_date (desktop_file->details->real_dir_file,
				       date_type,
				       date);
}

static char *
desktop_directory_file_get_where_string (DoryFile *file)
{
...
monitor_destroy (gpointer data)
{
	...
	dory_file_monitor_remove
		(DORY_FILE (monitor->desktop_file->details->real_dir_file), monitor);
	g_free (monitor);
}

static void
dory_desktop_directory_file_set_metadata (DoryFile           *file,
                                          const char         *key,
                                          const char         *value)
{
    gchar *name;
    ...
    dory_desktop_set_metadata_string (file, name, key, value);

    g_free (name);
}

static void
dory_desktop_directory_file_set_metadata_as_list (DoryFile           *file,
                                                  const char         *key,
                                                  char              **value)
{
    gchar *name;
    ...
    dory_desktop_set_metadata_stringv (file, name, key, (const gchar **) value);

    g_free (name);
}

static gchar *
dory_desktop_directory_file_get_metadata (DoryFile           *file,
                                          const char         *key)
{
    gchar *name;
    gchar *string;
    ...
    string = dory_desktop_get_metadata_string (file, name, key);
...
static gchar **
dory_desktop_directory_file_get_metadata_as_list (DoryFile           *file,
                                                  const char         *key)
{
    gchar *name;
    gchar **stringv;
    ...
    stringv = dory_desktop_get_metadata_stringv (file, name, key);
...
static void
dory_desktop_directory_file_init (DoryDesktopDirectoryFile *desktop_file)
{
	DoryDesktopDirectory *desktop_directory;
	DoryDirectory *real_dir;
	DoryFile *real_dir_file;

	desktop_file->details = G_TYPE_INSTANCE_GET_PRIVATE (desktop_file,
							     DORY_TYPE_DESKTOP_DIRECTORY_FILE,
							     DoryDesktopDirectoryFileDetails);

	desktop_directory = DORY_DESKTOP_DIRECTORY (dory_directory_get_by_uri (EEL_DESKTOP_URI));
	...
	real_dir = dory_desktop_directory_get_real_directory (desktop_directory);
	real_dir_file = dory_directory_get_corresponding_file (real_dir);
	dory_directory_unref (real_dir);
...
desktop_finalize (GObject *object)
{
	...
	desktop_file = DORY_DESKTOP_DIRECTORY_FILE (object);
	...
	dory_file_unref (desktop_file->details->real_dir_file);
	dory_directory_unref (DORY_DIRECTORY (desktop_directory));

	G_OBJECT_CLASS (dory_desktop_directory_file_parent_class)->finalize (object);
}

static void
dory_desktop_directory_file_class_init (DoryDesktopDirectoryFileClass *klass)
{
    ...
    file_class->set_metadata = dory_desktop_directory_file_set_metadata;
	file_class->get_metadata = dory_desktop_directory_file_get_metadata;
    file_class->set_metadata_as_list = dory_desktop_directory_file_set_metadata_as_list;
	file_class->get_metadata_as_list = dory_desktop_directory_file_get_metadata_as_list;

	g_type_class_add_private (klass, sizeof (DoryDesktopDirectoryFileDetails));
}
