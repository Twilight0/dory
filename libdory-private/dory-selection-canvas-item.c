...
struct _DorySelectionCanvasItemDetails {
...
G_DEFINE_TYPE (DorySelectionCanvasItem, dory_selection_canvas_item, EEL_TYPE_CANVAS_ITEM);

#define DASH_ON 0.8
#define DASH_OFF 1.7
static void
dory_selection_canvas_item_draw (EelCanvasItem *item,
				     cairo_t *cr,
				     cairo_region_t *region)
{
	DorySelectionCanvasItem *self;
	double x1, y1, x2, y2;
	int cx1, cy1, cx2, cy2;
	double i2w_dx, i2w_dy;

	self = DORY_SELECTION_CANVAS_ITEM (item);
...
static double
dory_selection_canvas_item_point (EelCanvasItem *item,
				      double x,
				      double y,
				      int cx,
				      int cy,
...
static void
dory_selection_canvas_item_update (EelCanvasItem *item,
				       double i2w_dx,
				       double i2w_dy,
				       gint flags)
{
	...
	if (EEL_CANVAS_ITEM_CLASS (dory_selection_canvas_item_parent_class)->update)
		(* EEL_CANVAS_ITEM_CLASS (dory_selection_canvas_item_parent_class)->update) (item, i2w_dx, i2w_dy, flags);
...
static void
dory_selection_canvas_item_translate (EelCanvasItem *item,
					  double dx,
					  double dy)
{
	DorySelectionCanvasItem *self;
...
static void
dory_selection_canvas_item_bounds (EelCanvasItem *item,
				       double *x1,
				       double *y1,
				       double *x2,
				       double *y2)
...
{
	DorySelectionCanvasItem *self = user_data;
...
void
dory_selection_canvas_item_fade_out (DorySelectionCanvasItem *self,
...
static void
dory_selection_canvas_item_dispose (GObject *obj)
{
	DorySelectionCanvasItem *self = DORY_SELECTION_CANVAS_ITEM (obj);
	...
	G_OBJECT_CLASS (dory_selection_canvas_item_parent_class)->dispose (obj);
}

static void
do_set_fill (DorySelectionCanvasItem *self,
	     gboolean fill_set)
{
...
static void
do_set_outline (DorySelectionCanvasItem *self,
...
static void
dory_selection_canvas_item_set_property (GObject *object,
					     guint param_id,
					     const GValue *value,
					     GParamSpec *pspec)
{
	...
	self = DORY_SELECTION_CANVAS_ITEM (object);
...
static void
dory_selection_canvas_item_get_property (GObject *object,
					     guint param_id,
					     GValue *value,
					     GParamSpec *pspec)
{
...
static void
dory_selection_canvas_item_class_init (DorySelectionCanvasItemClass *klass)
{
	...
	gobject_class->set_property = dory_selection_canvas_item_set_property;
	gobject_class->get_property = dory_selection_canvas_item_get_property;
	gobject_class->dispose = dory_selection_canvas_item_dispose;

	item_class->draw = dory_selection_canvas_item_draw;
	item_class->point = dory_selection_canvas_item_point;
	item_class->update = dory_selection_canvas_item_update;
	item_class->bounds = dory_selection_canvas_item_bounds;
	item_class->translate = dory_selection_canvas_item_translate;
	...
	g_type_class_add_private (klass, sizeof (DorySelectionCanvasItemDetails));
}

static void
dory_selection_canvas_item_init (DorySelectionCanvasItem *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, DORY_TYPE_SELECTION_CANVAS_ITEM,
						  DorySelectionCanvasItemDetails);
}
