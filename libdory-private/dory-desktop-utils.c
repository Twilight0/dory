...
void
dory_desktop_utils_get_monitor_work_rect (gint num, GdkRectangle *rect)
{
#if GTK_CHECK_VERSION (3, 22, 0)
    ensure_display ();
...
void
dory_desktop_utils_get_monitor_geometry (gint num, GdkRectangle *rect)
{
#if GTK_CHECK_VERSION (3, 22, 0)
    ensure_display ();
    g_return_if_fail (num >= 0 && num < gdk_display_get_n_monitors (default_display));
...
gint
dory_desktop_utils_get_primary_monitor (void)
{
#if GTK_CHECK_VERSION (3, 22, 0)
    gint n_mon, i;
...
gint
dory_desktop_utils_get_monitor_for_widget (GtkWidget *widget)
{
    GdkWindow *window;
    GtkWidget *toplevel;
    gint monitor;
...
gint
dory_desktop_utils_get_num_monitors (void)
{
    ensure_screen ();
...
gboolean
dory_desktop_utils_get_monitor_cloned (gint monitor, gint x_primary)
{
...
gint
dory_desktop_utils_get_scale_factor (void)
{
    guint scale;
...
gboolean
dory_desktop_utils_configure_layer_shell (GtkWindow *window,
                                          gint       monitor_num,
                                          gboolean   keyboard_on_demand)
{
    if (eel_check_is_wayland ()) {
        ...
        gtk_layer_set_namespace (window, "dory-desktop");
