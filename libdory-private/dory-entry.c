...
#include <config.h>
#include "dory-entry.h"

#include <string.h>
#include "dory-global-preferences.h"
#include "dory-undo-signal-handlers.h"
#include <gdk/gdkkeysyms.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

struct DoryEntryDetails {
	gboolean user_edit;
...
static void dory_entry_editable_init (GtkEditableInterface *iface);

G_DEFINE_TYPE_WITH_CODE (DoryEntry, dory_entry, GTK_TYPE_ENTRY,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_EDITABLE,
						dory_entry_editable_init));

static GtkEditableInterface *parent_editable_interface = NULL;

static void
dory_entry_init (DoryEntry *entry)
{
	entry->details = g_new0 (DoryEntryDetails, 1);
	
	entry->details->user_edit = TRUE;

	dory_undo_set_up_dory_entry_for_undo (entry);
}

GtkWidget *
dory_entry_new (void)
{
	return gtk_widget_new (DORY_TYPE_ENTRY, NULL);
}

GtkWidget *
dory_entry_new_with_max_length (guint16 max)
{
	GtkWidget *widget;

	widget = gtk_widget_new (DORY_TYPE_ENTRY, NULL);
...
static void
dory_entry_finalize (GObject *object)
{
	DoryEntry *entry;

	entry = DORY_ENTRY (object);
	...
	G_OBJECT_CLASS (dory_entry_parent_class)->finalize (object);
}

static gboolean
dory_entry_key_press (GtkWidget *widget, GdkEventKey *event)
{
	DoryEntry *entry;
	...
	result = GTK_WIDGET_CLASS (dory_entry_parent_class)->key_press_event (widget, event);
...
static gboolean
dory_entry_motion_notify (GtkWidget *widget, GdkEventMotion *event)
{
	int result;
	...
	result = GTK_WIDGET_CLASS (dory_entry_parent_class)->motion_notify_event (widget, event);
 ...
 **/
void
dory_entry_select_all (DoryEntry *entry)
{
	g_return_if_fail (DORY_IS_ENTRY (entry));
...
select_all_at_idle (gpointer callback_data)
{
	...
	entry = DORY_ENTRY (callback_data);

	dory_entry_select_all (entry);
 ...
 **/
void
dory_entry_select_all_at_idle (DoryEntry *entry)
{
	g_return_if_fail (DORY_IS_ENTRY (entry));
...
void
dory_entry_set_text (DoryEntry *entry, const gchar *text)
{
	g_return_if_fail (DORY_IS_ENTRY (entry));
...
static void
dory_entry_set_selection_bounds (GtkEditable *editable,
				     int start_pos,
				     int end_pos)
{
...
static gboolean
dory_entry_button_press (GtkWidget *widget,
			     GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (dory_entry_parent_class)->button_press_event (widget, event);
...
static gboolean
dory_entry_button_release (GtkWidget *widget,
			       GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (dory_entry_parent_class)->button_release_event (widget, event);
...
static void
dory_entry_insert_text (GtkEditable *editable, const gchar *text,
			    int length, int *position)
{
	DoryEntry *entry;

	entry = DORY_ENTRY(editable);
...
static void 
dory_entry_delete_text (GtkEditable *editable, int start_pos, int end_pos)
{
	DoryEntry *entry;
 ...
 * this with type-completion behavior in DoryLocationBar (see bug 5313).
 * There's a FIXME comment that seems to be about this same issue in
 * gtk+/gtkselection.c, gtk_selection_clear.
 */
static gboolean
dory_entry_selection_clear (GtkWidget *widget,
			        GdkEventSelection *event)
{
	...
	return GTK_WIDGET_CLASS (dory_entry_parent_class)->selection_clear_event (widget, event);
}

static void
dory_entry_editable_init (GtkEditableInterface *iface)
{
	parent_editable_interface = g_type_interface_peek_parent (iface);

	iface->insert_text = dory_entry_insert_text;
	iface->delete_text = dory_entry_delete_text;
	iface->set_selection_bounds = dory_entry_set_selection_bounds;
...
static void
dory_entry_class_init (DoryEntryClass *class)
{
	...
	widget_class->button_press_event = dory_entry_button_press;
	widget_class->button_release_event = dory_entry_button_release;
	widget_class->key_press_event = dory_entry_key_press;
	widget_class->motion_notify_event = dory_entry_motion_notify;
	widget_class->selection_clear_event = dory_entry_selection_clear;
	
	gobject_class->finalize = dory_entry_finalize;
	...
	signals[USER_CHANGED] = g_signal_new
		("user_changed",
		 ...
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (DoryEntryClass, user_changed),
		 NULL, NULL,
	...
	signals[SELECTION_CHANGED] = g_signal_new
		("selection_changed",
		 G_TYPE_FROM_CLASS (class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (DoryEntryClass, selection_changed),
		 NULL, NULL,
		 g_cclosure_marshal_VOID__VOID,
		 G_TYPE_NONE, 0);
}
...
void
dory_entry_set_special_tab_handling (DoryEntry *entry,
					 gboolean special_tab_handling)
{
	g_return_if_fail (DORY_IS_ENTRY (entry));
