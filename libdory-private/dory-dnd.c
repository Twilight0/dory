...
/* dory-dnd.c - Common Drag & drop handling code shared by the icon container
   and the list view.
...
void
dory_drag_init (DoryDragInfo     *drag_info,
...
void
dory_drag_finalize (DoryDragInfo *drag_info)
{
	gtk_target_list_unref (drag_info->target_list);
	dory_drag_destroy_selection_list (drag_info->selection_list);
...
/* Functions to deal with DoryDragSelectionItems.  */

DoryDragSelectionItem *
dory_drag_selection_item_new (void)
{
	return g_new0 (DoryDragSelectionItem, 1);
}

static void
drag_selection_item_destroy (DoryDragSelectionItem *item)
{
	g_free (item->uri);
	g_free (item);
}
...
void
dory_drag_destroy_selection_list (GList *list)
{
	GList *p;
...
char **
dory_drag_uri_array_from_selection_list (const GList *selection_list)
{
	GList *uri_list;
	char **uris;

	uri_list = dory_drag_uri_list_from_selection_list (selection_list);
	uris = dory_drag_uri_array_from_list (uri_list);
...
GList *
dory_drag_uri_list_from_selection_list (const GList *selection_list)
{
	DoryDragSelectionItem *selection_item;
	GList *uri_list;
	const GList *l;

	uri_list = NULL;
	for (l = selection_list; l != NULL; l = l->next) {
		selection_item = (DoryDragSelectionItem *) l->data;
...
char **
dory_drag_uri_array_from_list (const GList *uri_list)
{
	const GList *l;
	char **uris;
	int i;
...
GList *
dory_drag_uri_list_from_array (const char **uris)
{
	GList *uri_list;
	int i;
...
GList *
dory_drag_build_selection_list (GtkSelectionData *data)
{
	GList *result;
	const guchar *p, *oldp;
	int size;
...
static gboolean
dory_drag_file_local_internal (const char *target_uri_string,
...
gboolean
dory_drag_uris_local (const char *target_uri,
			  const GList *source_uri_list)
{
	/* must have at least one item */
	g_assert (source_uri_list);
	
	return dory_drag_file_local_internal (target_uri, source_uri_list->data);
}

gboolean
dory_drag_items_local (const char *target_uri_string,
			   const GList *selection_list)
{
	/* must have at least one item */
	g_assert (selection_list);

	return dory_drag_file_local_internal (target_uri_string,
						  ((DoryDragSelectionItem *)selection_list->data)->uri);
}

gboolean
dory_drag_items_in_trash (const GList *selection_list)
{
	 ...
	 * and currently dropped items can only be from the same container
	 */
	return eel_uri_is_trash (((DoryDragSelectionItem *)selection_list->data)->uri);
}

gboolean
dory_drag_items_on_desktop (const GList *selection_list)
{
	char *uri;
	 ...
	 * and currently dropped items can only be from the same container
	 */
	uri = ((DoryDragSelectionItem *)selection_list->data)->uri;
	if (eel_uri_is_desktop (uri)) {
		return TRUE;
	}
	
	desktop = dory_get_desktop_location ();
...
GdkDragAction
dory_drag_default_drop_action_for_netscape_url (GdkDragContext *context)
{
...
static gboolean
check_same_fs (DoryFile    *target_file,
               DoryFile    *source_file,
               const gchar *source_fs_for_desktop)
{
    ...
    if (target_file != NULL && source_file != NULL) {
        source_id = dory_file_get_filesystem_id (source_file);
        target_id = dory_file_get_filesystem_id (target_file);
...
source_is_deletable (GFile *file)
{
	...
	/* if there's no a cached DoryFile, it returns NULL */
	naut_file = dory_file_get_existing (file);
	if (naut_file == NULL) {
		return FALSE;
	}
	
	ret = dory_file_can_delete (naut_file);
	dory_file_unref (naut_file);
...
uri_contains_desktop (const gchar *uri)
{
    ...
    real_desktop_uri = dory_get_desktop_directory_uri ();
...
void
dory_drag_default_drop_action_for_icons (GdkDragContext *context,
                                         ...
                                         gboolean       *can_delete_source)
{
	...
	DoryFile *dropped_file, *target_file;
    ...
    dropped_uri = ((DoryDragSelectionItem *)items->data)->uri;
    ...
    dropped_file = dory_file_get_existing_by_uri (dropped_uri);

    /* To/from desktop preparation - since we are separate processes, we don't have the full filesystem
     * info on the source and destination - we only have the destination info.  Creating a DoryFile for it
	...
	target_file = dory_file_get_existing_by_uri (target_uri_string);
	...
	if (eel_uri_is_trash (target_uri_string)) {
		...
		dory_file_unref (dropped_file);
		dory_file_unref (target_file);
		return;

	} else if (dropped_file != NULL && dory_file_is_launcher (dropped_file)) {
		if (actions & GDK_ACTION_MOVE) {
			*action = GDK_ACTION_MOVE;
		}
		dory_file_unref (dropped_file);
		...
		target = dory_get_desktop_location ();

		dory_file_unref (target_file);
		target_file = dory_file_get (target);
	...
	} else if (target_file != NULL && dory_file_is_archive (target_file)) {
		*action = GDK_ACTION_COPY;
...
GdkDragAction
dory_drag_default_drop_action_for_uri_list (GdkDragContext *context,
						const char *target_uri_string)
{
...
gboolean
dory_drag_drag_data_get (GtkWidget *widget,
...
GdkDragAction
dory_drag_drop_action_ask (GtkWidget *widget,
			       GdkDragAction actions)
{
	GtkWidget *menu;
	GtkWidget *menu_item;
...
gboolean
dory_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
...
void
dory_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
...
void
dory_drag_autoscroll_start (DoryDragInfo *drag_info,
				GtkWidget        *widget,
				GSourceFunc       callback,
				gpointer          user_data)
{
	if (dory_drag_autoscroll_in_scroll_region (widget)) {
...
void
dory_drag_autoscroll_stop (DoryDragInfo *drag_info)
{
	if (drag_info->auto_scroll_timeout_id != 0) {
		g_source_remove (drag_info->auto_scroll_timeout_id);
		drag_info->auto_scroll_timeout_id = 0;
...
gboolean
dory_drag_selection_includes_special_link (GList *selection_list)
{
	GList *node;
	char *uri;

	for (node = selection_list; node != NULL; node = node->next) {
		uri = ((DoryDragSelectionItem *) node->data)->uri;
