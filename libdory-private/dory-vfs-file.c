/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-vfs-file.c: Subclass of DoryFile to help implement the
...
G_DEFINE_TYPE (DoryVFSFile, dory_vfs_file, DORY_TYPE_FILE);

static void
vfs_file_monitor_add (DoryFile *file,
...
static void
vfs_file_monitor_remove (DoryFile *file,
			 gconstpointer client)
{
...
static void
vfs_file_call_when_ready (DoryFile *file,
...
static void
vfs_file_cancel_call_when_ready (DoryFile *file,
				 DoryFileCallback callback,
				 gpointer callback_data)
{
...
static gboolean
vfs_file_check_if_ready (DoryFile *file,
			 DoryFileAttributes file_attributes)
{
				...
				gpointer callback_data)
{
	...
	if (new_info != NULL) {
		if (dory_file_update_info (file, new_info)) {
...
static void
vfs_file_set_metadata (DoryFile           *file,
		       const char             *key,
		       const char             *value)
{
	GFileInfo *info;
...
static void
vfs_file_set_metadata_as_list (DoryFile           *file,
			       const char             *key,
			       char                  **value)
{
	GFile *location;
...
static gboolean
vfs_file_get_item_count (DoryFile *file,
			 guint *count,
...
vfs_file_get_deep_counts (DoryFile *file,
			  ...
			  goffset *total_size)
{
	...
	if (file->details->deep_counts_status != DORY_REQUEST_NOT_STARTED) {
		if (directory_count != NULL) {
	...
	type = dory_file_get_file_type (file);
	if (type == G_FILE_TYPE_UNKNOWN
	    || type == G_FILE_TYPE_DIRECTORY) {
		return DORY_REQUEST_NOT_STARTED;
	}
...
static gboolean
vfs_file_get_date (DoryFile *file,
...
static char *
vfs_file_get_where_string (DoryFile *file)
{
    ...
    if (!dory_file_is_in_recent (file) &&
        !dory_file_is_in_favorites (file)) {
        location = dory_file_ref (file);
    } else {
        activation_location = dory_file_get_activation_location (file);
        location = dory_file_get (activation_location);
        g_object_unref (activation_location);
    }

    where_string = dory_file_get_parent_uri_for_display (location);
			 ...
			 gpointer callback_data)
{
	...
	mounted_on = g_file_mount_mountable_finish (G_FILE (source_object),
						    res, &error);
	dory_file_operation_complete (op, mounted_on, error);
	if (mounted_on) {
			 ...
			 gpointer callback_data)
{
    ...
    dory_file_operation_complete (op, G_FILE (source_object), error);
	if (error) {
		g_error_free (error);
	}
}
...
static void
vfs_file_unmount (DoryFile                   *file,
		  GMountOperation                *mount_op,
		  GCancellable                   *cancellable,
		  DoryFileOperationCallback   callback,
		  gpointer                        callback_data)
...
static void
vfs_file_poll_for_media (DoryFile *file)
{
	DoryFileOperation *op;
	GFile *location;

	op = dory_file_operation_new (file, NULL, NULL);
...
static void
dory_vfs_file_init (DoryVFSFile *file)
{
}

static void
dory_vfs_file_class_init (DoryVFSFileClass *klass)
{
	DoryFileClass *file_class = DORY_FILE_CLASS (klass);
