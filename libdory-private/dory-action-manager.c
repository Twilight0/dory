...
} DoryActionManagerPrivate;

struct _DoryActionManager 
{
    GObject parent_instance;
};

G_DEFINE_TYPE_WITH_PRIVATE (DoryActionManager, dory_action_manager, G_TYPE_OBJECT)

static void     refresh_actions                 (DoryActionManager *action_manager, const gchar *directory);
static void     queue_refresh_actions           (DoryActionManager *action_manager, const gchar *directory);
static void     add_action_to_action_list       (DoryActionManager *action_manager, const gchar *path);
                 ...
                 gpointer           user_data)
{
    ...
    action_manager = DORY_ACTION_MANAGER (user_data);
...
{
    g_return_if_fail (DORY_IS_ACTION_MANAGER (user_data));

    DoryActionManager *action_manager = DORY_ACTION_MANAGER (user_data);
...
static void
add_directory_to_actions_directory_list (DoryActionManager *action_manager,
                                         const gchar       *path)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
...
static void
process_directory_actions (DoryActionManager *action_manager,
                           const gchar       *directory)
{
    ...
    disabled_actions = g_settings_get_strv (dory_plugin_preferences, DORY_PLUGIN_PREFERENCES_DISABLED_ACTIONS);
...
static void
set_up_actions_directories (DoryActionManager *action_manager)
{
    gchar *path;
    gchar **data_dirs;
    guint i;
...
static char *
create_action_name (DoryActionManager *manager,
                    const char        *uri)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (manager);
    GString *s;
...
static void
on_action_condition_changed (DoryActionManager *action_manager)
{
...
static void
add_action_to_action_list (DoryActionManager *action_manager,
                           const gchar       *path)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    ...
    action = dory_action_new (action_name, path);
...
static void
void_actions_for_directory (DoryActionManager *action_manager,
                            const gchar       *directory)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    ...
    for (l = priv->actions; l != NULL; l = l->next) {
        DoryAction *action = DORY_ACTION (l->data);
...
static void
reload_actions_layout (DoryActionManager *action_manager)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    GFile *file;
    GFileInfo *info;
...
static void
dory_config_dir_changed (GFileMonitor      *monitor,
                         GFile             *file,
                         GFile             *other_file,
                         GFileMonitorEvent  event_type,
                         gpointer           user_data)
...
static void
monitor_dory_config_dir (DoryActionManager *action_manager)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    GFile *file;
    GFileMonitor *monitor;

    g_autofree gchar *path = g_build_filename (g_get_user_config_dir (), "dory", NULL);

    file = g_file_new_for_path (path);
    monitor = g_file_monitor_directory (file, G_FILE_MONITOR_WATCH_MOVES, NULL, NULL);
    g_object_unref (file);
    g_signal_connect (monitor, "changed", G_CALLBACK (dory_config_dir_changed), action_manager);
    priv->dir_monitors = g_list_prepend (priv->dir_monitors, monitor);
}
...
{
    DoryAction *action_a = (DoryAction *) a;
    DoryAction *action_b = (DoryAction *) b;
...
static void
queue_refresh_actions (DoryActionManager *action_manager,
                       const gchar       *directory)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
...
static void
refresh_actions (DoryActionManager *action_manager,
                 const gchar       *directory)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
...
static void
dory_action_manager_init (DoryActionManager *action_manager)
{
}

void
dory_action_manager_constructed (GObject *object)
{
    G_OBJECT_CLASS (dory_action_manager_parent_class)->constructed (object);

    DoryActionManager *action_manager = DORY_ACTION_MANAGER (object);
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    ...
    monitor_dory_config_dir (action_manager);
    reload_actions_layout (action_manager);
    refresh_actions (action_manager, NULL);

    g_signal_connect (dory_plugin_preferences,
                      "changed::" DORY_PLUGIN_PREFERENCES_DISABLED_ACTIONS,
                      G_CALLBACK (plugin_prefs_changed), action_manager);
}

DoryActionManager *
dory_action_manager_new (void)
{
    return g_object_new (DORY_TYPE_ACTION_MANAGER, NULL);
}

static void
dory_action_manager_dispose (GObject *object)
{
    DoryActionManager *action_manager = DORY_ACTION_MANAGER (object);
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    ...
    g_clear_object (&priv->json_parser);
    g_signal_handlers_disconnect_by_func (dory_plugin_preferences, G_CALLBACK (plugin_prefs_changed), action_manager);

    G_OBJECT_CLASS (dory_action_manager_parent_class)->dispose (object);
}

static void
dory_action_manager_finalize (GObject *object)
{
    DoryActionManager *action_manager = DORY_ACTION_MANAGER (object);
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    ...
    G_OBJECT_CLASS (dory_action_manager_parent_class)->finalize (object);
}

static void
dory_action_manager_class_init (DoryActionManagerClass *klass)
{
    GObjectClass         *object_class = G_OBJECT_CLASS(klass);
    object_class->finalize = dory_action_manager_finalize;
    object_class->dispose = dory_action_manager_dispose;
    object_class->constructed = dory_action_manager_constructed;

    signals[CHANGED] = g_signal_new ("changed",
                                     DORY_TYPE_ACTION_MANAGER,
                                     G_SIGNAL_RUN_LAST,
                                     0,
                                     NULL, NULL, NULL,
                                     G_TYPE_NONE, 0);
...
GList *
dory_action_manager_list_actions (DoryActionManager *action_manager)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
...
DoryAction *
dory_action_manager_get_action (DoryActionManager *action_manager,
                                const gchar       *uuid)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
    DoryAction *action;
...
gchar *
dory_action_manager_get_system_directory_path (const gchar *data_dir)
{
    g_autofree gchar *dory_path = NULL;
    g_autofree gchar *target = NULL;

    dory_path = g_build_filename (data_dir, "dory", NULL);

    // For symbolic links, we try to figure out its actual path
    // to prevent possible duplicate right-click menu items.
    target = g_file_read_link (dory_path, NULL);

    if (target) {
        return g_build_filename (target, "actions", NULL);
    } else {
        return g_build_filename (dory_path, "actions", NULL);
    }
}

gchar *
dory_action_manager_get_user_directory_path (void)
{
    return g_build_filename (g_get_user_data_dir (), "dory", "actions", NULL);
}
...
typedef struct
{
    ...
    DoryActionManagerIterFunc         func;
    gpointer                          user_data;
} ActionsIterData;
            ...
            const gchar               *path)
{
    ...
    if (g_strcmp0 (type, "action") == 0) {
        DoryAction *action;
        g_autofree gchar *lookup_uuid = dory_make_action_uuid_for_path (uuid);

        action = dory_action_manager_get_action (idata->action_manager, lookup_uuid);
        ...
        if (user_label != NULL) {
            dory_action_override_label (action, user_label);
        }

        if (user_icon != NULL) {
            dory_action_override_icon (action, user_icon);
        }
...
static gboolean
iter_actions (DoryActionManager  *action_manager,
...
static JsonReader *
get_layout_reader (DoryActionManager *action_manager)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
...
void
dory_action_manager_iterate_actions (DoryActionManager                *action_manager,
                                     DoryActionManagerIterFunc         func,
                                     gpointer                          user_data)
{
    DoryActionManagerPrivate *priv = dory_action_manager_get_instance_private (action_manager);
...
void
dory_action_manager_add_action_ui (DoryActionManager   *manager,
...
void
dory_action_manager_update_action_states (DoryActionManager *action_manager,
                                          GtkActionGroup    *action_group,
                                          GList             *selection,
                                          DoryFile          *parent,
                                          gboolean           for_places,
                                          ...
                                          GtkWindow         *window)
{
    ...
    for (l = actions; l != NULL; l = l->next) {
        ...
        if (for_accelerators && !DORY_ACTION (l->data)->has_accel) {
            DEBUG ("Skipping '%s' (no accelerator assigned to this action)", DORY_ACTION (l->data)->uuid);
            continue;
        }

        dory_action_update_display_state (DORY_ACTION (l->data), selection, parent, for_places, window);
    }
