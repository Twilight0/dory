/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-directory-async.c: Dory directory model state machine.
...
struct DirectoryLoadState {
	...
	DoryFile *load_directory_file;
	int load_file_count;
};

struct MimeListState {
	DoryDirectory *directory;
	DoryFile *mime_list_file;
	GCancellable *cancellable;
	GFileEnumerator *enumerator;
...
typedef struct {
	DoryFile *file; /* Which file, NULL means all. */
	union {
		DoryDirectoryCallback directory;
		DoryFileCallback file;
	} callback;
...
typedef gboolean (* FileCheck) (DoryFile *);
...
static void     deep_count_load                               (DeepCountState         *state,
							       GFile                  *location);
static gboolean request_is_satisfied                          (DoryDirectory      *directory,
							       DoryFile           *file,
							       Request                 request);
static void     cancel_loading_attributes                     (DoryDirectory      *directory,
							       DoryFileAttributes  file_attributes);
static void     add_all_files_to_work_queue                   (DoryDirectory      *directory);
static void     link_info_done                                (DoryDirectory      *directory,
							       DoryFile           *file,
							       const char             *uri,
							       const char             *name,
							       GIcon                  *icon,
							       ...
							       gboolean                is_foreign);
static void     move_file_to_low_priority_queue               (DoryDirectory      *directory,
							       DoryFile           *file);
static void     move_file_to_extension_queue                  (DoryDirectory      *directory,
							       DoryFile           *file);
static void     dory_directory_invalidate_file_attributes (DoryDirectory      *directory,
							       DoryFileAttributes  file_attributes);

/* Some helpers for case-insensitive strings.
 * Move to dory-glib-extensions?
 */
...
#if 0
static void
dory_directory_verify_request_counts (DoryDirectory *directory)
{
	GList *l;
 ...
 */
static gboolean
async_job_start (DoryDirectory *directory,
		 const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
/* End a job. */
static void
async_job_end (DoryDirectory *directory,
	       const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
async_job_wake_up (void)
{
	...
	while (async_job_count < MAX_ASYNC_JOBS) {
		...
		dory_directory_async_state_changed
			(DORY_DIRECTORY (value));
	}
	already_waking_up = FALSE;
}

static void
directory_count_cancel (DoryDirectory *directory)
{
...
static void
deep_count_cancel (DoryDirectory *directory)
{
	if (directory->details->deep_count_in_progress != NULL) {
		g_assert (DORY_IS_FILE (directory->details->deep_count_file));
		
		g_cancellable_cancel (directory->details->deep_count_in_progress->cancellable);

		directory->details->deep_count_file->details->deep_counts_status = DORY_REQUEST_NOT_STARTED;
...
static void
mime_list_cancel (DoryDirectory *directory)
{
...
static void
link_info_cancel (DoryDirectory *directory)
{
...
static void
thumbnail_cancel (DoryDirectory *directory)
{
...
static void
mount_cancel (DoryDirectory *directory)
{
...
static void
file_info_cancel (DoryDirectory *directory)
{
...
static void
favorite_check_cancel (DoryDirectory *directory)
{
...
static void
new_files_cancel (DoryDirectory *directory)
{
	GList *l;
	NewFilesState *state;
...
static GList *
find_monitor (DoryDirectory *directory,
	      DoryFile *file,
	      gconstpointer client)
{
	Monitor monitor;
...
static void
remove_monitor_link (DoryDirectory *directory,
		     GList *link)
{
	Monitor *monitor;
...
static void
remove_monitor (DoryDirectory *directory,
		DoryFile *file,
		gconstpointer client)
{
...
Request
dory_directory_set_up_request (DoryFileAttributes file_attributes)
{
	Request request;

	request = 0;
	
	if ((file_attributes & DORY_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_DIRECTORY_COUNT);
	}
	
	if ((file_attributes & DORY_FILE_ATTRIBUTE_DEEP_COUNTS) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_DEEP_COUNT);
	}

	if ((file_attributes & DORY_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_MIME_LIST);
	}
	if ((file_attributes & DORY_FILE_ATTRIBUTE_INFO) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}
	if (file_attributes & DORY_FILE_ATTRIBUTE_LINK_INFO) {
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
		REQUEST_SET_TYPE (request, REQUEST_LINK_INFO);
	}

	if ((file_attributes & DORY_FILE_ATTRIBUTE_EXTENSION_INFO) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_EXTENSION_INFO);
	}

	if (file_attributes & DORY_FILE_ATTRIBUTE_THUMBNAIL) {
		REQUEST_SET_TYPE (request, REQUEST_THUMBNAIL);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}

	if (file_attributes & DORY_FILE_ATTRIBUTE_MOUNT) {
		REQUEST_SET_TYPE (request, REQUEST_MOUNT);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}

	if (file_attributes & DORY_FILE_ATTRIBUTE_FILESYSTEM_INFO) {
		REQUEST_SET_TYPE (request, REQUEST_FILESYSTEM_INFO);
	}

    if (file_attributes & DORY_FILE_ATTRIBUTE_FAVORITE_CHECK) {
...
static void
mime_db_changed_callback (GObject *ignore, DoryDirectory *dir)
{
	DoryFileAttributes attrs;

	g_assert (dir != NULL);
	g_assert (dir->details != NULL);

	attrs = DORY_FILE_ATTRIBUTE_INFO |
		DORY_FILE_ATTRIBUTE_LINK_INFO |
		DORY_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;

	dory_directory_force_reload_internal (dir, attrs);
}

void
dory_directory_monitor_add_internal (DoryDirectory *directory,
					 DoryFile *file,
					 gconstpointer client,
					 ...
					 gpointer callback_data)
{
	...
	monitor->client = client;
	monitor->request = dory_directory_set_up_request (file_attributes);
	 ...
	 * dory almost always shows the whole directory anyway, and
	 * it allows us to avoid one file monitor per file in a directory.
	 */
	if (directory->details->monitor == NULL) {
		directory->details->monitor = dory_monitor_directory (directory->details->location);
	}
...
static void
set_file_unconfirmed (DoryFile *file, gboolean unconfirmed)
{
	DoryDirectory *directory;
...
{
	show_hidden_files = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_HIDDEN_FILES);
}

static gboolean
should_skip_file (DoryDirectory *directory, GFileInfo *info)
{
...
static void
process_files_changed_while_being_added (DoryDirectory *directory)
{
    if (directory->details->new_files_in_progress_changes == NULL)
    {
        return;
    ...
    dory_directory_notify_files_changed (directory->details->new_files_in_progress_changes);
...
dequeue_pending_idle_callback (gpointer callback_data)
{
	...
	directory = DORY_DIRECTORY (callback_data);
	...
	/* Build a list of DoryFile objects. */
	for (node = pending_file_info; node != NULL; node = node->next) {
		...
		file = dory_directory_find_file_by_name (directory, name);
		if (file != NULL) {
			...
			if (!file->details->is_added) {
				 ...
				 * This can happen if someone called dory_file_get_by_uri()
				 * on a file in the folder before the add signal was
				 * emitted */
				dory_file_ref (file);
				file->details->is_added = TRUE;
				added_files = g_list_prepend (added_files, file);
			} else if (dory_file_update_info (file, file_info)) {
				/* File changed, notify about the change. */
				dory_file_ref (file);
				changed_files = g_list_prepend (changed_files, file);
			}
		} else {
			/* new file, create a dory file object and add it to the list */
			file = dory_file_new_from_info (directory, file_info);
	...
	dory_directory_emit_change_signals (directory, changed_files);
	dory_file_list_free (changed_files);
	dory_directory_emit_files_added (directory, added_files);
	dory_file_list_free (added_files);

	if (directory->details->directory_loaded &&
	    !directory->details->directory_loaded_sent_notification) {
		/* Send the done_loading signal. */
		dory_directory_emit_done_loading (directory);
...
void
dory_directory_schedule_dequeue_pending (DoryDirectory *directory)
{
...
static void
directory_load_one (DoryDirectory *directory,
		    GFileInfo *info)
{
	if (info == NULL) {
		return;
	...
	dory_directory_schedule_dequeue_pending (directory);
}

static void
directory_load_cancel (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
file_list_cancel (DoryDirectory *directory)
{
...
static void
directory_load_done (DoryDirectory *directory,
		     GError *error)
{
	GList *node;
	...
	if (error != NULL) {
		...
		for (node = directory->details->file_list;
			...
			set_file_unconfirmed (DORY_FILE (node->data), FALSE);
		}

		dory_directory_emit_load_error (directory, error);
	}
...
void
dory_directory_monitor_remove_internal (DoryDirectory *directory,
					    DoryFile *file,
					    gconstpointer client)
{
	g_assert (DORY_IS_DIRECTORY (directory));
	g_assert (file == NULL || DORY_IS_FILE (file));
...
FileMonitors *
dory_directory_remove_file_monitors (DoryDirectory *directory,
					 DoryFile *file)
{
...
void
dory_directory_add_file_monitors (DoryDirectory *directory,
				      DoryFile *file,
...
static void
ready_callback_call (DoryDirectory *directory,
...
void
dory_directory_call_when_ready_internal (DoryDirectory *directory,
					     DoryFile *file,
					     DoryFileAttributes file_attributes,
					     ...
					     gpointer callback_data)
{
	...
	g_assert (directory == NULL || DORY_IS_DIRECTORY (directory));
	g_assert (file == NULL || DORY_IS_FILE (file));
	...
	callback.request = dory_directory_set_up_request (file_attributes);
...
gboolean      
dory_directory_check_if_ready_internal (DoryDirectory *directory,
					    DoryFile *file,
					    DoryFileAttributes file_attributes)
{
	Request request;
...
static void
remove_callback_link_keep_data (DoryDirectory *directory,
				GList *link)
{
	ReadyCallback *callback;
...
static void
remove_callback_link (DoryDirectory *directory,
		      GList *link)
{
	ReadyCallback *callback;
...
void
dory_directory_cancel_callback_internal (DoryDirectory *directory,
					     DoryFile *file,
					     DoryDirectoryCallback directory_callback,
					     DoryFileCallback file_callback,
					     gpointer callback_data)
		    ...
		    gpointer user_data)
{
	...
	directory = dory_directory_ref (state->directory);
...
void
dory_directory_get_info_for_new_files (DoryDirectory *directory,
					   GList *location_list)
{
	NewFilesState *state;
	GFile *location;
...
void
dory_async_destroying_file (DoryFile *file)
{
	DoryDirectory *directory;
	gboolean changed;
	GList *node, *next;
...
static gboolean
lacks_directory_count (DoryFile *file)
{
	return !file->details->directory_count_is_up_to_date
		&& dory_file_should_show_directory_item_count (file);
}

static gboolean
should_get_directory_count_now (DoryFile *file)
{
...
static gboolean
lacks_favorite_check (DoryFile *file)
{
...
static gboolean
lacks_filesystem_info (DoryFile *file)
{
	return dory_file_is_directory (file) && !file->details->filesystem_info_is_up_to_date;
}

static gboolean
lacks_deep_count (DoryFile *file)
{
	return file->details->deep_counts_status != DORY_REQUEST_DONE;
}

static gboolean
lacks_mime_list (DoryFile *file)
{
	return !file->details->mime_list_is_up_to_date;
}

static gboolean
should_get_mime_list (DoryFile *file)
{
...
static gboolean
lacks_extension_info (DoryFile *file)
{
    return file->details->load_deferred_attrs > DORY_FILE_LOAD_DEFERRED_ATTRS_NO &&
        file->details->pending_info_providers != NULL;
}

static gboolean
lacks_thumbnail (DoryFile *file)
{
    return file->details->load_deferred_attrs > DORY_FILE_LOAD_DEFERRED_ATTRS_NO &&
...
static gboolean
has_problem (DoryDirectory *directory, DoryFile *file, FileCheck problem)
{
	GList *node;
...
static gboolean
request_is_satisfied (DoryDirectory *directory,
		      DoryFile *file,
		      Request request)
{
...
static void
schedule_call_ready_callbacks (DoryDirectory *directory)
{
 ...
 * before then */
static gboolean
call_ready_callbacks (DoryDirectory *directory)
{
	gboolean found_any;
	GList *node, *next;
	ReadyCallback *callback;
...
gboolean
dory_directory_has_active_request_for_file (DoryDirectory *directory,
						DoryFile *file)
{
	GList *node;
	ReadyCallback *callback;
...
gboolean
dory_directory_is_anyone_monitoring_file_list (DoryDirectory *directory)
{
...
gboolean
dory_directory_is_file_list_monitored (DoryDirectory *directory) 
{
	return directory->details->file_list_monitored;
}

static void
mark_all_files_unconfirmed (DoryDirectory *directory)
{
	GList *node;
	DoryFile *file;
...
directory_load_state_free (DirectoryLoadState *state)
{
	...
	dory_file_unref (state->load_directory_file);
	g_object_unref (state->cancellable);
	g_free (state);
}
...
static void
start_monitoring_file_list (DoryDirectory *directory)
{
	DirectoryLoadState *state;
	...
	if (!directory->details->file_list_monitored) {
		...
		dory_file_list_ref (directory->details->file_list);
	}
...
void
dory_directory_stop_monitoring_file_list (DoryDirectory *directory)
{
	if (!directory->details->file_list_monitored) {
		g_assert (directory->details->directory_load_in_progress == NULL);
		return;
	...
	dory_file_list_unref (directory->details->file_list);
	directory->details->directory_loaded = FALSE;
}

static void
file_list_start_or_stop (DoryDirectory *directory)
{
	if (dory_directory_is_anyone_monitoring_file_list (directory)) {
		start_monitoring_file_list (directory);
	} else {
		dory_directory_stop_monitoring_file_list (directory);
	}
}

void
dory_file_invalidate_count_and_mime_list (DoryFile *file)
{
	DoryFileAttributes attributes;
	
	attributes = DORY_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		DORY_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;
	
	dory_file_invalidate_attributes (file, attributes);
}
 ...
 */
void
dory_directory_invalidate_count_and_mime_list (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
dory_directory_invalidate_file_attributes (DoryDirectory      *directory,
					       DoryFileAttributes  file_attributes)
{
	GList *node;
	...
	for (node = directory->details->file_list; node != NULL; node = node->next) {
		dory_file_invalidate_attributes_internal (DORY_FILE (node->data),
							      file_attributes);
	}

	if (directory->details->as_file != NULL) {
		dory_file_invalidate_attributes_internal (directory->details->as_file,
							      file_attributes);
	}
}

void
dory_directory_force_reload_internal (DoryDirectory     *directory,
					  DoryFileAttributes file_attributes)
{
	/* invalidate attributes that are getting reloaded for all files */
	dory_directory_invalidate_file_attributes (directory, file_attributes);
...
static void
directory_count_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
count_children_done (DoryDirectory *directory,
		     DoryFile *count_file,
		     gboolean succeeded,
		     int count)
{
	g_assert (DORY_IS_FILE (count_file));
...
directory_count_state_free (DirectoryCountState *state)
{
	...
	g_object_unref (state->cancellable);
	dory_directory_unref (state->directory);
	g_free (state);
}
...
static void
directory_count_start (DoryDirectory *directory,
		       DoryFile *file,
		       gboolean *doing_io)
{
	DirectoryCountState *state;
	...
	*doing_io = TRUE;
	...
	state->directory = dory_directory_ref (directory);
	state->cancellable = g_cancellable_new ();
...
deep_count_next_dir (DeepCountState *state)
{
	...
	if (state->deep_count_subdirectories != NULL) {
		...
		file->details->deep_counts_status = DORY_REQUEST_DONE;
		directory->details->deep_count_file = NULL;
		directory->details->deep_count_in_progress = NULL;
	...
	dory_file_updated_deep_count_in_progress (file);
...
static void
deep_count_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
deep_count_start (DoryDirectory *directory,
		  DoryFile *file,
		  gboolean *doing_io)
{
	GFile *location;
	...
	*doing_io = TRUE;
	...
	/* Start counting. */
	file->details->deep_counts_status = DORY_REQUEST_IN_PROGRESS;
...
static void
mime_list_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
mime_list_start (DoryDirectory *directory,
		 DoryFile *file,
		 gboolean *doing_io)
{
	MimeListState *state;
		     ...
		     gpointer user_data)
{
	...
	g_assert (DORY_IS_FILE (get_info_file));
	...
	if (info == NULL) {
		if (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_FOUND) {
			/* mark file as gone */
			dory_file_mark_gone (get_info_file);
		}
		get_info_file->details->file_info_is_up_to_date = TRUE;
		dory_file_clear_info (get_info_file);
		get_info_file->details->get_info_failed = TRUE;
		get_info_file->details->get_info_error = error;
	} else {
		dory_file_update_info (get_info_file, info);
		g_object_unref (info);
	}

	dory_file_changed (get_info_file);
	dory_file_unref (get_info_file);
...
static void
file_info_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
file_info_start (DoryDirectory *directory,
		 DoryFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
favorite_check_callback (gpointer user_data)
{
    ...
    g_assert (DORY_IS_FILE (favorite_check_file));
     ...
     * least long enough to send the change notification. 
     */
    dory_file_ref (favorite_check_file);

    gchar *uri;
    gboolean is_favorite;

    uri = dory_file_get_uri (favorite_check_file);
    ...
    if (!dory_file_is_in_favorites (favorite_check_file) && 
        is_favorite != dory_file_get_is_favorite (favorite_check_file)) {
        dory_file_set_is_favorite (favorite_check_file, is_favorite);
        dory_file_changed (favorite_check_file);
    }

    g_free (uri);
    dory_file_unref (favorite_check_file);
...
static void
favorite_check_stop (DoryDirectory *directory)
{
    DoryFile *file;
...
static void
favorite_check_start (DoryDirectory *directory,
                      DoryFile *file,
                      gboolean *doing_io)
{
    FavoriteCheckState *state;
		 ...
		 gboolean is_launcher)
{
	...
	if (dory_file_is_local (file)) {
		location = dory_file_get_location (file);
		res = dory_is_in_system_dir (location);
		g_object_unref (location);
	}
...
static void
link_info_done (DoryDirectory *directory,
		DoryFile *file,
		const char *uri,
		const char *name, 
		GIcon *icon,
		...
		gboolean is_foreign)
{
	...
	if (is_trusted) {
		dory_file_set_display_name (file, name, name, TRUE);
	} else {
		dory_file_set_display_name (file, NULL, NULL, TRUE);
	}
...
static void
link_info_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
link_info_got_data (DoryDirectory *directory,
		    DoryFile *file,
		    gboolean result,
		    goffset bytes_read,
		    char *file_contents)
{
	...
	/* Handle the case where we read the Dory link. */
	if (result) {
		link_uri = dory_file_get_uri (file);
		dory_link_get_link_info_given_file_contents (file_contents, bytes_read, link_uri,
...
static void
link_info_dory_link_read_callback (GObject *source_object,
				       GAsyncResult *res,
				       gpointer user_data)
{
...
static void
link_info_start (DoryDirectory *directory,
		 DoryFile *file,
		 gboolean *doing_io)
{
	GFile *location;
	gboolean dory_style_link;
	LinkInfoReadState *state;
	...
	*doing_io = TRUE;
	...
	dory_style_link = dory_file_is_dory_link (file);
	location = dory_file_get_location (file);
	...
	if (!dory_style_link) {
		...
		g_file_load_contents_async (location,
					    state->cancellable,
					    link_info_dory_link_read_callback,
					    state);
	}
	g_object_unref (location);
}
...
static void
thumbnail_done (DoryDirectory *directory,
		DoryFile *file,
		GdkPixbuf *pixbuf,
		gboolean tried_original)
{
...
static void
thumbnail_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
thumbnail_got_pixbuf (DoryDirectory *directory,
		      DoryFile *file,
		      GdkPixbuf *pixbuf,
		      gboolean tried_original)
{
				...
				gpointer user_data)
{
	...
	/* cf. dory_file_get_icon() */
	max_thumbnail_size = DORY_ICON_SIZE_LARGEST * cached_thumbnail_size / DORY_ICON_SIZE_STANDARD;
...
static void
thumbnail_start (DoryDirectory *directory,
		 DoryFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
static void
mount_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
got_mount (MountState *state, GMount *mount)
{
	...
	file = dory_file_ref (state->file);

	file->details->mount_is_up_to_date = TRUE;
	dory_file_set_mount (file, mount);
			       ...
			       gpointer user_data)
{
	...
	if (mount) {
		...
		location = dory_file_get_location (state->file);
...
static void
mount_start (DoryDirectory *directory,
	     DoryFile *file,
	     gboolean *doing_io)
{
	GFile *location;
	...
	*doing_io = TRUE;
	...
	if (file->details->type == G_FILE_TYPE_MOUNTABLE) {
		...
		mount = NULL;
		target = dory_file_get_activation_location (file);
		if (target != NULL) {
...
static void
filesystem_info_cancel (DoryDirectory *directory)
{
	if (directory->details->filesystem_info_state != NULL) {
...
static void
filesystem_info_stop (DoryDirectory *directory)
{
	DoryFile *file;
...
static void
filesystem_info_start (DoryDirectory *directory,
		       DoryFile *file,
		       gboolean *doing_io)
{
	GFile *location;
...
static void
extension_info_cancel (DoryDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		if (directory->details->extension_info_idle > 0) {
			...
			dory_info_provider_cancel_update 
...
static void
extension_info_stop (DoryDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		DoryFile *file;
...
static void
finish_info_provider (DoryDirectory *directory,
		      DoryFile *file,
		      DoryInfoProvider *provider)
{
	...
	if (file->details->pending_info_providers == NULL) {
		dory_file_info_providers_done (file);
	}
...
info_provider_idle_callback (gpointer user_data)
{
	...
	if (response->handle != directory->details->extension_info_in_progress
		...
		g_warning ("Unexpected plugin response.  This probably indicates a bug in a Dory extension: handle=%p", response->handle);
	} else {
		DoryFile *file;
		async_job_end (directory, "extension info");
...
static void
info_provider_callback (DoryInfoProvider *provider,
			DoryOperationHandle *handle,
			DoryOperationResult result,
			gpointer user_data)
{
	...
	response->directory = DORY_DIRECTORY (user_data);
...
static void
extension_info_start (DoryDirectory *directory,
		      DoryFile *file,
		      gboolean *doing_io)
{
	DoryInfoProvider *provider;
	...
	*doing_io = TRUE;
	...
	result = dory_info_provider_update_file_info
		(provider, 
		 DORY_FILE_INFO (file), 
		 update_complete, 
		 &handle);

	if (result == DORY_OPERATION_COMPLETE ||
	    result == DORY_OPERATION_FAILED) {
...
static void
start_or_stop_io (DoryDirectory *directory)
{
	DoryFile *file;
	gboolean doing_io;
	...
	while (!dory_file_queue_is_empty (directory->details->high_priority_queue)) {
		file = dory_file_queue_head (directory->details->high_priority_queue);
	...
	while (!dory_file_queue_is_empty (directory->details->low_priority_queue)) {
		file = dory_file_queue_head (directory->details->low_priority_queue);
	...
	while (!dory_file_queue_is_empty (directory->details->extension_queue)) {
		file = dory_file_queue_head (directory->details->extension_queue);
 ...
 */
void
dory_directory_async_state_changed (DoryDirectory *directory)
{
...
void
dory_directory_cancel (DoryDirectory *directory)
{
...
static void
cancel_directory_count_for_file (DoryDirectory *directory,
				 DoryFile      *file)
{
	if (directory->details->count_in_progress != NULL &&
	    directory->details->count_in_progress->count_file == file) {
...
static void
cancel_deep_counts_for_file (DoryDirectory *directory,
			     DoryFile      *file)
{
	if (directory->details->deep_count_file == file) {
		deep_count_cancel (directory);
...
static void
cancel_mime_list_for_file (DoryDirectory *directory,
			   DoryFile      *file)
{
	if (directory->details->mime_list_in_progress != NULL &&
	    directory->details->mime_list_in_progress->mime_list_file == file) {
...
static void
cancel_file_info_for_file (DoryDirectory *directory,
			   DoryFile      *file)
{
	if (directory->details->get_info_file == file) {
		file_info_cancel (directory);
...
static void
cancel_favorite_check_for_file (DoryDirectory *directory,
                                DoryFile      *file)
{
    if (directory->details->favorite_check_file == file) {
        favorite_check_cancel (directory);
...
static void
cancel_thumbnail_for_file (DoryDirectory *directory,
			   DoryFile      *file)
{
	if (directory->details->thumbnail_state != NULL &&
	    directory->details->thumbnail_state->file == file) {
...
static void
cancel_mount_for_file (DoryDirectory *directory,
			   DoryFile      *file)
{
	if (directory->details->mount_state != NULL &&
	    directory->details->mount_state->file == file) {
...
static void
cancel_filesystem_info_for_file (DoryDirectory *directory,
				 DoryFile      *file)
{
	if (directory->details->filesystem_info_state != NULL &&
	    directory->details->filesystem_info_state->file == file) {
...
static void
cancel_link_info_for_file (DoryDirectory *directory,
			   DoryFile      *file)
{
	if (directory->details->link_info_read_state != NULL &&
	    directory->details->link_info_read_state->file == file) {
...
static void
cancel_loading_attributes (DoryDirectory *directory,
			   DoryFileAttributes file_attributes)
{
	Request request;
...
void
dory_directory_cancel_loading_file_attributes (DoryDirectory      *directory,
						   DoryFile           *file,
						   DoryFileAttributes  file_attributes)
{
	Request request;
...
void
dory_directory_add_file_to_work_queue (DoryDirectory *directory,
					   DoryFile *file)
{
	g_return_if_fail (file->details->directory == directory);

	dory_file_queue_enqueue (directory->details->high_priority_queue,
				     file);
}


static void
add_all_files_to_work_queue (DoryDirectory *directory)
{
	GList *node;
	DoryFile *file;
...
void
dory_directory_remove_file_from_work_queue (DoryDirectory *directory,
						DoryFile *file)
{
	dory_file_queue_remove (directory->details->high_priority_queue,
				    file);
	dory_file_queue_remove (directory->details->low_priority_queue,
				    file);
	dory_file_queue_remove (directory->details->extension_queue,
				    file);
}


static void
move_file_to_low_priority_queue (DoryDirectory *directory,
				 DoryFile *file)
{
	/* Must add before removing to avoid ref underflow */
	dory_file_queue_enqueue (directory->details->low_priority_queue,
				     file);
	dory_file_queue_remove (directory->details->high_priority_queue,
				    file);
}
...
static void
move_file_to_extension_queue (DoryDirectory *directory,
			      DoryFile *file)
{
	/* Must add before removing to avoid ref underflow */
	dory_file_queue_enqueue (directory->details->extension_queue,
				     file);
	dory_file_queue_remove (directory->details->low_priority_queue,
				    file);
}
