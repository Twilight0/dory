...
/* dory-icon-container.c - Icon container widget.
...
#include "dory-selection-canvas-item.h"
#include "dory-desktop-utils.h"
...
#define DEBUG_FLAG DORY_DEBUG_ICON_CONTAINER
...
/* Copied from DoryIconContainer */
#define DORY_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT 5

/* Copied from DoryFile */
...
} DoryIconContainerAccessiblePrivate;

static GType         dory_icon_container_accessible_get_type (void);

static void          preview_selected_items                         (DoryIconContainer *container);
static void          activate_selected_items                        (DoryIconContainer *container);
static void          activate_selected_items_alternate              (DoryIconContainer *container,
								     DoryIcon          *icon);
static void          compute_stretch                                (StretchState          *start,
								     StretchState          *current);
static DoryIcon *get_first_selected_icon                        (DoryIconContainer *container);
static DoryIcon *get_nth_selected_icon                          (DoryIconContainer *container,
								     int                    index);
static gboolean      has_multiple_selection                         (DoryIconContainer *container);
static gboolean      all_selected                                   (DoryIconContainer *container);
static gboolean      has_selection                                  (DoryIconContainer *container);
static void          icon_destroy                                   (DoryIconContainer *container,
								     DoryIcon          *icon);

static gboolean      is_renaming                                    (DoryIconContainer *container);
static gboolean      is_renaming_pending                            (DoryIconContainer *container);
static void          process_pending_icon_to_rename                 (DoryIconContainer *container);
...
static void          handle_vadjustment_changed                     (GtkAdjustment         *adjustment,
								     DoryIconContainer *container);
static GList *       dory_icon_container_get_selected_icons (DoryIconContainer *container);
static void          queue_update_visible_icons                 (DoryIconContainer *container, gint delay);
static void          reveal_icon                                    (DoryIconContainer *container,
								     DoryIcon *icon);

static void         text_ellipsis_limit_changed_container_callback  (gpointer callback_data);

static int compare_icons_horizontal (DoryIconContainer *container,
				     DoryIcon *icon_a,
				     DoryIcon *icon_b);

static int compare_icons_vertical (DoryIconContainer *container,
				   DoryIcon *icon_a,
				   DoryIcon *icon_b);

static void remove_search_entry_timeout (DoryIconContainer *container);

static gboolean handle_icon_slow_two_click (DoryIconContainer *container,
                                            DoryIcon *icon,
                                            GdkEventButton *event);

static void schedule_align_icons (DoryIconContainer *container);

static gpointer accessible_parent_class;

static GQuark accessible_private_data_quark = 0;

static const char *dory_icon_container_accessible_action_names[] = {
	"activate",
	"menu",
	NULL
};

static const char *dory_icon_container_accessible_action_descriptions[] = {
	"Activate selected items",
	"Popup context menu",
	NULL
};

G_DEFINE_TYPE (DoryIconContainer, dory_icon_container, EEL_TYPE_CANVAS);

/* The DoryIconContainer signals.  */
enum {
	ACTIVATE,
	ACTIVATE_ALTERNATE,
	ACTIVATE_PREVIEWER,
...
static void
tooltip_prefs_changed_callback (DoryIconContainer *container)
{
    container->details->show_desktop_tooltips = g_settings_get_boolean (dory_preferences,
                                                                        DORY_PREFERENCES_TOOLTIPS_DESKTOP);
    container->details->show_icon_view_tooltips = g_settings_get_boolean (dory_preferences,
                                                                          DORY_PREFERENCES_TOOLTIPS_ICON_VIEW);

    container->details->tooltip_flags = dory_global_preferences_get_tooltip_flags ();

    dory_icon_container_request_update_all (container);
}

/* Functions dealing with DoryIcons.  */

static gboolean
clicked_on_text (DoryIconContainer *container,
                          DoryIcon *icon,
                    GdkEventButton *event)
{
    if (icon == NULL)
    ...
    icon_rect = dory_icon_canvas_item_get_text_rectangle (icon->item, TRUE);
...
static gboolean
clicked_on_icon (DoryIconContainer *container,
                          DoryIcon *icon,
                    GdkEventButton *event)
{
    if (icon == NULL)
    ...
    icon_rect = dory_icon_canvas_item_get_icon_rectangle (icon->item);
    eel_canvas_window_to_world (EEL_CANVAS (container), event->x, event->y, &eventX, &eventY);
...
gboolean
dory_icon_container_icon_is_positioned (const DoryIcon *icon)
{
	return icon->x != ICON_UNPOSITIONED_VALUE && icon->y != ICON_UNPOSITIONED_VALUE;
}

static void
icon_get_size (DoryIconContainer *container,
	       DoryIcon *icon,
	       guint *size)
{
	if (size != NULL) {
		*size = MAX (dory_get_icon_size_for_zoom_level (container->details->zoom_level)
			       * icon->scale, DORY_ICON_SIZE_SMALLEST);
	}
}
 ...
 */
static void
icon_set_size (DoryIconContainer *container,
	       DoryIcon *icon,
	       guint icon_size,
	       gboolean snap,
	       gboolean update_position)
{
	...
	scale = (double) icon_size /
		dory_get_icon_size_for_zoom_level
		(container->details->zoom_level);
	dory_icon_container_move_icon (container, icon,
					   icon->x, icon->y,
					   scale, FALSE,
...
static void
emit_stretch_started (DoryIconContainer *container, DoryIcon *icon)
{
...
static void
emit_stretch_ended (DoryIconContainer *container, DoryIcon *icon)
{
	g_signal_emit (container,
...
static void
icon_toggle_selected (DoryIconContainer *container,
		      DoryIcon *icon)
{
	...
	if (icon == container->details->stretch_icon) {
		...
		dory_icon_canvas_item_set_show_stretch_handles (icon->item, FALSE);
		/* snap the icon if necessary */
		if (container->details->keep_aligned) {
			dory_icon_container_move_icon (container,
							   icon,
							   icon->x, icon->y,
							   icon->scale,
							   FALSE, TRUE, TRUE);
...
static gboolean
icon_set_selected (DoryIconContainer *container,
		   DoryIcon *icon,
		   gboolean select)
{
...
/* Utility functions for DoryIconContainer.  */

gboolean
dory_icon_container_scroll (DoryIconContainer *container,
...
static void
pending_icon_to_reveal_destroy_callback (DoryIconCanvasItem *item,
					 DoryIconContainer *container)
{
...
static DoryIcon*
get_pending_icon_to_reveal (DoryIconContainer *container)
{
	return container->details->pending_icon_to_reveal;
}

static void
set_pending_icon_to_reveal (DoryIconContainer *container, DoryIcon *icon)
{
	DoryIcon *old_icon;
...
static void
item_get_canvas_bounds (DoryIconContainer *container,
                        EelCanvasItem     *item,
                        EelIRect          *bounds,
                        gboolean           safety_pad)
{
...
static void
icon_get_row_and_column_bounds (DoryIconContainer *container,
				DoryIcon *icon,
				EelIRect *bounds,
				gboolean safety_pad)
{
...
static void
reveal_icon (DoryIconContainer *container,
	     DoryIcon *icon)
{
	...
	if (!dory_icon_container_icon_is_positioned (icon)) {
	...
	if (dory_icon_container_is_auto_layout (container)) {
...
static void
process_pending_icon_to_reveal (DoryIconContainer *container)
{
	DoryIcon *pending_icon_to_reveal;
...
keyboard_icon_reveal_timeout_callback (gpointer data)
{
	...
	container = DORY_ICON_CONTAINER (data);
...
static void
unschedule_keyboard_icon_reveal (DoryIconContainer *container)
{
...
static void
schedule_keyboard_icon_reveal (DoryIconContainer *container,
			       DoryIcon *icon)
{
	DoryIconContainerDetails *details;
...
static void
clear_keyboard_focus (DoryIconContainer *container)
{
...
inline static void
emit_atk_focus_tracker_notify (DoryIcon *icon)
{
...
static void
set_keyboard_focus (DoryIconContainer *container,
		    DoryIcon *icon)
{
	g_assert (icon != NULL);
...
static void
set_keyboard_rubberband_start (DoryIconContainer *container,
			       DoryIcon *icon)
{
...
static void
clear_keyboard_rubberband_start (DoryIconContainer *container)
{
	container->details->keyboard_rubberband_start = NULL;
}

/* carbon-copy of eel_canvas_group_bounds(), but
 * for DoryIconContainerItems it returns the
				   ...
				   DoryIconCanvasItemBoundsUsage usage)
{
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (!DORY_IS_ICON_CANVAS_ITEM (child)) {
			continue;
		}
		...
		if (child->flags & EEL_CANVAS_ITEM_VISIBLE) {
			...
			if (!DORY_IS_ICON_CANVAS_ITEM (child) ||
				...
				dory_icon_canvas_item_get_bounds_for_layout (DORY_ICON_CANVAS_ITEM (child),
										 &minx, &miny, &maxx, &maxy);
			} else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
				dory_icon_canvas_item_get_bounds_for_entire_item (DORY_ICON_CANVAS_ITEM (child),
										      &minx, &miny, &maxx, &maxy);
			} else {
...
void
dory_icon_container_get_all_icon_bounds (DoryIconContainer *container,
		     double *x1, double *y1,
		     double *x2, double *y2,
		     DoryIconCanvasItemBoundsUsage usage)
{
...
void
dory_icon_container_reset_scroll_region (DoryIconContainer *container)
{
...
void
dory_icon_container_update_scroll_region (DoryIconContainer *container)
{
	double x1, y1, x2, y2;
	...
	if (dory_icon_container_get_is_fixed_size (container)) {
	...
	reset_scroll_region = container->details->reset_scroll_region_trigger
		|| dory_icon_container_is_empty (container)
		|| dory_icon_container_is_auto_layout (container);
	 ...
	 */
	if (!dory_icon_container_is_empty (container)) {
		container->details->reset_scroll_region_trigger = FALSE;
	}

	dory_icon_container_get_all_icon_bounds (container, &x1, &y1, &x2, &y2, BOUNDS_USAGE_FOR_ENTIRE_ITEM);
	 ...
	 */
	if (dory_icon_container_is_layout_vertical (container)) {
	...
	step_increment = dory_get_icon_size_for_zoom_level
...
{
	DoryIconContainerClass *klass;
	const DoryIcon *icon_a, *icon_b;

	icon_a = a;
	icon_b = b;
	klass  = DORY_ICON_CONTAINER_GET_CLASS (icon_container);

	return klass->compare_icons (icon_container, icon_a->data, icon_b->data);
}

static void
align_icons (DoryIconContainer *container)
{
    DORY_ICON_CONTAINER_GET_CLASS (container)->align_icons (container);
}

static void
redo_layout_internal (DoryIconContainer *container)
{
    container->details->fixed_text_height = -1;

    if (DORY_ICON_CONTAINER_GET_CLASS (container)->finish_adding_new_icons != NULL) {
        DORY_ICON_CONTAINER_GET_CLASS (container)->finish_adding_new_icons (container);
    }
    ...
    if (container->details->auto_layout && container->details->drag_state != DRAG_STATE_STRETCH) {
        ...
        DORY_ICON_CONTAINER_GET_CLASS (container)->lay_down_icons (container, container->details->icons, 0);
	}
	...
	dory_icon_container_update_scroll_region (container);
...
redo_layout_callback (gpointer callback_data)
{
	...
	container = DORY_ICON_CONTAINER (callback_data);
...
static void
unschedule_redo_layout (DoryIconContainer *container)
{
...
static void
schedule_redo_layout (DoryIconContainer *container)
{
	if (container->details->idle_id == 0
...
void
dory_icon_container_redo_layout (DoryIconContainer *container)
{
...
static void
reload_icon_positions (DoryIconContainer *container)
{
    DORY_ICON_CONTAINER_GET_CLASS (container)->reload_icon_positions (container);
}
...
static void
invalidate_label_sizes (DoryIconContainer *container)
{
	GList *p;
	DoryIcon *icon;
...
static void
update_icons (DoryIconContainer *container)
{
    GList *p;
    DoryIcon *icon;
...
static gboolean
select_range (DoryIconContainer *container,
	      DoryIcon *icon1,
	      DoryIcon *icon2,
...
static gboolean
select_one_unselect_others (DoryIconContainer *container,
			    DoryIcon *icon_to_select)
{
	gboolean selection_changed;
	GList *p;
	DoryIcon *icon;
...
static gboolean
unselect_all (DoryIconContainer *container)
{
...
static void
rubberband_select (DoryIconContainer *container,
		   ...
		   const EelDRect *current_rect)
{
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		is_in = dory_icon_canvas_item_hit_test_rectangle (icon->item, canvas_rect);
...
rubberband_timeout_callback (gpointer data)
{
	DoryIconContainer *container;
	GtkWidget *widget;
	DoryIconRubberbandInfo *band_info;
	int x, y;
	double x1, y1, x2, y2;
	...
	dory_icon_container_scroll (container, x_scroll, y_scroll);
...
static void
start_rubberbanding (DoryIconContainer *container,
		     GdkEventButton *event)
{
	...
	band_info->selection_rectangle = eel_canvas_item_new
		(eel_canvas_root
		 (EEL_CANVAS (container)),
		 DORY_TYPE_SELECTION_CANVAS_ITEM,
		 "x1", band_info->start_x,
		 "y1", band_info->start_y,
		 "x2", band_info->start_x,
		 "y2", band_info->start_y,
...
static void
stop_rubberbanding (DoryIconContainer *container,
		    guint32 time)
{
	DoryIconRubberbandInfo *band_info;
	GList *icons;
	...
	if (enable_animation) {
		dory_selection_canvas_item_fade_out (DORY_SELECTION_CANVAS_ITEM (band_info->selection_rectangle), 150);
	} else {
	...
	icons = dory_icon_container_get_selected_icons (container);
...
typedef gboolean (* IsBetterIconFunction) (DoryIconContainer *container,
					   DoryIcon *start_icon,
					   DoryIcon *best_so_far,
					   DoryIcon *candidate,
					   void *data);

static DoryIcon *
find_best_icon (DoryIconContainer *container,
		DoryIcon *start_icon,
		IsBetterIconFunction function,
		void *data)
{
	GList *p;
	DoryIcon *best, *candidate;
...
static DoryIcon *
find_best_selected_icon (DoryIconContainer *container,
			 DoryIcon *start_icon,
			 IsBetterIconFunction function,
			 void *data)
{
...
static int
compare_icons_by_uri (DoryIconContainer *container,
		      DoryIcon *icon_a,
		      DoryIcon *icon_b)
{
	char *uri_a, *uri_b;
	...
	uri_a = dory_icon_container_get_icon_uri (container, icon_a);
	uri_b = dory_icon_container_get_icon_uri (container, icon_b);
...
static int
get_cmp_point_x (DoryIconContainer *container,
		 EelDRect icon_rect)
{
	if (container->details->label_position == DORY_ICON_LABEL_POSITION_BESIDE) {
...
static int
get_cmp_point_y (DoryIconContainer *container,
		 EelDRect icon_rect)
{
	if (container->details->label_position == DORY_ICON_LABEL_POSITION_BESIDE) {
...
static int
compare_icons_horizontal (DoryIconContainer *container,
			  DoryIcon *icon_a,
			  DoryIcon *icon_b)
{
	EelDRect world_rect;
	...
	world_rect = dory_icon_canvas_item_get_icon_rectangle (icon_a->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 ...
		 &ax,
		 NULL);
	world_rect = dory_icon_canvas_item_get_icon_rectangle (icon_b->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static int
compare_icons_vertical (DoryIconContainer *container,
			DoryIcon *icon_a,
			DoryIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_horizontal_first (DoryIconContainer *container,
				DoryIcon *icon_a,
				DoryIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_vertical_first (DoryIconContainer *container,
			      DoryIcon *icon_a,
			      DoryIcon *icon_b)
{
	EelDRect world_rect;
...
static gboolean
leftmost_in_top_row (DoryIconContainer *container,
		     DoryIcon *start_icon,
		     DoryIcon *best_so_far,
		     DoryIcon *candidate,
		     void *data)
...
static gboolean
rightmost_in_top_row (DoryIconContainer *container,
		      DoryIcon *start_icon,
		      DoryIcon *best_so_far,
		      DoryIcon *candidate,
		      void *data)
...
static gboolean
rightmost_in_bottom_row (DoryIconContainer *container,
			 DoryIcon *start_icon,
			 DoryIcon *best_so_far,
			 DoryIcon *candidate,
			 void *data)
...
static int
compare_with_start_row (DoryIconContainer *container,
			DoryIcon *icon)
{
	EelCanvasItem *item;
...
static int
compare_with_start_column (DoryIconContainer *container,
			   DoryIcon *icon)
{
	EelCanvasItem *item;
...
static gboolean
same_row_right_side_leftmost (DoryIconContainer *container,
			      DoryIcon *start_icon,
			      DoryIcon *best_so_far,
			      DoryIcon *candidate,
			      void *data)
...
static gboolean
same_row_left_side_rightmost (DoryIconContainer *container,
			      DoryIcon *start_icon,
			      DoryIcon *best_so_far,
			      DoryIcon *candidate,
			      void *data)
...
static gboolean
next_row_leftmost (DoryIconContainer *container,
		   DoryIcon *start_icon,
	           DoryIcon *best_so_far,
		   DoryIcon *candidate,
		   void *data)
...
static gboolean
next_row_rightmost (DoryIconContainer *container,
		    DoryIcon *start_icon,
		    DoryIcon *best_so_far,
		    DoryIcon *candidate,
		    void *data)
...
static gboolean
next_column_bottommost (DoryIconContainer *container,
			DoryIcon *start_icon,
			DoryIcon *best_so_far,
			DoryIcon *candidate,
			void *data)
...
static gboolean
previous_row_rightmost (DoryIconContainer *container,
		        DoryIcon *start_icon,
			DoryIcon *best_so_far,
			DoryIcon *candidate,
			void *data)
...
static gboolean
same_column_above_lowest (DoryIconContainer *container,
			  DoryIcon *start_icon,
			  DoryIcon *best_so_far,
			  DoryIcon *candidate,
			  void *data)
...
static gboolean
same_column_below_highest (DoryIconContainer *container,
			   DoryIcon *start_icon,
			   DoryIcon *best_so_far,
			   DoryIcon *candidate,
			   void *data)
...
static gboolean
previous_column_highest (DoryIconContainer *container,
			 DoryIcon *start_icon,
			 DoryIcon *best_so_far,
			 DoryIcon *candidate,
			 void *data)
...
static gboolean
next_column_highest (DoryIconContainer *container,
		     DoryIcon *start_icon,
		     DoryIcon *best_so_far,
		     DoryIcon *candidate,
		     void *data)
...
static gboolean
previous_column_lowest (DoryIconContainer *container,
		        DoryIcon *start_icon,
			DoryIcon *best_so_far,
			DoryIcon *candidate,
			void *data)
...
static gboolean
last_column_lowest (DoryIconContainer *container,
		    DoryIcon *start_icon,
		    DoryIcon *best_so_far,
		    DoryIcon *candidate,
		    void *data)
...
static gboolean
closest_in_90_degrees (DoryIconContainer *container,
		       DoryIcon *start_icon,
		       DoryIcon *best_so_far,
		       DoryIcon *candidate,
		       void *data)
{
	...
	world_rect = dory_icon_canvas_item_get_icon_rectangle (candidate->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static void
keyboard_move_to (DoryIconContainer *container,
		  DoryIcon *icon,
		  DoryIcon *from,
		  GdkEventKey *event)
{
...
static void
keyboard_home (DoryIconContainer *container,
	       GdkEventKey *event)
{
	DoryIcon *from;
	DoryIcon *to;
...
static void
keyboard_end (DoryIconContainer *container,
	      GdkEventKey *event)
{
	DoryIcon *to;
	DoryIcon *from;
	...
	to = find_best_icon (container, NULL,
			     dory_icon_container_is_layout_vertical (container) ?
			     last_column_lowest :
			     rightmost_in_bottom_row,
			     NULL);
...
static void
record_arrow_key_start (DoryIconContainer *container,
			DoryIcon *icon,
			GtkDirectionType direction)
{
	EelDRect world_rect;

	world_rect = dory_icon_canvas_item_get_icon_rectangle (icon->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static void
keyboard_arrow_key (DoryIconContainer *container,
		    GdkEventKey *event,
		    GtkDirectionType direction,
...
static void
keyboard_right (DoryIconContainer *container,
		GdkEventKey *event)
{
	...
	keyboard_arrow_key (container,
			    ...
			    GTK_DIR_RIGHT,
			    rightmost_in_bottom_row,
			    dory_icon_container_is_layout_rtl (container) ?
...
static void
keyboard_left (DoryIconContainer *container,
	       GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_down (DoryIconContainer *container,
	       GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_up (DoryIconContainer *container,
	     GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_space (DoryIconContainer *container,
		GdkEventKey *event)
{
	DoryIcon *icon;
...
static void
select_previous_or_next_icon (DoryIconContainer *container,
			      gboolean next,
			      GdkEventKey *event)
{
	DoryIcon *icon;
...
destroy (GtkWidget *object)
{
	...
	container = DORY_ICON_CONTAINER (object);

        dory_icon_container_clear (container);
	...
	GTK_WIDGET_CLASS (dory_icon_container_parent_class)->destroy (object);
}
...
finalize (GObject *object)
{
	...
	details = DORY_ICON_CONTAINER (object)->details;
	...
	G_OBJECT_CLASS (dory_icon_container_parent_class)->finalize (object);
}
	       ...
	       GtkAllocation *allocation)
{
	...
	container = DORY_ICON_CONTAINER (widget);
	...
	GTK_WIDGET_CLASS (dory_icon_container_parent_class)->size_allocate (widget, allocation);
...
realize (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (dory_icon_container_parent_class)->realize (widget);

	container = DORY_ICON_CONTAINER (widget);

	/* Set up DnD.  */
	dory_icon_dnd_init (container);
...
unrealize (GtkWidget *widget)
{
	...
	dory_icon_dnd_fini (container);
	remove_search_entry_timeout (container);

	GTK_WIDGET_CLASS (dory_icon_container_parent_class)->unrealize (widget);
}
...
style_updated (GtkWidget *widget)
{
	 ...
	 */
	if (!dory_icon_container_get_is_desktop (container)) {
		GTK_WIDGET_CLASS (dory_icon_container_parent_class)->style_updated (widget);
	}
		    ...
		    GdkEventButton *event)
{
	...
	clicked_on_item = GTK_WIDGET_CLASS (dory_icon_container_parent_class)->button_press_event (widget, event);
    ...
    if (clicked_on_item) {
        DoryIcon *icon; // current icon which was clicked on
        ...
        if (clicked_on_icon (container, icon, event) &&
            icon == dory_icon_container_get_icon_being_renamed (container)) {
            dory_icon_container_end_renaming_mode (container, TRUE);
        }
...
static void
dory_icon_container_did_not_drag (DoryIconContainer *container,
				      GdkEventButton *event)
{
	DoryIconContainerDetails *details;
	gboolean selection_changed;
	...
	if (details->drag_icon != NULL &&
		...
		if (click_count == 0 &&
			 ...
			 * DoryList goes the other way because its "links" seem
			 * much more link-like.
			 */
			if (event->button == MIDDLE_BUTTON) {
				activate_selected_items_alternate (container, NULL);
    ...
    if (details->drag_icon != NULL &&
        ...
        if (!details->skip_rename_on_release)
            dory_icon_container_start_renaming_selected_item (container, FALSE);
    }
}

static gboolean
clicked_within_double_click_interval (DoryIconContainer *container)
{
	static gint64 last_click_time = 0;
	static gint click_count = 0;
	gint64 current_time;
...
static gboolean
clicked_within_slow_click_interval_on_text (DoryIconContainer *container, DoryIcon *icon, GdkEventButton *event)
{
...
static void
clear_drag_state (DoryIconContainer *container)
{
...
static gboolean
start_stretching (DoryIconContainer *container)
{
	DoryIconContainerDetails *details;
	DoryIcon *icon;
	GtkWidget *toplevel;
	...
	if (!dory_icon_canvas_item_hit_test_stretch_handles (icon->item,
...
static gboolean
update_stretch_at_idle (DoryIconContainer *container)
{
	DoryIconContainerDetails *details;
	DoryIcon *icon;
	double world_x, world_y;
	...
	dory_icon_container_icon_set_position (container, icon, world_x, world_y);
...
static void
continue_stretching (DoryIconContainer *container,
		     double world_x, double world_y)
{

	g_return_if_fail (DORY_IS_ICON_CONTAINER (container));
...
static gboolean
keyboard_stretching (DoryIconContainer *container,
		     GdkEventKey           *event)
{
	DoryIcon *icon;
	guint size;
...
static void
ungrab_stretch_icon (DoryIconContainer *container)
{
...
static void
end_stretching (DoryIconContainer *container,
		double world_x, double world_y)
{
	...
	if (dory_icon_container_is_layout_rtl (container)) {
		position.x = icon->saved_ltr_x = dory_icon_container_get_mirror_x_position (container, icon, icon->x);
	} else {
		position.x = icon->x;
	}
	position.y = icon->y;
...
static gboolean
undo_stretching (DoryIconContainer *container)
{
	DoryIcon *stretched_icon;
	...
	dory_icon_canvas_item_set_show_stretch_handles
		(stretched_icon->item, FALSE);

	dory_icon_container_icon_set_position (container, stretched_icon,
		      ...
		      GdkEventButton *event)
{
	...
	if (event->button == RUBBERBAND_BUTTON && details->rubberband_info.active) {
        ...
        return GTK_WIDGET_CLASS (dory_icon_container_parent_class)->button_release_event (widget, event);
	}
	...
	if (event->button == details->drag_button) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			if (!details->drag_started) {
				dory_icon_container_did_not_drag (container, event);
			} else {
				dory_icon_dnd_end_drag (container);
		     ...
		     GdkEventMotion *event)
{
	...
	if (details->drag_button != 0) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			...
			if (gtk_drag_check_threshold (widget,
				...
				dory_icon_dnd_begin_drag (container,
							      actions,
							      details->drag_button,
							      event,
							      canvas_x,
	...
	return GTK_WIDGET_CLASS (dory_icon_container_parent_class)->motion_notify_event (widget, event);
}

static void
dory_icon_container_search_position_func (DoryIconContainer *container,
					      GtkWidget *search_dialog)
{
    ...
    monitor_num = dory_desktop_utils_get_monitor_for_widget (GTK_WIDGET (container));
     ...
     */
    dory_desktop_utils_get_monitor_work_rect (monitor_num, &monitor);
...
static void
dory_icon_container_search_dialog_hide (GtkWidget *search_dialog,
					    DoryIconContainer *container)
{
...
static gboolean
dory_icon_container_search_entry_flush_timeout (gpointer data)
{
	DoryIconContainer *container = data;

	container->details->typeselect_flush_timeout = 0;
	dory_icon_container_search_dialog_hide (container->details->search_window, container);

	return FALSE;
}

static void
add_search_entry_timeout (DoryIconContainer *container)
{
	container->details->typeselect_flush_timeout =
		g_timeout_add_seconds (DORY_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT,
				       dory_icon_container_search_entry_flush_timeout,
				       container);
}

static void
remove_search_entry_timeout (DoryIconContainer *container)
{
...
static void
reset_search_entry_timeout (DoryIconContainer *container)
{
	remove_search_entry_timeout (container);
 ...
 */
static void
dory_icon_container_search_preedit_changed (GtkEntry *entry,
						gchar *preedit,
						DoryIconContainer *container)
{
...
static void
dory_icon_container_search_activate (GtkEntry *entry,
					 DoryIconContainer *container)
{
	dory_icon_container_search_dialog_hide (container->details->search_window,
						    container);
...
static gboolean
dory_icon_container_search_delete_event (GtkWidget *widget,
					     GdkEventAny *event,
					     DoryIconContainer *container)
{
	dory_icon_container_search_dialog_hide (widget, container);

	return TRUE;
}

static gboolean
dory_icon_container_search_button_press_event (GtkWidget *widget,
						   GdkEventButton *event,
						   DoryIconContainer *container)
{
	dory_icon_container_search_dialog_hide (widget, container);
...
static gboolean
dory_icon_container_search_entry_button_press_event (GtkWidget *widget,
							 GdkEventButton *event,
							 DoryIconContainer *container)
{
	reset_search_entry_timeout (container);
...
static void
dory_icon_container_search_populate_popup (GtkEntry *entry,
					       GtkMenu *menu,
					       DoryIconContainer *container)
{
	remove_search_entry_timeout (container);
...
void
dory_icon_container_get_icon_text (DoryIconContainer *container,
				       DoryIconData      *data,
				       char                 **editable_text,
				       ...
				       gboolean               include_invisible)
{
	...
	klass = DORY_ICON_CONTAINER_GET_CLASS (container);
...
static gboolean
dory_icon_container_search_iter (DoryIconContainer *container,
				     const char *key, gint n)
{
	...
	for (p = container->details->icons; p != NULL && count != n; p = p->next) {
		icon = p->data;
		dory_icon_container_get_icon_text (container, icon->data, &name,
						       NULL, NULL, NULL, TRUE);
...
static void
dory_icon_container_search_move (GtkWidget *window,
				     DoryIconContainer *container,
				     gboolean up)
{
	gboolean ret;
	...
	ret = dory_icon_container_search_iter (container, text,
	...
	if (ret) {
		...
		/* return to old iter */
		dory_icon_container_search_iter (container, text,
					container->details->selected_iter);
	}
}

static gboolean
dory_icon_container_search_scroll_event (GtkWidget *widget,
					     ...
					     DoryIconContainer *container)
{
	...
	if (event->direction == GDK_SCROLL_UP) {
		dory_icon_container_search_move (widget, container, TRUE);
		retval = TRUE;
	} else if (event->direction == GDK_SCROLL_DOWN) {
		dory_icon_container_search_move (widget, container, FALSE);
		retval = TRUE;
	}
...
static gboolean
dory_icon_container_search_key_press_event (GtkWidget *widget,
						GdkEventKey *event,
						DoryIconContainer *container)
{
	gboolean retval = FALSE;
	...
	if (event->keyval == GDK_KEY_Return && event->state & GDK_SHIFT_MASK) {
		dory_icon_container_search_dialog_hide (widget,
							    container);
...
static void
dory_icon_container_search_init (GtkWidget   *entry,
				     DoryIconContainer *container)
{
	gint ret;
	gint len;
	...
	ret = dory_icon_container_search_iter (container, text, 1);
...
static void
dory_icon_container_ensure_interactive_directory (DoryIconContainer *container)
{
	...
	g_signal_connect (container->details->search_window, "delete_event",
			  G_CALLBACK (dory_icon_container_search_delete_event),
			  container);
	g_signal_connect (container->details->search_window, "key_press_event",
			  G_CALLBACK (dory_icon_container_search_key_press_event),
			  container);
	g_signal_connect (container->details->search_window, "button_press_event",
			  G_CALLBACK (dory_icon_container_search_button_press_event),
			  container);
	g_signal_connect (container->details->search_window, "scroll_event",
			  G_CALLBACK (dory_icon_container_search_scroll_event),
			  container);
 ...
 */
static gboolean
dory_icon_container_start_interactive_search (DoryIconContainer *container)
{
	...
	dory_icon_container_ensure_interactive_directory (container);

	/* done, show it */
	dory_icon_container_search_position_func (container, container->details->search_window);
	...
	if (container->details->search_entry_changed_id == 0) {
		container->details->search_entry_changed_id =
			g_signal_connect (container->details->search_entry, "changed",
				G_CALLBACK (dory_icon_container_search_init),
				container);
	}
	...
	* really non-kosher. */
	...
	dory_icon_container_search_init (container->details->search_entry, container);

	return TRUE;
}

static gboolean
handle_popups (DoryIconContainer *container,
	       GdkEventKey           *event,
	       const char            *signal)
{
		 ...
		 GdkEventKey *event)
{
	...
	if (!handled) {
		handled = GTK_WIDGET_CLASS (dory_icon_container_parent_class)->key_press_event (widget, event);
	}
	...
	if (!handled &&
	    ...
	    if (!dory_icon_container_get_is_desktop (container)) {
	        ...
	        if (container->details->imcontext_changed ||
	            ...
	            if (dory_icon_container_start_interactive_search (container)) {
                        ...
                        cairo_t   *cr)
{
     ...
     * This is overridden by dory-icon-view-grid-container. */
    return;
}
...
get_accessible (GtkWidget *widget)
{
	...
	accessible = g_object_new
		(dory_icon_container_accessible_get_type (), NULL);
...
static void
real_lay_down_icons (DoryIconContainer *container,
                     GList             *icons,
                     double start_y)
{
    g_assert_not_reached ();
...
static void
real_icon_set_position (DoryIconContainer *container,
                        DoryIcon          *icon,
                        double x,
                        double y)
{
...
static void
real_move_icon (DoryIconContainer *container,
                DoryIcon *icon,
                int x, int y,
                double scale,
                gboolean raise,
...
static void
real_update_icon (DoryIconContainer *container,
                  DoryIcon *icon,
                  gboolean visible)
{
    g_assert_not_reached ();
...
static void
real_align_icons (DoryIconContainer *container)
{
    g_assert_not_reached ();
}

static void
real_icon_get_bounding_box (DoryIcon *icon,
...
static void
real_set_zoom_level (DoryIconContainer *container,
                     gint               new_level)
{
    g_assert_not_reached ();
}
...
static void
dory_icon_container_class_init (DoryIconContainerClass *class)
{
	...
	signals[SELECTION_CHANGED]
		= g_signal_new ("selection_changed",
		                ...
		                G_SIGNAL_RUN_LAST,
		                G_STRUCT_OFFSET (DoryIconContainerClass,
						 selection_changed),
		                NULL, NULL,
...
static void
update_selected (DoryIconContainer *container)
{
	GList *node;
	DoryIcon *icon;
...
{
	update_selected (DORY_ICON_CONTAINER (widget));
...
handle_focus_out_event (GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
	...
	dory_icon_container_end_renaming_mode (DORY_ICON_CONTAINER (widget), TRUE);
	update_selected (DORY_ICON_CONTAINER (widget));
...
static void
dory_icon_container_init (DoryIconContainer *container)
{
	DoryIconContainerDetails *details;

	details = g_new0 (DoryIconContainerDetails, 1);

	details->icon_set = g_hash_table_new (g_direct_hash, g_direct_equal);
	details->layout_timestamp = UNDEFINED_TIME;
	details->zoom_level = DORY_ZOOM_LEVEL_STANDARD;

	details->font_size_table[DORY_ZOOM_LEVEL_SMALLEST] = -2 * PANGO_SCALE;
	details->font_size_table[DORY_ZOOM_LEVEL_SMALLER] = -2 * PANGO_SCALE;
	details->font_size_table[DORY_ZOOM_LEVEL_SMALL] = -0 * PANGO_SCALE;
	details->font_size_table[DORY_ZOOM_LEVEL_STANDARD] = 0 * PANGO_SCALE;
	details->font_size_table[DORY_ZOOM_LEVEL_LARGE] = 0 * PANGO_SCALE;
	details->font_size_table[DORY_ZOOM_LEVEL_LARGER] = 0 * PANGO_SCALE;
	details->font_size_table[DORY_ZOOM_LEVEL_LARGEST] = 0 * PANGO_SCALE;

    details->fixed_text_height = -1;

    details->view_constants = g_new0 (DoryViewLayoutConstants, 1);
    ...
    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_TOOLTIPS_DESKTOP,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_TOOLTIPS_ICON_VIEW,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_TOOLTIP_FILE_TYPE,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_TOOLTIP_MOD_DATE,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_TOOLTIP_ACCESS_DATE,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);

    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_TOOLTIP_FULL_PATH,
                              G_CALLBACK (tooltip_prefs_changed_callback),
                              container);
...
static gboolean
handle_icon_double_click (DoryIconContainer *container,
			  DoryIcon *icon,
			  GdkEventButton *event)
{
	DoryIconContainerDetails *details;
...
static gboolean
handle_icon_slow_two_click (DoryIconContainer *container,
                                     DoryIcon *icon,
                               GdkEventButton *event)
{
    DoryIconContainerDetails *details;

    DoryFile *file = DORY_FILE (icon->data);

    if (!dory_file_can_rename (file))
        return FALSE;
    ...
    GList *selection = dory_icon_container_get_selection (container);
...
static gboolean
handle_icon_button_press (DoryIconContainer *container,
			  DoryIcon *icon,
			  GdkEventButton *event)
{
	DoryIconContainerDetails *details;
    ...
    GList *sel = dory_icon_container_get_selected_icons (container);
		     ...
		     gpointer data)
{
	...
	icon = DORY_ICON_CANVAS_ITEM (item)->user_data;
	g_assert (icon != NULL);
...
GtkWidget *
dory_icon_container_new (void)
{
	return gtk_widget_new (DORY_TYPE_ICON_CONTAINER, NULL);
}

/* Clear all of the icons in the container. */
void
dory_icon_container_clear (DoryIconContainer *container)
{
	DoryIconContainerDetails *details;
	GList *p;
...
gboolean
dory_icon_container_is_empty (DoryIconContainer *container)
{
	return container->details->icons == NULL;
}

DoryIconData *
dory_icon_container_get_first_visible_icon (DoryIconContainer *container)
{
	GList *l;
	DoryIcon *icon, *best_icon;
	double x, y;
	double x1, y1, x2, y2;
	double *pos, best_pos;
...
void
dory_icon_container_scroll_to_icon (DoryIconContainer  *container,
					DoryIconData       *data)
{
	GList *l;
	DoryIcon *icon;
	...
	dory_icon_container_layout_now (container);
	...
	while (l != NULL) {
		...
		if (icon->data == data &&
		    dory_icon_container_icon_is_positioned (icon)) {
...
void
dory_icon_container_for_each (DoryIconContainer *container,
				  DoryIconCallback callback,
				  gpointer callback_data)
{
	CallbackAndData callback_and_data;
...
static void
icon_destroy (DoryIconContainer *container,
	      DoryIcon *icon)
{
	DoryIconContainerDetails *details;
	gboolean was_selected;
	DoryIcon *icon_to_focus;
	GList *item;
...
static void
activate_selected_items (DoryIconContainer *container)
{
	GList *selection;

	g_return_if_fail (DORY_IS_ICON_CONTAINER (container));

	selection = dory_icon_container_get_selection (container);
	if (selection != NULL) {
	  	g_signal_emit (container,
				 signals[ACTIVATE], 0,
				 selection);
...
static void
preview_selected_items (DoryIconContainer *container)
{
	GList *selection;
	GArray *locations;
	guint idx;
	...
	selection = dory_icon_container_get_selection (container);
	locations = dory_icon_container_get_selected_icon_locations (container);
...
static void
activate_selected_items_alternate (DoryIconContainer *container,
				   DoryIcon *icon)
{
	GList *selection;
...
DoryIconInfo *
dory_icon_container_get_icon_images (DoryIconContainer *container,
                                     DoryIconData      *data,
                                     int                size,
...
static void
dory_icon_container_freeze_updates (DoryIconContainer *container)
{
	DoryIconContainerClass *klass;
...
static void
dory_icon_container_unfreeze_updates (DoryIconContainer *container)
{
	DoryIconContainerClass *klass;
...
static gboolean
update_visible_icons_cb (DoryIconContainer *container)
{
	GtkAdjustment *vadj, *hadj;
	double min_y, max_y;
	double min_x, max_x;
	...
	for (node = g_list_last (container->details->icons); node != NULL; node = node->prev) {
		...
		if (dory_icon_container_icon_is_positioned (icon)) {
			...
			if (visible) {
				dory_icon_canvas_item_set_is_visible (icon->item, TRUE);
                DoryFile *file = DORY_FILE (icon->data);
                ...
                if (!icon->ok_to_show_thumb) {
                    ...
                    if (dory_file_get_load_deferred_attrs (file) == DORY_FILE_LOAD_DEFERRED_ATTRS_NO) {
                        dory_file_set_load_deferred_attrs (file, DORY_FILE_LOAD_DEFERRED_ATTRS_YES);
                    }

                    dory_file_invalidate_attributes (file, DORY_FILE_DEFERRED_ATTRIBUTES);
                } else {
                    gchar *uri = dory_file_get_uri (file);
                    dory_thumbnail_prioritize (uri);
                    g_free (uri);
                }

                dory_icon_container_update_icon (container, icon);
			} else {
				dory_icon_canvas_item_set_is_visible (icon->item, FALSE);
			}
		}
	}
...
static void
queue_update_visible_icons(DoryIconContainer *container,
                           gint               delay)
{
    DoryIconContainerDetails *details = container->details;
...
static gboolean
is_old_or_unknown_icon_data (DoryIconContainer *container,
			     DoryIconData *data)
{
	time_t timestamp;
	gboolean success;
    ...
    is_transient = DORY_IS_DESKTOP_ICON_FILE (data) && container->details->keep_aligned;
...
gboolean
dory_icon_container_icon_is_new_for_monitor (DoryIconContainer *container,
                                             DoryIcon          *icon,
                                             gint               current_monitor)
{
    ...
    return dory_file_get_monitor_number (DORY_FILE (icon->data)) != current_monitor;
}

/**
 * dory_icon_container_add:
 * @container: A DoryIconContainer
 * @data: Icon data.
 *
 ...
 **/
gboolean
dory_icon_container_add (DoryIconContainer *container,
			     DoryIconData *data)
{
	DoryIconContainerDetails *details;
	DoryIcon *icon;
	...
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), FALSE);
	...
	item = EEL_CANVAS_ITEM (icon->item);
	band = DORY_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle;
	if (band) {
...
void
dory_icon_container_layout_now (DoryIconContainer *container)
{
	if (container->details->idle_id != 0) {
		unschedule_redo_layout (container);
		redo_layout_internal (container);
 ...
 **/
gboolean
dory_icon_container_remove (DoryIconContainer *container,
				DoryIconData *data)
{
	DoryIcon *icon;
 ...
 **/
void
dory_icon_container_request_update (DoryIconContainer *container,
					DoryIconData *data)
{
	DoryIcon *icon;
...
void
dory_icon_container_invalidate_labels (DoryIconContainer *container)
{
    GList *p;
    DoryIcon *icon;
    ...
    for (p = container->details->icons; p != NULL; p = p->next) {
        ...
        dory_icon_canvas_item_invalidate_label (icon->item);
    }
}
...
DoryZoomLevel
dory_icon_container_get_zoom_level (DoryIconContainer *container)
{
...
/**
 * dory_icon_container_request_update_all:
 ...
 **/
void
dory_icon_container_request_update_all (DoryIconContainer *container)
{
	GList *node;
	DoryIcon *icon;
 ...
 */
void
dory_icon_container_reveal (DoryIconContainer *container, DoryIconData *data)
{
	DoryIcon *icon;
...
GList *
dory_icon_container_get_real_selection (DoryIconContainer *container)
{
    GList *list, *p;
...
/**
 * dory_icon_container_get_selection:
 * @container: An icon container.
 *
 ...
 **/
GList *
dory_icon_container_get_selection (DoryIconContainer *container)
{
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), NULL);

    dory_icon_container_update_selection (container);

    return g_list_copy (container->details->current_selection);
}

/**
 * dory_icon_container_peek_selection:
 * @container: An icon container.
 *
 ...
 * DoryIconContainer and should not be freed.
 **/
GList *
dory_icon_container_peek_selection (DoryIconContainer *container)
{
    g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), NULL);
...
gint
dory_icon_container_get_selection_count (DoryIconContainer *container)
{
    g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), 0);
...
static GList *
dory_icon_container_get_selected_icons (DoryIconContainer *container)
{
	GList *list, *p;
 ...
 **/
void
dory_icon_container_invert_selection (DoryIconContainer *container)
{
	GList *p;
...
static GArray *
dory_icon_container_get_icon_locations (DoryIconContainer *container,
					    GList *icons)
{
	GArray *result;
	GList *node;
	...
	for (index = 0, node = icons; node != NULL; index++, node = node->next) {
	     	g_array_index (result, GdkPoint, index).x =
	     		((DoryIcon *)node->data)->x;
	     	g_array_index (result, GdkPoint, index).y =
			((DoryIcon *)node->data)->y;
	}
...
/**
 * dory_icon_container_get_selected_icon_locations:
 * @container: An icon container widget.
 *
 * Returns an array of GdkPoints of locations of the selected icons.
 **/
GArray *
dory_icon_container_get_selected_icon_locations (DoryIconContainer *container)
{
	GArray *result;
	GList *icons;
	...
	icons = dory_icon_container_get_selected_icons (container);
	result = dory_icon_container_get_icon_locations (container, icons);
	g_list_free (icons);
...
/**
 * dory_icon_container_select_all:
 * @container: An icon container widget.
 *
 * Select all the icons in @container at once.
 **/
void
dory_icon_container_select_all (DoryIconContainer *container)
{
	gboolean selection_changed;
	GList *p;
	DoryIcon *icon;
...
/**
 * dory_icon_container_select_first:
 * @container: An icon container widget.
 *
 ...
 **/
void
dory_icon_container_select_first (DoryIconContainer *container)
{
    DoryIcon *icon;
...
/**
 * dory_icon_container_select_list_unselect_others.
 * @container: An icon container widget.
 * @selection: A list of DoryIcon *.
 *
 * Set the selection to exactly the icons in @selection.
 **/
void
dory_icon_container_select_list_unselect_others (DoryIconContainer *container,
						     GList *selection)
{
	gboolean selection_changed;
	GHashTable *hash;
...
/**
 * dory_icon_container_unselect_all:
 * @container: An icon container widget.
 *
 * Deselect all the icons in @container.
 **/
void
dory_icon_container_unselect_all (DoryIconContainer *container)
{
...
/**
 * dory_icon_container_get_icon_by_uri:
 * @container: An icon container widget.
 ...
 **/
DoryIcon *
dory_icon_container_get_icon_by_uri (DoryIconContainer *container,
					 const char *uri)
{
	DoryIconContainerDetails *details;
	GList *p;
	...
	*/
	...
	for (p = details->icons; p != NULL; p = p->next) {
		...
		icon_uri = dory_icon_container_get_icon_uri
			(container, icon);
...
static DoryIcon *
get_nth_selected_icon (DoryIconContainer *container, int index)
{
	GList *p;
	DoryIcon *icon;
	int selection_count;
...
static DoryIcon *
get_first_selected_icon (DoryIconContainer *container)
{
        return get_nth_selected_icon (container, 1);
}

static gboolean
has_multiple_selection (DoryIconContainer *container)
{
        return get_nth_selected_icon (container, 2) != NULL;
}

static gboolean
all_selected (DoryIconContainer *container)
{
	GList *p;
	DoryIcon *icon;
...
static gboolean
has_selection (DoryIconContainer *container)
{
        return get_nth_selected_icon (container, 1) != NULL;
}

/**
 * dory_icon_container_show_stretch_handles:
 * @container: An icon container widget.
 *
 * Makes stretch handles visible on the first selected icon.
 **/
void
dory_icon_container_show_stretch_handles (DoryIconContainer *container)
{
	DoryIconContainerDetails *details;
	DoryIcon *icon;
	guint initial_size;
	...
	dory_icon_canvas_item_set_show_stretch_handles (icon->item, TRUE);
...
/**
 * dory_icon_container_has_stretch_handles
 * @container: An icon container widget.
 *
 * Returns true if the first selected item has stretch handles.
 **/
gboolean
dory_icon_container_has_stretch_handles (DoryIconContainer *container)
{
	DoryIcon *icon;
...
/**
 * dory_icon_container_is_stretched
 * @container: An icon container widget.
 *
 * Returns true if the any selected item is stretched to a size other than 1.0.
 **/
gboolean
dory_icon_container_is_stretched (DoryIconContainer *container)
{
	GList *p;
	DoryIcon *icon;
...
/**
 * dory_icon_container_unstretch
 * @container: An icon container widget.
 *
 * Gets rid of any icon stretching.
 **/
void
dory_icon_container_unstretch (DoryIconContainer *container)
{
	GList *p;
	DoryIcon *icon;
...
char *
dory_icon_container_get_icon_uri (DoryIconContainer *container,
				      DoryIcon *icon)
{
	char *uri;
...
char *
dory_icon_container_get_icon_drop_target_uri (DoryIconContainer *container,
				   	     	  DoryIcon *icon)
{
	char *uri;
...
void
dory_icon_container_update_tooltip_text (DoryIconContainer  *container,
 ...
 */
static void
reset_scroll_region_if_not_empty (DoryIconContainer *container)
{
	if (!dory_icon_container_is_empty (container)) {
		dory_icon_container_reset_scroll_region (container);
	}
}
 ...
 */
void
dory_icon_container_set_auto_layout (DoryIconContainer *container,
...
void
dory_icon_container_set_horizontal_layout (DoryIconContainer *container,
                                           gboolean           horizontal)
{
    GtkTextDirection dir;
    DoryIconLayoutMode layout_mode;
    ...
    if (dir == GTK_TEXT_DIR_LTR) {
        layout_mode = horizontal ? DORY_ICON_LAYOUT_L_R_T_B : DORY_ICON_LAYOUT_T_B_L_R;
    } else {
        layout_mode = horizontal ? DORY_ICON_LAYOUT_R_L_T_B : DORY_ICON_LAYOUT_T_B_R_L;
    }
...
gboolean
dory_icon_container_get_horizontal_layout (DoryIconContainer *container)
{
    g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), FALSE);
...
void
dory_icon_container_set_grid_adjusts (DoryIconContainer *container,
                                      gint               h_adjust,
                                      gint               v_adjust)
{
    g_return_if_fail (DORY_IS_ICON_CONTAINER (container));
...
gboolean
dory_icon_container_is_keep_aligned (DoryIconContainer *container)
{
...
static void
unschedule_align_icons (DoryIconContainer *container)
{
        if (container->details->align_idle_id != 0) {
		g_source_remove (container->details->align_idle_id);
		container->details->align_idle_id = 0;
...
static void
schedule_align_icons (DoryIconContainer *container)
{
	if (container->details->align_idle_id == 0
	    && container->details->has_been_allocated) {
...
void
dory_icon_container_set_keep_aligned (DoryIconContainer *container,
...
void
dory_icon_container_set_layout_mode (DoryIconContainer *container,
...
void
dory_icon_container_set_label_position (DoryIconContainer *container,
					    DoryIconLabelPosition position)
{
	g_return_if_fail (DORY_IS_ICON_CONTAINER (container));
 ...
 */
void
dory_icon_container_freeze_icon_positions (DoryIconContainer *container)
{
	gboolean changed;
	GList *p;
	DoryIcon *icon;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		position.y = icon->y;
		position.scale = icon->scale;
        position.monitor = dory_desktop_utils_get_monitor_for_widget (GTK_WIDGET (container));
...
void
dory_icon_container_sort (DoryIconContainer *container)
{
	gboolean changed;
...
gboolean
dory_icon_container_is_auto_layout (DoryIconContainer *container)
{
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), FALSE);
...
static void
pending_icon_to_rename_destroy_callback (DoryIconCanvasItem *item, DoryIconContainer *container)
{
...
static DoryIcon*
get_pending_icon_to_rename (DoryIconContainer *container)
{
	return container->details->pending_icon_to_rename;
}

static void
set_pending_icon_to_rename (DoryIconContainer *container, DoryIcon *icon)
{
	DoryIcon *old_icon;
...
static void
process_pending_icon_to_rename (DoryIconContainer *container)
{
	DoryIcon *pending_icon_to_rename;
...
static gboolean
is_renaming_pending (DoryIconContainer *container)
{
	return get_pending_icon_to_rename (container) != NULL;
}

static gboolean
is_renaming (DoryIconContainer *container)
{
 ...
 **/
void
dory_icon_container_start_renaming_selected_item (DoryIconContainer *container,
						      gboolean select_all)
{
	DoryIconContainerDetails *details;
	DoryIcon *icon;
	...
	editable_text = dory_icon_canvas_item_get_editable_text (icon->item);
	...
	dory_icon_container_freeze_updates (container);
	...
	if (details->rename_widget == NULL) {
		...
		if (details->label_position != DORY_ICON_LABEL_POSITION_BESIDE) {
	...
	icon_rect = dory_icon_canvas_item_get_icon_rectangle (icon->item);
    text_rect = dory_icon_canvas_item_get_text_rectangle (icon->item, TRUE);

	if (dory_icon_container_is_layout_vertical (container) &&
	    container->details->label_position == DORY_ICON_LABEL_POSITION_BESIDE) {
		/* for one-line editables, the width changes dynamically */
		width = -1;
	} else {
		width = dory_icon_canvas_item_get_max_text_width (icon->item);
	}
	...
	if (select_all) {
		...
		if (dory_file_is_directory (DORY_FILE (icon->data))) {
			start_offset = 0;
			end_offset = -1;
		} else {
	...
	dory_icon_canvas_item_set_renaming (icon->item, TRUE);
}

DoryIcon *
dory_icon_container_get_icon_being_renamed (DoryIconContainer *container)
{
    DoryIcon *rename_icon;
...
void
dory_icon_container_end_renaming_mode (DoryIconContainer *container, gboolean commit)
{
	DoryIcon *icon;
	...
	dory_icon_canvas_item_set_renaming (icon->item, FALSE);
    ...
    dory_icon_container_unfreeze_updates (container);
}

void
dory_icon_container_set_single_click_mode (DoryIconContainer *container,
...
void
dory_icon_container_set_click_to_rename_enabled (DoryIconContainer *container,
                                                           gboolean enabled)
{
    g_return_if_fail (DORY_IS_ICON_CONTAINER (container));
...
gboolean
dory_icon_container_get_is_fixed_size (DoryIconContainer *container)
{
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), FALSE);
...
void
dory_icon_container_set_is_fixed_size (DoryIconContainer *container,
...
gboolean
dory_icon_container_get_is_desktop (DoryIconContainer *container)
{
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), FALSE);
...
void
dory_icon_container_set_is_desktop (DoryIconContainer *container,
					   gboolean is_desktop)
{
    g_return_if_fail (DORY_IS_ICON_CONTAINER (container));
    ...
    if (is_desktop) {
        ...
        context = gtk_widget_get_style_context (GTK_WIDGET (container));
        gtk_style_context_add_class (context, "dory-desktop");
        ...
        g_signal_connect_swapped (dory_icon_view_preferences,
                                  "changed::" DORY_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT,
                                  G_CALLBACK (text_ellipsis_limit_changed_container_callback),
                                  container);
    }
}
...
void
dory_icon_container_set_filter_highlight (DoryIconContainer *container,
...
const char *
dory_icon_container_get_filter_highlight (DoryIconContainer *container)
{
    g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), NULL);
...
void
dory_icon_container_set_margins (DoryIconContainer *container,
				     int left_margin,
				     int right_margin,
				     int top_margin,
				     int bottom_margin)
...
void
dory_icon_container_set_use_drop_shadows (DoryIconContainer  *container,
...
void
dory_icon_container_set_font (DoryIconContainer *container,
				  const char *font)
{
	g_return_if_fail (DORY_IS_ICON_CONTAINER (container));
...
void
dory_icon_container_set_font_size_table (DoryIconContainer *container,
					     const int font_size_table[DORY_ZOOM_LEVEL_LARGEST + 1])
{
	...
	for (i = 0; i <= DORY_ZOOM_LEVEL_LARGEST; i++) {
...
/**
 * dory_icon_container_get_icon_description
 * @container: An icon container widget.
 * @data: Icon data
 *
 ...
 **/
char*
dory_icon_container_get_icon_description (DoryIconContainer *container,
				              DoryIconData      *data)
{
	DoryIconContainerClass *klass;
...
gboolean
dory_icon_container_get_allow_moves (DoryIconContainer *container)
{
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), FALSE);
...
void
dory_icon_container_set_allow_moves	(DoryIconContainer *container,
...
void
dory_icon_container_set_forced_icon_size (DoryIconContainer *container,
...
void
dory_icon_container_set_all_columns_same_width (DoryIconContainer *container,
 ...
 **/
void
dory_icon_container_set_highlighted_for_clipboard (DoryIconContainer *container,
...
/* DoryIconContainerAccessible */

static DoryIconContainerAccessiblePrivate *
accessible_get_priv (AtkObject *accessible)
{
	DoryIconContainerAccessiblePrivate *priv;
...
static gboolean
dory_icon_container_accessible_do_action (AtkAction *accessible, int i)
{
	GtkWidget *widget;
	DoryIconContainer *container;
	GList *selection;
	...
	case ACTION_MENU :
		...
		break;
	default :
		g_warning ("Invalid action passed to DoryIconContainerAccessible::do_action");
		return FALSE;
	}
	return TRUE;
}
...
static int
dory_icon_container_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
dory_icon_container_accessible_action_get_description (AtkAction *accessible,
							   int i)
{
	DoryIconContainerAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return dory_icon_container_accessible_action_descriptions[i];
	}
}

static const char *
dory_icon_container_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return dory_icon_container_accessible_action_names[i];
}

static const char *
dory_icon_container_accessible_action_get_keybinding (AtkAction *accessible,
							  int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
dory_icon_container_accessible_action_set_description (AtkAction *accessible,
							   int i,
...
static void
dory_icon_container_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = dory_icon_container_accessible_do_action;
	iface->get_n_actions = dory_icon_container_accessible_get_n_actions;
	iface->get_description = dory_icon_container_accessible_action_get_description;
	iface->get_name = dory_icon_container_accessible_action_get_name;
	iface->get_keybinding = dory_icon_container_accessible_action_get_keybinding;
	iface->set_description = dory_icon_container_accessible_action_set_description;
}

/* AtkSelection interface */

static void
dory_icon_container_accessible_update_selection (AtkObject *accessible)
{
	DoryIconContainer *container;
	DoryIconContainerAccessiblePrivate *priv;
	GList *l;
	...
	container = DORY_ICON_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
...
static void
dory_icon_container_accessible_selection_changed_cb (DoryIconContainer *container,
							 gpointer data)
{
...
static void
dory_icon_container_accessible_icon_added_cb (DoryIconContainer *container,
...
static void
dory_icon_container_accessible_icon_removed_cb (DoryIconContainer *container,
						    DoryIconData *icon_data,
						    gpointer data)
{
	DoryIcon *icon;
...
static void
dory_icon_container_accessible_cleared_cb (DoryIconContainer *container,
					       gpointer data)
{
...
static gboolean
dory_icon_container_accessible_add_selection (AtkSelection *accessible,
						  int i)
{
	GtkWidget *widget;
	DoryIconContainer *container;
	...
	if (icon) {
		...
		selection = g_list_prepend (selection,
					    icon->data);
		dory_icon_container_set_selection (container, selection);
...
static gboolean
dory_icon_container_accessible_clear_selection (AtkSelection *accessible)
{
	GtkWidget *widget;
	DoryIconContainer *container;
	...
	dory_icon_container_unselect_all (container);

	return TRUE;
}

static AtkObject *
dory_icon_container_accessible_ref_selection (AtkSelection *accessible,
						  int i)
{
	...
	dory_icon_container_accessible_update_selection (ATK_OBJECT (accessible));
	priv = accessible_get_priv (ATK_OBJECT (accessible));
...
static int
dory_icon_container_accessible_get_selection_count (AtkSelection *accessible)
{
	int count;
	DoryIconContainerAccessiblePrivate *priv;
...
static gboolean
dory_icon_container_accessible_is_child_selected (AtkSelection *accessible,
						      int i)
{
	DoryIconContainer *container;
	DoryIcon *icon;
...
static gboolean
dory_icon_container_accessible_remove_selection (AtkSelection *accessible,
						     int i)
{
	DoryIconContainer *container;
	DoryIconContainerAccessiblePrivate *priv;
...
static gboolean
dory_icon_container_accessible_select_all_selection (AtkSelection *accessible)
{
	DoryIconContainer *container;
	GtkWidget *widget;
	...
	dory_icon_container_select_all (container);

	return TRUE;
}

void
dory_icon_container_widget_to_file_operation_position (DoryIconContainer *container,
							   GdkPoint              *position)
{
	double x, y;
	...
	position->x -= dory_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
	position->y -= dory_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
}

static void
dory_icon_container_accessible_selection_interface_init (AtkSelectionIface *iface)
{
	iface->add_selection = dory_icon_container_accessible_add_selection;
	iface->clear_selection = dory_icon_container_accessible_clear_selection;
	iface->ref_selection = dory_icon_container_accessible_ref_selection;
	iface->get_selection_count = dory_icon_container_accessible_get_selection_count;
	iface->is_child_selected = dory_icon_container_accessible_is_child_selected;
	iface->remove_selection = dory_icon_container_accessible_remove_selection;
	iface->select_all_selection = dory_icon_container_accessible_select_all_selection;
}


static gint
dory_icon_container_accessible_get_n_children (AtkObject *accessible)
{
	DoryIconContainer *container;
	GtkWidget *widget;
	gint i;
...
static AtkObject*
dory_icon_container_accessible_ref_child (AtkObject *accessible, int i)
{
        AtkObject *atk_object;
        DoryIconContainer *container;
        DoryIcon *icon;
...
static void
dory_icon_container_accessible_initialize (AtkObject *accessible,
					       gpointer data)
{
	DoryIconContainer *container;
	DoryIconContainerAccessiblePrivate *priv;
	...
	priv = g_new0 (DoryIconContainerAccessiblePrivate, 1);
	g_object_set_qdata (G_OBJECT (accessible),
			    accessible_private_data_quark,
			    priv);

	if (GTK_IS_ACCESSIBLE (accessible)) {
		dory_icon_container_accessible_update_selection
			(ATK_OBJECT (accessible));

		container = DORY_ICON_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
		g_signal_connect (G_OBJECT (container), "selection_changed",
				  G_CALLBACK (dory_icon_container_accessible_selection_changed_cb),
				  accessible);
		g_signal_connect (G_OBJECT (container), "icon_added",
				  G_CALLBACK (dory_icon_container_accessible_icon_added_cb),
				  accessible);
		g_signal_connect (G_OBJECT (container), "icon_removed",
				  G_CALLBACK (dory_icon_container_accessible_icon_removed_cb),
				  accessible);
		g_signal_connect (G_OBJECT (container), "cleared",
				  G_CALLBACK (dory_icon_container_accessible_cleared_cb),
				  accessible);
	}
}

static void
dory_icon_container_accessible_finalize (GObject *object)
{
	DoryIconContainerAccessiblePrivate *priv;
	int i;
...
static void
dory_icon_container_accessible_class_init (AtkObjectClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	accessible_parent_class = g_type_class_peek_parent (klass);

	gobject_class->finalize = dory_icon_container_accessible_finalize;

	klass->get_n_children = dory_icon_container_accessible_get_n_children;
	klass->ref_child = dory_icon_container_accessible_ref_child;
	klass->initialize = dory_icon_container_accessible_initialize;

	accessible_private_data_quark = g_quark_from_static_string ("icon-container-accessible-private-data");
}

static GType
dory_icon_container_accessible_get_type (void)
{
        static GType type = 0;

        if (!type) {
                static GInterfaceInfo atk_action_info = {
                        (GInterfaceInitFunc) dory_icon_container_accessible_action_interface_init,
                        (GInterfaceFinalizeFunc) NULL,
                        NULL
                };

                static GInterfaceInfo atk_selection_info = {
                        (GInterfaceInitFunc) dory_icon_container_accessible_selection_interface_init,
                        (GInterfaceFinalizeFunc) NULL,
                        NULL
                };
		...
		type = eel_accessibility_create_derived_type
			("DoryIconContainerAccessible",
			 EEL_TYPE_CANVAS,
			 dory_icon_container_accessible_class_init);
...
gboolean
dory_icon_container_is_layout_rtl (DoryIconContainer *container)
{
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), 0);
...
gboolean
dory_icon_container_is_layout_vertical (DoryIconContainer *container)
{
	g_return_val_if_fail (DORY_IS_ICON_CONTAINER (container), FALSE);

	return (container->details->layout_mode == DORY_ICON_LAYOUT_T_B_L_R ||
		container->details->layout_mode == DORY_ICON_LAYOUT_T_B_R_L);
}

gint
dory_icon_container_get_max_layout_lines_for_pango (DoryIconContainer  *container)
{
    return DORY_ICON_CONTAINER_GET_CLASS (container)->get_max_layout_lines_for_pango (container);
}

gint
dory_icon_container_get_max_layout_lines (DoryIconContainer  *container)
{
    return DORY_ICON_CONTAINER_GET_CLASS (container)->get_max_layout_lines (container);
}

void
dory_icon_container_begin_loading (DoryIconContainer *container)
{
	gboolean dummy;

    clear_drag_state (container);

	if (dory_icon_container_get_store_layout_timestamps (container)) {
...
void
dory_icon_container_store_layout_timestamps_now (DoryIconContainer *container)
{
	DoryIcon *icon;
	GList *p;
	gboolean dummy;
...
void
dory_icon_container_end_loading (DoryIconContainer *container,
				     gboolean               all_icons_added)
{
	if (all_icons_added &&
	    dory_icon_container_get_store_layout_timestamps (container)) {
...
gboolean
dory_icon_container_get_store_layout_timestamps (DoryIconContainer *container)
{
...
void
dory_icon_container_set_store_layout_timestamps (DoryIconContainer *container,
...
gint
dory_icon_container_get_canvas_height (DoryIconContainer *container,
                                       GtkAllocation      allocation)
{
...
gint
dory_icon_container_get_canvas_width (DoryIconContainer *container,
                                      GtkAllocation      allocation)
{
...
double
dory_icon_container_get_mirror_x_position (DoryIconContainer *container, DoryIcon *icon, double x)
{
    ...
    return dory_icon_container_get_canvas_width (container, allocation) - x - (icon_bounds.x1 - icon_bounds.x0);
}

void
dory_icon_container_set_rtl_positions (DoryIconContainer *container)
{
    GList *l;
    DoryIcon *icon;
    double x;
    ...
    for (l = container->details->icons; l != NULL; l = l->next) {
        icon = l->data;
        x = dory_icon_container_get_mirror_x_position (container, icon, icon->saved_ltr_x);
        dory_icon_container_icon_set_position (container, icon, x, icon->y);
    }
}

void
dory_icon_container_sort_icons (DoryIconContainer *container,
        GList                **icons)
{
    DoryIconContainerClass *klass;
...
void
dory_icon_container_resort (DoryIconContainer *container)
{
    dory_icon_container_sort_icons (container, &container->details->icons);
}

void
dory_icon_container_icon_raise (DoryIconContainer *container, DoryIcon *icon)
{
...
void
dory_icon_container_finish_adding_icon (DoryIconContainer *container,
            DoryIcon *icon)
{
...
void
dory_icon_container_update_selection (DoryIconContainer *container)
{
    g_return_if_fail (DORY_IS_ICON_CONTAINER (container));
    ...
    container->details->current_selection = dory_icon_container_get_real_selection (container);
    container->details->current_selection_count = g_list_length (container->details->current_selection);
}

void
dory_icon_container_move_icon (DoryIconContainer *container,
                   DoryIcon *icon,
                   int x, int y,
                   double scale,
                   gboolean raise,
                   ...
                   gboolean update_position)
{
    DORY_ICON_CONTAINER_GET_CLASS (container)->move_icon (container,
                                                          icon,
                                                          x, y,
                                                          scale,
                                                          raise,
...
void
dory_icon_container_icon_set_position (DoryIconContainer *container,
                                       DoryIcon          *icon,
                                       gdouble            x,
                                       gdouble            y)
{
    DORY_ICON_CONTAINER_GET_CLASS (container)->icon_set_position (container, icon, x, y);
}

void
dory_icon_container_icon_get_bounding_box (DoryIconContainer *container,
                                           DoryIcon *icon,
                                           int *x1_return, int *y1_return,
                                           int *x2_return, int *y2_return,
                                           DoryIconCanvasItemBoundsUsage usage)
{
    DORY_ICON_CONTAINER_GET_CLASS (container)->icon_get_bounding_box (icon, x1_return, y1_return, x2_return, y2_return, usage);
}

void
dory_icon_container_update_icon (DoryIconContainer *container,
                                 DoryIcon          *icon)
{
    gboolean ok = FALSE;
    ...
    DORY_ICON_CONTAINER_GET_CLASS (container)->update_icon (container, icon, ok);
}

gint
dory_icon_container_get_additional_text_line_count (DoryIconContainer *container)
{
    return DORY_ICON_CONTAINER_GET_CLASS (container)->get_additional_text_line_count (container);
}

void
dory_icon_container_set_ok_to_load_deferred_attrs (DoryIconContainer *container,
                                                   gboolean           ok)
{
...
void
dory_self_check_icon_container (void)
{
