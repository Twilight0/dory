...
struct DorySearchDirectoryDetails {
	DoryQuery *query;
	gboolean modified;

	DorySearchEngine *engine;
...
typedef struct {
	...
	DoryFileAttributes wait_for_attributes;
	gboolean wait_for_file_list;
	GList *file_list;
	GHashTable *non_ready_hash;
} SearchCallback;

G_DEFINE_TYPE (DorySearchDirectory, dory_search_directory,
	       DORY_TYPE_DIRECTORY);

static void search_engine_hits_added (DorySearchEngine *engine, GList *hits, DorySearchDirectory *search);
static void search_engine_hits_subtracted (DorySearchEngine *engine, GList *hits, DorySearchDirectory *search);
static void search_engine_finished (DorySearchEngine *engine, DorySearchDirectory *search);
static void search_engine_error (DorySearchEngine *engine, const char *error, DorySearchDirectory *search);
static void search_callback_file_ready_callback (DoryFile *file, gpointer data);
static void file_changed (DoryFile *file, DorySearchDirectory *search);

static void
ensure_search_engine (DorySearchDirectory *search)
{
	if (!search->details->engine) {
		search->details->engine = dory_search_engine_new ();
...
static void
reset_file_list (DorySearchDirectory *search)
{
	...
	for (list = search->details->files; list != NULL; list = list->next) {
		...
		for (monitor_list = search->details->monitor_list; monitor_list; 
			...
			dory_file_monitor_remove (file, monitor);
		}

        dory_file_clear_search_result_data (file, (gpointer) search);

	}
	
	dory_file_list_free (search->details->files);
	search->details->files = NULL;

    // DEBUG_FSR_ACCOUNTING
    dory_search_engine_report_accounting ();
}

static void
start_or_stop_search_engine (DorySearchDirectory *search, gboolean adding)
{
	if (adding && (search->details->monitor_list ||
        ...
        dory_query_set_show_hidden (search->details->query,
                                    g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_HIDDEN_FILES));

		dory_search_engine_set_query (search->details->engine, search->details->query);

		reset_file_list (search);

		dory_search_engine_start (search->details->engine);
	} else if (!adding && (!search->details->monitor_list ||
		   !search->details->pending_callback_list) &&
		   search->details->engine &&
		   search->details->search_running) {
		search->details->search_running = FALSE;
		dory_search_engine_stop (search->details->engine);
	}

}

static void
file_changed (DoryFile *file, DorySearchDirectory *search)
{
	GList list;
	...
	dory_directory_emit_files_changed (DORY_DIRECTORY (search), &list);
}

static void
search_monitor_add (DoryDirectory *directory,
		    gconstpointer client,
		    gboolean monitor_hidden_files,
		    ...
		    gpointer callback_data)
{
	GList *list;
	SearchMonitor *monitor;
	DorySearchDirectory *search;
	DoryFile *file;

	search = DORY_SEARCH_DIRECTORY (directory);
	...
	for (list = search->details->files; list != NULL; list = list->next) {
		...
		/* Add monitors */
		dory_file_monitor_add (file, monitor, file_attributes);
	}
...
static void
search_monitor_remove_file_monitors (SearchMonitor *monitor, DorySearchDirectory *search)
{
	GList *list;
	DoryFile *file;
...
static void
search_monitor_destroy (SearchMonitor *monitor, DorySearchDirectory *search)
{
...
static void
search_monitor_remove (DoryDirectory *directory,
		       gconstpointer client)
{
	DorySearchDirectory *search;
	SearchMonitor *monitor;
...
cancel_call_when_ready (gpointer key, gpointer value, gpointer user_data)
{
	...
	dory_file_cancel_call_when_ready (file, search_callback_file_ready_callback,
					      search_callback);
}
...
search_callback_destroy (SearchCallback *search_callback)
{
	...
	dory_file_list_free (search_callback->file_list);
...
{
	search_callback->callback (DORY_DIRECTORY (search_callback->search_directory),
...
static void
search_callback_file_ready_callback (DoryFile *file, gpointer data)
{
...
search_callback_find (DorySearchDirectory *search, DoryDirectoryCallback callback, gpointer callback_data)
{
	SearchCallback *search_callback;
	GList *list;
...
static SearchCallback *
search_callback_find_pending (DorySearchDirectory *search, DoryDirectoryCallback callback, gpointer callback_data)
{
	SearchCallback *search_callback;
	GList *list;
...
static void
search_call_when_ready (DoryDirectory *directory,
			DoryFileAttributes file_attributes,
			gboolean wait_for_file_list,
			DoryDirectoryCallback callback,
			gpointer callback_data)
{
	...
	if (wait_for_file_list && !search->details->search_finished) {
		...
		search_callback->file_list = dory_file_list_copy (search->details->files);
...
static void
search_cancel_callback (DoryDirectory *directory,
			DoryDirectoryCallback callback,
			gpointer callback_data)
{
	DorySearchDirectory *search;
...
static void
search_engine_hits_added (DorySearchEngine *engine, GList *hits, 
			  DorySearchDirectory *search)
{
	GList *hit;
	GList *file_list;
    ...
    for (hit = hits; hit != NULL; hit = hit->next) {
        ...
        file = dory_file_get_by_uri (fsr->uri);
        if (dory_file_add_search_result_data (file, (gpointer) search, fsr)) {
            for (monitor_list = search->details->monitor_list; monitor_list; monitor_list = monitor_list->next) {
                monitor = monitor_list->data;

                /* Add monitors */
                dory_file_monitor_add (file, monitor, monitor->monitor_attributes);
            }
	...
	dory_directory_emit_files_added (DORY_DIRECTORY (search), file_list);
    g_list_free (file_list);

	file = dory_directory_get_corresponding_file (DORY_DIRECTORY (search));
	dory_file_emit_changed (file);
	dory_file_unref (file);
}

static void
search_engine_hits_subtracted (DorySearchEngine *engine, GList *hits, 
			       DorySearchDirectory *search)
{
	GList *hit_list;
	GList *monitor_list;
	...
	dory_directory_emit_files_changed (DORY_DIRECTORY (search), file_list);
...
{
	callback->file_list = dory_file_list_copy (callback->search_directory->details->files);
...
static void
search_engine_error (DorySearchEngine *engine, const char *error_message, DorySearchDirectory *search)
{
	GError *error;

	error = g_error_new_literal (G_IO_ERROR, G_IO_ERROR_FAILED,
				     error_message);
	dory_directory_emit_load_error (DORY_DIRECTORY (search),
					    error);
	g_error_free (error);
}

static void
search_engine_finished (DorySearchEngine *engine, DorySearchDirectory *search)
{
	search->details->search_finished = TRUE;

	dory_directory_emit_done_loading (DORY_DIRECTORY (search));
...
static void
search_force_reload (DoryDirectory *directory)
{
	DorySearchDirectory *search;
...
static gboolean
search_are_all_files_seen (DoryDirectory *directory)
{
	DorySearchDirectory *search;
...
static gboolean
search_is_not_empty (DoryDirectory *directory)
{
	DorySearchDirectory *search;
...
static gboolean
search_contains_file (DoryDirectory *directory,
		      DoryFile *file)
{
    return dory_file_has_search_result (file, (gpointer) directory);
}

static GList *
search_get_file_list (DoryDirectory *directory)
{
	DorySearchDirectory *search;

	search = DORY_SEARCH_DIRECTORY (directory);

	return dory_file_list_copy (search->details->files);
}


static gboolean
search_is_editable (DoryDirectory *directory)
{
	return FALSE;
}
...
search_dispose (GObject *object)
{
	...
	search = DORY_SEARCH_DIRECTORY (object);
	...
	G_OBJECT_CLASS (dory_search_directory_parent_class)->dispose (object);
}
...
search_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (dory_search_directory_parent_class)->finalize (object);
}

static void
dory_search_directory_init (DorySearchDirectory *search)
{
	search->details = g_new0 (DorySearchDirectoryDetails, 1);
}

static void
dory_search_directory_class_init (DorySearchDirectoryClass *class)
{
	DoryDirectoryClass *directory_class;

	G_OBJECT_CLASS (class)->dispose = search_dispose;
	G_OBJECT_CLASS (class)->finalize = search_finalize;

	directory_class = DORY_DIRECTORY_CLASS (class);
...
char *
dory_search_directory_generate_new_uri (void)
{
...
void
dory_search_directory_set_query (DorySearchDirectory *search,
				     DoryQuery *query)
{
	DoryDirectory *dir;
	DoryFile *as_file;
	...
	dir = DORY_DIRECTORY (search);
	as_file = dir->details->as_file;
	if (as_file != NULL) {
		dory_search_directory_file_update_display_name (DORY_SEARCH_DIRECTORY_FILE (as_file));
	}
}

DoryQuery *
dory_search_directory_get_query (DorySearchDirectory *search)
{
...
gboolean
dory_search_directory_is_modified (DorySearchDirectory *search)
{
