...
/* dory-file-utilities.c - implementation of file manipulation routines.
...
#define DORY_USER_DIRECTORY_NAME "dory"
...
static GFile *dory_find_file_insensitive_next (GFile *parent, const gchar *name);

char *
dory_compute_title_for_location (GFile *location)
{
	DoryFile *file;
	char *title;
    char *builder;
	...
	builder = NULL;
	if (location) {
		file = dory_file_get (location);
		builder = dory_file_get_description (file);
		if (builder == NULL) {
			builder = dory_file_get_display_name (file);
		}
    ...
    if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_FULL_PATH_TITLES)) {
        gchar *uri, *path;
        file = dory_file_get (location);
...
// TODO: Maybe this can replace dory_compute_title_for_location() all around?
char *
dory_compute_search_title_for_location (GFile *location)
{
    GFile *home_file;
...
/**
 * dory_get_user_directory:
 *
 * Get the path for the directory containing dory settings.
 *
 * Return value: the directory path.
 **/
char *
dory_get_user_directory (void)
{
	char *user_directory = NULL;

	user_directory = g_build_filename (g_get_user_config_dir (),
					   DORY_USER_DIRECTORY_NAME,
					   NULL);

	if (!g_file_test (user_directory, G_FILE_TEST_EXISTS)) {
		g_mkdir_with_parents (user_directory, DEFAULT_DORY_DIRECTORY_MODE);
		/* FIXME bugzilla.gnome.org 41286:
		 * How should we handle the case where this mkdir fails?
		 * Note that dory_application_startup will refuse to launch if this
		 * directory doesn't get created, so that case is OK. But the directory
		 * could be deleted after Dory was launched, and perhaps
...
/**
 * dory_get_accel_map_file:
 *
 * Get the path for the filename containing dory accelerator map.
 * The filename need not exist.
 *
 * Return value: the filename path, or NULL if the home directory could not be found
 **/
char *
dory_get_accel_map_file (void)
{
	...
	if (override) {
		return g_build_filename (override, "accels/dory", NULL);
	} else {
		return g_build_filename (g_get_home_dir (), ".gnome2/accels/dory", NULL);
	}
}

/**
 * dory_get_scripts_directory_path:
 *
 * Get the path for the directory containing dory scripts.
 *
 * Return value: the directory path containing dory scripts
 **/
char *
dory_get_scripts_directory_path (void)
{
	return g_build_filename (g_get_user_data_dir (), "dory", "scripts", NULL);
}
...
{
	g_signal_emit_by_name (dory_signaller_get_current (),
			       "user_dirs_changed");
	...
	if (cached_xdg_dirs != NULL) {
		for (i = 0; cached_xdg_dirs[i].type != NULL; i++) {
			if (cached_xdg_dirs[i].file != NULL) {
				dory_file_monitor_remove (cached_xdg_dirs[i].file,
							      &cached_xdg_dirs[i]);
				g_signal_handlers_disconnect_by_func (cached_xdg_dirs[i].file,
								      G_CALLBACK (xdg_dir_changed),
								      &cached_xdg_dirs[i]);
				dory_file_unref (cached_xdg_dirs[i].file);
			}
...
update_xdg_dir_cache (void)
{
	...
	for (i = 0 ; cached_xdg_dirs[i].type != NULL; i++) {
		...
		if (strcmp (cached_xdg_dirs[i].path, g_get_home_dir ()) != 0) {
			...
			cached_xdg_dirs[i].file = dory_file_get_by_uri (uri);
			dory_file_monitor_add (cached_xdg_dirs[i].file,
						   &cached_xdg_dirs[i],
...
char *
dory_get_xdg_dir (const char *type)
{
	int i;
...
{
	if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_DESKTOP_IS_HOME_DIR)) {
		return g_strdup (g_get_home_dir());
	} else {
		return dory_get_xdg_dir ("DESKTOP");
	}
}

/**
 * dory_get_desktop_directory:
 *
 ...
 **/
char *
dory_get_desktop_directory (void)
{
...
GFile *
dory_get_desktop_location (void)
{
	char *desktop_directory;
	GFile *res;
...
/**
 * dory_get_desktop_directory_uri:
 *
 ...
 **/
char *
dory_get_desktop_directory_uri (void)
{
	char *desktop_path;
	char *desktop_uri;
...
char *
dory_get_desktop_directory_uri_no_create (void)
{
	char *desktop_path;
	char *desktop_uri;
...
char *
dory_get_home_directory_uri (void)
{
...
gboolean
dory_should_use_templates_directory (void)
{
	char *dir;
	gboolean res;

	dir = dory_get_xdg_dir ("TEMPLATES");
...
char *
dory_get_templates_directory (void)
{
	return dory_get_xdg_dir ("TEMPLATES");
}

void
dory_ensure_valid_templates_directory (void)
{
    ...
    templates_dir = dory_get_xdg_dir ("TEMPLATES");
    ...
    if (g_strcmp0 (templates_dir, home_dir) == 0) {
        ...
        g_mkdir (fixed_templates_dir, DEFAULT_DORY_DIRECTORY_MODE);
        ...
        g_mkdir (templates_dir, DEFAULT_DORY_DIRECTORY_MODE);
    }
}

char *
dory_get_templates_directory_uri (void)
{
	char *directory, *uri;

	directory = dory_get_templates_directory ();
...
char *
dory_get_searches_directory (void)
{
	char *user_dir;
	char *searches_dir;

	user_dir = dory_get_user_directory ();
	searches_dir = g_build_filename (user_dir, "searches", NULL);
	g_free (user_dir);

	if (!g_file_test (searches_dir, G_FILE_TEST_EXISTS))
		g_mkdir (searches_dir, DEFAULT_DORY_DIRECTORY_MODE);
...
gboolean
dory_is_home_directory_file (GFile *dir,
				 const char *filename)
{
	char *dirname;
...
gboolean
dory_is_home_directory (GFile *dir)
{
...
gboolean
dory_is_root_directory (GFile *dir)
{
...
gboolean
dory_is_desktop_directory_file (GFile *dir,
				    const char *file)
{
...
gboolean
dory_is_desktop_directory (GFile *dir)
{
...
/**
 * dory_get_gmc_desktop_directory:
 *
 ...
 **/
char *
dory_get_gmc_desktop_directory (void)
{
	return g_build_filename (g_get_home_dir (), LEGACY_DESKTOP_DIRECTORY_NAME, NULL);
}

char *
dory_get_data_file_path (const char *partial_path)
{
	char *path;
	char *user_directory;

	/* first try the user's home directory */
	user_directory = dory_get_user_directory ();
	...
	path = g_build_filename (DORY_DATADIR, partial_path, NULL);
	if (g_file_test (path, G_FILE_TEST_EXISTS)) {
		return path;
	}
	g_free (path);
...
char *
dory_ensure_unique_file_name (const char *directory_uri,
...
char *
dory_unique_temporary_file_name (void)
{
	const char *prefix = "/tmp/dory-temp-file";
	char *file_name;
	int fd;
...
GFile *
dory_find_existing_uri_in_hierarchy (GFile *location)
{
	GFileInfo *info;
	GFile *tmp;
...
dory_find_file_insensitive (GFile *parent, const gchar *name)
{
	...
	for (i = 0; (component = split_path[i]) != NULL; i++) {
		if (!(next = dory_find_file_insensitive_next (file,
		                                                  component))) {
			/* File does not exist */
			g_object_unref (file);
			file = NULL;
...
static GFile *
dory_find_file_insensitive_next (GFile *parent, const gchar *name)
{
...
gboolean
dory_is_file_roller_installed (void)
{
 ...
 * (e.g. whenever Dory is doing file operations).
 *
 * Returns: an integer cookie, which must be passed to
 *    dory_uninhibit_power_manager() to resume
 *    normal power management.
 */
int
dory_inhibit_power_manager (const char *message)
{
...
/**
 * dory_uninhibit_power_manager:
 * @cookie: the cookie value returned by dory_inhibit_power_manager()
 *
 ...
 */
void
dory_uninhibit_power_manager (gint cookie)
{
	GDBusConnection *connection;
	GVariant *result;
	GError *error = NULL;
...
gboolean
dory_is_in_system_dir (GFile *file)
{
...
GHashTable *
dory_trashed_files_get_original_directories (GList *files,
						 GList **unhandled_files)
{
	GHashTable *directories;
	DoryFile *file, *original_file, *original_dir;
	GList *l, *m;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		original_file = dory_file_get_trash_original_file (file);

		original_dir = NULL;
		if (original_file != NULL) {
			original_dir = dory_file_get_parent (original_file);
		}

		if (original_dir != NULL) {
			if (directories == NULL) {
				directories = g_hash_table_new_full (g_direct_hash, g_direct_equal,
								     (GDestroyNotify) dory_file_unref,
								     (GDestroyNotify) dory_file_list_free);
			}
			dory_file_ref (original_dir);
			m = g_hash_table_lookup (directories, original_dir);
			if (m != NULL) {
				g_hash_table_steal (directories, original_dir);
				dory_file_unref (original_dir);
			}
			m = g_list_append (m, dory_file_ref (file));
			g_hash_table_insert (directories, original_dir, m);
		} else if (unhandled_files != NULL) {
			*unhandled_files = g_list_append (*unhandled_files, dory_file_ref (file));
		}
...
locations_from_file_list (GList *file_list)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		file = DORY_FILE (l->data);
		ret = g_list_prepend (ret, dory_file_get_location (file));
	}
			   ...
			   gpointer user_data)
{
	...
	for (l = original_dirs; l != NULL; l = l->next) {
		original_dir = DORY_FILE (l->data);
		original_dir_location = dory_file_get_location (original_dir);

		files = g_hash_table_lookup (data->original_dirs_hash, original_dir);
		locations = locations_from_file_list (files);

		dory_file_operations_move
			(locations, NULL,
...
void
dory_restore_files_from_trash (GList *files,
				   GtkWindow *parent_window)
{
	DoryFile *file;
	GHashTable *original_dirs_hash;
	...
	original_dirs_hash = dory_trashed_files_get_original_directories (files, &unhandled_files);
	...
	dory_file_list_unref (unhandled_files);
}
...
void
dory_get_x_content_types_for_mount_async (GMount *mount,
					      DoryMountGetContent callback,
					      GCancellable *cancellable,
					      gpointer user_data)
{
	...
	cached = g_object_get_data (G_OBJECT (mount), "dory-content-type-cache");
	if (cached != NULL) {
		if (callback) {
...
char **
dory_get_cached_x_content_types_for_mount (GMount *mount)
{
	char **cached;
...
debug_icon_names (const gchar *format, ...)
{
    ...
    if (g_once_init_enter (&once_init)) {
        debug_removable_device_icons = g_getenv ("DORY_DEBUG_DEVICE_ICONS") != NULL;
...
gchar *
dory_get_mount_icon_name (GMount *mount)
{
...
gchar *
dory_get_volume_icon_name (GVolume *volume)
{
    GtkIconTheme *icon_theme;
    GIcon *gicon;
    gchar *icon_name;
...
gchar *
dory_get_drive_icon_name (GDrive *drive)
{
    GtkIconTheme *icon_theme;
    GIcon *gicon;
    gchar *icon_name;
...
gchar *
dory_get_best_guess_file_mimetype (const gchar *filename,
                                   GFileInfo   *info,
                                   goffset      size)
{
...
gboolean
dory_treating_root_as_normal (void)
{
    ...
    if (g_once_init_enter (&once_init)) {
        GSettings *prefs = g_settings_new("org.dory.preferences");
...
GMount *
dory_get_mount_for_location_safe (GFile *location)
{
...
gboolean
dory_location_is_network_safe (GFile *location)
{
    GVolume *volume;
    GMount *mount;
    gboolean is_network = FALSE;

    mount = dory_get_mount_for_location_safe (location);
    if (mount != NULL) {
...
gboolean
dory_path_is_network_safe (const gchar *path)
{
    g_autoptr(GFile) location = g_file_new_for_path (path);

    return dory_location_is_network_safe (location);
}

#if !defined (DORY_OMIT_SELF_CHECK)

void
dory_self_check_file_utilities (void)
{
}
