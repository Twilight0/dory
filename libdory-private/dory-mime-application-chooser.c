...
/*
 *  dory-mime-application-chooser.c: an mime-application chooser
 *
...
#include <config.h>
#include "dory-mime-application-chooser.h"
...
struct _DoryMimeApplicationChooserDetails {
	GList *files;
	char *uri;
...
G_DEFINE_TYPE (DoryMimeApplicationChooser, dory_mime_application_chooser, GTK_TYPE_BOX);

static void
add_clicked_cb (GtkButton *button,
		gpointer user_data)
{
	DoryMimeApplicationChooser *chooser = user_data;
	GAppInfo *info;
...
reset_clicked_cb (GtkButton *button,
                  gpointer   user_data)
{
	DoryMimeApplicationChooser *chooser;

	chooser = DORY_MIME_APPLICATION_CHOOSER (user_data);
...
static void
custom_app_set (DoryMimeApplicationChooser *chooser,
...
static gchar *
get_extension_from_file (DoryFile *nfile)
{
	char *name;
	char *extension;

	name = dory_file_get_name (nfile);
...
static void
dory_mime_application_chooser_apply_labels (DoryMimeApplicationChooser *chooser)
{
	...
	if (chooser->details->files != NULL) {
		...
		if (g_content_type_is_unknown (chooser->details->content_type)) {
			extension = get_extension_from_file (DORY_FILE (chooser->details->files->data));
...
static void
dory_mime_application_chooser_build_ui (DoryMimeApplicationChooser *chooser)
{
...
static void
dory_mime_application_chooser_init (DoryMimeApplicationChooser *chooser)
{
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE (chooser, DORY_TYPE_MIME_APPLICATION_CHOOSER,
							DoryMimeApplicationChooserDetails);
...
static void
dory_mime_application_chooser_constructed (GObject *object)
{
	DoryMimeApplicationChooser *chooser = DORY_MIME_APPLICATION_CHOOSER (object);

	if (G_OBJECT_CLASS (dory_mime_application_chooser_parent_class)->constructed != NULL)
		G_OBJECT_CLASS (dory_mime_application_chooser_parent_class)->constructed (object);

    gtk_widget_set_halign (GTK_WIDGET (object), GTK_ALIGN_CENTER);

	dory_mime_application_chooser_build_ui (chooser);
	dory_mime_application_chooser_apply_labels (chooser);
}

static void
dory_mime_application_chooser_finalize (GObject *object)
{
	DoryMimeApplicationChooser *chooser;

	chooser = DORY_MIME_APPLICATION_CHOOSER (object);
	...
	G_OBJECT_CLASS (dory_mime_application_chooser_parent_class)->finalize (object);
}

static void
dory_mime_application_chooser_get_property (GObject *object,
						guint property_id,
						GValue *value,
						GParamSpec *pspec)
{
...
static void
dory_mime_application_chooser_set_property (GObject *object,
						guint property_id,
						const GValue *value,
						GParamSpec *pspec)
{
...
static void
dory_mime_application_chooser_class_init (DoryMimeApplicationChooserClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);
	gobject_class->set_property = dory_mime_application_chooser_set_property;
	gobject_class->get_property = dory_mime_application_chooser_get_property;
	gobject_class->finalize = dory_mime_application_chooser_finalize;
	gobject_class->constructed = dory_mime_application_chooser_constructed;
	...
	g_type_class_add_private (class, sizeof (DoryMimeApplicationChooserDetails));
}

GtkWidget *
dory_mime_application_chooser_new (const char *uri,
                                        GList *files,
                                    ...
                                    GtkWidget *ok_button)
{
	...
	chooser = g_object_new (DORY_TYPE_MIME_APPLICATION_CHOOSER,
				"uri", uri,
				"files", files,
...
GAppInfo *
dory_mime_application_chooser_get_info (DoryMimeApplicationChooser *chooser)
{
    if (chooser->details->custom_info == NULL)
...
const gchar *
dory_mime_application_chooser_get_uri (DoryMimeApplicationChooser *chooser)
{
