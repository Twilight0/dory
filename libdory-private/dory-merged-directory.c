/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-merged-directory.c: Subclass of DoryDirectory to implement the
...
#include <config.h>
#include "dory-merged-directory.h"
...
struct DoryMergedDirectoryDetails {
	GList *directories;
...
G_DEFINE_TYPE (DoryMergedDirectory, dory_merged_directory,
	       DORY_TYPE_DIRECTORY);
...
merged_callback_destroy (MergedCallback *merged_callback)
{
	...
	g_assert (DORY_IS_MERGED_DIRECTORY (merged_callback->merged));

	g_list_free (merged_callback->non_ready_directories);
	dory_file_list_free (merged_callback->merged_file_list);
...
merged_callback_check_done (MergedCallback *merged_callback)
{
	...
	(* merged_callback->callback) (DORY_DIRECTORY (merged_callback->merged),
...
static void
directory_ready_callback (DoryDirectory *directory,
			  GList *files,
			  gpointer callback_data)
{
...
static void
merged_call_when_ready (DoryDirectory *directory,
			...
			gpointer callback_data)
{
	...
	merged = DORY_MERGED_DIRECTORY (directory);
...
static void
merged_cancel_callback (DoryDirectory *directory,
			DoryDirectoryCallback callback,
			gpointer callback_data)
{
	DoryMergedDirectory *merged;
	...
	for (node = merged_callback->non_ready_directories; node != NULL; node = node->next) {
		dory_directory_cancel_callback
			(node->data,
			 directory_ready_callback, merged_callback);
	}
	merged_callback_destroy (merged_callback);
...
static void
build_merged_callback_list (DoryDirectory *directory,
			    GList *file_list,
			    gpointer callback_data)
{
	GList **merged_list;
	...
	*merged_list = g_list_concat (*merged_list,
				      dory_file_list_copy (file_list));
}

/* Create a monitor on each of the directories in the list. */
static void
merged_monitor_add (DoryDirectory *directory,
		    ...
		    gpointer callback_data)
{
	...
	dory_file_list_free (merged_callback_list);
}

static void
merged_monitor_destroy (DoryMergedDirectory *merged, MergedMonitor *monitor)
{
	GList *node;

	/* Call through to the real directory remove calls. */
	for (node = merged->details->directories; node != NULL; node = node->next) {
		dory_directory_file_monitor_remove (node->data, monitor);
	}
...
static void
merged_monitor_remove (DoryDirectory *directory,
		       gconstpointer client)
{
	DoryMergedDirectory *merged;
	MergedMonitor *monitor;
...
static void
merged_force_reload (DoryDirectory *directory)
{
	DoryMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		dory_directory_force_reload (node->data);
	}
}

/* Return true if any directory in the list does. */
static gboolean
merged_contains_file (DoryDirectory *directory,
		      DoryFile *file)
{
	DoryMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		if (dory_directory_contains_file (node->data, file)) {
			return TRUE;
		}
	}
	return FALSE;
...
static gboolean
merged_are_all_files_seen (DoryDirectory *directory)
{
	DoryMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		if (!dory_directory_are_all_files_seen (node->data)) {
			return FALSE;
		}
	}
	return TRUE;
...
/* Return true if any directory in the list does. */
static gboolean
merged_is_not_empty (DoryDirectory *directory)
{
	DoryMergedDirectory *merged;
	GList *node;
	...
	for (node = merged->details->directories; node != NULL; node = node->next) {
		if (dory_directory_is_not_empty (node->data)) {
			return TRUE;
		}
	}
	return FALSE;
...
static GList *
merged_get_file_list (DoryDirectory *directory)
{
	...
	dirs_file_list = NULL;
	dir_list = DORY_MERGED_DIRECTORY (directory)->details->directories;

	for (cur_node = dir_list; cur_node != NULL; cur_node = cur_node->next) {
		DoryDirectory *cur_dir;

		cur_dir = DORY_DIRECTORY (cur_node->data);
		dirs_file_list = g_list_concat (dirs_file_list,
						 dory_directory_get_file_list (cur_dir));
	}

	merged_dir_file_list = DORY_DIRECTORY_CLASS 
		(dory_merged_directory_parent_class)->get_file_list (directory);

	return g_list_concat (dirs_file_list, merged_dir_file_list);
}

static void
forward_files_added_cover (DoryDirectory *real_directory,
			   GList *files,
			   gpointer callback_data)
{
	dory_directory_emit_files_added (DORY_DIRECTORY (callback_data), files);
}

static void
forward_files_changed_cover (DoryDirectory *real_directory,
			     GList *files,
			     gpointer callback_data)
{
	dory_directory_emit_files_changed (DORY_DIRECTORY (callback_data), files);
}

static void
done_loading_callback (DoryDirectory *real_directory,
		       DoryMergedDirectory *merged)
{
	...
	if (merged->details->directories_not_done_loading == NULL) {
		dory_directory_emit_done_loading (DORY_DIRECTORY (merged));
	}
}
		       ...
		       gpointer callback_data)
{
	...
	monitor = value;
	dory_directory_file_monitor_add
		(DORY_DIRECTORY (callback_data), monitor,
...
static void
merged_add_real_directory (DoryMergedDirectory *merged,
			   DoryDirectory *real_directory)
{
	g_return_if_fail (DORY_IS_MERGED_DIRECTORY (merged));
	g_return_if_fail (DORY_IS_DIRECTORY (real_directory));
	g_return_if_fail (!DORY_IS_MERGED_DIRECTORY (real_directory));
	g_return_if_fail (g_list_find (merged->details->directories, real_directory) == NULL);

	/* Add to our list of directories. */
	dory_directory_ref (real_directory);
...
void
dory_merged_directory_add_real_directory (DoryMergedDirectory *merged,
					      DoryDirectory *real_directory)
{
	g_return_if_fail (DORY_IS_MERGED_DIRECTORY (merged));
	g_return_if_fail (DORY_IS_DIRECTORY (real_directory));
...
GList *
dory_merged_directory_get_real_directories (DoryMergedDirectory *merged)
{
					...
					gpointer callback_data)
{
	merged_callback_remove_directory
		(value, DORY_DIRECTORY (callback_data));
}
...
monitor_remove_directory (gpointer key,
			  ...
			  gpointer callback_data)
{
	dory_directory_file_monitor_remove
		(DORY_DIRECTORY (callback_data), value);
}

static void
real_directory_notify_files_removed (DoryDirectory *real_directory)
{
	GList *files, *l;

	files = dory_directory_get_file_list (real_directory);
	...
	if (files) {
		dory_directory_notify_files_removed_by_uri (files);
	}
...
static void
merged_remove_real_directory (DoryMergedDirectory *merged,
			      DoryDirectory *real_directory)
{
	g_return_if_fail (DORY_IS_MERGED_DIRECTORY (merged));
	g_return_if_fail (DORY_IS_DIRECTORY (real_directory));
	...
	merged->details->directories_not_done_loading = g_list_remove
		(merged->details->directories_not_done_loading, real_directory);
	dory_directory_unref (real_directory);
}

void
dory_merged_directory_remove_real_directory (DoryMergedDirectory *merged,
						 DoryDirectory *real_directory)
{
	g_return_if_fail (DORY_IS_MERGED_DIRECTORY (merged));
...
merged_finalize (GObject *object)
{
	...
	merged = DORY_MERGED_DIRECTORY (object);
	...
	dory_directory_list_free (merged->details->directories);
	g_list_free (merged->details->directories_not_done_loading);

	G_OBJECT_CLASS (dory_merged_directory_parent_class)->finalize (object);
}

static void
dory_merged_directory_init (DoryMergedDirectory *merged)
{
	merged->details = G_TYPE_INSTANCE_GET_PRIVATE (merged, DORY_TYPE_MERGED_DIRECTORY,
						       DoryMergedDirectoryDetails);
...
static void
dory_merged_directory_class_init (DoryMergedDirectoryClass *class)
{
	 ...
	 * the files from each of the directories in DoryMergedDirectory->details->directories.
         */
	...
	g_type_class_add_private (class, sizeof (DoryMergedDirectoryDetails));

	signals[ADD_REAL_DIRECTORY] 
		= g_signal_new ("add_real_directory",
		                G_TYPE_FROM_CLASS (class),
		                G_SIGNAL_RUN_LAST,
		                G_STRUCT_OFFSET (DoryMergedDirectoryClass, 
						 add_real_directory),
		                NULL, NULL,
