/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-link.c: .desktop link files.
...
#define DORY_LINK_GENERIC_TAG	"Link"
#define DORY_LINK_TRASH_TAG 	"X-dory-trash"
#define DORY_LINK_MOUNT_TAG 	"FSDevice"
#define DORY_LINK_HOME_TAG 		"X-dory-home"
...
gboolean
dory_link_local_create (const char     *directory_uri,
			    ...
			    gboolean        unique_filename)
{
	...
	if (eel_uri_is_desktop (directory_uri)) {
		real_directory_uri = dory_get_desktop_directory_uri ();
	} else {
		real_directory_uri = g_strdup (directory_uri);
	}

	if (unique_filename) {
		uri = dory_ensure_unique_file_name (real_directory_uri,
							base_name, ".desktop");
		if (uri == NULL) {
	...
	dory_directory_notify_files_added (&dummy_list);
	...
	if (point != NULL) {
		...
		dummy_list.next = NULL;
		dummy_list.prev = NULL;
		dory_directory_schedule_position_set (&dummy_list);
	}
...
static gboolean
dory_link_local_set_key (const char *uri,
			     const char *key,
			     const char *value,
			     gboolean    localize)
{
...
gboolean
dory_link_local_set_text (const char *uri,
			      const char *text)
{
	return dory_link_local_set_key (uri, "Name", text, TRUE);
}


gboolean
dory_link_local_set_icon (const char        *uri,
			      const char        *icon)
{
	return dory_link_local_set_key (uri, "Icon", icon, FALSE);
}

char *
dory_link_local_get_text (const char *path)
{
	return slurp_key_string (path, "Name", TRUE);
}

static char *
dory_link_get_link_uri_from_desktop (GKeyFile *key_file, const char *desktop_file_uri)
{
	GFile *file, *parent;
	char *type;
	char *retval;
	...
	} else if ((strcmp (type, DORY_LINK_GENERIC_TAG) == 0) ||
		   (strcmp (type, DORY_LINK_MOUNT_TAG) == 0) ||
		   (strcmp (type, DORY_LINK_TRASH_TAG) == 0) ||
		   (strcmp (type, DORY_LINK_HOME_TAG) == 0)) {
...
static char *
dory_link_get_link_name_from_desktop (GKeyFile *key_file)
{
	return g_key_file_get_locale_string (key_file, MAIN_GROUP, "Name", NULL, NULL);
}

static GIcon *
dory_link_get_link_icon_from_desktop (GKeyFile *key_file)
{
	...
	if (icon_str == NULL) {
		if (g_strcmp0 (type, "Application") == 0) {
			...
			icon_str = g_strdup (DORY_ICON_FOLDER);
...
char *
dory_link_local_get_link_uri (const char *uri)
{
	GKeyFile *key_file;
	char *retval;
	...
	retval = dory_link_get_link_uri_from_desktop (key_file, uri);
	g_key_file_free (key_file);
...
void
dory_link_get_link_info_given_file_contents (const char  *file_contents,
						 int          link_file_size,
						 const char  *file_uri,
						 char       **uri,
						 char       **name,
						 ...
						 gboolean    *is_foreign)
{
	...
	*uri = dory_link_get_link_uri_from_desktop (key_file, file_uri);
	*name = dory_link_get_link_name_from_desktop (key_file);
	*icon = dory_link_get_link_icon_from_desktop (key_file);
