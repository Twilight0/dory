...
/* dory-column-chooser.h - A column chooser widget
...
#include <config.h>
#include "dory-column-chooser.h"

#include <string.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

#include "dory-column-utilities.h"

struct _DoryColumnChooserDetails
{
	GtkTreeView *view;
	GtkListStore *store;
...
G_DEFINE_TYPE(DoryColumnChooser, dory_column_chooser, GTK_TYPE_BOX);

static void dory_column_chooser_constructed (GObject *object);

static void
dory_column_chooser_set_property (GObject *object,
				      guint param_id,
				      const GValue *value,
				      GParamSpec *pspec)
{
	DoryColumnChooser *chooser;

	chooser = DORY_COLUMN_CHOOSER (object);
...
static void
dory_column_chooser_class_init (DoryColumnChooserClass *chooser_class)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (chooser_class);

	oclass->set_property = dory_column_chooser_set_property;
	oclass->constructed = dory_column_chooser_constructed;

	signals[CHANGED] = g_signal_new
		("changed",
		 G_TYPE_FROM_CLASS (chooser_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (DoryColumnChooserClass,
				  changed),
		 NULL, NULL,
	...
	g_type_class_add_private (chooser_class, sizeof (DoryColumnChooserDetails));
}

static void 
update_buttons (DoryColumnChooser *chooser)
{
...
static void
list_changed (DoryColumnChooser *chooser) 
{
			  ...
			  gpointer user_data)
{
	...
	chooser = DORY_COLUMN_CHOOSER (user_data);
...
{
	update_buttons (DORY_COLUMN_CHOOSER (user_data));
}
...
row_deleted_callback (GtkTreeModel *model, 
		       ...
		       gpointer user_data)
{
	list_changed (DORY_COLUMN_CHOOSER (user_data));
}

static void
add_tree_view (DoryColumnChooser *chooser)
{
	GtkWidget *scrolled;
	GtkWidget *view;
	GtkListStore *store;
...
{
	g_signal_emit (DORY_COLUMN_CHOOSER (user_data), 
...
static void
add_buttons (DoryColumnChooser *chooser)
{
	GtkWidget *box;
	GtkWidget *separator;
...
static void
populate_tree (DoryColumnChooser *chooser)
{
	GList *columns;
	GList *l;

	columns = dory_get_columns_for_file (chooser->details->file);
...
static void
dory_column_chooser_constructed (GObject *object)
{
	DoryColumnChooser *chooser;
...
static void
dory_column_chooser_init (DoryColumnChooser *chooser)
{	
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE ((chooser), DORY_TYPE_COLUMN_CHOOSER, DoryColumnChooserDetails);
...
static void 
set_visible_columns (DoryColumnChooser *chooser,
...
static char **
get_column_names (DoryColumnChooser *chooser, gboolean only_visible)
{
	GPtrArray *ret;
	GtkTreeIter iter;
...
static gboolean
get_column_iter (DoryColumnChooser *chooser, 
		 DoryColumn *column,
		 GtkTreeIter *iter)
{
	char *column_name;

	g_object_get (DORY_COLUMN (column), "name", &column_name, NULL);
...
static void
set_column_order (DoryColumnChooser *chooser,
		  char **column_order)

{
	...
	columns = dory_get_columns_for_file (chooser->details->file);	
	columns = dory_sort_columns (columns, column_order);
	...
	for (l = columns; l != NULL; l = l->next) {
		...
		if (get_column_iter (chooser, DORY_COLUMN (l->data), &iter)) {
			GtkTreeIter before;
			if (path) {
				gtk_tree_model_get_iter (GTK_TREE_MODEL (chooser->details->store),
							 &before, path);
...
void
dory_column_chooser_set_settings (DoryColumnChooser *chooser,
				      char **visible_columns,
				      char **column_order)
{
	g_return_if_fail (DORY_IS_COLUMN_CHOOSER (chooser));
...
void
dory_column_chooser_get_settings (DoryColumnChooser *chooser,
				      char ***visible_columns,
				      char ***column_order)
{
	g_return_if_fail (DORY_IS_COLUMN_CHOOSER (chooser));
...
GtkWidget *
dory_column_chooser_new (DoryFile *file)
{
	return g_object_new (DORY_TYPE_COLUMN_CHOOSER, "file", file, NULL);
}
