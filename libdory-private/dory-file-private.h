...
#ifndef DORY_FILE_PRIVATE_H
#define DORY_FILE_PRIVATE_H
...
#define DORY_FILE_DEFAULT_ATTRIBUTES				\
...
struct DoryFileDetails
{
	...
	/* Info you might get from a link (.desktop, .directory or dory link) */
	GIcon *custom_icon;
	char *activation_uri;
	...
	/* DoryInfoProviders that need to be run for this file */
	...
	/* Set by the DoryDirectory while it's loading the file
	...
	eel_boolean_bit deep_counts_status      : 2; /* DoryRequestStatus */
    ...
    DoryFileMetaState pinning;
    DoryFileMetaState favorite;
...
DoryFile *dory_file_new_from_info                  (DoryDirectory      *directory,
							    GFileInfo              *info);
void          dory_file_emit_changed                   (DoryFile           *file);
void          dory_file_mark_gone                      (DoryFile           *file);

void          dory_file_set_directory                  (DoryFile           *file,
							    DoryDirectory      *directory);
gboolean      dory_file_get_date                       (DoryFile           *file,
							    DoryDateType        date_type,
							    time_t                 *date);
void          dory_file_updated_deep_count_in_progress (DoryFile           *file);


void          dory_file_clear_info                     (DoryFile           *file);
/* Compare file's state with a fresh file info struct, return FALSE if
 * no change, update file and return TRUE if the file info contains
 * new state.  */
gboolean      dory_file_update_info                    (DoryFile           *file,
							    GFileInfo              *info);
gboolean      dory_file_update_name                    (DoryFile           *file,
							    const char             *name);
gboolean      dory_file_update_metadata_from_info      (DoryFile           *file,
							    GFileInfo              *info);

gboolean      dory_file_update_name_and_directory      (DoryFile           *file,
							    const char             *name,
							    DoryDirectory      *directory);

gboolean      dory_file_set_display_name               (DoryFile           *file,
 ...
 */
void                   dory_file_invalidate_attributes_internal     (DoryFile           *file,
									 DoryFileAttributes  file_attributes);
DoryFileAttributes dory_file_get_all_attributes                 (void);
gboolean               dory_file_is_self_owned                      (DoryFile           *file);
void                   dory_file_invalidate_count_and_mime_list     (DoryFile           *file);
gboolean               dory_file_rename_in_progress                 (DoryFile           *file);
void                   dory_file_invalidate_extension_info_internal (DoryFile           *file);
void                   dory_file_info_providers_done                (DoryFile           *file);


/* Thumbnailing: */
void          dory_file_set_is_thumbnailing            (DoryFile           *file,
							    gboolean                is_thumbnailing);

DoryFileOperation *dory_file_operation_new      (DoryFile                  *file,
							 DoryFileOperationCallback  callback,
							 gpointer                       callback_data);
void                   dory_file_operation_free     (DoryFileOperation         *op);
void                   dory_file_operation_complete (DoryFileOperation         *op,
							 GFile                         *result_location,
							 GError                        *error);
void                   dory_file_operation_cancel   (DoryFileOperation         *op);
#endif
