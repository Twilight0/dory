...
/* dory-global-preferences.c - Dory specific preference keys and
                                   functions.
...
#define DEBUG_FLAG DORY_DEBUG_PREFERENCES
#include <libdory-private/dory-debug.h>

GSettings *dory_preferences;
GSettings *dory_icon_view_preferences;
GSettings *dory_list_view_preferences;
GSettings *dory_compact_view_preferences;
GSettings *dory_desktop_preferences;
GSettings *dory_tree_sidebar_preferences;
GSettings *dory_window_state;
GSettings *gtk_filechooser_preferences;
GSettings *dory_plugin_preferences;
GSettings *dory_menu_config_preferences;
GSettings *dory_search_preferences;
...
DoryDateFormat  prefs_current_date_format;

GTimer    *dory_startup_timer;
 ...
 */
char *
dory_global_preferences_get_default_folder_viewer_preference_as_iid (void)
{
	int preference_value;
	const char *viewer_iid;

	preference_value =
		g_settings_get_enum (dory_preferences, DORY_PREFERENCES_DEFAULT_FOLDER_VIEWER);

	if (preference_value == DORY_DEFAULT_FOLDER_VIEWER_LIST_VIEW) {
		viewer_iid = DORY_LIST_VIEW_IID;
	} else if (preference_value == DORY_DEFAULT_FOLDER_VIEWER_COMPACT_VIEW) {
		viewer_iid = DORY_COMPACT_VIEW_IID;
	} else {
		viewer_iid = DORY_ICON_VIEW_IID;
	}
...
gboolean
dory_global_preferences_get_inherit_folder_viewer_preference (void)
{
    return inherit_folder_view_preference;
}

gboolean
dory_global_preferences_get_ignore_view_metadata (void)
{
    return ignore_view_metadata;
}

gboolean
dory_global_preferences_get_inherit_show_thumbnails_preference (void)
{
    return inherit_show_thumbnails_preference;
}

int
dory_global_preferences_get_size_prefix_preference (void)
{
...
char *
dory_global_preferences_get_desktop_iid (void)
{
    gboolean use_grid;
    const char *viewer_iid;

    use_grid = g_settings_get_boolean (dory_desktop_preferences, DORY_PREFERENCES_USE_DESKTOP_GRID);

    if (use_grid) {
        viewer_iid = DORY_DESKTOP_ICON_GRID_VIEW_IID;
    } else {
        viewer_iid = DORY_DESKTOP_ICON_VIEW_IID;
    }
...
gint
dory_global_preferences_get_tooltip_flags (void)
{
    DoryFileTooltipFlags flags = DORY_FILE_TOOLTIP_FLAGS_NONE;

    if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_TOOLTIP_FILE_TYPE))
        flags |= DORY_FILE_TOOLTIP_FLAGS_FILE_TYPE;
    if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_TOOLTIP_MOD_DATE))
        flags |= DORY_FILE_TOOLTIP_FLAGS_MOD_DATE;
    if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_TOOLTIP_ACCESS_DATE))
        flags |= DORY_FILE_TOOLTIP_FLAGS_ACCESS_DATE;
    if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_TOOLTIP_FULL_PATH))
        flags |= DORY_FILE_TOOLTIP_FLAGS_PATH;
    if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_TOOLTIP_CREATED_DATE))
        flags |= DORY_FILE_TOOLTIP_FLAGS_CREATED_DATE;
    return flags;
}

gboolean
dory_global_preferences_should_load_plugin (const gchar *name, const gchar *key)
{
    gchar **disabled_list = g_settings_get_strv (dory_plugin_preferences, key);
...
{
    g_signal_connect (dory_preferences,
                      "changed::" DORY_PREFERENCES_IGNORE_VIEW_METADATA,
                      G_CALLBACK (boolean_changed_cb), &ignore_view_metadata);

    boolean_changed_cb (dory_preferences, DORY_PREFERENCES_IGNORE_VIEW_METADATA, &ignore_view_metadata);

    g_signal_connect (dory_preferences,
                      "changed::" DORY_PREFERENCES_INHERIT_FOLDER_VIEWER,
                      G_CALLBACK (boolean_changed_cb), &inherit_folder_view_preference);

    boolean_changed_cb (dory_preferences, DORY_PREFERENCES_INHERIT_FOLDER_VIEWER, &inherit_folder_view_preference);

    g_signal_connect (dory_preferences,
                      "changed::" DORY_PREFERENCES_INHERIT_SHOW_THUMBNAILS,
                      G_CALLBACK (boolean_changed_cb), &inherit_show_thumbnails_preference);

    boolean_changed_cb (dory_preferences, DORY_PREFERENCES_INHERIT_SHOW_THUMBNAILS, &inherit_show_thumbnails_preference);

    g_signal_connect (dory_preferences,
                      "changed::" DORY_PREFERENCES_SIZE_PREFIXES,
                      G_CALLBACK (enum_changed_cb), &size_prefixes_preference);

    enum_changed_cb (dory_preferences, DORY_PREFERENCES_SIZE_PREFIXES, &size_prefixes_preference);
}


gchar **
dory_global_preferences_get_fileroller_mimetypes (void)
{
...
gchar *
dory_global_preferences_get_mono_system_font (void)
{
    return g_settings_get_string (gnome_interface_preferences, DORY_PREFERENCES_MONO_FONT_NAME);
}
...
gchar *
dory_global_preferences_get_mono_font_family_match (const gchar *in_family_name)
{
    static gsize once_init = 0;
...
{
    prefs_current_date_format = g_settings_get_enum (dory_preferences, DORY_PREFERENCES_DATE_FORMAT);
...
void
dory_global_preferences_init (void)
{
	...
	dory_preferences = g_settings_new("org.dory.preferences");
	dory_window_state = g_settings_new("org.dory.window-state");
	dory_icon_view_preferences = g_settings_new("org.dory.icon-view");
	dory_list_view_preferences = g_settings_new("org.dory.list-view");
	dory_compact_view_preferences = g_settings_new("org.dory.compact-view");
	dory_desktop_preferences = g_settings_new("org.dory.desktop");
    ...
    eel_debug_call_at_shutdown (dory_global_preferences_finalize);
}

void
dory_global_preferences_finalize (void)
{
    ...
    g_object_unref (dory_preferences);
    g_object_unref (dory_window_state);
    g_object_unref (dory_icon_view_preferences);
    g_object_unref (dory_list_view_preferences);
    g_object_unref (dory_compact_view_preferences);
    g_object_unref (dory_desktop_preferences);
    g_object_unref (dory_tree_sidebar_preferences);
    g_object_unref (dory_plugin_preferences);
    g_object_unref (dory_menu_config_preferences);
    g_object_unref (dory_search_preferences);
