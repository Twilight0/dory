/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-desktop-directory.c: Subclass of DoryDirectory to implement
...
struct DoryDesktopDirectoryDetails {
...
typedef struct {
	DoryDesktopDirectory *desktop_dir;
...
G_DEFINE_TYPE (DoryDesktopDirectory, dory_desktop_directory,
	       DORY_TYPE_DIRECTORY);

static gboolean
desktop_contains_file (DoryDirectory *directory,
		       DoryFile *file)
{
	DoryDesktopDirectory *desktop;

	desktop = DORY_DESKTOP_DIRECTORY (directory);

	if (dory_directory_contains_file (desktop->details->real_directory, file)) {
		return TRUE;
	}
...
merged_callback_destroy (MergedCallback *merged_callback)
{
	...
	g_assert (DORY_IS_DESKTOP_DIRECTORY (merged_callback->desktop_dir));
...
merged_callback_check_done (MergedCallback *merged_callback)
{
	...
	(* merged_callback->callback) (DORY_DIRECTORY (merged_callback->desktop_dir),
...
static void
desktop_call_when_ready (DoryDirectory *directory,
...
static void
desktop_cancel_callback (DoryDirectory *directory,
			DoryDirectoryCallback callback,
			gpointer callback_data)
{
	DoryDesktopDirectory *desktop;
...
merged_monitor_destroy (MergedMonitor *monitor)
{
	...
	dory_directory_file_monitor_remove (desktop->details->real_directory, monitor);

	dory_directory_monitor_remove_internal (DORY_DIRECTORY (desktop), NULL, monitor);
...
static void
desktop_monitor_add (DoryDirectory *directory,
...
static void
desktop_monitor_remove (DoryDirectory *directory,
		       gconstpointer client)
{
	DoryDesktopDirectory *desktop;
	MergedMonitor *monitor;
...
static void
desktop_force_reload (DoryDirectory *directory)
{
	DoryDesktopDirectory *desktop;

	desktop = DORY_DESKTOP_DIRECTORY (directory);

	dory_directory_force_reload (desktop->details->real_directory);
...
static gboolean
desktop_are_all_files_seen (DoryDirectory *directory)
{
	DoryDesktopDirectory *desktop;

	desktop = DORY_DESKTOP_DIRECTORY (directory);

	if (!dory_directory_are_all_files_seen (desktop->details->real_directory)) {
		return FALSE;
	}
...
static gboolean
desktop_is_not_empty (DoryDirectory *directory)
{
	DoryDesktopDirectory *desktop;

	desktop = DORY_DESKTOP_DIRECTORY (directory);

	if (dory_directory_is_not_empty (desktop->details->real_directory)) {
		return TRUE;
	}
...
static GList *
desktop_get_file_list (DoryDirectory *directory)
{
	GList *real_dir_file_list, *desktop_dir_file_list = NULL;
	
	real_dir_file_list = dory_directory_get_file_list
				(DORY_DESKTOP_DIRECTORY (directory)->details->real_directory);
	desktop_dir_file_list = DORY_DIRECTORY_CLASS (dory_desktop_directory_parent_class)->get_file_list (directory);

	return g_list_concat (real_dir_file_list, desktop_dir_file_list);
}

DoryDirectory *
dory_desktop_directory_get_real_directory (DoryDesktopDirectory *desktop)
{
	dory_directory_ref (desktop->details->real_directory);
...
desktop_finalize (GObject *object)
{
	...
	desktop = DORY_DESKTOP_DIRECTORY (object);

	dory_directory_unref (desktop->details->real_directory);
	...
	G_OBJECT_CLASS (dory_desktop_directory_parent_class)->finalize (object);
}
...
done_loading_callback (DoryDirectory *real_directory,
		       DoryDesktopDirectory *desktop)
{
     dory_directory_emit_done_loading (DORY_DIRECTORY (desktop));
}
...
static void
update_desktop_directory (DoryDesktopDirectory *desktop)
{
	char *desktop_path;
	char *desktop_uri;
	DoryDirectory *real_directory;
	...
	real_directory = dory_directory_get_by_uri (desktop_uri);
	g_free (desktop_uri);
	g_free (desktop_path);
...
{
	update_desktop_directory (DORY_DESKTOP_DIRECTORY (data));
	dory_directory_force_reload (DORY_DIRECTORY (data));
}

static void
dory_desktop_directory_init (DoryDesktopDirectory *desktop)
{
	desktop->details = g_new0 (DoryDesktopDirectoryDetails, 1);
	...
	update_desktop_directory (DORY_DESKTOP_DIRECTORY (desktop));
...
static void
dory_desktop_directory_class_init (DoryDesktopDirectoryClass *class)
{
	 ...
	 * in DoryDesktopDirectory->details->real_directory,
