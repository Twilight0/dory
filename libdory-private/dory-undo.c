...
/* dory-undo.c - public interface for objects that implement
...
#include "dory-undo-transaction.h"
#include <gtk/gtk.h>

#define DORY_UNDO_MANAGER_DATA "Dory undo manager"

/* Register a simple undo action by calling dory_undo_register_full. */
void
dory_undo_register (GObject *target,
			DoryUndoCallback callback,
			...
			const char *redo_menu_item_hint)
{
	 ...
	 * undo manager.
	 */
	dory_undo_register_full (&single_atom_list,
				     target,
				     operation_name,
...
void
dory_undo_register_full (GList *atoms,
			     ...
			     const char *redo_menu_item_hint)
{
	...
	transaction = dory_undo_transaction_new (operation_name,
						     undo_menu_item_label,
						     undo_menu_item_hint, 
						     redo_menu_item_label,
						     redo_menu_item_hint);
	for (p = atoms; p != NULL; p = p->next) {
		dory_undo_transaction_add_atom (transaction, p->data);
	}
	dory_undo_transaction_add_to_undo_manager
		(transaction,
		 dory_undo_get_undo_manager (undo_manager_search_start_object));
...
void
dory_undo_unregister (GObject *target)
{
	/* Perhaps this should also unregister all children if called on a
	 * GtkContainer? That might be handy.
	 */
	dory_undo_transaction_unregister_object (target);
}

void
dory_undo (GObject *undo_manager_search_start_object)
{
	DoryUndoManager *manager;

	g_return_if_fail (G_IS_OBJECT (undo_manager_search_start_object));

	manager = dory_undo_get_undo_manager (undo_manager_search_start_object);
	if (manager != NULL) {
		dory_undo_manager_undo (manager);
	}
}

DoryUndoManager *
dory_undo_get_undo_manager (GObject *start_object)
{
	DoryUndoManager *manager;
	GtkWidget *parent;
	...
	manager = g_object_get_data (start_object, DORY_UNDO_MANAGER_DATA);
	if (manager != NULL) {
		return manager;
	}
	...
	if (GTK_IS_WIDGET (start_object)) {
		...
		if (parent != NULL) {
			manager = dory_undo_get_undo_manager (G_OBJECT (parent));
			if (manager != NULL) {
				return manager;
			}
		}
		...
		if (GTK_IS_WINDOW (start_object)) {
			...
			if (transient_parent != NULL) {
				manager = dory_undo_get_undo_manager (G_OBJECT (transient_parent));
				if (manager != NULL) {
					return manager;
				}
			}
...
void
dory_undo_attach_undo_manager (GObject *object,
				   DoryUndoManager *manager)
{
	g_return_if_fail (G_IS_OBJECT (object));

	if (manager == NULL) {
		g_object_set_data (object, DORY_UNDO_MANAGER_DATA, NULL);
	} else {
		g_object_ref (manager);
		g_object_set_data_full
			(object, DORY_UNDO_MANAGER_DATA,
...
void
dory_undo_share_undo_manager (GObject *destination_object,
				  GObject *source_object)
{
	DoryUndoManager *manager;

	manager = dory_undo_get_undo_manager (source_object);
	dory_undo_attach_undo_manager (destination_object, manager);
}
