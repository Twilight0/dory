...
typedef struct {
	...
	GList *tail;
	GMutex mutex;
} DoryFileChangesQueue;

static DoryFileChangesQueue *
dory_file_changes_queue_new (void)
{
	DoryFileChangesQueue *result;

	result = g_new0 (DoryFileChangesQueue, 1);
...
static DoryFileChangesQueue *
dory_file_changes_queue_get (void)
{
	static DoryFileChangesQueue *file_changes_queue;

	if (file_changes_queue == NULL) {
		file_changes_queue = dory_file_changes_queue_new ();
	}
...
static void
dory_file_changes_queue_add_common (DoryFileChangesQueue *queue,
	DoryFileChange *new_item)
{
...
void
dory_file_changes_queue_file_added (GFile *location)
{
	DoryFileChange *new_item;
	DoryFileChangesQueue *queue;

	queue = dory_file_changes_queue_get();

	new_item = g_new0 (DoryFileChange, 1);
	new_item->kind = CHANGE_FILE_ADDED;
	new_item->from = g_object_ref (location);
	dory_file_changes_queue_add_common (queue, new_item);
}

void
dory_file_changes_queue_file_changed (GFile *location)
{
	DoryFileChange *new_item;
	DoryFileChangesQueue *queue;
...
void
dory_file_changes_queue_file_removed (GFile *location)
{
	DoryFileChange *new_item;
	DoryFileChangesQueue *queue;
...
void
dory_file_changes_queue_file_moved (GFile *from,
					GFile *to)
{
	DoryFileChange *new_item;
	DoryFileChangesQueue *queue;
	...
	new_item = g_new (DoryFileChange, 1);
...
void
dory_file_changes_queue_schedule_position_set (GFile *location,
						   GdkPoint point,
                           int monitor)
{
	DoryFileChange *new_item;
...
void
dory_file_changes_queue_schedule_position_remove (GFile *location)
{
	DoryFileChange *new_item;
	DoryFileChangesQueue *queue;
...
static DoryFileChange *
dory_file_changes_queue_get_change (DoryFileChangesQueue *queue)
{
	GList *new_tail;
	DoryFileChange *result;
...
{
	GList *p;
	DoryFileChangesQueuePosition *item;
 ...
 * in a list to the different dory_directory_notify calls
 */
void
dory_file_changes_consume_changes (gboolean consume_all)
{
	DoryFileChange *change;
	GList *additions, *changes, *deletions, *moves;
	GList *position_set_requests;
	GFilePair *pair;
	DoryFileChangesQueuePosition *position_set;
	guint chunk_count;
	DoryFileChangesQueue *queue;
	...
	for (chunk_count = 0; ; chunk_count++) {
		change = dory_file_changes_queue_get_change (queue);
		...
		if (flush_needed) {
			...
			if (position_set_requests != NULL) {
				...
				dory_directory_schedule_position_set (position_set_requests);
		...
		case CHANGE_POSITION_SET:
			position_set = g_new (DoryFileChangesQueuePosition, 1);
