...
static void refresh_bookmarks_menu (DoryWindow *window);
...
remove_bookmarks_for_uri_if_yes (GtkDialog *dialog, int response, gpointer callback_data)
{
	...
	if (response == GTK_RESPONSE_YES) {
		...
		dory_bookmark_list_delete_items_with_uri (window->details->bookmark_list, uri);
	}
...
static void
show_bogus_bookmark_window (DoryWindow *window,
...
static GtkWindow *
get_or_create_bookmarks_window (DoryWindow *window)
{
 ...
 **/
void
dory_bookmarks_exiting (void)
{
	if (bookmarks_window != NULL) {
		dory_bookmarks_window_save_geometry (bookmarks_window);
 ...
 */
void
dory_window_add_bookmark_for_current_location (DoryWindow *window)
{
	DoryBookmark *bookmark;
	DoryWindowSlot *slot;
	DoryBookmarkList *list;

	slot = dory_window_get_active_slot (window);
	bookmark = slot->current_location_bookmark;
	list = window->details->bookmark_list;

	if (!dory_bookmark_list_contains (list, bookmark)) {
		dory_bookmark_list_append (list, bookmark); 
	}
}

void
dory_window_edit_bookmarks (DoryWindow *window)
{
	GtkWindow *dialog;
...
static void
remove_bookmarks_menu_items (DoryWindow *window)
{
	GtkUIManager *ui_manager;
	
	ui_manager = dory_window_get_ui_manager (window);
...
bookmark_holder_new (DoryBookmark *bookmark, 
		     DoryWindow *window,
		     GCallback refresh_callback,
		     DoryBookmarkFailedCallback failed_callback)
{
...
activate_bookmark_in_menu_item (GtkAction *action, gpointer user_data)
{
    ...
    location = dory_bookmark_get_location (holder->bookmark);
    slot = dory_window_get_active_slot (holder->window);
    dory_window_slot_open_location (slot, location, dory_event_get_window_open_flags ());
    g_object_unref (location);
}

void
dory_menus_append_bookmark_to_menu (DoryWindow *window, 
					DoryBookmark *bookmark, 
...
static void
update_bookmarks (DoryWindow *window)
{
	...
	if (window->details->bookmark_list == NULL) {
		window->details->bookmark_list = dory_bookmark_list_get_default ();
	}

	bookmarks = window->details->bookmark_list;

	ui_manager = dory_window_get_ui_manager (DORY_WINDOW (window));
	...
	bookmark_count = dory_bookmark_list_length (bookmarks);

	for (index = 0; index < bookmark_count; ++index) {
		bookmark = dory_bookmark_list_item_at (bookmarks, index);

		dory_menus_append_bookmark_to_menu
			(DORY_WINDOW (window),
			 bookmark,
...
static void
refresh_bookmarks_menu (DoryWindow *window)
{
	g_assert (DORY_IS_WINDOW (window));
