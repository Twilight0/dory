...
G_DEFINE_TYPE (DoryStatusBar, dory_status_bar, GTK_TYPE_BOX);

static void
dory_status_bar_init (DoryStatusBar *bar)
{
    bar->window = NULL;
}

static void
dory_status_bar_set_property (GObject        *object,
                              guint           arg_id,
                              const GValue   *value,
                              GParamSpec     *pspec)
{
    DoryStatusBar *self = DORY_STATUS_BAR (object);
...
static void
dory_status_bar_get_property (GObject      *object,
                              guint         arg_id,
                              GValue       *value,
                              GParamSpec   *pspec)
{
...
static void
dory_status_bar_dispose (GObject *object)
{
    DoryStatusBar *bar = DORY_STATUS_BAR (object);

    bar->window = NULL;

    G_OBJECT_CLASS (dory_status_bar_parent_class)->dispose (object);
}

static void
action_places_toggle_callback (GtkButton *button, DoryStatusBar *bar)
{
    dory_window_set_sidebar_id (DORY_WINDOW (bar->window), DORY_WINDOW_SIDEBAR_PLACES);

    dory_status_bar_sync_button_states (bar);
}

static void
action_treeview_toggle_callback (GtkButton *button, DoryStatusBar *bar)
{
    dory_window_set_sidebar_id (DORY_WINDOW (bar->window), DORY_WINDOW_SIDEBAR_TREE);

    dory_status_bar_sync_button_states (bar);
}

static void
action_show_sidebar_callback (GtkButton *button, DoryStatusBar *bar)
{
    dory_window_show_sidebar (bar->window);
}

static void
action_hide_sidebar_callback (GtkButton *button, DoryStatusBar *bar)
{
    dory_window_hide_sidebar (bar->window);
}

static void
sidebar_state_changed_cb (gpointer pointer, gboolean state, gpointer user_data)
{
    dory_status_bar_sync_button_states (DORY_STATUS_BAR (user_data));
}
...
{
    DoryStatusBar *bar = DORY_STATUS_BAR (user_data);
    gdouble val = gtk_range_get_value (GTK_RANGE (zoom_slider));

    DoryWindowSlot *slot = dory_window_get_active_slot (bar->window);

    if (!DORY_IS_WINDOW_SLOT (slot))
        return;

    DoryView *view = slot->content_view;

    if (!DORY_IS_VIEW (view))
        return;

    dory_view_zoom_to_level (view, (int) val);
}

#define SLIDER_WIDTH 100

static void
dory_status_bar_constructed (GObject *object)
{
    DoryStatusBar *bar = DORY_STATUS_BAR (object);
    G_OBJECT_CLASS (dory_status_bar_parent_class)->constructed (object);
    ...
    GtkIconSize size = gtk_icon_size_from_name (DORY_STATUSBAR_ICON_SIZE_NAME);
    ...
    icon = gtk_image_new_from_icon_name ("dory-sidebar-places-symbolic", size);
    ...
    button = gtk_toggle_button_new ();
    icon = gtk_image_new_from_icon_name ("dory-sidebar-tree-symbolic", size);
    gtk_button_set_image (GTK_BUTTON (button), icon);
    ...
    icon = gtk_image_new_from_icon_name ("dory-sidebar-hide-symbolic", size);
    gtk_button_set_image (GTK_BUTTON (button), icon);
    ...
    button = gtk_button_new ();
    icon = gtk_image_new_from_icon_name ("dory-sidebar-show-symbolic", size);
    gtk_button_set_image (GTK_BUTTON (button), icon);
    ...
    GtkWidget *zoom_slider = gtk_scale_new_with_range (GTK_ORIENTATION_HORIZONTAL,
                                                       (gdouble) DORY_ZOOM_LEVEL_SMALLEST,
                                                       (gdouble) DORY_ZOOM_LEVEL_LARGEST,
                                                       1.0);
    ...
    g_signal_connect_object (DORY_WINDOW (bar->window), "notify::show-sidebar",
                             G_CALLBACK (sidebar_state_changed_cb), bar, G_CONNECT_AFTER);

    g_signal_connect_object (DORY_WINDOW (bar->window), "notify::sidebar-view-id",
...
static void
dory_status_bar_class_init (DoryStatusBarClass *status_bar_class)
{
    GObjectClass *oclass;

    oclass = G_OBJECT_CLASS (status_bar_class);

    oclass->set_property = dory_status_bar_set_property;
    oclass->get_property = dory_status_bar_get_property;

    oclass->dispose = dory_status_bar_dispose;
    oclass->constructed = dory_status_bar_constructed;
...
GtkWidget *
dory_status_bar_new (DoryWindow *window)
{
    return g_object_new (DORY_TYPE_STATUS_BAR,
...
GtkWidget *
dory_status_bar_get_real_statusbar (DoryStatusBar *bar)
{
    return bar->real_statusbar;
}

void
dory_status_bar_sync_button_states (DoryStatusBar *bar)
{
    const gchar *sidebar_id = dory_window_get_sidebar_id (DORY_WINDOW (bar->window));

    gboolean sidebar_visible = dory_window_get_show_sidebar (DORY_WINDOW (bar->window));
    ...
    if (g_strcmp0 (sidebar_id, DORY_WINDOW_SIDEBAR_PLACES) == 0) {
...
void
dory_status_bar_sync_zoom_widgets (DoryStatusBar *bar)
{
    ...
    DoryZoomLevel zoom_level = dory_view_get_zoom_level (DORY_VIEW (view));
