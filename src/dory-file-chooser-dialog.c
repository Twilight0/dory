                              ...
                              const gchar *suggested_name)
{
    ...
    GError *error = NULL;
    if (!gtk_builder_add_from_resource (builder, "/org/dory/dory-file-chooser-dialog.glade", &error)) {
        g_warning ("Failed to load file chooser UI resource: %s", error->message);
        g_object_unref (builder);
        return NULL;
    }
    
    GtkWidget *dialog = GTK_WIDGET (gtk_builder_get_object (builder, "dory_file_chooser_dialog"));
    if (title && *title) {
...
GSList *
dory_file_chooser_dialog_get_selected_uris (GtkDialog *dialog)
{
    DialogData *data = g_object_get_data (G_OBJECT (dialog), "dialog-data");
    if (!data)
        return NULL;
        
    g_message ("dory_file_chooser_dialog_get_selected_uris: action=%d, selected_uris_count=%d, current_folder=%p",
...
gchar *
dory_file_chooser_dialog_get_selected_uri (GtkDialog *dialog)
{
    DialogData *data = g_object_get_data (G_OBJECT (dialog), "dialog-data");
    if (!data)
        return NULL;
