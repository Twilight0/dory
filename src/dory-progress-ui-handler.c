...
/*
 * dory-progress-ui-handler.c: file operation progress user interface.
 *
...
#include "dory-progress-info-widget.h"
...
#include <libdory-private/dory-progress-info-manager.h>

#include <libxapp/xapp-gtk-window.h>
#include <libxapp/xapp-status-icon.h>

struct _DoryProgressUIHandlerPriv {
...
G_DEFINE_TYPE (DoryProgressUIHandler, dory_progress_ui_handler, G_TYPE_OBJECT);

static void
status_icon_activate_cb (XAppStatusIcon        *icon,
                         guint                  button,
                         guint                  _time,
                         DoryProgressUIHandler *self)
{
...
static void
progress_ui_handler_ensure_status_icon (DoryProgressUIHandler *self)
{
	XAppStatusIcon *status_icon;
...
static void
progress_ui_handler_update_status_icon (DoryProgressUIHandler *self)
{
	gchar *tooltip;
...
static void
ensure_first_separator_hidden (DoryProgressUIHandler *self)
{
    ...
    DoryProgressInfoWidgetPriv *priv = DORY_PROGRESS_INFO_WIDGET (l->data)->priv;
...
static void
progress_ui_handler_sort_by_active (DoryProgressUIHandler *self)
{
    ...
    for (iter = l; iter != NULL; iter = iter->next) {
        DoryProgressInfoWidgetPriv *priv = DORY_PROGRESS_INFO_WIDGET (iter->data)->priv;

        if (dory_progress_info_get_is_started (priv->info)) {
            if (first_pending > 0) {
...
static void
progress_ui_handler_ensure_window (DoryProgressUIHandler *self)
{
    DoryProgressUIHandlerPriv *priv = DORY_PROGRESS_UI_HANDLER (self)->priv;
...
static void
progress_ui_handler_add_to_window (DoryProgressUIHandler *self,
				   DoryProgressInfo *info)
{
	GtkWidget *progress;

	progress = dory_progress_info_widget_new (info);
...
static void
progress_ui_handler_show_complete_notification (DoryProgressUIHandler *self)
{
	...
	g_application_send_notification (G_APPLICATION (dory_application_get_singleton ()), NULL, complete_notification);
	g_object_unref (complete_notification);
}

static void
progress_ui_handler_hide_status (DoryProgressUIHandler *self)
{
	if (self->priv->status_icon != NULL) {
        self->priv->should_show_status_icon = FALSE;
...
static void
progress_info_changed_cb (DoryProgressInfo *info,
			   DoryProgressUIHandler *self)
{	
	if (g_list_length(self->priv->infos) > 0) {
        DoryProgressInfo *first_info = (DoryProgressInfo *) g_list_first(self->priv->infos)->data;
        GList *l;
        g_autofree gchar *status = dory_progress_info_get_status (first_info);
        double progress = 0.0;
        int i = 0;
        for (l = self->priv->infos; l != NULL; l = l->next) {
            if (dory_progress_info_get_is_finished (l->data)) {
                continue;
            }
            progress = (progress + dory_progress_info_get_progress (l->data)) / (double) ++i;
        }
        if (progress > 0) {
...
static void
progress_info_started_cb (DoryProgressUIHandler *self)
{
    progress_ui_handler_sort_by_active (self);
    ensure_first_separator_hidden (self);
}
...
static void
handle_new_progress_info (DoryProgressUIHandler *self,
			  DoryProgressInfo *info)
{
	...
	if (self->priv->active_infos == 1) {
        ...
        gtk_window_present (GTK_WINDOW (self->priv->progress_window));
        gchar *details = dory_progress_info_get_details (info);
...
static TimeoutData *
timeout_data_new (DoryProgressUIHandler *self,
		  DoryProgressInfo *info)
{
	TimeoutData *retval;
...
{
	DoryProgressInfo *info = data->info;
	DoryProgressUIHandler *self = data->self;

	if (dory_progress_info_get_is_paused (info)) {
		return TRUE;
	}

	if (!dory_progress_info_get_is_finished (info)) {
...
static void
release_application (DoryProgressInfo *info,
		     DoryProgressUIHandler *self)
{
	DoryApplication *app;
...
static void
progress_info_queued_cb (DoryProgressInfo *info,
			  DoryProgressUIHandler *self)
{
	DoryApplication *app;
	TimeoutData *data;
...
static void
new_progress_info_cb (DoryProgressInfoManager *manager,
		      DoryProgressInfo *info,
		      DoryProgressUIHandler *self)
{
...
static void
dory_progress_ui_handler_dispose (GObject *obj)
{
	DoryProgressUIHandler *self = DORY_PROGRESS_UI_HANDLER (obj);

	g_clear_object (&self->priv->manager);

	G_OBJECT_CLASS (dory_progress_ui_handler_parent_class)->dispose (obj);
}

static void
dory_progress_ui_handler_init (DoryProgressUIHandler *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, DORY_TYPE_PROGRESS_UI_HANDLER,
						  DoryProgressUIHandlerPriv);

	self->priv->manager = dory_progress_info_manager_new ();
...
static void
dory_progress_ui_handler_class_init (DoryProgressUIHandlerClass *klass)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (klass);
	oclass->dispose = dory_progress_ui_handler_dispose;
	
	g_type_class_add_private (klass, sizeof (DoryProgressUIHandlerPriv));
}

DoryProgressUIHandler *
dory_progress_ui_handler_new (void)
{
	return g_object_new (DORY_TYPE_PROGRESS_UI_HANDLER, NULL);
}
