...
#include <libdory-private/dory-column-chooser.h>
#include <libdory-private/dory-column-utilities.h>
...
#include <libdory-private/dory-tree-view-drag-dest.h>
#include <libdory-private/dory-clipboard.h>

#define DEBUG_FLAG DORY_DEBUG_LIST_VIEW
#include <libdory-private/dory-debug.h>

struct DoryListViewDetails {
	...
	DoryTreeViewDragDest *drag_dest;
...
static GList *dory_list_view_get_selection                   (DoryView   *view);
static void   dory_list_view_update_selection                (DoryView *view);
static GList *dory_list_view_get_selection_for_file_transfer (DoryView   *view);
static void   dory_list_view_set_zoom_level                  (DoryListView        *view,
								  DoryZoomLevel  new_level,
								  gboolean           always_set_level);
static void   dory_list_view_scale_font_size                 (DoryListView        *view,
								  DoryZoomLevel  new_level);
static void   dory_list_view_scroll_to_file                  (DoryListView        *view,
								  DoryFile      *file);
static void   dory_list_view_rename_callback                 (DoryFile      *file,
								  GFile             *result_location,
								  GError            *error,
								  gpointer           callback_data);

static void dory_list_view_start_renaming_file               (DoryView *view,
                                                              DoryFile *file,
                                                              gboolean  select_all);

static void   apply_columns_settings                             (DoryListView *list_view,
                                                                  char **column_order,
                                                                  char **visible_columns);
static char **get_visible_columns                                (DoryListView *list_view);
static char **get_default_visible_columns                        (DoryListView *list_view);
static char **get_column_order                                   (DoryListView *list_view);
static char **get_default_column_order                           (DoryListView *list_view);

static void   set_columns_settings_from_metadata_and_preferences (DoryListView *list_view);
static void   queue_update_visible_icons (DoryListView *view, gint delay);
static DoryZoomLevel dory_list_view_get_zoom_level (DoryView *view);
static void   prioritize_visible_files (DoryListView *view);

G_DEFINE_TYPE (DoryListView, dory_list_view, DORY_TYPE_VIEW);

static gint click_policy = DORY_CLICK_POLICY_SINGLE;
...
{
	DoryFileSortType default_sort_order;
	...
	retval = dory_file_get_default_sort_attribute (file, reversed);

	if (retval == NULL) {
		default_sort_order = g_settings_get_enum (dory_preferences,
							  DORY_PREFERENCES_DEFAULT_SORT_ORDER);
		default_sort_reversed = g_settings_get_boolean (dory_preferences,
...
static void dory_list_view_update_filter_text (DoryView *view, const char *filter_text);
static void dory_list_view_select_first        (DoryView *view);

static void
tooltip_prefs_changed_callback (DoryListView *view)
{
    view->details->show_tooltips = g_settings_get_boolean (dory_preferences,
                                                           DORY_PREFERENCES_TOOLTIPS_LIST_VIEW);

    view->details->tooltip_flags = dory_global_preferences_get_tooltip_flags ();
}

static void
expanders_enabled_changed_cb (DoryListView *view)
{
    gboolean enabled;
    DoryWindowSlot *slot;

    g_return_if_fail (DORY_IS_LIST_VIEW (view));
    g_return_if_fail (GTK_IS_TREE_VIEW (view->details->tree_view) && view->details->tree_view != NULL);

    enabled = g_settings_get_boolean (dory_list_view_preferences,
                                      DORY_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION);

    gtk_tree_view_collapse_all (view->details->tree_view);
    gtk_tree_view_set_show_expanders (view->details->tree_view, enabled);
    dory_list_model_set_expansion_enabled (view->details->model, enabled);

    slot = dory_view_get_dory_window_slot (DORY_VIEW (view));
    if (slot != NULL) {
...
static void
preview_selected_items (DoryListView *view)
{
	GList *file_list;

	file_list = dory_list_view_get_selection (DORY_VIEW (view));

	if (file_list != NULL) {
		dory_view_preview_files (DORY_VIEW (view),
					     file_list, NULL);
...
static void
activate_selected_items (DoryListView *view)
{
	GList *file_list;
	...
	dory_view_activate_files (DORY_VIEW (view),
				      file_list,
				      0, TRUE);
	dory_file_list_free (file_list);
...
static void
activate_selected_items_alternate (DoryListView *view,
				   DoryFile *file,
...
static void
dory_list_view_did_not_drag (DoryListView *view,
				 GdkEventButton *event)
{
	...
	if (gtk_tree_view_get_path_at_pos (tree_view, event->x, event->y,
		...
		if ((click_policy == DORY_CLICK_POLICY_SINGLE)
        ...
        if (view->details->rename_on_release) {
            DoryFile *file = dory_list_model_file_for_path (view->details->model, path);
            dory_list_view_start_renaming_file (DORY_VIEW (view),
                                                file,
...
static void
stop_drag_check (DoryListView *view)
{
	view->details->drag_button = 0;
}

static cairo_surface_t *
get_drag_surface (DoryListView *view)
{
	GtkTreeModel *model;
	GtkTreePath *path;
	GtkTreeIter iter;
	...
	if (gtk_tree_view_get_path_at_pos (view->details->tree_view,
		...
		gtk_tree_model_get (model, &iter,
				    dory_list_model_get_column_id_from_zoom_level (view->details->zoom_level),
				    &ret,
				    -1);
			...
			gpointer callback_data)
{
	...
	view = DORY_LIST_VIEW (callback_data);
    ...
    if (view->details->drag_button != 0 && !view->details->rubber_banding) {
        ...
        if (!source_target_list) {
            source_target_list = dory_list_model_get_drag_target_list ();
        }
...
query_tooltip_callback (GtkWidget *widget,
                        ...
                        gpointer user_data)
{
    DoryListView *list_view;
    gboolean ret;

    ret = FALSE;

    list_view = DORY_LIST_VIEW (user_data);
        ...
        if (gtk_tree_view_get_tooltip_context (GTK_TREE_VIEW (widget), &x, &y,
            ...
            if (!gtk_tree_view_is_blank_at_pos (GTK_TREE_VIEW (widget), x, y, NULL, NULL, NULL, NULL)) {
                ...
                if (file) {
                    ...
                    tooltip_text = dory_file_construct_tooltip (file,
                                                                list_view->details->tooltip_flags,
                                                                dory_view_get_model (DORY_VIEW (list_view)));
...
static void
do_popup_menu (GtkWidget *widget, DoryListView *view, GdkEventButton *event)
{
 	if (tree_view_has_selection (GTK_TREE_VIEW (widget))) {
		dory_view_pop_up_selection_context_menu (DORY_VIEW (view), event);
	} else {
                dory_view_pop_up_background_context_menu (DORY_VIEW (view), event);
	}
}

static void
row_activated_callback (GtkTreeView *treeview, GtkTreePath *path,
			GtkTreeViewColumn *column, DoryListView *view)
{
	activate_selected_items (view);
}
...
columns_reordered_callback (AtkObject *atk,
                            gpointer user_data)
{
    DoryListView *view = DORY_LIST_VIEW (user_data);

    gchar **columns;
    GList *vis_columns = NULL;
    int i;
    DoryFile *file = dory_view_get_directory_as_file (DORY_VIEW (view));
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        dory_window_set_ignore_meta_column_order (dory_view_get_dory_window (DORY_VIEW (view)), list);
    } else if (dory_file_is_in_search (file)) {
        gchar **column_array = string_array_from_string_glist (list);

        g_settings_set_strv (dory_search_preferences,
                             DORY_PREFERENCES_SEARCH_VISIBLE_COLUMNS,
                             (const gchar **) column_array);
    } else {
        dory_file_set_metadata_list (file,
                                     DORY_METADATA_KEY_LIST_VIEW_COLUMN_ORDER,
                                     list);
    }
...
static gboolean
clicked_on_text_in_name_cell (DoryListView *view, GtkTreePath *path, GdkEventButton *event)
{
    gboolean ret = FALSE;

    DoryListViewDetails *details = view->details;
    ...
    if (g_settings_get_boolean (dory_list_view_preferences, DORY_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION)) {
...
static gboolean
clicked_within_double_click_interval (DoryListView *view)
{
...
static gboolean
clicked_within_slow_click_interval_on_text (DoryListView *view, GtkTreePath *path, GdkEventButton *event)
{
...
static gboolean
handle_icon_double_click (DoryListView *view, GtkTreePath *path, GdkEventButton *event, gboolean on_expander)
{
    ...
    if (clicked_within_double_click_interval (view) &&
        ...
        if (!button_event_modifies_selection (event)) {
            ...
            return TRUE;
               ...
               (event->state & GDK_SHIFT_MASK) != 0) {
            DoryFile *file;
            file = dory_list_model_file_for_path (view->details->model, path);
            if (file != NULL) {
...
static gboolean
handle_icon_slow_two_click (DoryListView *view, GtkTreePath *path, GdkEventButton *event)
{
    ...
    file = dory_list_model_file_for_path (view->details->model, path);
    can_rename = dory_file_can_rename (file);
    dory_file_unref (file);
...
button_press_callback (GtkWidget *widget, GdkEventButton *event, gpointer callback_data)
{
    ...
    if (event->type == GDK_2BUTTON_PRESS || event->type == GDK_3BUTTON_PRESS) {
        if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_CLICK_DOUBLE_PARENT_FOLDER) &&
            ...
            if (!gtk_tree_view_get_path_at_pos (tree_view, event->x, event->y,
                                                NULL, NULL, NULL, NULL)) {
                DoryWindowSlot *slot = dory_view_get_dory_window_slot (DORY_VIEW (view));
    ...
    if (!dory_view_get_active (DORY_VIEW (view)) && gtk_tree_selection_count_selected_rows (selection) > 0) {
        DoryWindowSlot *slot = dory_view_get_dory_window_slot (DORY_VIEW (view));
        dory_window_slot_make_hosting_pane_active (slot);
        return GDK_EVENT_STOP;
    }

	dory_list_model_set_drag_view
		(DORY_LIST_MODEL (gtk_tree_view_get_model (tree_view)),
		 tree_view,
		 event->x, event->y);
	...
	if (gtk_tree_view_get_path_at_pos (tree_view, event->x, event->y,
					   &path, NULL, NULL, NULL)) {
        if (g_settings_get_boolean (dory_list_view_preferences,
                                      DORY_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION)) {
			 ...
			 gpointer callback_data)
{
	...
	if (event->button == view->details->drag_button) {
		...
		if (!view->details->drag_started &&
			...
			dory_list_view_did_not_drag (view, event);
		}
	}
	return FALSE;
}
...
static void
subdirectory_done_loading_callback (DoryDirectory *directory, DoryListView *view)
{
	dory_list_model_subdirectory_done_loading (view->details->model, directory);
...
row_expanded_callback (GtkTreeView *treeview, GtkTreeIter *iter, GtkTreePath *path, gpointer callback_data)
{
	...
	if (dory_list_model_load_subdirectory (view->details->model, path, &directory)) {
		char *uri;
		...
		dory_view_add_subdirectory (DORY_VIEW (view), directory);
        dory_list_model_set_expanding (view->details->model, directory);

		if (dory_directory_are_all_files_seen (directory)) {
			dory_list_model_subdirectory_done_loading (view->details->model,
								 directory);
		} else {
...
unload_file_timeout (gpointer data)
{
	...
	if (unload_data->view != NULL) {
		...
		if (dory_list_model_get_tree_iter_from_file (model,
							   unload_data->file,
			...
			if (!gtk_tree_view_row_expanded (unload_data->view->details->tree_view,
							 path)) {
				dory_list_model_unload_subdirectory (model, &iter);
			}
			gtk_tree_path_free (path);
		}
	...
	if (unload_data->directory) {
		dory_directory_unref (unload_data->directory);
	}
	dory_file_unref (unload_data->file);
	g_free (unload_data);
	return FALSE;
}
...
row_collapsed_callback (GtkTreeView *treeview, GtkTreeIter *iter, GtkTreePath *path, gpointer callback_data)
{
	...
	if (gtk_tree_model_iter_parent (model, &parent, iter)) {
		gtk_tree_model_get (model, &parent,
				    DORY_LIST_MODEL_SUBDIRECTORY_COLUMN, &directory,
				    -1);
	}
...
static void
subdirectory_unloaded_callback (DoryListModel *model,
				DoryDirectory *directory,
				gpointer callback_data)
{
	DoryListView *view;
	...
	g_signal_handlers_disconnect_by_func (directory,
					      G_CALLBACK (subdirectory_done_loading_callback),
					      view);
	dory_view_remove_subdirectory (DORY_VIEW (view), directory);
}
...
key_press_callback (GtkWidget *widget, GdkEventKey *event, gpointer callback_data)
{
    ...
    if (event->keyval == GDK_KEY_slash ||
        ...
        if (gtk_bindings_activate_event (G_OBJECT (dory_view_get_dory_window (view)), event)) {
            return GDK_EVENT_STOP;
        }
    }
	...
	case GDK_KEY_F10:
		if (event->state & GDK_CONTROL_MASK) {
			dory_view_pop_up_background_context_menu (view, &button_event);
			handled = TRUE;
		}
		break;
	case GDK_KEY_Right:
	...
	case GDK_KEY_Left:
		...
		break;
	case GDK_KEY_space:
		if (dory_view_activate_filter (view, event)) {
			handled = TRUE;
			break;
		}
		if (event->state & GDK_CONTROL_MASK) {
		...
		if (!gtk_widget_has_focus (GTK_WIDGET (DORY_LIST_VIEW (view)->details->tree_view))) {
			handled = FALSE;
			break;
		}
		if ((event->state & GDK_SHIFT_MASK) != 0) {
			activate_selected_items_alternate (DORY_LIST_VIEW (view), NULL, TRUE);
		} else {
			preview_selected_items (DORY_LIST_VIEW (view));
		}
		handled = TRUE;
		break;
	case GDK_KEY_Return:
	case GDK_KEY_KP_Enter:
		if ((event->state & GDK_SHIFT_MASK) != 0) {
			...
			activate_selected_items (DORY_LIST_VIEW (view));
		}
		handled = TRUE;
		break;
	case GDK_KEY_v:
	...
	if (!handled) {
		handled = dory_view_activate_filter (view, event);
	}
...
static void
set_ok_to_load_deferred_attrs (DoryListView  *list_view,
                       gboolean       ok)
{
...
static void
dory_list_view_reveal_selection (DoryView *view)
{
	GList *selection;
	...
	if (selection != NULL) {
		...
		list_view = DORY_LIST_VIEW (view);
		file = selection->data;
		if (dory_list_model_get_first_iter_for_file (list_view->details->model, file, &iter)) {
			      ...
			      DoryListView *view)
{
	...
	sort_attr = dory_list_model_get_attribute_from_sort_column_id (view->details->model, sort_column_id);

	default_sort_column_id = dory_list_model_get_sort_column_id_from_attribute (view->details->model,
										  g_quark_from_string (get_default_sort_order (file, &default_sort_reversed)));
	default_sort_attr = dory_list_model_get_attribute_from_sort_column_id (view->details->model, default_sort_column_id);

        if (dory_global_preferences_get_ignore_view_metadata ())
                dory_window_set_ignore_meta_sort_column (dory_view_get_dory_window (DORY_VIEW (view)),
                                                         g_quark_to_string (sort_attr));
        else if (dory_file_is_in_search (file)) {
            g_settings_set_string (dory_search_preferences, DORY_PREFERENCES_SEARCH_SORT_COLUMN, g_quark_to_string (sort_attr));
        } else {
            dory_file_set_metadata (file, DORY_METADATA_KEY_LIST_VIEW_SORT_COLUMN,
	...
	if (view->details->last_sort_attr != sort_attr &&
		...
		if (sort_attr == default_sort_attr) {
			...
			reversed = g_settings_get_boolean (dory_preferences,
							   DORY_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER);
		} else {
			reversed = dory_file_is_date_sort_attribute_q (sort_attr);
		}
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        dory_window_set_ignore_meta_sort_direction (dory_view_get_dory_window (DORY_VIEW (view)),
                                                    reversed ? SORT_DESCENDING : SORT_ASCENDING);
    } else if (dory_file_is_in_search (file)) {
        g_settings_set_boolean (dory_search_preferences, DORY_PREFERENCES_SEARCH_REVERSE_SORT, reversed);
    } else {
        reversed_attr = (reversed ? (char *)"true" : (char *)"false");
        dory_file_set_metadata (file, DORY_METADATA_KEY_LIST_VIEW_SORT_REVERSED,
                                default_reversed_attr, reversed_attr);
    }

	/* Make sure selected item(s) is visible after sort */
	dory_list_view_reveal_selection (DORY_VIEW (view));
				  ...
				  DoryListView *list_view)
{
	...
	dory_clipboard_set_up_editable
		(GTK_EDITABLE (entry),
		 dory_view_get_ui_manager (DORY_VIEW (list_view)),
		 FALSE);
}
				...
				DoryListView    *view)
{
	...
	dory_view_set_is_renaming (DORY_VIEW (view), FALSE);
	dory_view_unfreeze_updates (DORY_VIEW (view));
}
		      ...
		      DoryListView    *view)
{
	...
	if (strcmp (new_text, view->details->original_name) != 0) {
		view->details->renaming_file = dory_file_ref (file);
		view->details->rename_done = FALSE;
		dory_rename_file (file, new_text, dory_list_view_rename_callback, g_object_ref (view));
...
static char *
get_root_uri_callback (DoryTreeViewDragDest *dest,
		       gpointer user_data)
{
	DoryListView *view;

	view = DORY_LIST_VIEW (user_data);

	return dory_view_get_uri (DORY_VIEW (view));
}
...
static DoryFile *
get_file_for_path_callback (DoryTreeViewDragDest *dest,
			    GtkTreePath *path,
			    gpointer user_data)
{
    DoryListView *view;
    ...
    return dory_list_model_file_for_path (view->details->model, path);
}

/* Handles an URL received from Mozilla */
static void
list_view_handle_netscape_url (DoryTreeViewDragDest *dest, const char *encoded_url,
			       const char *target_uri, GdkDragAction action, int x, int y, DoryListView *view)
{
...
static void
list_view_handle_uri_list (DoryTreeViewDragDest *dest, const char *item_uris,
			   const char *target_uri,
			   GdkDragAction action, int x, int y, DoryListView *view)
{
...
static void
list_view_handle_text (DoryTreeViewDragDest *dest, const char *text,
		       const char *target_uri,
		       GdkDragAction action, int x, int y, DoryListView *view)
{
...
static void
list_view_handle_raw (DoryTreeViewDragDest *dest, const char *raw_data,
...
static void
move_copy_items_callback (DoryTreeViewDragDest *dest,
                            ...
                            DoryListView *list_view)
{
    ...
    file = dory_view_get_directory_as_file (DORY_VIEW (list_view));
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        dory_window_set_ignore_meta_visible_columns (dory_view_get_dory_window (DORY_VIEW (list_view)), list);
    } else if (dory_file_is_in_search (file)) {
        gchar **column_array = string_array_from_string_glist (list);
        ...
        dory_file_set_metadata_list (file,
                                     DORY_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS,
                                     list);
                                ...
                                DoryListView *list_view)
{
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        DoryWindow *window = dory_view_get_dory_window (DORY_VIEW (list_view));
        dory_window_set_ignore_meta_visible_columns (window, NULL);
        dory_window_set_ignore_meta_column_order (window, NULL);
    } else {
        dory_file_set_metadata_list (file, DORY_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
        dory_file_set_metadata_list (file, DORY_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
    }

    if (dory_file_is_in_search (file)) {
        g_settings_reset (dory_search_preferences, DORY_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
    }
                                 ...
                                 DoryListView *list_view)
{
    ...
    dory_list_model_set_temporarily_disable_sort (list_view->details->model, active);
}
                       ...
                       DoryListView *list_view)
{
    ...
    valid_columns = dory_get_columns_for_file (file);
    ...
    g_strfreev (visible_columns);
    dory_column_list_free (valid_columns);
    ...
    gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
                                    dory_list_model_get_temporarily_disable_sort (list_view->details->model));
...
static void
apply_columns_settings (DoryListView *list_view,
			char **column_order,
			char **visible_columns)
{
	GList *all_columns;
	...
	all_columns = dory_get_columns_for_file (file);
	all_columns = dory_sort_columns (all_columns, column_order);
	...
	dory_column_list_free (all_columns);
			 ...
			 DoryListView        *view)
{
	...
	gtk_tree_model_get (model, iter,
                ...
                DORY_LIST_MODEL_TEXT_WEIGHT_COLUMN, &weight,
			    -1);
	...
	if (text != NULL && dory_list_model_get_filter_active (view->details->model)) {
		const char *filter_text = dory_view_get_filter_text (DORY_VIEW (view));
		PangoAttrList *match_attrs = dory_fzy_match_attrs (filter_text, text);
...
{
	DoryWindowSlot *slot;
	DoryListView *list_view = DORY_LIST_VIEW (user_data);

	/* make the corresponding slot (and the pane that contains it) active */
	slot = dory_view_get_dory_window_slot (DORY_VIEW (list_view));
	dory_window_slot_make_hosting_pane_active (slot);
...
static void
prioritize_visible_files (DoryListView *view)
{
    DoryFile *last_file;
    // GList *queue_list, *l;
    GdkRectangle vrect;
    ...
    gtk_tree_view_get_visible_rect (view->details->tree_view,
                                    &vrect);
    icon_size = dory_get_list_icon_size_for_zoom_level (dory_list_view_get_zoom_level (DORY_VIEW (view)));
    ...
    while (cy > start_y) {
        if (gtk_tree_view_get_path_at_pos (view->details->tree_view,
            ...
            gtk_tree_model_get (GTK_TREE_MODEL (view->details->model),
                                &iter,
                                DORY_LIST_MODEL_ICON_SHOWN, &shown,
                                DORY_LIST_MODEL_FILE_COLUMN, &file, -1);
            ...
            if (file != NULL && file != last_file) {
                ...
                if (dory_file_is_thumbnailing (file)) {
...
static gboolean
update_visible_icons_cb (DoryListView *view)
{
...
static void
queue_update_visible_icons(DoryListView *view,
                           gint          delay)
{
...
static gint
get_icon_scale_callback (DoryListModel *model,
                         DoryListView  *view)
{
...
static gint
get_filter_match_callback (DoryListModel *model,
                           DoryFile      *file,
                           DoryListView  *view)
{
    return dory_view_get_filter_match (DORY_VIEW (view), file);
}
...
static void
update_date_fonts (DoryListView *view)
{
    g_return_if_fail (DORY_IS_LIST_VIEW (view));
    DoryDateFontChoice mono_pref;
    gchar *font_name;
    ...
    mono_pref = g_settings_get_enum (dory_preferences, DORY_PREFERENCES_DATE_FONT_CHOICE);

    if (g_settings_get_enum (dory_preferences, DORY_PREFERENCES_DATE_FORMAT) == DORY_DATE_FORMAT_INFORMAL ||
        mono_pref == DORY_DATE_FONT_CHOICE_NONE ||
        g_strstr_len (font_name, -1, "Mono")) {
        date_name = g_strdup (font_name);
    } else {
        if (mono_pref == DORY_DATE_FONT_CHOICE_AUTO) {
            PangoFontDescription *font_desc = pango_font_description_from_string (font_name);
            const gchar *current_font_family = pango_font_description_get_family (font_desc);

            if (current_font_family != NULL) {
                date_family = dory_global_preferences_get_mono_font_family_match (current_font_family);
            } else {
            ...
            date_name = dory_global_preferences_get_mono_system_font ();
        }
    }
...
static void
create_and_set_up_tree_view (DoryListView *view)
{
    ...
    gtk_tree_view_set_show_expanders (view->details->tree_view,
                                      g_settings_get_boolean (dory_list_view_preferences,
                                                              DORY_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION));
    g_signal_connect_swapped (dory_list_view_preferences,
                              "changed::" DORY_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION,
	...
	view->details->drag_dest =
		dory_tree_view_drag_dest_new (view->details->tree_view, TRUE);
	...
	view->details->model = g_object_new (DORY_TYPE_LIST_MODEL, NULL);
    dory_list_model_set_expansion_enabled (view->details->model,
                                           g_settings_get_boolean (dory_list_view_preferences,
                                                                   DORY_PREFERENCES_LIST_VIEW_ENABLE_EXPANSION));

	gtk_tree_view_set_model (view->details->tree_view, GTK_TREE_MODEL (view->details->model));
	/* Need the model for the dnd drop icon "accept" change */
	dory_list_model_set_drag_view (DORY_LIST_MODEL (view->details->model),
	...
	dory_columns = dory_get_all_columns ();

	for (l = dory_columns; l != NULL; l = l->next) {
		...
		dory_column = DORY_COLUMN (l->data);
		...
		column_num = dory_list_model_add_column (view->details->model,
						       dory_column);
		...
		if (!strcmp (name, "name")) {
			...
			gtk_tree_view_column_set_attributes (view->details->file_name_column,
							     cell,
							     "surface", DORY_LIST_MODEL_SMALLEST_ICON_COLUMN,
							     NULL);
            ...
            gtk_tree_view_column_set_attributes (column, cell,
                                                 "text", column_num,
                                                 "weight", DORY_LIST_MODEL_TEXT_WEIGHT_COLUMN,
                                                 NULL);
    ...
    g_signal_connect_swapped (dory_preferences, "changed::" DORY_PREFERENCES_DATE_FONT_CHOICE, G_CALLBACK (update_date_fonts), view);
    g_signal_connect_swapped (gnome_interface_preferences, "changed::" DORY_PREFERENCES_MONO_FONT_NAME, G_CALLBACK (update_date_fonts), view);
	dory_column_list_free (dory_columns);

	default_visible_columns = g_settings_get_strv (dory_list_view_preferences,
						       DORY_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
	default_column_order = g_settings_get_strv (dory_list_view_preferences,
						    DORY_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER);
...
static void
dory_list_view_add_file (DoryView *view, DoryFile *file, DoryDirectory *directory)
{
	DoryListModel *model;
	...
	model = DORY_LIST_VIEW (view)->details->model;
	dory_list_model_add_file (model, file, directory);
    queue_update_visible_icons (DORY_LIST_VIEW (view), INITIAL_UPDATE_VISIBLE_DELAY);
}

static char **
get_default_visible_columns (DoryListView *list_view)
{
    DoryFile *file;
    ...
    if (dory_file_is_in_recent (file)) {
    ...
    return g_settings_get_strv (dory_list_view_preferences,
                                DORY_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
}

static char **
get_visible_columns (DoryListView *list_view)
{
	DoryFile *file;
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        visible_columns = dory_window_get_ignore_meta_visible_columns (dory_view_get_dory_window (DORY_VIEW (list_view)));
    } else {
        if (dory_file_is_in_search (file)) {
            gchar **modified_cols;

            modified_cols = g_settings_get_strv (dory_search_preferences, DORY_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
            ...
            visible_columns = dory_file_get_metadata_list (file, DORY_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS);
        }
    }
...
static char **
get_default_column_order (DoryListView *list_view)
{
    DoryFile *file;
...
static char **
get_column_order (DoryListView *list_view)
{
	DoryFile *file;
	GList *column_order;
	char **ret;
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        column_order = dory_window_get_ignore_meta_column_order (dory_view_get_dory_window (DORY_VIEW (list_view)));
    } else {
        if (dory_file_is_in_search (file)) {
            gchar **modified_cols;
            modified_cols = g_settings_get_strv (dory_search_preferences, DORY_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
            ...
            column_order = dory_file_get_metadata_list (file, DORY_METADATA_KEY_LIST_VIEW_COLUMN_ORDER);
        }
    }
...
static void
set_columns_settings_from_metadata_and_preferences (DoryListView *list_view)
{
	char **column_order;
	char **visible_columns;
...
static void
set_sort_order_from_metadata_and_preferences (DoryListView *list_view)
{
	char *sort_attribute;
	int sort_column_id;
	DoryFile *file;
	...
	file = dory_view_get_directory_as_file (DORY_VIEW (list_view));
        ...
        if (dory_global_preferences_get_ignore_view_metadata ())
                sort_attribute = g_strdup (dory_window_get_ignore_meta_sort_column (dory_view_get_dory_window (DORY_VIEW (list_view))));
        else if (dory_file_is_in_search (file)) {
            sort_attribute = g_settings_get_string (dory_search_preferences, DORY_PREFERENCES_SEARCH_SORT_COLUMN);
        } else {
            sort_attribute = dory_file_get_metadata (file,
                                                     DORY_METADATA_KEY_LIST_VIEW_SORT_COLUMN,
                                                     NULL);
        }
	sort_column_id = dory_list_model_get_sort_column_id_from_attribute (list_view->details->model,
	...
	if (sort_column_id == -1) {
		sort_column_id =
			dory_list_model_get_sort_column_id_from_attribute (list_view->details->model,
									 g_quark_from_string (default_sort_order));
	}

    if (dory_global_preferences_get_ignore_view_metadata ()) {
        gint dir = dory_window_get_ignore_meta_sort_direction (dory_view_get_dory_window (DORY_VIEW (list_view)));
        sort_reversed = dir > SORT_NULL ? dir == SORT_DESCENDING : default_sort_reversed;
    } else if (dory_file_is_in_search (file)) {
        sort_reversed = g_settings_get_boolean (dory_search_preferences, DORY_PREFERENCES_SEARCH_REVERSE_SORT);
    } else {
        sort_reversed = dory_file_get_boolean_metadata (file,
                                                        DORY_METADATA_KEY_LIST_VIEW_SORT_REVERSED,
...
get_default_zoom_level (void) {
	...
	default_zoom_level = g_settings_get_enum (dory_list_view_preferences,
						  DORY_PREFERENCES_LIST_VIEW_DEFAULT_ZOOM_LEVEL);

	if (default_zoom_level <  DORY_ZOOM_LEVEL_SMALLEST
	    || DORY_ZOOM_LEVEL_LARGEST < default_zoom_level) {
		default_zoom_level = DORY_ZOOM_LEVEL_SMALL;
	}
...
static void
set_zoom_level_from_metadata_and_preferences (DoryListView *list_view)
{
	DoryFile *file;
	int level;

	if (dory_view_supports_zooming (DORY_VIEW (list_view))) {
		file = dory_view_get_directory_as_file (DORY_VIEW (list_view));
        if (dory_global_preferences_get_ignore_view_metadata ()) {
            gchar *uri;
            ...
            if (eel_uri_is_search (uri)) {
                ...
                gint ignore_level;
                ignore_level = dory_window_get_ignore_meta_zoom_level (dory_view_get_dory_window (DORY_VIEW (list_view)));
            ...
            level = dory_file_get_integer_metadata (file,
							    DORY_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL,
							    get_default_zoom_level ());
        }
		dory_list_view_set_zoom_level (list_view, level, TRUE);
...
static void
dory_list_view_begin_loading (DoryView *view)
{
	DoryListView *list_view;
    ...
    dory_list_model_set_view_directory (list_view->details->model, dory_view_get_model (view));

    AtkObject *atk = gtk_widget_get_accessible (GTK_WIDGET (DORY_LIST_VIEW (view)->details->tree_view));
...
static void
stop_cell_editing (DoryListView *list_view)
{
	GtkTreeViewColumn *column;
...
static void
dory_list_view_clear (DoryView *view)
{
	DoryListView *list_view;
	...
	if (list_view->details->model != NULL) {
		...
		dory_list_model_clear (list_view->details->model);
	}
...
static void
dory_list_view_rename_callback (DoryFile *file,
				    GFile *result_location,
				    GError *error,
				    gpointer callback_data)
{
	...
	if (view->details->renaming_file) {
		...
		if (error != NULL) {
			 ...
			 */
			dory_file_unref (view->details->renaming_file);
...
static void
dory_list_view_file_changed (DoryView *view, DoryFile *file, DoryDirectory *directory)
{
	DoryListView *listview;
	GtkTreeIter iter;
	GtkTreePath *file_path;

	listview = DORY_LIST_VIEW (view);

	dory_list_model_file_changed (listview->details->model, file, directory);
	...
	if (listview->details->renaming_file != NULL &&
		 ...
		 */
		if (dory_list_model_get_tree_iter_from_file (listview->details->model, file, directory, &iter)) {
		...
		dory_file_unref (listview->details->renaming_file);
...
static char *
dory_list_view_get_backing_uri (DoryView *view)
{
	DoryListView *list_view;
	DoryListModel *list_model;
	DoryFile *file;
	GtkTreeView *tree_view;
	GtkTreeSelection *selection;
	GtkTreePath *path;
	...
	g_return_val_if_fail (DORY_IS_LIST_VIEW (view), NULL);
	...
	if (length == 1) {
		...
		file = dory_list_model_file_for_path (list_model, path);
		if (file == NULL) {
	...
	if (uri == NULL && length > 0) {
		 ...
		 * is handled by DoryView::get_backing_directory.) */
		...
		if (is_common && !is_root) {
			...
			file = dory_list_model_file_for_path (list_model, path);
			g_assert (file != NULL);
			uri = dory_file_get_parent_uri (file);
			dory_file_unref (file);
	...
	return DORY_VIEW_CLASS (dory_list_view_parent_class)->get_backing_uri (view);
}

static void
dory_list_view_get_selection_foreach_func (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
	GList **list;
	DoryFile *file;
...
static GList *
dory_list_view_get_selection (DoryView *view)
{
	GList *list;

	list = NULL;

	gtk_tree_selection_selected_foreach (gtk_tree_view_get_selection (DORY_LIST_VIEW (view)->details->tree_view),
					     dory_list_view_get_selection_foreach_func, &list);

	return g_list_reverse (list);
}

static GList *
dory_list_view_peek_selection (DoryView *view)
{
    DoryListView *list_view = DORY_LIST_VIEW (view);

    if (list_view->details->current_selection_count == -1) {
        dory_list_view_update_selection (DORY_VIEW (list_view));
    }
...
static gint
dory_list_view_get_selection_count (DoryView *view)
{
    DoryListView *list_view = DORY_LIST_VIEW (view);
...
static void
dory_list_view_update_selection (DoryView *view)
{
    DoryListView *list_view = DORY_LIST_VIEW (view);
    ...
    list_view->details->current_selection = dory_list_view_get_selection (view);
    list_view->details->current_selection_count = g_list_length (list_view->details->current_selection);
}

static void
dory_list_view_get_selection_for_file_transfer_foreach_func (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
	DoryFile *file;
	struct SelectionForeachData *selection_data;
...
static GList *
dory_list_view_get_selection_for_file_transfer (DoryView *view)
{
	struct SelectionForeachData selection_data;

	selection_data.list = NULL;
	selection_data.selection = gtk_tree_view_get_selection (DORY_LIST_VIEW (view)->details->tree_view);

	gtk_tree_selection_selected_foreach (selection_data.selection,
					     dory_list_view_get_selection_for_file_transfer_foreach_func, &selection_data);
...
static guint
dory_list_view_get_item_count (DoryView *view)
{
	g_return_val_if_fail (DORY_IS_LIST_VIEW (view), 0);

	return dory_list_model_get_length (DORY_LIST_VIEW (view)->details->model);
}

static gboolean
dory_list_view_is_empty (DoryView *view)
{
	return dory_list_model_is_empty (DORY_LIST_VIEW (view)->details->model);
}

static void
dory_list_view_end_file_changes (DoryView *view)
{
	DoryListView *list_view;
...
static void
dory_list_view_remove_file (DoryView *view, DoryFile *file, DoryDirectory *directory)
{
	GtkTreePath *path;
	GtkTreePath *file_path;
	GtkTreeIter iter;
	...
	if (dory_list_model_get_tree_iter_from_file (list_view->details->model, file, directory, &iter)) {
		...
		dory_list_model_remove_file (list_view->details->model, file, directory);
...
static void
dory_list_view_set_selection (DoryView *view, GList *selection)
{
	DoryListView *list_view;
	GtkTreeSelection *tree_selection;
	GList *node;
	...
	for (node = selection; node != NULL; node = node->next) {
		file = node->data;
		iters = dory_list_model_get_all_iters_for_file (list_view->details->model, file);
...
static void
dory_list_view_invert_selection (DoryView *view)
{
	DoryListView *list_view;
	GtkTreeSelection *tree_selection;
	GList *node;
	...
	gtk_tree_selection_selected_foreach (tree_selection,
					     dory_list_view_get_selection_foreach_func, &selection);
...
static void
dory_list_view_select_all (DoryView *view)
{
	gtk_tree_selection_select_all (gtk_tree_view_get_selection (DORY_LIST_VIEW (view)->details->tree_view));
}

static void
dory_list_view_merge_menus (DoryView *view)
{
  DoryListView *list_view;
  ...
  DORY_VIEW_CLASS (dory_list_view_parent_class)->merge_menus (view);
  ...
  list_view->details->list_merge_id =
    gtk_ui_manager_add_ui_from_resource (ui_manager, "/org/dory/dory-list-view-ui.xml", NULL);

  list_view->details->menus_ready = TRUE;
}

static void
dory_list_view_unmerge_menus (DoryView *view)
{
  DoryListView *list_view;
  GtkUIManager *ui_manager;
  ...
  DORY_VIEW_CLASS (dory_list_view_parent_class)->unmerge_menus (view);
...
static void
dory_list_view_update_menus (DoryView *view)
{
	DoryListView *list_view;
	...
	DORY_VIEW_CLASS (dory_list_view_parent_class)->update_menus (view);
}

/* Reset sort criteria and zoom level to match defaults */
static void
dory_list_view_reset_to_defaults (DoryView *view)
{
	DoryFile *file;

	file = dory_view_get_directory_as_file (view);

    g_signal_handlers_block_by_func (DORY_LIST_VIEW (view)->details->tree_view,
                                     columns_reordered_callback,
                                     DORY_LIST_VIEW (view));

    if (dory_global_preferences_get_ignore_view_metadata ()) {
        DoryWindow *window = dory_view_get_dory_window (DORY_VIEW (view));
        dory_window_set_ignore_meta_sort_column (window, NULL);
        dory_window_set_ignore_meta_sort_direction (window, SORT_NULL);
        dory_window_set_ignore_meta_zoom_level (window, DORY_ZOOM_LEVEL_NULL);
        dory_window_set_ignore_meta_column_order (window, NULL);
        dory_window_set_ignore_meta_visible_columns (window, NULL);
    } else if (dory_file_is_in_search (file)) {
        g_settings_reset (dory_search_preferences, DORY_PREFERENCES_SEARCH_VISIBLE_COLUMNS);
        g_settings_reset (dory_search_preferences, DORY_PREFERENCES_SEARCH_SORT_COLUMN);
        g_settings_reset (dory_search_preferences, DORY_PREFERENCES_SEARCH_REVERSE_SORT);
    } else {
        dory_file_set_metadata (file, DORY_METADATA_KEY_LIST_VIEW_SORT_COLUMN, NULL, NULL);
        dory_file_set_metadata (file, DORY_METADATA_KEY_LIST_VIEW_SORT_REVERSED, NULL, NULL);
        dory_file_set_metadata (file, DORY_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL, NULL, NULL);
        dory_file_set_metadata_list (file, DORY_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
        dory_file_set_metadata_list (file, DORY_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
    }
    ...
    default_columns = get_default_visible_columns (DORY_LIST_VIEW (view));
    default_order = get_default_column_order (DORY_LIST_VIEW (view));
    apply_columns_settings (DORY_LIST_VIEW (view), default_order, default_columns);

    g_signal_handlers_unblock_by_func (DORY_LIST_VIEW (view)->details->tree_view,
                                       columns_reordered_callback,
                                       DORY_LIST_VIEW (view));
}

static void
dory_list_view_scale_font_size (DoryListView *view,
				    DoryZoomLevel new_level)
{
	GList *l;
	...
	if (first_time) {
		first_time = FALSE;
		medium = DORY_ZOOM_LEVEL_SMALLER;
		pango_scale[medium] = PANGO_SCALE_MEDIUM;
		for (i = medium; i > DORY_ZOOM_LEVEL_SMALLEST; i--) {
			pango_scale[i - 1] = (1 / 1.2) * pango_scale[i];
		}
		for (i = medium; i < DORY_ZOOM_LEVEL_LARGEST; i++) {
				   ...
				   gboolean always_emit)
{
	...
	if (view->details->zoom_level == new_level) {
		if (always_emit) {
			g_signal_emit_by_name (DORY_VIEW(view), "zoom_level_changed");
		}
		return;
	}
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        ...
        dory_file_set_integer_metadata (file,
                                        DORY_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL,
                                        get_default_zoom_level (),
                                        new_level);
    }
	...
	column = dory_list_model_get_column_id_from_zoom_level (new_level);
	gtk_tree_view_column_set_attributes (view->details->file_name_column,
	...
	/* Scale text. */
	dory_list_view_scale_font_size (view, new_level);

	/* Make all rows the same size. */
	icon_size = dory_get_list_icon_size_for_zoom_level (new_level);
...
static void
dory_list_view_bump_zoom_level (DoryView *view, int zoom_increment)
{
	DoryListView *list_view;
	gint new_level;
	...
	if (new_level >= DORY_ZOOM_LEVEL_SMALLEST &&
	    new_level <= DORY_ZOOM_LEVEL_LARGEST) {
		dory_list_view_set_zoom_level (list_view, new_level, FALSE);
	}
}

static DoryZoomLevel
dory_list_view_get_zoom_level (DoryView *view)
{
	DoryListView *list_view;

	g_return_val_if_fail (DORY_IS_LIST_VIEW (view), DORY_ZOOM_LEVEL_STANDARD);
...
static void
dory_list_view_zoom_to_level (DoryView *view,
				  DoryZoomLevel zoom_level)
{
	DoryListView *list_view;
	...
	dory_list_view_set_zoom_level (list_view, zoom_level, FALSE);
}

static void
dory_list_view_restore_default_zoom_level (DoryView *view)
{
	DoryListView *list_view;
	...
	dory_list_view_set_zoom_level (list_view, get_default_zoom_level (), FALSE);
}

static DoryZoomLevel
dory_list_view_get_default_zoom_level (DoryView *view)
{
    g_return_val_if_fail (DORY_IS_LIST_VIEW (view), DORY_ZOOM_LEVEL_NULL);

    return get_default_zoom_level();
}

static gboolean
dory_list_view_can_zoom_in (DoryView *view)
{
	g_return_val_if_fail (DORY_IS_LIST_VIEW (view), FALSE);

	return DORY_LIST_VIEW (view)->details->zoom_level	< DORY_ZOOM_LEVEL_LARGEST;
}

static gboolean
dory_list_view_can_zoom_out (DoryView *view)
{
	g_return_val_if_fail (DORY_IS_LIST_VIEW (view), FALSE);

	return DORY_LIST_VIEW (view)->details->zoom_level > DORY_ZOOM_LEVEL_SMALLEST;
}

static void
dory_list_view_start_renaming_file (DoryView *view,
					DoryFile *file,
					gboolean select_all)
{
	DoryListView *list_view;
	...
	DORY_VIEW_CLASS (dory_list_view_parent_class)->start_renaming_file (view, file, select_all);

	/* Freeze updates to the view to prevent losing rename focus when the tree view updates */
	dory_view_freeze_updates (DORY_VIEW (view));
...
static void
dory_list_view_click_to_rename_mode_changed (DoryView *directory_view)
{
    DoryListView *view;

    g_assert (DORY_IS_LIST_VIEW (directory_view));

    view = DORY_LIST_VIEW (directory_view);

    view->details->click_to_rename = g_settings_get_boolean (dory_preferences,
                                                                  DORY_PREFERENCES_CLICK_TO_RENAME);
}

static void
dory_list_view_click_policy_changed (DoryView *directory_view)
{
	GdkWindow *win;
	GdkDisplay *display;
	DoryListView *view;
    ...
    click_policy = g_settings_get_enum (dory_preferences,
                                        DORY_PREFERENCES_CLICK_POLICY);

	/* ensure that we unset the hand cursor and refresh underlined rows */
	if (click_policy == DORY_CLICK_POLICY_DOUBLE) {
		if (view->details->hover_path != NULL) {
	...
	} else if (click_policy == DORY_CLICK_POLICY_SINGLE) {
		if (hand_cursor == NULL) {
...
default_sort_order_changed_callback (gpointer callback_data)
{
	...
	list_view = DORY_LIST_VIEW (callback_data);
...
static void
dory_list_view_sort_directories_first_changed (DoryView *view)
{
	DoryListView *list_view;

	list_view = DORY_LIST_VIEW (view);

	dory_list_model_set_should_sort_directories_first (list_view->details->model,
							 dory_view_should_sort_directories_first (view));
}

static void
dory_list_view_sort_favorites_first_changed (DoryView *view)
{
	DoryListView *list_view;

	list_view = DORY_LIST_VIEW (view);

	dory_list_model_set_should_sort_favorites_first (list_view->details->model,
							 dory_view_should_sort_favorites_first (view));
}

static int
dory_list_view_compare_files (DoryView *view, DoryFile *file1, DoryFile *file2)
{
	DoryListView *list_view;

	list_view = DORY_LIST_VIEW (view);
	return dory_list_model_compare_func (list_view->details->model, file1, file2);
}

static gboolean
dory_list_view_using_manual_layout (DoryView *view)
{
	g_return_val_if_fail (DORY_IS_LIST_VIEW (view), FALSE);
...
static void
dory_list_view_dispose (GObject *object)
{
	DoryListView *list_view;

	list_view = DORY_LIST_VIEW (object);

    g_signal_handlers_disconnect_by_func (gtk_settings_get_default (), update_date_fonts, list_view);
    g_signal_handlers_disconnect_by_func (dory_preferences, update_date_fonts, list_view);
    ...
    G_OBJECT_CLASS (dory_list_view_parent_class)->dispose (object);
}

static void
dory_list_view_finalize (GObject *object)
{
	DoryListView *list_view;
	...
	g_signal_handlers_disconnect_by_func (dory_preferences,
					      ...
					      list_view);
	g_signal_handlers_disconnect_by_func (dory_list_view_preferences,
	...
	G_OBJECT_CLASS (dory_list_view_parent_class)->finalize (object);
}

static char *
dory_list_view_get_first_visible_file (DoryView *view)
{
	DoryFile *file;
	GtkTreePath *path;
	GtkTreeIter iter;
...
static void
dory_list_view_scroll_to_file (DoryListView *view,
				   DoryFile *file)
{
	GtkTreePath *path;
	GtkTreeIter iter;

	if (!dory_list_model_get_first_iter_for_file (view->details->model, file, &iter)) {
		return;
	}
...
static void
list_view_scroll_to_file (DoryView *view,
			  const char *uri)
{
	DoryFile *file;
	...
	if (uri != NULL) {
		...
		if (file != NULL) {
			dory_list_view_scroll_to_file (DORY_LIST_VIEW (view), file);
			dory_file_unref (file);
		}
	}
}
...
static void
list_view_notify_clipboard_info (DoryClipboardMonitor *monitor,
                                 ...
                                 DoryListView *view)
{
	...
	if (info != NULL && info->cut) {
		dory_list_model_set_highlight_for_files (view->details->model, info->files);
	} else {
		dory_list_model_set_highlight_for_files (view->details->model, NULL);
	}
}

static void
dory_list_view_end_loading (DoryView *view,
				gboolean all_files_seen)
{
	DoryClipboardMonitor *monitor;
	DoryClipboardInfo *info;

    set_ok_to_load_deferred_attrs (DORY_LIST_VIEW (view), TRUE);

	monitor = dory_clipboard_monitor_get ();
	info = dory_clipboard_monitor_get_clipboard_info (monitor);

	list_view_notify_clipboard_info (monitor, info, DORY_LIST_VIEW (view));
}

static const char *
dory_list_view_get_id (DoryView *view)
{
	return DORY_LIST_VIEW_ID;
}

static const gchar *
dory_list_view_get_sort_attribute (DoryView *view)
{
	DoryListView *list_view = DORY_LIST_VIEW (view);
	gint sort_column_id;
	GtkSortType order;
	...
	attribute = dory_list_model_get_attribute_from_sort_column_id (
...
static void
dory_list_view_class_init (DoryListViewClass *class)
{
	DoryViewClass *dory_view_class;

	dory_view_class = DORY_VIEW_CLASS (class);

	G_OBJECT_CLASS (class)->dispose = dory_list_view_dispose;
	G_OBJECT_CLASS (class)->finalize = dory_list_view_finalize;

	dory_view_class->add_file = dory_list_view_add_file;
	dory_view_class->begin_loading = dory_list_view_begin_loading;
	dory_view_class->end_loading = dory_list_view_end_loading;
	dory_view_class->bump_zoom_level = dory_list_view_bump_zoom_level;
	dory_view_class->can_zoom_in = dory_list_view_can_zoom_in;
	dory_view_class->can_zoom_out = dory_list_view_can_zoom_out;
        dory_view_class->click_policy_changed = dory_list_view_click_policy_changed;
	dory_view_class->clear = dory_list_view_clear;
	dory_view_class->file_changed = dory_list_view_file_changed;
	dory_view_class->get_backing_uri = dory_list_view_get_backing_uri;
	dory_view_class->get_selection = dory_list_view_get_selection;
    dory_view_class->peek_selection = dory_list_view_peek_selection;
    dory_view_class->get_selection_count = dory_list_view_get_selection_count;
	dory_view_class->get_selection_for_file_transfer = dory_list_view_get_selection_for_file_transfer;
	dory_view_class->get_item_count = dory_list_view_get_item_count;
	dory_view_class->is_empty = dory_list_view_is_empty;
	dory_view_class->remove_file = dory_list_view_remove_file;
    dory_view_class->merge_menus = dory_list_view_merge_menus;
    dory_view_class->unmerge_menus = dory_list_view_unmerge_menus;
	dory_view_class->update_menus = dory_list_view_update_menus;
	dory_view_class->reset_to_defaults = dory_list_view_reset_to_defaults;
	dory_view_class->restore_default_zoom_level = dory_list_view_restore_default_zoom_level;
    dory_view_class->get_default_zoom_level = dory_list_view_get_default_zoom_level;
	dory_view_class->reveal_selection = dory_list_view_reveal_selection;
	dory_view_class->select_all = dory_list_view_select_all;
	dory_view_class->set_selection = dory_list_view_set_selection;
	dory_view_class->invert_selection = dory_list_view_invert_selection;
	dory_view_class->compare_files = dory_list_view_compare_files;
	dory_view_class->update_filter_text = dory_list_view_update_filter_text;
	dory_view_class->select_first = dory_list_view_select_first;
	dory_view_class->sort_directories_first_changed = dory_list_view_sort_directories_first_changed;
	dory_view_class->sort_favorites_first_changed = dory_list_view_sort_favorites_first_changed;
	dory_view_class->start_renaming_file = dory_list_view_start_renaming_file;
	dory_view_class->get_zoom_level = dory_list_view_get_zoom_level;
	dory_view_class->zoom_to_level = dory_list_view_zoom_to_level;
	dory_view_class->end_file_changes = dory_list_view_end_file_changes;
	dory_view_class->using_manual_layout = dory_list_view_using_manual_layout;
	dory_view_class->get_view_id = dory_list_view_get_id;
	dory_view_class->get_first_visible_file = dory_list_view_get_first_visible_file;
	dory_view_class->scroll_to_file = list_view_scroll_to_file;
    dory_view_class->click_to_rename_mode_changed = dory_list_view_click_to_rename_mode_changed;
	dory_view_class->get_sort_attribute = dory_list_view_get_sort_attribute;
}

static void
dory_list_view_init (DoryListView *list_view)
{
	list_view->details = g_new0 (DoryListViewDetails, 1);
	...
	g_signal_connect_swapped (dory_preferences,
				  ...
				  list_view);
	g_signal_connect_swapped (dory_list_view_preferences,
				  "changed::" DORY_PREFERENCES_LIST_VIEW_DEFAULT_ZOOM_LEVEL,
				  G_CALLBACK (default_zoom_level_changed_callback),
				  list_view);
	g_signal_connect_swapped (dory_list_view_preferences,
				  "changed::" DORY_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS,
				  G_CALLBACK (default_visible_columns_changed_callback),
				  list_view);
	g_signal_connect_swapped (dory_list_view_preferences,
				  "changed::" DORY_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER,
				  G_CALLBACK (default_column_order_changed_callback),
				  list_view);

    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_TOOLTIPS_LIST_VIEW,
	...
	dory_list_view_click_policy_changed (DORY_VIEW (list_view));
    dory_list_view_click_to_rename_mode_changed (DORY_VIEW (list_view));

	dory_list_view_sort_directories_first_changed (DORY_VIEW (list_view));
	dory_list_view_sort_favorites_first_changed (DORY_VIEW (list_view));

    list_view->details->current_selection_count = -1;

	/* ensure that the zoom level is always set in begin_loading */
	list_view->details->zoom_level = DORY_ZOOM_LEVEL_SMALLEST - 1;
...
static DoryView *
dory_list_view_create (DoryWindowSlot *slot)
{
	DoryListView *view;

	view = g_object_new (DORY_TYPE_LIST_VIEW,
			     "window-slot", slot,
			     NULL);
...
static gboolean
dory_list_view_supports_uri (const char *uri,
				 GFileType file_type,
...
static DoryViewInfo dory_list_view = {
	(char *)DORY_LIST_VIEW_ID,
	...
	dory_list_view_create,
	dory_list_view_supports_uri
};

void
dory_list_view_register (void)
{
	dory_list_view.view_combo_label = _(dory_list_view.view_combo_label);
	dory_list_view.view_menu_label_with_mnemonic = _(dory_list_view.view_menu_label_with_mnemonic);
	dory_list_view.error_label = _(dory_list_view.error_label);
	dory_list_view.startup_error_label = _(dory_list_view.startup_error_label);
	dory_list_view.display_location_label = _(dory_list_view.display_location_label);

	dory_view_factory_register (&dory_list_view);
}

GtkTreeView*
dory_list_view_get_tree_view (DoryListView *list_view)
{
	return list_view->details->tree_view;
}

static void
dory_list_view_update_filter_text (DoryView   *view,
                                   const char *filter_text)
{
    DoryListView *list_view = DORY_LIST_VIEW (view);

    dory_list_model_set_filter_active (list_view->details->model,
                                       filter_text != NULL && filter_text[0] != '\0');
}

static void
dory_list_view_select_first (DoryView *view)
{
    DoryListView *list_view = DORY_LIST_VIEW (view);
    GtkTreeModel *model = GTK_TREE_MODEL (list_view->details->model);
    GtkTreeIter iter;
