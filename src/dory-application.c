...
G_DEFINE_TYPE (DoryApplication, dory_application, GTK_TYPE_APPLICATION);

struct _DoryApplicationPriv {
	DoryProgressUIHandler *progress_handler;

    gboolean cache_problem;
    gboolean ignore_cache_problem;
};

static DoryApplication *singleton = NULL;
...
add_fallback_mandatory_css_provider (const gchar *theme_name)
{
    ...
    if (!g_strstr_len (css, -1, "dory")) {
        g_warning ("The theme appears to have no dory support.  Adding some...");

        init_fallback_css = load_file_contents_from_resource ("/org/dory/dory-style-fallback-mandatory.css",
                                                              &error);
...
process_system_theme (GtkSettings *gtk_settings)
{
    ...
    if (!is_known_supported_theme (theme_name)) {
        g_warning ("Current gtk theme is not known to have dory support (%s) - checking...", theme_name);
...
init_icons_and_styles (void)
{
    ...
    gtk_icon_theme_append_search_path (gtk_icon_theme_get_default (),
                       DORY_DATADIR G_DIR_SEPARATOR_S "icons");

    gtk_icon_size_register (DORY_STATUSBAR_ICON_SIZE_NAME,
                            DORY_STATUSBAR_ICON_SIZE,
                            DORY_STATUSBAR_ICON_SIZE);

    add_css_provider_at_priority ("/org/dory/dory-style-fallback.css",
                                  GTK_STYLE_PROVIDER_PRIORITY_FALLBACK);

    add_css_provider_at_priority ("/org/dory/dory-style-application.css",
                                  GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);

    GtkSettings *gtk_settings = gtk_settings_get_default ();
    /* We create our own 'runtime theme' when we encounter one that doesn't
     * include dory support. We must listen for the theme changing so this
...
save_accel_map (gpointer data)
{
    if (save_of_accel_map_requested) {
        ...
        accel_map_filename = dory_get_accel_map_file ();
        ...
        gpointer user_data)
{
    if (!save_of_accel_map_requested) {
        ...
        g_timeout_add_seconds (DORY_ACCEL_MAP_SAVE_DELAY, 
                               save_accel_map, NULL);
    }
}
...
static void
menu_provider_items_updated_handler (DoryMenuProvider *provider, GtkWidget* parent_window, gpointer data)
{
...
init_menu_provider_callback (void)
{
    ...
    for (l = providers; l != NULL; l = l->next) {
        DoryMenuProvider *provider = DORY_MENU_PROVIDER (l->data);
...
gboolean
dory_application_check_required_directory (DoryApplication *application,
                                           gchar           *path)
{
    gboolean ret;

    g_assert (DORY_IS_APPLICATION (application));
    ...
    if (!ret) {
        ...
        error_string = g_strdup_printf (_("Dory could not create the required folder \"%s\"."),
                                        path);
        detail_string = _("Before running Dory, please create the following folder, or "
                          "set permissions such that Dory can create it.");
...
void
dory_application_open_location (DoryApplication *application,
                                GFile           *location,
                                GFile           *selection,
                                const char      *startup_id,
                                const gboolean  open_in_tabs)
{
    DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (application))->open_location (application,
                                                                              location,
                                                                              selection,
                                                                              startup_id,
                                                                              open_in_tabs);
...
void
dory_application_show_items (DoryApplication *application,
                             GFile          **uris,
                             gint             n_uris,
                             const char      *startup_id)
{
    DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (application))->show_items (application,
                                                                           uris,
                                                                           n_uris,
                                                                           startup_id);
}
...
DoryWindow *
dory_application_create_window (DoryApplication *application,
                                GdkScreen       *screen)
{
    return DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (application))->create_window (application,
                                                                              screen);
}

void
dory_application_notify_unmount_done (DoryApplication *application,
                                          const gchar *message)
{
    DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (application))->notify_unmount_done (application,
                                                                                    message);
}

void
dory_application_notify_unmount_show (DoryApplication *application,
                                          const gchar *message)
{
    DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (application))->notify_unmount_show (application,
                                                                                    message);
}

void
dory_application_close_all_windows (DoryApplication *application)
{
    DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (application))->close_all_windows (application);
}

static GObject *
dory_application_constructor (GType type,
				  guint n_construct_params,
				  GObjectConstructParam *construct_params)
{
        GObject *retval;

        retval = G_OBJECT_CLASS (dory_application_parent_class)->constructor
                (type, n_construct_params, construct_params);

        singleton = DORY_APPLICATION (retval);
...
static void
dory_application_init (DoryApplication *application)
{
	GSimpleAction *action;

	application->priv = G_TYPE_INSTANCE_GET_PRIVATE (application,
                                                     DORY_TYPE_APPLICATION,
                                                     DoryApplicationPriv);

    if (g_getenv("DORY_BENCHMARK_LOADING"))
        dory_startup_timer = g_timer_new ();
	...
	g_signal_connect_swapped (action, "activate",
				  G_CALLBACK (dory_application_quit), application);

	g_object_unref (action);
}

void
dory_application_quit (DoryApplication *self)
{
    ...
    DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (self))->continue_quit (self);
	 ...
	 * Save the last non-desktop window we find. */
	{
		...
		for (GList *l = windows; l != NULL; l = l->next) {
			...
			if (DORY_IS_DESKTOP_WINDOW (w)) {
				continue;
			}

			last_window = DORY_WINDOW (w);
			break;
		}

		if (last_window != NULL) {
			dory_window_save_session_state (last_window);
		}
	}
...
static void
dory_application_startup (GApplication *app)
{
	DoryApplication *self = DORY_APPLICATION (app);
	/* chain up to the GTK+ implementation early, so gtk_init()
	 * is called for us.
	 */
	G_APPLICATION_CLASS (dory_application_parent_class)->startup (app);

	/* create an undo manager */
	self->undo_manager = dory_undo_manager_new ();

	/* initialize preferences and create the global GSettings objects */
	dory_global_preferences_init ();

    /* Run desktop- or main- specific things */
    DORY_APPLICATION_CLASS (G_OBJECT_GET_CLASS (self))->continue_startup (self);

	/* register property pages */
	dory_image_properties_page_register ();

	/* initialize theming */
	init_icons_and_styles ();
	init_gtk_accels ();

	/* initialize dory modules */
	dory_module_setup ();
	...
	self->priv->progress_handler = dory_progress_ui_handler_new ();
...
static void
dory_application_quit_mainloop (GApplication *app)
{
	DEBUG ("Quitting mainloop");

    dory_icon_info_clear_caches ();
    save_accel_map (NULL);
    g_object_unref (DORY_APPLICATION (app)->undo_manager);
    g_clear_object (&mandatory_css_provider);

    dory_application_notify_unmount_done (DORY_APPLICATION (app), NULL);

	G_APPLICATION_CLASS (dory_application_parent_class)->quit_mainloop (app);
}

static void
dory_application_window_removed (GtkApplication *app,
				     GtkWindow *window)
{
	DoryPreviewer *previewer;

	/* chain to parent */
	GTK_APPLICATION_CLASS (dory_application_parent_class)->window_removed (app, window);
	...
	if (g_list_length (gtk_application_get_windows (app)) == 0) {
		...
		dory_previewer_call_close (previewer);
	}
}

static void
dory_application_class_init (DoryApplicationClass *class)
{
    ...
    object_class->constructor = dory_application_constructor;

    application_class = G_APPLICATION_CLASS (class);
    application_class->startup = dory_application_startup;
    application_class->quit_mainloop = dory_application_quit_mainloop;

    gtkapp_class = GTK_APPLICATION_CLASS (class);
    gtkapp_class->window_removed = dory_application_window_removed;

    g_type_class_add_private (class, sizeof (DoryApplicationPriv));
}

DoryApplication *
dory_application_initialize_singleton (GType object_type,
                                       ...
                                       ...)
{
    ...
    application = DORY_APPLICATION (g_object_new_valist (object_type, first_property_name, var_args));
    va_end (var_args);
...
DoryApplication *
dory_application_get_singleton (void)
{
    return singleton;
}

void
dory_application_check_thumbnail_cache (DoryApplication *application)
{
    application->priv->cache_problem = !dory_thumbnail_factory_check_status ();
}

gboolean
dory_application_get_cache_bad (DoryApplication *application)
{
    return application->priv->cache_problem;
}

void
dory_application_clear_cache_flag (DoryApplication *application)
{
    application->priv->cache_problem = FALSE;
}

void
dory_application_set_cache_flag (DoryApplication *application)
{
    application->priv->cache_problem = TRUE;
}

void
dory_application_ignore_cache_problem (DoryApplication *application)
{
    application->priv->ignore_cache_problem = TRUE;
}

gboolean
dory_application_get_cache_problem_ignored (DoryApplication *application)
{
