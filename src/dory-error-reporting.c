...
/* dory-error-reporting.h - implementation of file manager functions that report
 	                        errors to the user.
...
#define NEW_NAME_TAG "Dory: new name"
#define MAXIMUM_DISPLAYED_FILE_NAME_LENGTH	50

static void finish_rename (DoryFile *file, gboolean stop_timer, GError *error);

void
dory_report_error_loading_directory (DoryFile *file,
					 GError *error,
...
void
dory_report_error_setting_group (DoryFile *file,
				     GError *error,
				     GtkWindow *parent_window)
{
	char *file_name;
	...
	if (message == NULL) {
		...
		g_warning ("Hit unhandled case %s:%d in dory_report_error_setting_group", 
...
void
dory_report_error_setting_owner (DoryFile *file,
				     GError *error,
				     GtkWindow *parent_window)
{
	char *file_name;
...
void
dory_report_error_setting_permissions (DoryFile *file,
					   GError *error,
					   GtkWindow *parent_window)
{
	char *file_name;
...
typedef struct _DoryRenameData {
	char *name;
...
void
dory_report_error_renaming_file (DoryFile *file,
				     const char *new_name,
				     GError *error,
				     GtkWindow *parent_window)
{
	 ...
	 */
	original_name = dory_file_get_display_name (file);
	...
	if (message == NULL) {
		/* We should invent decent error messages for every case we actually experience. */
		g_warning ("Hit unhandled case %s:%d in dory_report_error_renaming_file", 
			   g_quark_to_string (error->domain), error->code);
		/* fall through */
...
static void
dory_rename_data_free (DoryRenameData *data)
{
	g_free (data->name);
	g_free (data);
}
...
static void
rename_callback (DoryFile *file, GFile *result_location,
		 GError *error, gpointer callback_data)
{
	...
	if (error &&
		...
		dory_report_error_renaming_file (file, data->name, error, NULL);
	}
...
cancel_rename_callback (gpointer callback_data)
{
	...
	finish_rename (DORY_FILE (callback_data), FALSE, error);
	g_error_free (error);
}

static void
finish_rename (DoryFile *file, gboolean stop_timer, GError *error)
{
	DoryRenameData *data;
	...
	dory_file_cancel (file, rename_callback, NULL);
	if (stop_timer) {
...
void
dory_rename_file (DoryFile *file,
		      const char *new_name,
		      DoryFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	g_object_set_data_full (G_OBJECT (file),
				NEW_NAME_TAG,
				data, (GDestroyNotify)dory_rename_data_free);
