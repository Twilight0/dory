...
/* dory-bookmarks-window.c - implementation of bookmark-editing window.
 */
...
static DoryBookmarkList *bookmarks = NULL;
static GtkTreeView	    *bookmark_list_widget = NULL; /* awkward name to distinguish from DoryBookmarkList */
...
static void     name_or_uri_field_activate                  (DoryEntry        *entry);
static void     dory_bookmarks_window_restore_geometry  (GtkWidget            *window);
static void     on_bookmark_list_changed                    (DoryBookmarkList *list,
							     gpointer              user_data);
...
/* We store a pointer to the bookmark in a column so when an item is moved
   ...
   dory_bookmark_list_insert_item() makes a copy of the bookmark, so we
...
static void
dory_bookmarks_window_response_callback (GtkDialog *dialog,
					     int response_id,
...
bookmarks_set_empty (gboolean empty)
{
	...
	if (empty) {
		...
		if (dory_bookmark_list_length (bookmarks) > 0 &&
 ...
 * @list: The DoryBookmarkList that this window will edit.
 *
 * Return value: A pointer to the new window.
 **/
GtkWindow *
create_bookmarks_window (DoryBookmarkList *list, GObject *undo_manager_source)
{
	GtkWidget         *window;
	...
	if (!gtk_builder_add_from_resource (builder,
					    "/org/dory/dory-bookmarks-window.glade",
					    NULL)) {
		return NULL;
	}
	...
	dory_undo_share_undo_manager (G_OBJECT (window), undo_manager_source);

	gtk_window_set_wmclass (GTK_WINDOW (window), "bookmarks", "Dory");
	dory_bookmarks_window_restore_geometry (window);
	...
	g_signal_connect (window, "response",
			  G_CALLBACK (dory_bookmarks_window_response_callback), NULL);
...
static void
dory_bookmarks_window_restore_geometry (GtkWidget *window)
{
	...
	window_geometry = dory_bookmark_list_get_window_geometry (bookmarks);
...
dory_bookmarks_window_save_geometry (GtkWindow *window)
{
	...
	if (gtk_widget_get_visible (GTK_WIDGET (window))) {
		...
		dory_bookmark_list_set_window_geometry (bookmarks, geometry_string);
		g_free (geometry_string);
	}
}

static void
on_bookmark_list_changed (DoryBookmarkList *bmarks, gpointer data)
{
	g_return_if_fail (DORY_IS_BOOKMARK_LIST (bmarks));
...
open_selected_bookmark (gpointer user_data, GdkScreen *screen)
{
	...
	location = dory_bookmark_get_location (selected);
	if (location == NULL) { 
		return;
	}
                        ...
                        gpointer   user_data)
{
    ...
    dory_bookmark_list_sort_ascending (bookmarks);
}
...
bookmarks_delete_bookmark (void)
{
    ...
    for (i = 0; i < dory_bookmark_list_length (bookmarks); i++) {
        DoryBookmark *bookmark_in_list = dory_bookmark_list_item_at (bookmarks, i);

        if (bookmark_in_list == bookmark) {
            dory_bookmark_list_delete_item_at (bookmarks, i);
            break;
        }
    }
    ...
    if (gtk_tree_model_get_iter_first (GTK_TREE_MODEL (store), &row_iter)) {
        do {
            ...
            if (bookmark != NULL) {
                ...
                for (i = 0; i < dory_bookmark_list_length (bookmarks); i++) {
                    DoryBookmark *old_bm = dory_bookmark_list_item_at (bookmarks, i);

                    if (old_bm == bookmark) {
                        dory_bookmark_list_move_item (bookmarks, i, new_index++);
                    }
                }
            } else {
                g_settings_set_int (dory_window_state, DORY_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT, row);
            }
		      ...
		      gpointer user_data)
{
	...
	if (selected) {
		name = dory_bookmark_get_name (selected);
		location = dory_bookmark_get_location (selected);
	...
	dory_entry_set_text (DORY_ENTRY (name_field),
				 name ? name : "");
	...
	dory_entry_set_text (DORY_ENTRY (uri_field),
...
{
	if (text_changed) {
		DoryBookmark *bookmark, *bookmark_in_list;
		const char *name;
		gchar *icon_name;
		guint selected_row;
		GtkTreeIter iter;
		...
		bookmark = dory_bookmark_new (location,
        ...
        if (selected_row > g_settings_get_int (dory_window_state, DORY_PREFERENCES_SIDEBAR_BOOKMARK_BREAKPOINT)) {
            selected_row--;
        }
		...
		g_signal_handler_block (bookmarks, 
					bookmark_list_changed_signal_id);
		dory_bookmark_list_delete_item_at (bookmarks, selected_row);
		dory_bookmark_list_insert_item (bookmarks, bookmark, selected_row);
		g_signal_handler_unblock (bookmarks, 
					  bookmark_list_changed_signal_id);
		...
		bookmark_in_list = dory_bookmark_list_item_at (bookmarks,
								   selected_row);

		name = dory_bookmark_get_name (bookmark_in_list);
		icon_name = dory_bookmark_get_icon_name (bookmark_in_list);
...
static void
name_or_uri_field_activate (DoryEntry *entry)
{
	g_assert (DORY_IS_ENTRY (entry));

	update_bookmark_from_text ();
	dory_entry_select_all_at_idle (entry);
}
...
restore_geometry (gpointer data)
{
	...
	dory_bookmarks_window_restore_geometry (GTK_WIDGET (data));
...
on_window_hide_event (GtkWidget *widget,
		      gpointer user_data)
{
	dory_bookmarks_window_save_geometry (GTK_WINDOW (widget));

	/* Disable undo for entry widgets */
	dory_undo_unregister (G_OBJECT (name_field));
	dory_undo_unregister (G_OBJECT (uri_field));
...
repopulate_now (void)
{
    ...
    bookmarks_length = dory_bookmark_list_length (bookmarks);
    ...
    while (index < bookmarks_length) {
        ...
        if (index == breakpoint && !breakpoint_added) {
            ...
            bookmark = dory_bookmark_list_item_at (bookmarks, index);
            bookmark_name = dory_bookmark_get_name (bookmark);
            bookmark_icon = dory_bookmark_get_icon_name (bookmark);
