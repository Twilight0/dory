...
G_DEFINE_TYPE (DoryExtensionConfigWidget, dory_extension_config_widget, DORY_TYPE_CONFIG_BASE_WIDGET);


typedef struct {
    DoryExtensionConfigWidget *widget;
...
static void
update_restart_visiblity (DoryExtensionConfigWidget *widget)
{
    GList *tmp = g_list_copy (widget->initial_extension_ids);
    gchar **new_settings = g_settings_get_strv (dory_plugin_preferences,
    		                                    DORY_PLUGIN_PREFERENCES_DISABLED_EXTENSIONS);
...
static void
detect_extensions (DoryExtensionConfigWidget *widget)
{
    gchar *out = NULL;
...
static void
refresh_widget (DoryExtensionConfigWidget *widget)
{
    ...
    dory_config_base_widget_set_default_buttons_sensitive (DORY_CONFIG_BASE_WIDGET (widget), widget->current_extensions != NULL);
}

static void
on_settings_changed (GSettings *settings, gchar *key, gpointer user_data)
{
    DoryExtensionConfigWidget *w = DORY_EXTENSION_CONFIG_WIDGET (user_data);
...
static void
on_enable_clicked (GtkWidget *button, DoryExtensionConfigWidget *widget)
{
    g_settings_set_strv (dory_plugin_preferences,
    		             DORY_PLUGIN_PREFERENCES_DISABLED_EXTENSIONS,
						 NULL);
...
static void
on_disable_clicked (GtkWidget *button, DoryExtensionConfigWidget *widget)
{
    GPtrArray *new_list = g_ptr_array_new ();
...
static void
on_restart_clicked (GtkWidget *button, DoryExtensionConfigWidget *widget)
{
    ...
    g_spawn_command_line_async ("sh -c \"dory --quit && sleep 1 && dory\"", NULL);
}

static void
dory_extension_config_widget_finalize (GObject *object)
{
    DoryExtensionConfigWidget *widget = DORY_EXTENSION_CONFIG_WIDGET (object);
...
static void
dory_extension_config_widget_class_init (DoryExtensionConfigWidgetClass *klass)
{
    GObjectClass *oclass;
...
static void
dory_extension_config_widget_init (DoryExtensionConfigWidget *self)
{
    self->current_extensions = NULL;
    self->initial_extension_ids = NULL;

    self->bl_handler = g_signal_connect (dory_plugin_preferences,
    		                             "changed::" DORY_PLUGIN_PREFERENCES_DISABLED_EXTENSIONS,
    ...
    gchar **init_settings = g_settings_get_strv (dory_plugin_preferences,
    		                                     DORY_PLUGIN_PREFERENCES_DISABLED_EXTENSIONS);
    guint i;
...
{
  return g_object_new (DORY_TYPE_EXTENSION_CONFIG_WIDGET, NULL);
}
