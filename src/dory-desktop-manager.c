...
#include "dory-blank-desktop-window.h"
#include "dory-desktop-window.h"
#include "dory-application.h"
#include "dory-cinnamon-dbus.h"
...
static gboolean layout_changed (DoryDesktopManager *manager);
...
} DoryDesktopManagerPrivate;

struct _DoryDesktopManager
{
    GtkWindow parent_object;

    DoryDesktopManagerPrivate *priv;
};

G_DEFINE_TYPE_WITH_PRIVATE (DoryDesktopManager, dory_desktop_manager, G_TYPE_OBJECT);

#define FETCH_PRIV(m) DoryDesktopManagerPrivate *priv = DORY_DESKTOP_MANAGER (m)->priv;
...
static RunState
get_run_state (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gint ret;
    GError *error;
    ...
    if (!dory_cinnamon_call_get_run_state_sync (priv->proxy,
                                                &ret,
                                                NULL,
                                                &error)) {
...
static gint
get_n_monitors (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gsize n_monitors, i;
    ...
    if (priv->current_run_state == RUN_STATE_FALLBACK) {
        ...
        n_monitors = dory_desktop_utils_get_num_monitors ();
    ...
    if (!dory_cinnamon_call_get_monitors_sync (priv->proxy,
                                               &monitors,
                                               NULL,
                                               &error)) {
...
static void
get_window_rect_for_monitor (DoryDesktopManager *manager,
                             gint                monitor,
                             GdkRectangle       *rect)
{
    FETCH_PRIV (manager);
    ...
    if (priv->current_run_state == RUN_STATE_FALLBACK) {
        ...
        dory_desktop_utils_get_monitor_geometry (monitor, &out_rect);

        goto out;
    }

    if (!dory_cinnamon_call_get_monitor_work_rect_sync (priv->proxy,
                                                        monitor,
                                                        &out_rect_var,
                                                        NULL,
                                                        &error)) {
    ...
    scale_factor = dory_desktop_utils_get_scale_factor ();
...
static void
close_all_windows (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
static void
queue_update_layout (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
static void
global_scale_changed (DoryDesktopManager *manager)
{
    g_return_if_fail (DORY_IS_DESKTOP_MANAGER (manager));
...
static void
create_new_desktop_window (DoryDesktopManager *manager,
                                         gint  monitor,
                                     gboolean  primary,
                                     gboolean  show_desktop)
{
    ...
    if (show_desktop) {
        window = GTK_WIDGET (dory_desktop_window_new (monitor));
    } else {
        window = GTK_WIDGET (dory_blank_desktop_window_new (monitor));
    }
    ...
    gtk_application_add_window (GTK_APPLICATION (dory_application_get_singleton ()),
                                GTK_WINDOW (window));
...
static gboolean
update_overlay_in_idle (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);

    if (manager->priv->overlay) {
        dory_desktop_overlay_update_in_place (priv->overlay);
    }
...
static gboolean
layout_changed (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gint n_monitors = 0;
    gint x_primary = 0;
    ...
    gchar *pref = g_settings_get_string (dory_desktop_preferences, DORY_PREFERENCES_DESKTOP_LAYOUT);

    if (g_strcmp0 (pref, "") == 0) {
        g_settings_set_string (dory_desktop_preferences, DORY_PREFERENCES_DESKTOP_LAYOUT, DESKTOPS_DEFAULT);
        g_free (pref);
    ...
    for (i = 0; i < n_monitors; i++) {
        ...
        } else if (!dory_desktop_utils_get_monitor_cloned (i, x_primary)) {
...
static void
on_bus_name_owner_changed (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
    gchar *name_owner;
...
static void
on_run_state_changed (DoryDesktopManager *manager)
{
    g_return_if_fail (DORY_IS_DESKTOP_MANAGER (manager));
    ...
    if (new_state == RUN_STATE_RUNNING || new_state == RUN_STATE_FALLBACK) {
        priv->current_run_state = new_state;
        layout_changed (manager);
        g_application_release (G_APPLICATION (dory_application_get_singleton ()));
    }
}

static void
on_monitors_changed (DoryDesktopManager *manager)
{
    g_return_if_fail (DORY_IS_DESKTOP_MANAGER (manager));
    ...
    for (l = priv->desktops; l != NULL; l = l->next) {
        ...
        if (DORY_IS_DESKTOP_WINDOW (info->window)) {
            dory_desktop_window_update_geometry (DORY_DESKTOP_WINDOW (info->window));
        }
        else
        if (DORY_IS_BLANK_DESKTOP_WINDOW (info->window)) {
            dory_blank_desktop_window_update_geometry (DORY_BLANK_DESKTOP_WINDOW (info->window));
        }
    }
}

static void
on_overlay_adjustments_changed (DoryDesktopOverlay      *overlay,
                                DoryWindow              *window,
                                gint                     h_percent,
                                gint                     v_percent,
                                DoryDesktopManager      *manager)
{
    g_return_if_fail (DORY_IS_DESKTOP_WINDOW (window));

    dory_desktop_window_set_grid_adjusts (DORY_DESKTOP_WINDOW (window),
                                          h_percent,
                                          v_percent);
}
                 ...
                 gpointer   *user_data)
{
    if (g_strcmp0 (signal_name, "RunStateChanged") == 0) {
        on_run_state_changed (DORY_DESKTOP_MANAGER (user_data));
    } 
    else
    if (g_strcmp0 (signal_name, "MonitorsChanged") == 0) {
        on_monitors_changed (DORY_DESKTOP_MANAGER (user_data));
    }
}

static gboolean
on_failsafe_timeout (DoryDesktopManager *manager)
{
    g_return_val_if_fail (DORY_IS_DESKTOP_MANAGER (manager), FALSE);
    ...
    g_warning ("dory-desktop: Desktop failsafe timeout reached, applying fallback behavior");
...
static void
connect_fallback_signals (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
on_proxy_created (GObject      *source,
                  ...
                  gpointer      user_data)
{
    DoryDesktopManager *manager = DORY_DESKTOP_MANAGER (user_data);
    FETCH_PRIV (manager);

    priv->proxy = dory_cinnamon_proxy_new_for_bus_finish (res, NULL);
...
static gboolean
fallback_startup_idle_cb (DoryDesktopManager *manager)
{
    on_run_state_changed (manager);
...
static void
dory_desktop_manager_dispose (GObject *object)
{
    DoryDesktopManager *manager = DORY_DESKTOP_MANAGER (object);
    FETCH_PRIV (manager);

    DEBUG ("Disposing DoryDesktopManager");

    close_all_windows (manager);

    g_clear_object (&priv->overlay);

    g_signal_handlers_disconnect_by_func (dory_desktop_preferences, queue_update_layout, manager);
    g_signal_handlers_disconnect_by_func (dory_preferences, queue_update_layout, manager);
    ...
    G_OBJECT_CLASS (dory_desktop_manager_parent_class)->dispose (object);
}

static void
dory_desktop_manager_finalize (GObject *object)
{
    FETCH_PRIV (object);
    ...
    DEBUG ("Finalizing DoryDesktopManager");

    G_OBJECT_CLASS (dory_desktop_manager_parent_class)->finalize (object);
}

static void
dory_desktop_manager_class_init (DoryDesktopManagerClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = dory_desktop_manager_finalize;
  object_class->dispose = dory_desktop_manager_dispose;
}

static void
dory_desktop_manager_init (DoryDesktopManager *manager)
{
    DoryDesktopManagerPrivate *priv;

    manager->priv = G_TYPE_INSTANCE_GET_PRIVATE (manager, DORY_TYPE_DESKTOP_MANAGER, DoryDesktopManagerPrivate);
    ...
    g_signal_connect_swapped (dory_desktop_preferences, 
                              "changed::" DORY_PREFERENCES_SHOW_DESKTOP,
                              G_CALLBACK (queue_update_layout),
                              manager);

    g_signal_connect_swapped (dory_desktop_preferences,
                              "changed::" DORY_PREFERENCES_DESKTOP_LAYOUT,
                              G_CALLBACK (queue_update_layout),
                              manager);

    g_signal_connect_swapped (dory_desktop_preferences,
                              "changed::" DORY_PREFERENCES_USE_DESKTOP_GRID,
                              G_CALLBACK (queue_update_layout),
                              manager);
    ...
    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_DESKTOP_IS_HOME_DIR,
                              G_CALLBACK (queue_update_layout),
                              manager);

    g_signal_connect_swapped (dory_preferences,
                              "changed::" DORY_PREFERENCES_SHOW_ORPHANED_DESKTOP_ICONS,
                              G_CALLBACK (queue_update_layout),
                              manager);
    ...
    g_application_hold (G_APPLICATION (dory_application_get_singleton ()));

    priv->other_desktop = !is_cinnamon_desktop ();
    if (!priv->other_desktop && !eel_check_is_wayland ()) {
         g_message ("dory-desktop: session is x11 cinnamon, establishing proxy");

        dory_cinnamon_proxy_new_for_bus (G_BUS_TYPE_SESSION,
...
static DoryDesktopManager *_manager = NULL;

DoryDesktopManager*
dory_desktop_manager_get (void)
{
    if (_manager == NULL) {
        _manager = g_object_new (DORY_TYPE_DESKTOP_MANAGER, NULL);
    }
...
gboolean
dory_desktop_manager_has_desktop_windows (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
gboolean
dory_desktop_manager_get_monitor_is_active (DoryDesktopManager *manager,
                                                          gint  monitor)
{
    FETCH_PRIV (manager);
    GList *iter;
...
gboolean
dory_desktop_manager_get_monitor_is_primary (DoryDesktopManager *manager,
                                                           gint  monitor)
{
    FETCH_PRIV (manager);
    GList *iter;
...
gboolean
dory_desktop_manager_get_primary_only (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
...
DoryActionManager *
dory_desktop_manager_get_action_manager (void)
{
    g_return_val_if_fail (_manager != NULL, NULL);
    FETCH_PRIV (_manager);

    if (priv->action_manager == NULL) {
        priv->action_manager = dory_action_manager_new ();
    }
...
void
dory_desktop_manager_get_window_rect_for_monitor (DoryDesktopManager *manager,
                                                  gint                monitor,
                                                  GdkRectangle       *rect)
{
...
void
dory_desktop_manager_get_margins (DoryDesktopManager *manager,
                                  gint                monitor,
                                  gint               *left,
                                  gint               *right,
                                  gint               *top,
                                  gint               *bottom)
{
    ...
    DEBUG ("DoryDesktopManager get_margins: monitor=%d proxy_owned=%d other_desktop=%d use_layer_shell=%d",
    ...
    if (priv->proxy_owned && !priv->other_desktop) {
        *left = *right = *top = *bottom = 0;
        ...
        DEBUG ("DoryDesktopManager get_margins: returning 0 margins (Cinnamon running)");
    ...
    if (monitor != dory_desktop_utils_get_primary_monitor ()) {
        *left = *right = *top = *bottom = 25;

        return;
    }

    dory_desktop_utils_get_monitor_geometry (monitor, &geometry);
    dory_desktop_utils_get_monitor_work_rect (monitor, &work_rect);
...
GtkWindow *
dory_desktop_manager_get_window_for_monitor (DoryDesktopManager *manager,
                                             gint                monitor)
{
    GtkWindow *window;
    GList *iter;
...
void
dory_desktop_manager_get_overlay_info (DoryDesktopManager *manager,
                                       gint                monitor,
                                       ...
                                       gint               *v_adjust)
{
    ...
    window = dory_desktop_manager_get_window_for_monitor (manager, monitor);

    if (DORY_IS_DESKTOP_WINDOW (window) &&
        dory_desktop_window_get_grid_adjusts (DORY_DESKTOP_WINDOW (window),
                                              h_adjust,
                                              v_adjust)) {

        *action_group = dory_desktop_window_get_action_group (DORY_DESKTOP_WINDOW (window));
    } else {
        *action_group = NULL;
    }
}
...
void
dory_desktop_manager_show_desktop_overlay (DoryDesktopManager *manager,
                                           gint                monitor)
{
    FETCH_PRIV (manager);

    if (priv->overlay == NULL) {
        priv->overlay = dory_desktop_overlay_new ();
    ...
    dory_desktop_overlay_show (priv->overlay, monitor);
}

gboolean
dory_desktop_manager_get_is_cinnamon         (DoryDesktopManager *manager)
{
    FETCH_PRIV (manager);
