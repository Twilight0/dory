...
struct DoryImagePropertiesPageDetails {
...
typedef struct {
        GObject parent;
} DoryImagePropertiesPageProvider;

typedef struct {
        GObjectClass parent;
} DoryImagePropertiesPageProviderClass;


static GType dory_image_properties_page_provider_get_type (void);
static void  property_page_provider_iface_init                (DoryPropertyPageProviderIface *iface);


G_DEFINE_TYPE (DoryImagePropertiesPage, dory_image_properties_page, GTK_TYPE_BOX);

G_DEFINE_TYPE_WITH_CODE (DoryImagePropertiesPageProvider, dory_image_properties_page_provider, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (DORY_TYPE_PROPERTY_PAGE_PROVIDER,
						property_page_provider_iface_init));

static void
dory_image_properties_page_finalize (GObject *object)
{
	DoryImagePropertiesPage *page;

	page = DORY_IMAGE_PROPERTIES_PAGE (object);
	...
	G_OBJECT_CLASS (dory_image_properties_page_parent_class)->finalize (object);
}
		     ...
		     gpointer      data)
{
	...
	page = DORY_IMAGE_PROPERTIES_PAGE (data);
...
static void
append_item (DoryImagePropertiesPage *page,
	     const char                  *name,
	     const char                  *value)
{
...
static gboolean
append_tag_value_pair (DoryImagePropertiesPage *page,
		       ExifData *data,
		       ExifTag   tag,
		       char     *description)
{
...
#ifdef HAVE_EXEMPI
static void
append_xmp_value_pair (DoryImagePropertiesPage *page,
		       XmpPtr      xmp,
		       const char *ns,
		       const char *propname,
		       char       *descr)
...
static gboolean
append_option_value_pair (DoryImagePropertiesPage *page,
			  GdkPixbuf                   *pixbuf,
			  const char                  *key,
			  char                        *description)
{
...
static void
append_basic_info (DoryImagePropertiesPage *page)
{
...
static void
append_options_info (DoryImagePropertiesPage *page)
{
	GdkPixbuf *pixbuf;
...
static void
append_exif_info (DoryImagePropertiesPage *page)
{
#ifdef HAVE_EXIF
	ExifData *exifdata;
...
static void
append_xmp_info (DoryImagePropertiesPage *page)
{
#ifdef HAVE_EXEMPI
...
static void
load_finished (DoryImagePropertiesPage *page)
{
	GtkWidget *label;
			...
			gpointer         callback_data)
{
	...
	page = DORY_IMAGE_PROPERTIES_PAGE (callback_data);
		    ...
		    gpointer      user_data)
{
	...
	DoryImagePropertiesPage *page = data->page;
	GFile *file;
	...
	if (stream) {
		...
		mime_type = dory_file_info_get_mime_type (data->info);
...
static void
load_location (DoryImagePropertiesPage *page,
	       DoryFileInfo	       *info)
{
	GFile *file;
	char *uri;
	...
	g_assert (DORY_IS_IMAGE_PROPERTIES_PAGE (page));
	g_assert (info != NULL);

	page->details->cancellable = g_cancellable_new ();

	uri = dory_file_info_get_uri (info);
...
static void
dory_image_properties_page_class_init (DoryImagePropertiesPageClass *class)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (class);

	object_class->finalize = dory_image_properties_page_finalize;

	g_type_class_add_private (object_class, sizeof(DoryImagePropertiesPageDetails));
}

static void
dory_image_properties_page_init (DoryImagePropertiesPage *page)
{
	GtkWidget *sw;

	page->details = G_TYPE_INSTANCE_GET_PRIVATE (page,
						     DORY_TYPE_IMAGE_PROPERTIES_PAGE,
						     DoryImagePropertiesPageDetails);
...
static GList *
get_property_pages (DoryPropertyPageProvider *provider,
                    GList *files)
{
	GList *pages;
	DoryFileInfo *file;
	...
	pages = NULL;
        ...
        file = DORY_FILE_INFO (files->data);

	mime_type = dory_file_info_get_mime_type (file);
	if (mime_type != NULL && is_mime_type_supported (mime_type)) {
		DoryImagePropertiesPage *page;
		DoryPropertyPage *real_page;

		page = g_object_new (dory_image_properties_page_get_type (), NULL);
		load_location (page, file);

		real_page = dory_property_page_new ("DoryImagePropertiesPage::property_page",
...
static void
property_page_provider_iface_init (DoryPropertyPageProviderIface *iface)
{
...
static void
dory_image_properties_page_provider_init (DoryImagePropertiesPageProvider *sidebar)
{
}

static void
dory_image_properties_page_provider_class_init (DoryImagePropertiesPageProviderClass *class)
{
}

void
dory_image_properties_page_register (void)
{
        dory_module_add_type (dory_image_properties_page_provider_get_type ());
}
