...
/* dory-mime-actions.c - uri-specific versions of mime action functions
...
#define DEBUG_FLAG DORY_DEBUG_MIME
...
typedef struct {
	...
	DoryFileListHandle *files_handle;
...
launch_location_from_file (DoryFile *file)
{
	LaunchLocation *location;
	location = g_new (LaunchLocation, 1);
	location->file = dory_file_ref (file);
	location->uri = dory_file_get_uri (file);
				 ...
				 const char *uri)
{
	dory_file_unref (location->file);
	g_free (location->uri);
	location->file = dory_file_get_by_uri (uri);
...
static GList*
filter_dory_handler (GList *apps)
{
	GList *l, *next;
	GAppInfo *application;
	const char *id;
...
static gboolean
dory_mime_actions_check_if_required_attributes_ready (DoryFile *file)
{
	DoryFileAttributes attributes;
	gboolean ready;

	attributes = dory_mime_actions_get_required_file_attributes ();
	ready = dory_file_check_if_ready (file, attributes);

	return ready;
}

DoryFileAttributes
dory_mime_actions_get_required_file_attributes (void)
{
...
static gboolean
file_has_local_path (DoryFile *file)
{
	GFile *location;
	char *path;
	gboolean res;
...
GAppInfo *
dory_mime_get_default_application_for_file (DoryFile *file)
{
	GAppInfo *app = NULL;
	char *mime_type;
	char *uri_scheme;

	if (!dory_mime_actions_check_if_required_attributes_ready (file)) {
        if (file_has_local_path (file)) {
            return NULL;
        }
	}

    uri_scheme = dory_file_get_uri_scheme (file);
...
static int
file_compare_by_mime_type (DoryFile *file_a,
			   DoryFile *file_b)
{
	char *mime_type_a, *mime_type_b;
	int ret;

	mime_type_a = dory_file_get_mime_type (file_a);
	mime_type_b = dory_file_get_mime_type (file_b);
...
static int
file_compare_by_parent_uri (DoryFile *file_a,
			    DoryFile *file_b) {
	char *parent_uri_a, *parent_uri_b;
	int ret;

	parent_uri_a = dory_file_get_parent_uri (file_a);
	parent_uri_b = dory_file_get_parent_uri (file_b);
...
GList *
dory_mime_get_applications_for_file (DoryFile *file)
{
	char *mime_type;
	char *uri_scheme;
	GList *result;
	...
	return filter_dory_handler (result);
}

GAppInfo *
dory_mime_get_default_application_for_files (GList *files)
{
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_app = dory_mime_get_default_application_for_file (file);
...
GList *
dory_mime_get_applications_for_files (GList *files)
{
	GList *l, *sorted_files;
	DoryFile *file;
	GList *one_ret, *ret;
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_ret = dory_mime_get_applications_for_file (file);
...
static void
report_broken_symbolic_link (GtkWindow *parent_window, DoryFile *file)
{
	char *target_path;
	char *display_name;
	char *prompt;
	...
	target_path = dory_file_get_symbolic_link_target_path (file);
	if (target_path == NULL) {
...
get_executable_text_file_action (GtkWindow *parent_window, DoryFile *file)
{
	GtkDialog *dialog;
	char *file_name;
	char *prompt;
	...
	g_assert (dory_file_contains_text (file));

	preferences_value = g_settings_get_enum	(dory_preferences,
						 DORY_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION);
	switch (preferences_value) {
	case DORY_EXECUTABLE_TEXT_LAUNCH:
		return ACTIVATION_ACTION_LAUNCH;
	case DORY_EXECUTABLE_TEXT_DISPLAY:
		return ACTIVATION_ACTION_OPEN_IN_APPLICATION;
	case DORY_EXECUTABLE_TEXT_ASK:
		break;
	default:
		/* Complain non-fatally, since preference data can't be trusted */
		g_warning ("Unknown value %d for DORY_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION",
			   preferences_value);

	}


	file_name = dory_file_get_display_name (file);
...
gboolean
dory_mime_file_opens_in_view (DoryFile *file)
{
...
static gboolean
dory_mime_file_is_image (DoryFile *file) {
	return dory_file_is_mime_type(file, "image/*");
}

static ActivationAction
get_activation_action (DoryFile *file, gboolean is_desktop)
{
	...
	if (activation_uri == NULL) {
		activation_uri = dory_file_get_uri (file);
	}
	...
	if (dory_file_is_launchable (file)) {
		...
		if (!executable_path) {
			action = ACTIVATION_ACTION_DO_NOTHING;
		} else if (dory_file_contains_text (file)) {
	...
	if (action == ACTIVATION_ACTION_DO_NOTHING) {
		if (dory_mime_file_opens_in_view (file) && !is_desktop) {
...
gboolean
dory_mime_file_opens_in_external_app (DoryFile *file)
{
 ...
 *
 * Construct a list of ApplicationLaunchParameters from a list of DoryFiles,
			    ...
			    GList **unhandled_uris)
{
	...
	*unhandled_uris = NULL;
	...
	for (l = uris; l != NULL; l = l->next) {
		...
		app = dory_mime_get_default_application_for_file (file);
		if (app != NULL) {
			app_uris = NULL;
...
static gboolean
file_was_cancelled (DoryFile *file)
{
	GError *error;
...
static gboolean
file_was_not_mounted (DoryFile *file)
{
	GError *error;
...
activation_parameters_free (ActivateParameters *parameters)
{
	...
	dory_file_list_free (parameters->mountables);
	dory_file_list_free (parameters->start_mountables);
	dory_file_list_free (parameters->not_mounted);
...
cancel_activate_callback (gpointer callback_data)
{
	...
	if (parameters->files_handle) {
		dory_file_list_cancel_call_when_ready (parameters->files_handle);
...
activate_parameters_special_free (ActivateParametersSpecial *parameters_special)
{
    ...
    dory_file_unref (parameters_special->file);
    g_free (parameters_special);
}

static void
make_exec_callback (DoryFile *file,
                       GFile *res_loc,
                      GError *error,
                     gpointer callback_data)
{
    ...
    if (error == NULL) {
        gchar *path = dory_file_get_path (params->file);
        dory_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (params->parent_window)),
                                              path, FALSE, NULL);
        g_free (path);
    }
...
run_open_with_dialog (ActivateParametersSpecial *params)
{
    ...
    mime_type = dory_file_get_mime_type (params->file);
    uri = dory_file_get_uri (params->file);
                                 ...
                                 ActivateParametersSpecial *parameters)
{
    ...
    case RESPONSE_RUN:
        ;
        guint32 existing = dory_file_get_permissions (parameters->file);
        dory_file_set_permissions (parameters->file,
                                   existing | (UNIX_PERM_USER_EXEC|UNIX_PERM_GROUP_EXEC|UNIX_PERM_OTHER_EXEC),
                                   (DoryFileOperationCallback) make_exec_callback,
                                   parameters);
        break;
...
application_unhandled_uri (ActivateParameters *parameters, char *uri)
{
    ...
    DoryFile *file = dory_file_get_existing_by_uri (uri);
    ...
    parameters_special->file = dory_file_ref (file);

    enable_exec_button = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_MIME_MAKE_EXECUTABLE);
    ...
    param_uri = dory_file_get_uri (parameters_special->file);
    if (param_uri != NULL) {
        gtk_dialog_add_button (GTK_DIALOG (dialog),
                   _("Choose a program"), RESPONSE_OPEN_WITH);
    }

    if (!dory_file_can_set_permissions (file) && enable_exec_button) {
				      ...
				      ActivateParametersSpecial *parameters)
{
	...
	case RESPONSE_DISPLAY:
		{
			GAppInfo *app;
			app = dory_mime_get_default_application_for_file (parameters->file);
			if (app != NULL) {
				GList file_list = { 0 };

				file_list.data = parameters->file;
				dory_launch_application (app, &file_list, parameters->parent_window);
				g_object_unref (app);
			}
		}
		break;
	case RESPONSE_MARK_TRUSTED:
		file = dory_file_get_location (parameters->file);
		dory_file_mark_desktop_file_trusted (file,
							 parameters->parent_window,
							 TRUE,
							 NULL, NULL);
		g_object_unref (file);
		       ...
		       DoryFile *file)
{
	...
	if (!dory_file_is_trusted_link (file)) {
	...
	dory_launch_desktop_file (screen, uri, NULL,
				      parameters->parent_window);
	g_free (uri);
}
							 ...
							 gpointer callback_data)
{
	...
	return DORY_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->compare_files (view, file1, file2);
}
...
add_sorted_view_uris (GList *uris,
					  ...
					  ActivateParameters *parameters)
{
	DoryDirectory *current_directory;
	DoryFile *file;
	...
	current_directory = dory_view_get_model (parameters->slot->content_view);
	files = dory_directory_get_file_list (current_directory);
	files = g_list_sort_with_data (files, compare_files_by_view_order, parameters->slot->content_view);

	for (l = files; l != NULL; l = l->next) {
		file = l->data;

		if (dory_mime_file_is_image (file)) {
			mime_type = dory_file_get_mime_type (file);
...
// allows an external sort order to be defined, adds all supported images in current dory view to @uris.
static void
launch_application (GAppInfo *application,
				GList *uris,
				ActivateParameters *parameters)
{
	DoryFile *single_file;
	...
	if (uris != NULL && uris->next == NULL && accepts_uri_list) {
		sortable_image_viewers = g_settings_get_strv (dory_preferences,
													  DORY_PREFERENCES_IMAGE_VIEWERS_WITH_EXTERNAL_SORT);
		...
		if (sortable_image_viewers != NULL &&
			...
			single_file = dory_file_get_by_uri (uris->data);
			if (dory_mime_file_is_image (single_file)) {
				add_sorted_view_uris (uris, application, parameters);
			}
			dory_file_unref (single_file);
		}
		g_strfreev (sortable_image_viewers);
	}

	dory_launch_application_by_uri (application,
					uris,
					parameters->parent_window);
}
...
activate_files (ActivateParameters *parameters)
{
    ...
    if (parameters->slot != NULL) {
        window = dory_window_slot_get_window (parameters->slot);

        launch_location_is_desktop = DORY_IS_DESKTOP_WINDOW (window);
    }
	...
	for (l = launch_files; l != NULL; l = l->next) {
		...
		dory_launch_application_from_command (screen, quoted_path, FALSE, NULL);
		g_free (quoted_path);
		g_free (executable_path);
		g_free (uri);
	...
	for (l = launch_in_terminal_files; l != NULL; l = l->next) {
		...
		dory_launch_application_from_command (screen, quoted_path, TRUE, NULL);
	...
	if (count > 1) {
		if ((parameters->flags & DORY_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0 &&
	...
	if (parameters->slot != NULL &&
		...
		if ((flags & DORY_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
		    g_settings_get_enum (dory_preferences, DORY_PREFERENCES_NEW_TAB_POSITION) ==
		...
		for (l = open_in_view_files; l != NULL; l = l->next) {
            ...
            if (g_file_is_native (initial_location) &&
                (dory_file_is_in_admin (file) ||
                 !dory_file_can_read (file) ||
            ...
            dory_window_slot_open_location (parameters->slot, final_location, flags);
	...
	if (open_in_app_parameters != NULL ||
	    unhandled_open_in_app_uris != NULL) {
		if ((parameters->flags & DORY_WINDOW_OPEN_FLAG_CLOSE_BEHIND) != 0 &&
		    window != NULL) {
...
activation_mount_not_mounted (ActivateParameters *parameters)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		loc = l->data;
		next = l->next;
		dory_file_invalidate_all_attributes (loc->file);
	}

	files = get_file_list_for_launch_locations (parameters->locations);
	dory_file_list_call_when_ready
		(files,
		 dory_mime_actions_get_required_file_attributes () | DORY_FILE_ATTRIBUTE_LINK_INFO,
					 ...
					 gpointer callback_data)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		...
		if (dory_file_is_broken_symbolic_link (file) && !dory_file_is_in_favorites (file)) {
			launch_location_free (location);
			parameters->locations = g_list_delete_link (parameters->locations, l);
			pause_activation_timed_cancel (parameters);
		...
		if (dory_file_get_file_type (file) == G_FILE_TYPE_MOUNTABLE &&
		    !dory_file_has_activation_uri (file)) {
	...
	for (l = parameters->locations; l != NULL; l = l->next) {
		 ...
		 */
		uri = dory_file_get_activation_uri (location->file);
		if (uri != NULL) {
...
static void
activation_mountable_mounted (DoryFile  *file,
			      ...
			      gpointer       callback_data)
{
	...
	if (error == NULL) {
		...
		target_file = dory_file_get (result_location);
...
static void
activation_mountable_started (DoryFile  *file,
			      GFile         *gfile_of_file,
			      GError        *error,
			      gpointer       callback_data)
{
 ...
 * @files: A GList of DoryFiles to activate.
 *
 **/
void
dory_mime_activate_files (GtkWindow *parent_window,
			      DoryWindowSlot *slot,
			      GList *files,
			      ...
			      gboolean user_confirmation)
{
	...
	if (file_count == 1) {
		file_name = dory_file_get_display_name (files->data);
 ...
 * @view: FMDirectoryView in question.
 * @file: A DoryFile representing the file in this view to activate.
 * @use_new_window: Should this item be opened in a new window?
 *
 **/

void
dory_mime_activate_file (GtkWindow *parent_window,
			     DoryWindowSlot *slot,
			     DoryFile *file,
			     const char *launch_directory,
			     DoryWindowOpenFlags flags)
{
	...
	dory_mime_activate_files (parent_window, slot, files, launch_directory, flags, FALSE);
	g_list_free (files);
}

void
dory_mime_launch_fm_and_select_file (GFile *file)
{
    GAppInfo *info;
    GList *list;
