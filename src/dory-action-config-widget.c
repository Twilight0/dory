...
#include <libdory-private/dory-action-symbols.h>
#include "dory-global-preferences.h"

G_DEFINE_TYPE (DoryActionConfigWidget, dory_action_config_widget, DORY_TYPE_CONFIG_BASE_WIDGET);


typedef struct {
    DoryActionConfigWidget *widget;
...
on_check_toggled(GtkWidget *button, ActionProxy *proxy)
{
    ...
    gchar **blacklist = g_settings_get_strv (dory_plugin_preferences, DORY_PLUGIN_PREFERENCES_DISABLED_ACTIONS);
    ...
    g_signal_handler_block (dory_plugin_preferences, proxy->widget->bl_handler);
    g_settings_set_strv (dory_plugin_preferences,
    		             DORY_PLUGIN_PREFERENCES_DISABLED_ACTIONS,
						 (const gchar * const *) new_list_ptr);
    g_signal_handler_unblock (dory_plugin_preferences, proxy->widget->bl_handler);
...
static void
populate_from_directory (DoryActionConfigWidget *widget, const gchar *path)
{
    GDir *dir;
...
static void
refresh_widget (DoryActionConfigWidget *widget)
{
    gchar **data_dirs;
    gchar *path;
    guint i;
    ...
    if (widget->actions == NULL) {
        ...
        gtk_widget_set_sensitive (GTK_WIDGET (DORY_CONFIG_BASE_WIDGET (widget)->listbox), FALSE);
    } else {
        GList *l;
        gchar **blacklist = g_settings_get_strv (dory_plugin_preferences,
        		                                 DORY_PLUGIN_PREFERENCES_DISABLED_ACTIONS);
    ...
    dory_config_base_widget_set_default_buttons_sensitive (DORY_CONFIG_BASE_WIDGET (widget), widget->actions != NULL);
}

static void
on_settings_changed (GSettings *settings, gchar *key, gpointer user_data)
{
    DoryActionConfigWidget *w = DORY_ACTION_CONFIG_WIDGET (user_data);

    refresh_widget (w);
}

static void
on_enable_clicked (GtkWidget *button, DoryActionConfigWidget *widget)
{
    g_settings_set_strv (dory_plugin_preferences,
    		             DORY_PLUGIN_PREFERENCES_DISABLED_ACTIONS,
						 NULL);
...
static void
on_disable_clicked (GtkWidget *button, DoryActionConfigWidget *widget)
{
    GPtrArray *new_list = g_ptr_array_new ();
...
static void
on_open_folder_clicked (GtkWidget *button, DoryActionConfigWidget *widget)
{
    gchar *path = NULL;
    path = g_build_filename (g_get_user_data_dir (), "dory", "actions", NULL);
...
static void
on_layout_editor_clicked (GtkWidget *button, DoryActionConfigWidget *widget)
{
    g_spawn_command_line_async ("dory-action-layout-editor", NULL);
}
...
on_dir_changed (GFileMonitor     *monitor,
                ...
                gpointer          user_data)
{
    DoryActionConfigWidget *widget = DORY_ACTION_CONFIG_WIDGET (user_data);

    refresh_widget (widget);
}

static void
try_monitor_path (DoryActionConfigWidget *widget, const gchar *path)
{
...
static void setup_dir_monitors (DoryActionConfigWidget *widget)
{
...
static void
dory_action_config_widget_finalize (GObject *object)
{
    DoryActionConfigWidget *widget = DORY_ACTION_CONFIG_WIDGET (object);
    ...
    g_signal_handler_disconnect (dory_plugin_preferences, widget->bl_handler);

    G_OBJECT_CLASS (dory_action_config_widget_parent_class)->finalize (object);
}

static void
dory_action_config_widget_class_init (DoryActionConfigWidgetClass *klass)
{
    GObjectClass *oclass;
    oclass = G_OBJECT_CLASS (klass);

    oclass->finalize = dory_action_config_widget_finalize;
}

static void
dory_action_config_widget_init (DoryActionConfigWidget *self)
{
    self->actions = NULL;

    self->bl_handler = g_signal_connect (dory_plugin_preferences, 
    ...
    g_signal_connect (dory_config_base_widget_get_enable_button (DORY_CONFIG_BASE_WIDGET (self)), "clicked",
                                                                 G_CALLBACK (on_enable_clicked), self);

    g_signal_connect (dory_config_base_widget_get_disable_button (DORY_CONFIG_BASE_WIDGET (self)), "clicked",
...
GtkWidget *
dory_action_config_widget_new (void)
{
  return g_object_new (DORY_TYPE_ACTION_CONFIG_WIDGET, NULL);
}
