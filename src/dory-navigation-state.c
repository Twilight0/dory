...
struct _DoryNavigationStateDetails {
...
G_DEFINE_TYPE (DoryNavigationState, dory_navigation_state, G_TYPE_OBJECT);

static void
clear_bindings (DoryNavigationState *self)
{
...
static void
update_bindings (DoryNavigationState *self)
{
	gint length, idx;
	GBinding *binding;
...
static void
dory_navigation_state_init (DoryNavigationState *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, DORY_TYPE_NAVIGATION_STATE,
						  DoryNavigationStateDetails);
}

static void
dory_navigation_state_set_property (GObject *object,
					guint property_id,
					const GValue *value,
					GParamSpec *pspec)
{
	DoryNavigationState *self = DORY_NAVIGATION_STATE (object);

	switch (property_id) {
	case PROP_SLAVE:
		self->priv->slave = g_value_dup_object (value);
		dory_navigation_state_add_group (self, g_value_get_object (value));
		break;
	case PROP_MASTER:
...
static void
dory_navigation_state_finalize (GObject *obj)
{
	DoryNavigationState *self = DORY_NAVIGATION_STATE (obj);

	g_strfreev (self->priv->action_names);

	G_OBJECT_CLASS (dory_navigation_state_parent_class)->finalize (obj);	
}

static void
dory_navigation_state_dispose (GObject *obj)
{
	DoryNavigationState *self = DORY_NAVIGATION_STATE (obj);
	...
	G_OBJECT_CLASS (dory_navigation_state_parent_class)->dispose (obj);
}

static void
dory_navigation_state_class_init (DoryNavigationStateClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->dispose = dory_navigation_state_dispose;
	oclass->finalize = dory_navigation_state_finalize;
	oclass->set_property = dory_navigation_state_set_property;
	...
	g_type_class_add_private (klass, sizeof (DoryNavigationStateDetails));
}

DoryNavigationState *
dory_navigation_state_new (GtkActionGroup *slave,
			       const gchar **action_names)
{
	return g_object_new (DORY_TYPE_NAVIGATION_STATE,
			     "slave", slave,
...
void
dory_navigation_state_set_master (DoryNavigationState *self,
				      GtkActionGroup *master)
{
...
void
dory_navigation_state_add_group (DoryNavigationState *self,
...
void
dory_navigation_state_sync_all (DoryNavigationState *self)
{
	GList *l;
	gint length, idx;
...
GtkActionGroup *
dory_navigation_state_get_master (DoryNavigationState *self)
{
	return self->priv->master;
}

void
dory_navigation_state_set_boolean (DoryNavigationState *self,
				       const gchar *action_name,
				       gboolean value)
{
	GtkAction *action;
