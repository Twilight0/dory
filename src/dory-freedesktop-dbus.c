...
#include "dory-freedesktop-generated.h"

/* We share the same debug domain as dory-dbus-manager */
...
G_DEFINE_TYPE (DoryFreedesktopDBus, dory_freedesktop_dbus, G_TYPE_OBJECT);
			       ...
			       gpointer data)
{
	...
	application = DORY_APPLICATION (g_application_get_default ());
	...
	dory_application_show_items (application,
	                             (GFile **) files->pdata,
	                             files->len,
	                             startup_id);
				 ...
				 gpointer data)
{
	...
	for (i = 0; uris[i] != NULL; i++) {
		...
		dory_application_open_location (application, file, NULL, startup_id, FALSE);
					 ...
					 gpointer data)
{
	...
	for (i = 0; uris[i] != NULL; i++) {
		files = g_list_prepend (files, dory_file_get_by_uri (uris[i]));
        }

	files = g_list_reverse (files);

	dory_properties_window_present (files, NULL, startup_id);
...
static void
dory_freedesktop_dbus_dispose (GObject *object)
{
	...
	G_OBJECT_CLASS (dory_freedesktop_dbus_parent_class)->dispose (object);
}

static void
dory_freedesktop_dbus_class_init (DoryFreedesktopDBusClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = dory_freedesktop_dbus_dispose;
}

static void
dory_freedesktop_dbus_init (DoryFreedesktopDBus *fdb)
{
...
void
dory_freedesktop_dbus_set_open_locations (DoryFreedesktopDBus *fdb,
					      const gchar **locations)
{
	g_return_if_fail (DORY_IS_FREEDESKTOP_DBUS (fdb));
...
DoryFreedesktopDBus *
dory_freedesktop_dbus_new (void)
{	
	return g_object_new (dory_freedesktop_dbus_get_type (),
                             NULL);
}
