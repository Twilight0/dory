...
#define DORY_X_CONTENT_BAR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), DORY_TYPE_X_CONTENT_BAR, DoryXContentBarPrivate))

struct DoryXContentBarPrivate
{
	GtkWidget *label;
	GtkWidget *button;
...
G_DEFINE_TYPE (DoryXContentBar, dory_x_content_bar, GTK_TYPE_INFO_BAR)
			 ...
			 gpointer user_data)
{
	...
	if (default_app != NULL) {
		dory_launch_application_for_mount (default_app, bar->priv->mount,
...
static void
dory_x_content_bar_set_x_content_type (DoryXContentBar *bar, const char *x_content_type)
{					  
	char *message;
	char *description;
	GAppInfo *default_app;
...
static void
dory_x_content_bar_set_mount (DoryXContentBar *bar, GMount *mount)
{
...
static void
dory_x_content_bar_set_property (GObject      *object,
				     guint         prop_id,
				     const GValue *value,
				     GParamSpec   *pspec)
{
	...
	bar = DORY_X_CONTENT_BAR (object);

	switch (prop_id) {
	case PROP_MOUNT:
		dory_x_content_bar_set_mount (bar, G_MOUNT (g_value_get_object (value)));
		break;
	case PROP_X_CONTENT_TYPE:
		dory_x_content_bar_set_x_content_type (bar, g_value_get_string (value));
		break;
	default:
...
static void
dory_x_content_bar_get_property (GObject    *object,
				     guint       prop_id,
				     GValue     *value,
				     GParamSpec *pspec)
{
	DoryXContentBar *bar = DORY_X_CONTENT_BAR (object);
...
static void
dory_x_content_bar_finalize (GObject *object)
{
	DoryXContentBar *bar = DORY_X_CONTENT_BAR (object);
	...
	if (bar->priv->mount != NULL)
        ...
        G_OBJECT_CLASS (dory_x_content_bar_parent_class)->finalize (object);
}

static void
dory_x_content_bar_class_init (DoryXContentBarClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);
	object_class->get_property = dory_x_content_bar_get_property;
	object_class->set_property = dory_x_content_bar_set_property;
	object_class->finalize = dory_x_content_bar_finalize;

	g_type_class_add_private (klass, sizeof (DoryXContentBarPrivate));
...
static void
dory_x_content_bar_init (DoryXContentBar *bar)
{
	GtkWidget *content_area;

	bar->priv = DORY_X_CONTENT_BAR_GET_PRIVATE (bar);
...
GtkWidget *
dory_x_content_bar_new (GMount *mount, 
			    const char *x_content_type)
{
	return g_object_new (DORY_TYPE_X_CONTENT_BAR, 
			     "mount", mount,
