/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   dory-view-factory.c: register and create DoryViews
...
void
dory_view_factory_register (DoryViewInfo *view_info)
{
	g_return_if_fail (view_info != NULL);
	g_return_if_fail (view_info->id != NULL);
	g_return_if_fail (dory_view_factory_lookup (view_info->id) == NULL);
	
	registered_views = g_list_append (registered_views, view_info);
}

const DoryViewInfo *
dory_view_factory_lookup (const char *id)
{
	GList *l;
	DoryViewInfo *view_info;
...
DoryView *
dory_view_factory_create (const char *id,
			      DoryWindowSlot *slot)
{
	const DoryViewInfo *view_info;
	DoryView *view;

	view_info = dory_view_factory_lookup (id);
	if (view_info == NULL) {
		return NULL;
	}
...
gboolean
dory_view_factory_view_supports_uri (const char *id,
					 GFile *location,
					 GFileType file_type,
...
GList *
dory_view_factory_get_views_for_uri (const char *uri,
					 GFileType file_type,
					 const char *mime_type)
{
	GList *l, *res;
