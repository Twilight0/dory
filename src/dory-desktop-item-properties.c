...
static void
dory_desktop_item_properties_url_drag_data_received (GtkWidget *widget, GdkDragContext *context,
                                                         int x, int y,
...
static void
dory_desktop_item_properties_exec_drag_data_received (GtkWidget *widget, GdkDragContext *context,
                                                          int x, int y,
                                                          GtkSelectionData *selection_data,
                                                          guint info, guint time,
                                                          GtkEntry *entry)
{
	...
	file = dory_file_get_by_uri (uris[0]);
	...
	for (l = entries; l; l = l->next) {
		...
		if (item_entry->filename) {
			...
			g_signal_connect (entry, "drag_data_received",
					  G_CALLBACK (dory_desktop_item_properties_url_drag_data_received),
					  entry);
			...
			g_signal_connect (entry, "drag_data_received",
					  G_CALLBACK (dory_desktop_item_properties_exec_drag_data_received),
					  entry);
		}
	}
...
static void
dory_desktop_item_properties_create_begin (const char *uri,
                                               GtkWidget *box)
{
	GFile *location;
...
GtkWidget *
dory_desktop_item_properties_make_box (GtkSizeGroup *label_size_group,
                                           GList *files)
{
	DoryFileInfo *info;
	char *uri;
...
gboolean
dory_desktop_item_properties_should_show (GList *files)
{
	DoryFileInfo *info;
	...
	if (!dory_file_info_is_mime_type (info, "application/x-desktop")) {
		return FALSE;
	}
