...
#include "dory-trash-bar.h"
#include "dory-view-factory.h"
#include "dory-x-content-bar.h"
...
static void begin_location_change                     (DoryWindowSlot         *slot,
                                                       GFile                      *location,
                                                       GFile                      *previous_location,
                                                       GList                      *new_selection,
                                                       DoryLocationChangeType  type,
                                                       guint                       distance,
                                                       const char                 *scroll_pos,
                                                       gboolean                    mount,
                                                       DoryWindowGoToCallback      callback,
                                                       gpointer                    user_data);
static void free_location_change                      (DoryWindowSlot         *slot);
static void end_location_change                       (DoryWindowSlot         *slot);
static void cancel_location_change                    (DoryWindowSlot         *slot);
static void got_file_info_for_view_selection_callback (DoryFile               *file,
						       gpointer                    callback_data);
static void create_content_view                       (DoryWindowSlot         *slot,
						       const char                 *view_id);
static void display_view_selection_failure            (DoryWindow             *window,
						       DoryFile               *file,
						       GFile                      *location,
						       GError                     *error);
static void load_new_location                         (DoryWindowSlot         *slot,
						       GFile                      *location,
						       GList                      *selection,
						       gboolean                    tell_current_content_view,
						       gboolean                    tell_new_content_view);
static void location_has_really_changed               (DoryWindowSlot         *slot);
static void update_for_new_location                   (DoryWindowSlot         *slot);

/* set_displayed_location:
 */
static void
set_displayed_location (DoryWindowSlot *slot, GFile *location)
{
        ...
        if (slot->current_location_bookmark == NULL || location == NULL) {
                ...
                bookmark_location = dory_bookmark_get_location (slot->current_location_bookmark);
    ...
    if (recreate) {
        ...
        slot->current_location_bookmark = (location == NULL) ?
                            NULL : dory_bookmark_new (location, NULL, NULL, NULL);
    }
}

static void
check_bookmark_location_matches (DoryBookmark *bookmark, GFile *location)
{
        GFile *bookmark_location;
        char *bookmark_uri, *uri;

	bookmark_location = dory_bookmark_get_location (bookmark);
 ...
 */
static void
check_last_bookmark_location_matches_slot (DoryWindowSlot *slot)
{
...
static void
handle_go_back (DoryWindowSlot *slot,
		GFile *location)
{
	guint i;
	GList *link;
	...
	check_bookmark_location_matches (DORY_BOOKMARK (g_list_nth_data (slot->back_list,
	...
	for (i = 0; i < slot->location_change_distance; ++i) {
		bookmark = DORY_BOOKMARK (slot->back_list->data);
		slot->back_list =
...
static void
handle_go_forward (DoryWindowSlot *slot,
		   GFile *location)
{
	guint i;
	GList *link;
	...
	check_bookmark_location_matches (DORY_BOOKMARK (g_list_nth_data (slot->forward_list,
									     slot->location_change_distance)),
					 location);
	g_assert (slot->location != NULL);
	...
	for (i = 0; i < slot->location_change_distance; ++i) {
		bookmark = DORY_BOOKMARK (slot->forward_list->data);
		slot->forward_list =
			g_list_remove (slot->back_list, bookmark);
		slot->back_list =
			g_list_prepend (slot->forward_list, bookmark);
...
static void
handle_go_elsewhere (DoryWindowSlot *slot,
		     GFile *location)
{
...
static void
viewed_file_changed_callback (DoryFile *file,
                              DoryWindowSlot *slot)
{
        GFile *new_location;
	...
	g_assert (file == slot->viewed_file);
        ...
        if (!dory_file_is_not_yet_confirmed (file)) {
                slot->viewed_file_seen = TRUE;
        }

	was_in_trash = slot->viewed_file_in_trash;

	slot->viewed_file_in_trash = is_in_trash = dory_file_is_in_trash (file);

    /* Close window if the file it's viewing has been deleted or moved to trash. */
    if (dory_file_is_gone (file) || (is_in_trash && !was_in_trash)) {
        DoryFile *parent;
        ...
        if (parent != NULL) {
            parent_is_desktop = dory_file_is_desktop_directory (parent);
        ...
        */
        if (slot->viewed_file_seen) {
            ...
            */
            ...
            if (parent) {
                go_to_file = dory_find_existing_uri_in_hierarchy (parent);
            ...
            if (go_to_file != NULL) {
                ...
                * inexistant children to show up anymore */
                if (slot == slot->pane->active_slot) {
                    /* multiview-TODO also update DoryWindowSlot
                    * [which as of writing doesn't save/store any path bar state]
                    */
                    dory_path_bar_clear_buttons (DORY_PATH_BAR (slot->pane->path_bar));
                }

                dory_window_slot_open_location (slot, go_to_file, 0);
                g_object_unref (go_to_file);
            } else {
                dory_window_slot_go_home (slot, FALSE);
            }
        }
    } else {
        new_location = dory_file_get_location (file);
...
static void
update_history (DoryWindowSlot *slot,
                DoryLocationChangeType type,
                GFile *new_location)
{
        switch (type) {
        case DORY_LOCATION_CHANGE_STANDARD:
		handle_go_elsewhere (slot, new_location);
                return;
        case DORY_LOCATION_CHANGE_RELOAD:
                /* for reload there is no work to do */
                return;
        case DORY_LOCATION_CHANGE_BACK:
                handle_go_back (slot, new_location);
                return;
        case DORY_LOCATION_CHANGE_FORWARD:
...
static void
cancel_viewed_file_changed_callback (DoryWindowSlot *slot)
{
        DoryFile *file;
        ...
        if (file != NULL) {
                g_signal_handlers_disconnect_by_func (G_OBJECT (file),
						      ...
						      slot);
                dory_file_monitor_remove (file, &slot->viewed_file);
        }
}
...
void
dory_window_slot_open_location_full (DoryWindowSlot *slot,
					 GFile *location,
...
{
	DoryWindow *window;
        DoryWindow *target_window;
        DoryWindowPane *pane;
        DoryWindowSlot *target_slot;
	DoryWindowOpenFlags slot_flags;
	GFile *old_location;
	...
	old_uri = dory_window_slot_get_location_uri (slot);
	if (old_uri == NULL) {
	...
	is_desktop = DORY_IS_DESKTOP_WINDOW (window);

	if (is_desktop) {
		use_same = !dory_desktop_window_loaded (DORY_DESKTOP_WINDOW (window));

		/* if we're requested to open a new tab on the desktop, open a window
		 * instead.
		 */
		if (flags & DORY_WINDOW_OPEN_FLAG_NEW_TAB) {
			flags ^= DORY_WINDOW_OPEN_FLAG_NEW_TAB;
			flags |= DORY_WINDOW_OPEN_FLAG_NEW_WINDOW;
		}
	} else {
		use_same |= g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_ALWAYS_USE_BROWSER);
	}

	g_assert (!((flags & DORY_WINDOW_OPEN_FLAG_NEW_WINDOW) != 0 &&
		    (flags & DORY_WINDOW_OPEN_FLAG_NEW_TAB) != 0));

	/* and if the flags specify so, this is overridden */
	if ((flags & DORY_WINDOW_OPEN_FLAG_SEARCH) != 0) {
		use_same = TRUE;
	}
	else if ((flags & DORY_WINDOW_OPEN_FLAG_NEW_WINDOW) != 0) {
		use_same = FALSE;
	}
	...
	if (use_same) {
        ...
        target_window = dory_application_create_window (app, gtk_window_get_screen (GTK_WINDOW (window)));
	}

    old_location = dory_window_slot_get_location (slot);

    g_assert (target_window != NULL);

	/* if the flags say we want a new tab, open a slot in the current window */
	if ((flags & DORY_WINDOW_OPEN_FLAG_NEW_TAB) != 0) {
		...
		if (new_slot_position == DORY_NEW_TAB_POSITION_END) {
			slot_flags = DORY_WINDOW_OPEN_SLOT_APPEND;
		}

		target_slot = dory_window_pane_open_slot (dory_window_get_active_pane (window),
							      slot_flags);
	}

	/* close the current window if the flags say so */
	if ((flags & DORY_WINDOW_OPEN_FLAG_CLOSE_BEHIND) != 0) {
		if (!is_desktop) {
	...
	if (target_slot == NULL) {
		if (target_window == window) {
			...
			target_slot = dory_window_get_active_slot (target_window);
		}
	}
    ...
    begin_location_change (target_slot,
                           ...
                           0, NULL,
                           (flags & DORY_WINDOW_OPEN_FLAG_MOUNT),
                           callback,
                           user_data);
...
const char *
dory_window_slot_get_content_view_id (DoryWindowSlot *slot)
{
	if (slot->content_view == NULL) {
		return NULL;
	}
	return dory_view_get_view_id (slot->content_view);
}

gboolean
dory_window_slot_content_view_matches_iid (DoryWindowSlot *slot,
					       const char *iid)
{
	if (slot->content_view == NULL) {
		return FALSE;
	}
	return g_strcmp0 (dory_view_get_view_id (slot->content_view), iid) == 0;
}

static gboolean
report_callback (DoryWindowSlot *slot,
		 GError *error)
{
	if (slot->open_callback != NULL) {
		slot->open_callback (dory_window_slot_get_window (slot),
 ...
 * @window: The DoryWindow whose location should be changed.
 ...
 */
static void
begin_location_change (DoryWindowSlot        *slot,
                       GFile                 *location,
                       GFile                 *previous_location,
                       GList                 *new_selection,
                       DoryLocationChangeType type,
                       ...
                       gpointer               user_data)
{
	...
	g_assert (slot != NULL);
        ...
        g_assert (type == DORY_LOCATION_CHANGE_BACK
                  || type == DORY_LOCATION_CHANGE_FORWARD
                  || distance == 0);
	...
	if (new_selection == NULL && previous_location != NULL &&
		...
		if (parent != NULL) {
			new_selection = parent_selection =
				g_list_prepend (NULL, dory_file_get (from_folder));
			g_object_unref (parent);
		}
	...
	dory_window_slot_set_allow_stop (slot, TRUE);
	dory_window_slot_set_status (slot, " ", NULL, FALSE);
	 ...
	 */
	if (type == DORY_LOCATION_CHANGE_RELOAD || mount) {
		force_reload = TRUE;
	} else if (!dory_monitor_active ()) {
		force_reload = TRUE;
	} else {
		force_reload = !dory_directory_is_local (directory);
	}

	if (force_reload) {
        file = dory_directory_get_corresponding_file (directory);
        dory_file_invalidate_all_attributes (file);
	...
	if (parent_selection != NULL) {
        ...
        if (slot->current_location_bookmark != NULL &&
            slot->content_view != NULL) {
                current_pos = dory_view_get_first_visible_file (slot->content_view);
                dory_bookmark_set_scroll_pos (slot->current_location_bookmark, current_pos);
                g_free (current_pos);
        }

	/* Get the info needed for view selection */

        slot->determine_view_file = dory_file_get (location);
	...
	dory_file_call_when_ready (slot->determine_view_file,
			    ...
			    gpointer user_data)
{
	...
	slot->determine_view_file = dory_file_get (slot->pending_location);
	...
	if (!g_file_mount_enclosing_volume_finish (G_FILE (source_object), res, &error)) {
		...
		dory_file_invalidate_all_attributes (slot->determine_view_file);
		dory_file_call_when_ready (slot->determine_view_file,
					       DORY_FILE_ATTRIBUTE_INFO,
					       got_file_info_for_view_selection_callback,
					       slot);
...
static void
got_file_info_for_view_selection_callback (DoryFile *file,
					   gpointer callback_data)
{
        GError *error;
	char *view_id;
	...
	if (slot->mount_error) {
		...
		error = dory_file_get_file_info_error (file);
	}
	...
	parent_file = dory_file_get_parent (file);
	if ((parent_file != NULL) &&
	    dory_file_get_file_type (file) == G_FILE_TYPE_REGULAR) {
		if (slot->pending_selection != NULL) {
			g_list_free_full (slot->pending_selection, (GDestroyNotify) dory_file_unref);
		}

		g_clear_object (&slot->pending_location);
		g_free (slot->pending_scroll_to);

		slot->pending_location = dory_file_get_parent_location (file);
		slot->pending_selection = g_list_prepend (NULL, dory_file_ref (file));
		slot->determine_view_file = parent_file;
		slot->pending_scroll_to = dory_file_get_uri (file);
	...
	view_id = NULL;
		...
		mimetype = dory_file_get_mime_type (file);

		/* Look in metadata for view */
		if (dory_global_preferences_get_inherit_folder_viewer_preference ()) {
        if (dory_global_preferences_get_ignore_view_metadata ()) {
        view_id = g_strdup (dory_window_get_ignore_meta_view_id (window));
        } else {
            parent_file = file;
            dory_file_ref(parent_file); // Do this once for the initial file
            while (parent_file) {
                view_id = dory_file_get_metadata (parent_file, DORY_METADATA_KEY_DEFAULT_VIEW, NULL);
                tmp = dory_file_get_parent (parent_file);
                dory_file_unref(parent_file);
                if (view_id != NULL) {
                    parent_file = NULL;
                } else {
        ...
        view_id = dory_global_preferences_get_ignore_view_metadata () ? g_strdup (dory_window_get_ignore_meta_view_id (window)) :
                                                                        dory_file_get_metadata (file, DORY_METADATA_KEY_DEFAULT_VIEW, NULL);
    }

    if (view_id != NULL &&
		    !dory_view_factory_view_supports_uri (view_id,
							      location,
							      dory_file_get_file_type (file),
							      mimetype)) {
			g_free (view_id);
			view_id = NULL;
		}
		...
		if (view_id == NULL) {
            gchar *name, *uri;
            name = dory_file_get_name (file);
            uri = dory_file_get_uri (file);

            if (g_strcmp0 (name, "x-dory-search") == 0) {
                view_id = g_strdup (DORY_LIST_VIEW_IID);
            } else if (eel_uri_is_desktop (uri)) {
                view_id = dory_global_preferences_get_desktop_iid ();
            } else {
                view_id = dory_global_preferences_get_default_folder_viewer_preference_as_iid ();
            }
	...
	if (view_id != NULL) {
		...
		if (!gtk_widget_get_visible (GTK_WIDGET (window))) {
			...
			if (g_list_length (gtk_application_get_windows (GTK_APPLICATION (app))) == 1) {
				...
				if (!dory_is_root_directory (location)) {
					if (!dory_is_home_directory (location)) {
						dory_window_slot_go_home (slot, FALSE);
					} else {
						GFile *root;
			...
			if (slot->location == NULL) {
				...
				viewed_file = dory_file_get (slot->location);
				dory_window_slot_set_viewed_file (slot, viewed_file);
				dory_file_monitor_add (viewed_file, &slot->viewed_file, 0);
				g_signal_connect_object (viewed_file, "changed",
							 G_CALLBACK (viewed_file_changed_callback), slot, 0);
				dory_file_unref (viewed_file);

                gchar *path = dory_file_get_path (file);
                DORY_WINDOW_GET_CLASS (window)->prompt_for_location(window, path);
                g_free (path);
 ...
 */
static void
create_content_view (DoryWindowSlot *slot,
                     const char     *view_id)
{
    DoryWindow *window;
    DoryView *view;
    ...
    if (slot->content_view != NULL &&
        g_strcmp0 (dory_view_get_view_id (slot->content_view),
        view_id) == 0) {
        ...
        view = dory_view_factory_create (view_id, slot);
        slot->new_content_view = view;
        dory_window_connect_content_view (window, slot->new_content_view);
    }

    if (DORY_IS_DESKTOP_WINDOW (window)) {
        DoryDesktopDirectory *directory;

        directory = DORY_DESKTOP_DIRECTORY (dory_directory_get (slot->pending_location));
        directory->display_number = dory_desktop_window_get_monitor (DORY_DESKTOP_WINDOW (window));

        dory_directory_unref (DORY_DIRECTORY (directory));
    }
    ...
    if (slot->pending_location != NULL) {
        ...
        selection = dory_view_get_selection (slot->content_view);
        load_new_location (slot,
                           slot->location,
                           selection,
                           FALSE,
...
static void
load_new_location (DoryWindowSlot *slot,
		   GFile *location,
		   GList *selection,
		   gboolean tell_current_content_view,
		   gboolean tell_new_content_view)
{
	...
	/* Note, these may recurse into report_load_underway */
		...
		view = slot->content_view;
		dory_view_load_location (slot->content_view, location);
        }
 ...
 */
void
dory_window_report_load_underway (DoryWindow *window,
				      DoryView *view)
{
	DoryWindowSlot *slot;
...
static void
dory_window_emit_location_change (DoryWindow *window,
				      GFile *location)
{
	char *uri;
...
static void
dory_window_slot_emit_location_change (DoryWindowSlot *slot,
					   GFile *from,
					   GFile *to)
{
	char *from_uri = NULL;
 ...
 */
void
dory_window_report_location_change (DoryWindow *window)
{
	DoryWindowSlot *slot;
	GFile *location;
	...
	if (location != NULL) {
		dory_window_emit_location_change (window, location);
	}
}

static void
real_setup_loading_floating_bar (DoryWindowSlot *slot)
{
	...
	dory_floating_bar_set_label (DORY_FLOATING_BAR (slot->floating_bar),
					 DORY_IS_SEARCH_DIRECTORY (dory_view_get_model (slot->content_view)) ?
					 _("Searching...") : _("Loading..."));
	dory_floating_bar_set_show_spinner (DORY_FLOATING_BAR (slot->floating_bar),
						TRUE);
	dory_floating_bar_add_action (DORY_FLOATING_BAR (slot->floating_bar),
					  GTK_STOCK_STOP,
					  DORY_FLOATING_BAR_ACTION_ID_STOP);
...
static void
setup_loading_floating_bar (DoryWindowSlot *slot)
{
...
static void
location_has_really_changed (DoryWindowSlot *slot)
{
	DoryWindow *window;
	GtkWidget *widget;
	GFile *location_copy;
	...
	if (slot->new_content_view != NULL) {
		...
		if (gtk_widget_get_parent (widget) == NULL) {
			dory_window_slot_set_content_view_widget (slot, slot->new_content_view);
		}
	...
	if (location_copy != NULL) {
		if (slot == dory_window_get_active_slot (window)) {
			dory_window_emit_location_change (window, location_copy);
		}
...
static void
slot_add_extension_extra_widgets (DoryWindowSlot *slot)
{
	GList *providers, *l;
	GtkWidget *widget;
	char *uri;
	...
	providers = dory_module_get_extensions_for_type (DORY_TYPE_LOCATION_WIDGET_PROVIDER);
	window = dory_window_slot_get_window (slot);

	uri = g_file_get_uri (slot->location);
	for (l = providers; l != NULL; l = l->next) {
		DoryLocationWidgetProvider *provider;

		provider = DORY_LOCATION_WIDGET_PROVIDER (l->data);
		widget = dory_location_widget_provider_get_widget (provider, uri, GTK_WIDGET (window));
		if (widget != NULL) {
			dory_window_slot_add_extra_location_widget (slot, widget);
		}
	}
	g_free (uri);
...
static void
dory_window_slot_show_x_content_bar (DoryWindowSlot *slot, GMount *mount, const char **x_content_types)
{
	unsigned int n;
	...
	for (n = 0; x_content_types[n] != NULL; n++) {
		...
		if (default_app != NULL)  {
			GtkWidget *bar;
			bar = dory_x_content_bar_new (mount, x_content_types[n]);
			gtk_widget_show (bar);
			dory_window_slot_add_extra_location_widget (slot, bar);
...
static void
dory_window_slot_show_trash_bar (DoryWindowSlot *slot)
{
	GtkWidget *bar;
	DoryView *view;

	view = dory_window_slot_get_current_view (slot);
	bar = dory_trash_bar_new (view);
	gtk_widget_show (bar);
...
static void
maybe_show_interesting_folder_bar (DoryWindowSlot *slot)
{
    GtkWidget *bar = dory_interesting_folder_bar_new_for_location (dory_window_slot_get_current_view(slot),
                                                                   slot->location);
		       ...
		       gpointer user_data)
{
	...
	if (x_content_types != NULL && x_content_types[0] != NULL) {
		dory_window_slot_show_x_content_bar (slot, data->mount, x_content_types);
	}
		...
		gpointer user_data)
{
	...
	if (mount != NULL) {
		...
		if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_MEDIA_HANDLING_DETECT_CONTENT)) {
...
/* Handle the changes for the DoryWindow itself. */
static void
update_for_new_location (DoryWindowSlot *slot)
{
	DoryWindow *window;
        GFile *new_location;
        DoryFile *file;
	...
	dory_window_slot_emit_location_change (slot, slot->location, new_location);

        /* Set the new location. */
	g_clear_object (&slot->location);
	slot->location = new_location;

        /* Create a DoryFile for this location, so we can catch it
         * if it goes away.
         */
	cancel_viewed_file_changed_callback (slot);
	file = dory_file_get (slot->location);
	dory_window_slot_set_viewed_file (slot, file);
	slot->viewed_file_seen = !dory_file_is_not_yet_confirmed (file);
	slot->viewed_file_in_trash = dory_file_is_in_trash (file);
	dory_file_monitor_add (file, &slot->viewed_file, 0);
	...
	if (slot == dory_window_get_active_slot (window)) {
		...
		dory_window_pane_sync_up_actions (slot->pane);
        ...
        dory_window_sync_create_folder_button (window);

		/* Load menus from dory extensions for this location */
		dory_window_load_extension_menus (window);
	}

	if (location_really_changed) {
        dory_window_clear_secondary_pane_location (window);
		dory_window_slot_remove_extra_location_widgets (slot);

		directory = dory_directory_get (slot->location);

		if (dory_directory_is_in_trash (directory)) {
			dory_window_slot_show_trash_bar (slot);
		}

        maybe_show_interesting_folder_bar (slot);

        dory_window_slot_check_bad_cache_bar (slot);
    ...
    dory_window_sync_menu_bar (window);
}

/* A location load previously announced by load_underway
 * has been finished */
void
dory_window_report_load_complete (DoryWindow *window,
				      DoryView *view)
{
	DoryWindowSlot *slot;
	...
	if (view == slot->content_view) {
		if (slot->pending_scroll_to != NULL) {
			dory_view_scroll_to_file (slot->content_view,
...
static void
remove_loading_floating_bar (DoryWindowSlot *slot)
{
	if (slot->loading_timeout_id != 0) {
		g_source_remove (slot->loading_timeout_id);
		slot->loading_timeout_id = 0;
...
static void
end_location_change (DoryWindowSlot *slot)
{
	char *uri;
	...
	dory_window_slot_set_allow_stop (slot, FALSE);
...
static void
free_location_change (DoryWindowSlot *slot)
{
	DoryWindow *window;
	...
	if (slot->mount_cancellable != NULL) {
		...
		dory_view_stop_loading (slot->new_content_view);
		window->details->temporarily_ignore_view_signals = FALSE;

		dory_window_disconnect_content_view (window, slot->new_content_view);
        	g_object_unref (slot->new_content_view);
                slot->new_content_view = NULL;
        }
}
...
static void
cancel_location_change (DoryWindowSlot *slot)
{
	GList *selection;
...
static void
display_view_selection_failure (DoryWindow *window, DoryFile *file,
	...
	if (error == NULL) {
		if (dory_file_is_directory (file)) {
			...
			detail_message = g_strdup
				(_("Dory has no installed viewer capable of displaying the folder."));
		} else {
			error_message = g_strdup_printf
				(_("Could not display \"%s\"."),
				 uri_for_display);
		...
		case G_IO_ERROR_NOT_SUPPORTED:
			...
			if (scheme_string != NULL) {
				detail_message = g_strdup_printf (_("Dory cannot handle \"%s\" locations."),
								  scheme_string);
			} else {
				detail_message = g_strdup (_("Dory cannot handle this kind of location."));
			}
...
void
dory_window_slot_stop_loading (DoryWindowSlot *slot)
{
	DoryWindow *window;

	window = dory_window_slot_get_window (slot);

	dory_view_stop_loading (slot->content_view);
...
void
dory_window_slot_set_content_view (DoryWindowSlot *slot,
				       const char *id)
{
	DoryFile *file;
	char *uri;
	...
	if (dory_window_slot_content_view_matches_iid (slot, id)) {
        	return;
        }
    ...
    if (dory_global_preferences_get_ignore_view_metadata ()) {
        dory_window_set_ignore_meta_view_id (dory_window_slot_get_window (slot), id);
    } else {
        dory_file_set_metadata (file, DORY_METADATA_KEY_DEFAULT_VIEW, NULL, id);
    }
    ...
    if (dory_view_get_selection_count (slot->content_view) == 0) {
            /* If there is no selection, queue a scroll to the same icon that
             * is currently visible */
            slot->pending_scroll_to = dory_view_get_first_visible_file (slot->content_view);
    }
	slot->location_change_type = DORY_LOCATION_CHANGE_RELOAD;

        create_content_view (slot, id);
}

void
dory_window_manage_views_close_slot (DoryWindowSlot *slot)
{
	if (slot->content_view != NULL) {
		dory_window_disconnect_content_view (dory_window_slot_get_window (slot),
							 slot->content_view);
	}
...
void
dory_window_back_or_forward (DoryWindow *window,
				 gboolean back,
				 guint distance,
				 DoryWindowOpenFlags flags)
{
	...
	location = dory_bookmark_get_location (bookmark);
	...
	if (flags != 0) {
		...
		old_location = dory_window_slot_get_location (slot);
		scroll_pos = dory_bookmark_get_scroll_pos (bookmark);
		begin_location_change
			(slot,
			 location, old_location, NULL,
			 back ? DORY_LOCATION_CHANGE_BACK : DORY_LOCATION_CHANGE_FORWARD,
			 distance,
			 scroll_pos,
             FALSE,
			 NULL, NULL);
...
void
dory_window_slot_force_reload (DoryWindowSlot *slot)
{
	GFile *location;
        char *current_pos;
	GList *selection;
	...
	begin_location_change
		(slot, location, location, selection,
		 DORY_LOCATION_CHANGE_RELOAD, 0, current_pos,
		 FALSE, NULL, NULL);
        g_free (current_pos);
	g_object_unref (location);
	g_list_free_full (selection, g_object_unref);
...
static void
clear_thumbnails_for_view (DoryView *view)
{
    DoryDirectory *directory;
    GList *file_list, *l;
    ...
    for (l = file_list; l != NULL; l = l->next) {
        DoryFile *file = DORY_FILE (l->data);

        dory_file_delete_thumbnail (file);
    }
...
void
dory_window_slot_queue_reload (DoryWindowSlot *slot,
                               gboolean        clear_thumbs)
{
	...
	if (slot->pending_location != NULL
	    || slot->content_view == NULL
	    || dory_view_get_loading (slot->content_view)) {
		/* there is a reload in flight */
		slot->needs_reload = TRUE;
		return;
	}

    if (clear_thumbs && !dory_file_is_in_favorites (slot->viewed_file)) {
...
void
dory_window_slot_check_bad_cache_bar (DoryWindowSlot *slot)
{
    int show_image_thumbs;
    if (DORY_IS_DESKTOP_WINDOW (dory_window_slot_get_window (slot)))
        return;
    ...
    if (show_image_thumbs != DORY_SPEED_TRADEOFF_NEVER &&
        dory_application_get_cache_bad (dory_application_get_singleton ()) &&
        !dory_application_get_cache_problem_ignored (dory_application_get_singleton ())) {
        if (slot->cache_bar != NULL) {
            gtk_widget_show (slot->cache_bar);
        } else {
            GtkWidget *bad_bar = dory_thumbnail_problem_bar_new (dory_window_slot_get_current_view (slot));
            if (bad_bar) {
                gtk_widget_show (bad_bar);
                dory_window_slot_add_extra_location_widget (slot, bad_bar);
