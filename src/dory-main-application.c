...
/*
 * dory-main-application: Dory application subclass for standard windows
 *
...
#include "dory-main-application.h"
...
#include <libdory-private/dory-dbus-manager.h>
#include <libdory-private/dory-directory-private.h>
...
#include <libdory-private/dory-lib-self-check-functions.h>
...
#include <libdory-private/dory-search-engine-advanced.h>
...
#define DORY_ACCEL_MAP_SAVE_DELAY 30

/* Disable the self-check functionality */
#define DORY_OMIT_SELF_CHECK "omit"

#define DORY_NOTIFICATION_UNMOUNT_ICON_NAME  "media-removable"
#define DORY_NOTIFICATION_UNMOUNT_ID_PENDING "unmount-pending"
#define DORY_NOTIFICATION_UNMOUNT_ID_DONE    "unmount-done"

static void     mount_removed_callback            (GVolumeMonitor            *monitor,
						   GMount                    *mount,
						   DoryMainApplication       *application);
static void     mount_added_callback              (GVolumeMonitor            *monitor,
						   GMount                    *mount,
						   DoryMainApplication       *application);

G_DEFINE_TYPE (DoryMainApplication, dory_main_application, DORY_TYPE_APPLICATION);

struct _DoryMainApplicationPriv {
	GVolumeMonitor *volume_monitor;

	DoryDBusManager *dbus_manager;
	DoryFreedesktopDBus *fdb_manager;
	DoryFileChooserDBus *file_chooser_dbus;

	gchar *geometry;
};

static void
dory_main_application_send_notification (DoryApplication *application,
                                         const gchar *title,
                                         const gchar *body,
...
{
	DoryMainApplication *app = DORY_MAIN_APPLICATION (application);
...
static void
dory_main_application_notify_unmount_done (DoryApplication *application,
                                           const gchar     *message)
{
	DoryMainApplication *app = DORY_MAIN_APPLICATION (application);
	gchar **strings;
	...
	g_application_withdraw_notification (G_APPLICATION (app), DORY_NOTIFICATION_UNMOUNT_ID_PENDING);

	g_return_if_fail (message != NULL);
	strings = g_strsplit (message, "\n", 2);

	dory_main_application_send_notification (application, strings[0], strings[1],
	                                         DORY_NOTIFICATION_UNMOUNT_ICON_NAME,
	                                         DORY_NOTIFICATION_UNMOUNT_ID_DONE,
...
static void
dory_main_application_notify_unmount_show (DoryApplication *application,
                                           const gchar     *message)
{
	gchar **strings;
	...
	dory_main_application_send_notification (application, strings[0], strings[1],
	                                         DORY_NOTIFICATION_UNMOUNT_ICON_NAME,
	                                         DORY_NOTIFICATION_UNMOUNT_ID_PENDING,
	                                         G_NOTIFICATION_PRIORITY_URGENT);
	g_strfreev (strings);
}

static void
dory_main_application_close_all_windows (DoryApplication *self)
{
	GList *list_copy;
	GList *l;
	...
	for (l = list_copy; l != NULL; l = l->next) {
		if (DORY_IS_WINDOW (l->data)) {
			DoryWindow *window;

			window = DORY_WINDOW (l->data);
...
static DoryWindow *
dory_main_application_create_window (DoryApplication *application,
                                     GdkScreen       *screen)
{
	DoryWindow *window;
	...
	g_return_val_if_fail (DORY_IS_APPLICATION (application), NULL);

    window = dory_window_new (GTK_APPLICATION (application), screen);
    ...
    geometry_string = g_settings_get_string (dory_window_state, DORY_WINDOW_STATE_GEOMETRY);

    if (DORY_MAIN_APPLICATION (application)->priv->geometry == NULL && 
		...
		eel_gtk_window_set_initial_geometry_from_string 
			(GTK_WINDOW (window), 
			 geometry_string,
			 DORY_WINDOW_MIN_WIDTH,
			 DORY_WINDOW_MIN_HEIGHT,
			 TRUE);
	}

	g_free (geometry_string);

    dory_undo_manager_attach (application->undo_manager, G_OBJECT (window));
		      ...
		      DoryMainApplication *application)
{
	...
	directory = dory_directory_get_existing (root);
	g_object_unref (root);
	if (directory != NULL) {
			...
			DoryMainApplication *application)
{
	...
	DoryWindow *window;
	DoryWindowSlot *slot;
	DoryWindowSlot *force_no_close_slot;
	GFile *root, *computer;
	gchar *uri;
	guint n_slots;
	...
	for (node = window_list; node != NULL; node = node->next) {
		...
		if (window != NULL) {
			...
			for (lp = window->details->panes; lp != NULL; lp = lp->next) {
				DoryWindowPane *pane;
				pane = (DoryWindowPane*) lp->data;
				for (l = pane->slots; l != NULL; l = l->next) {
					slot = l->data;
					n_slots++;
					if (dory_window_slot_should_close_with_mount (slot, mount)) {
	...
	for (node = close_list; node != NULL; node = node->next) {
		...
		if (slot != force_no_close_slot) {
            if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_CLOSE_DEVICE_VIEW_ON_EJECT))
                dory_window_pane_close_slot (slot->pane, slot);
            else
                dory_window_slot_go_home (slot, FALSE);
		} else {
			computer = g_file_new_for_path (g_get_home_dir ());
			dory_window_slot_open_location (slot, computer, 0);
...
static void
open_window (DoryMainApplication *application,
	     GFile *location, GdkScreen *screen, const char *geometry)
{
	...
	window = dory_main_application_create_window (DORY_APPLICATION (application),
						     screen);
...
static void
open_tabs (DoryMainApplication *application,
         GFile **locations,
         guint n_files,
         GdkScreen *screen,
         const char *geometry)
{
    ...
    dory_window_go_to (window, locations[0]);
    g_free (uri);
    for (int i = 1; i < n_files; i++) {
        ...
        g_debug ("Opening new tab at uri %s\n", uri);
        dory_window_go_to_tab (window, locations[tab]);
        g_free (uri);
    }
...
static void
open_tabs_in_existing_window (DoryMainApplication *application,
                              GFile **locations,
                              guint n_files,
                              GdkScreen *screen,
                              const char *geometry)
{
    ...
    for (l = list_copy; l != NULL; l = l->next) {
        if (DORY_IS_WINDOW (l->data)) {
            ...
            dory_notebook_set_current_page_relative (DORY_NOTEBOOK (pane->notebook), n_files);
...
static void
open_windows (DoryMainApplication *application,
	      GFile **files,
	      gint n_files,
	      GdkScreen *screen,
	      const char *geometry,
	      ...
	      gboolean open_in_existing_window)
{
	...
	if (files == NULL || files[0] == NULL) {
		...
		window = dory_main_application_create_window (DORY_APPLICATION (application), screen);
		...
		do_restore = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_RESTORE_TABS_ON_STARTUP);

		if (!do_restore || !dory_window_restore_saved_tabs (window)) {
			/* Fall back to a safe default location */
			GFile *home = g_file_new_for_path (g_get_home_dir ());
			dory_window_go_to (window, home);
			g_object_unref (home);
		}
	} else {
...
static void
dory_main_application_open_location (DoryApplication     *application,
                                     GFile               *location,
                                     GFile               *selection,
                                     ...
                                     const gboolean      open_in_tabs)
{
	...
	window = dory_main_application_create_window (application, gdk_screen_get_default ());
	gtk_window_set_startup_id (GTK_WINDOW (window), startup_id);

	if (selection != NULL) {
		sel_list = g_list_prepend (sel_list, dory_file_get (selection));
	}

	if(open_in_tabs){
		dory_window_slot_open_location_full (dory_window_get_active_slot (window), location,
						 DORY_WINDOW_OPEN_FLAG_NEW_TAB, sel_list, NULL, NULL);
	} else {
		dory_window_slot_open_location_full (dory_window_get_active_slot (window), location,
	...
	if (sel_list != NULL) {
		dory_file_list_free (sel_list);
	}
}

static DoryWindowSlot *
find_existing_slot_for_location (DoryApplication *application,
                                 GFile           *location)
{
	GList *windows;
	GList *l;
	...
	for (l = windows; l != NULL; l = l->next) {
		...
		for (p = window->details->panes; p != NULL; p = p->next) {
			DoryWindowPane *pane = p->data;
			GList *s;

			for (s = pane->slots; s != NULL; s = s->next) {
				DoryWindowSlot *slot = s->data;
				GFile *slot_location;
				gboolean match;

				slot_location = dory_window_slot_get_location (slot);
...
static void
present_window (DoryWindow *window)
{
	if (eel_check_is_wayland ()) {
		gtk_window_present (GTK_WINDOW (window));
	} else {
...
static void
dory_main_application_show_items (DoryApplication *application,
                                  GFile          **uris,
                                  gint             n_uris,
                                  const char      *startup_id)
{
	...
	while (g_hash_table_iter_next (&iter, &key, &value)) {
		...
		if (parent != NULL) {
			for (j = 0; j < group->len; j++) {
				sel_list = g_list_prepend (sel_list,
				                           dory_file_get ((GFile *) g_ptr_array_index (group, j)));
			}
		...
		if (existing_slot != NULL) {
			DoryWindow *window = dory_window_slot_get_window (existing_slot);
			DoryView *view;

			dory_window_slot_make_hosting_pane_active (existing_slot);

			if (sel_list != NULL) {
				view = dory_window_slot_get_current_view (existing_slot);
				if (view != NULL) {
					dory_view_set_selection (view, sel_list);
				}
			}
			...
			dory_window_slot_open_location_full (dory_window_get_active_slot (window), target,
			                                     0, sel_list, NULL, NULL);
		}
...
static void
dory_main_application_open (GApplication *app,
                            GFile       **files,
                            gint          n_files,
                            const gchar  *options)
{
	DoryMainApplication *self = DORY_MAIN_APPLICATION (app);
	...
	if (select_mode) {
		dory_application_show_items (DORY_APPLICATION (app), files, n_files, NULL);
...
static void
dory_main_application_init (DoryMainApplication *application)
{
    application->priv = G_TYPE_INSTANCE_GET_PRIVATE (application,
                                                     DORY_TYPE_MAIN_APPLICATION,
                                                     DoryMainApplicationPriv);
}

static void
dory_main_application_finalize (GObject *object)
{
    DoryMainApplication *application;

    application = DORY_MAIN_APPLICATION (object);

    dory_bookmarks_exiting ();
    ...
    G_OBJECT_CLASS (dory_main_application_parent_class)->finalize (object);
}

static gboolean
do_cmdline_sanity_checks (DoryMainApplication *self,
...
{
#ifndef DORY_OMIT_SELF_CHECK
	/* Run the checks (each twice) for dory and libdory-private. */

	dory_run_self_checks ();
	dory_run_lib_self_checks ();
...
static gboolean
dory_main_application_local_command_line (GApplication *application,
					 gchar ***arguments,
					 gint *exit_status)
{
	...
	DoryMainApplication *self = DORY_MAIN_APPLICATION (application);
	...
	const GOptionEntry options[] = {
		...
		{ "geometry", 'g', 0, G_OPTION_ARG_STRING, &self->priv->geometry,
		  N_("Create the initial window with the given geometry. "
             "Examples: dory --geometry=+100+100, dory --geometry=600x400, dory --geometry=600x400+100+100."), N_("GEOMETRY") },
        ...
        { "debug", 0, 0, G_OPTION_ARG_NONE, &debug,
          "Enable debugging code.  Example usage: 'DORY_DEBUG=Actions,Window dory --debug'.  Use DORY_DEBUG=help for more topics.", NULL },
	...
	*exit_status = EXIT_SUCCESS;
	...
	if (error != NULL) {
        g_debug ("Could not register dory as a service, trying as a remote: %s", error->message);
        g_clear_error (&error);
    } else {
    ...
    if (error != NULL) {
        g_printerr ("Could not register dory as a remote: %s\n", error->message);
        g_clear_error (&error);
...
static void
menu_state_changed_callback (DoryMainApplication *self)
{
    if (!g_settings_get_boolean (dory_window_state, DORY_WINDOW_STATE_START_WITH_MENU_BAR) &&
        !g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_DISABLE_MENU_WARNING)) {
        ...
        dialog = gtk_message_dialog_new (NULL,
                                         ...
                                         GTK_MESSAGE_INFO,
                                         GTK_BUTTONS_OK,
                                         _("Dory's main menu is now hidden"));
        ...
        g_settings_bind (dory_preferences,
                         DORY_PREFERENCES_DISABLE_MENU_WARNING,
                         checkbox,
                         "active",
...
static void
dory_main_application_continue_startup (DoryApplication *app)
{
	DoryMainApplication *self = DORY_MAIN_APPLICATION (app);

	/* create DBus manager */
	self->priv->dbus_manager = dory_dbus_manager_new ();
	self->priv->fdb_manager = dory_freedesktop_dbus_new ();
	self->priv->file_chooser_dbus = dory_file_chooser_dbus_new ();

    /* Check the user's ~/.config/dory directory and post warnings
	...
	dory_list_view_register ();
	dory_icon_view_compact_register ();
#if defined(ENABLE_EMPTY_VIEW) && ENABLE_EMPTY_VIEW
	dory_empty_view_register ();
#endif
    ...
    g_signal_connect_swapped (dory_window_state, "changed::" DORY_WINDOW_STATE_START_WITH_MENU_BAR,
...
static void
dory_main_application_quit_mainloop (GApplication *app)
{
    dory_main_application_notify_unmount_done (DORY_APPLICATION (app), NULL);

    G_APPLICATION_CLASS (dory_main_application_parent_class)->quit_mainloop (app);
}

static void
dory_main_application_class_init (DoryMainApplicationClass *class)
{
    ...
    object_class->finalize = dory_main_application_finalize;

    application_class = G_APPLICATION_CLASS (class);
    application_class->open = dory_main_application_open;
    application_class->local_command_line = dory_main_application_local_command_line;
    application_class->quit_mainloop = dory_main_application_quit_mainloop;

    dory_app_class = DORY_APPLICATION_CLASS (class);
    dory_app_class->open_location = dory_main_application_open_location;
    dory_app_class->show_items = dory_main_application_show_items;
    dory_app_class->create_window = dory_main_application_create_window;
    dory_app_class->notify_unmount_show = dory_main_application_notify_unmount_show;
    dory_app_class->notify_unmount_done = dory_main_application_notify_unmount_done;
    dory_app_class->close_all_windows = dory_main_application_close_all_windows;
    dory_app_class->continue_startup = dory_main_application_continue_startup;
    dory_app_class->continue_quit = dory_desktop_application_continue_quit;

    g_type_class_add_private (class, sizeof (DoryMainApplicationPriv));
}

DoryApplication *
dory_main_application_get_singleton (void)
{
    return dory_application_initialize_singleton (DORY_TYPE_MAIN_APPLICATION,
                                                  "application-id", "org.Dory",
