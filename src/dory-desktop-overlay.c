...
#include <libdory-extension/dory-desktop-preferences.h>
...
typedef struct
{
    ...
    gboolean syncing;
} DoryDesktopOverlayPrivate;

struct _DoryDesktopOverlay
{
    GObject parent_object;

    DoryDesktopOverlayPrivate *priv;
};
...
G_DEFINE_TYPE_WITH_PRIVATE (DoryDesktopOverlay, dory_desktop_overlay, G_TYPE_OBJECT)

static void
show_view_page (DoryDesktopOverlay *overlay)
{
    DoryDesktopOverlayPrivate *priv = dory_desktop_overlay_get_instance_private (overlay);
    GdkScreen *screen;
    GdkRectangle rect;
    gchar *plug_name;
...
static void
sync_controls (DoryDesktopOverlay *overlay,
               gboolean same_monitor)
{
    ...
    if (!dory_desktop_manager_get_monitor_is_active (priv->manager, priv->monitor)) {
        gtk_stack_set_visible_child_name (priv->view_substack, "substack_disabled");

        priv->dory_window = NULL;
        ...
        priv->dory_window = dory_desktop_manager_get_window_for_monitor (dory_desktop_manager_get (),
                                                                         priv->monitor);

        dory_desktop_manager_get_overlay_info (priv->manager,
                                               priv->monitor,
                                               &action_group,
                                               &h_adjust,
                                               &v_adjust);
    ...
    /* Actual handling is done in DoryDesktopIconGridView */
...
static void
signal_adjust_changed (DoryDesktopOverlay *overlay)
{
    DoryDesktopOverlayPrivate *priv = overlay->priv;
    double h_val, v_val;
...
{
    DoryDesktopOverlay *overlay = DORY_DESKTOP_OVERLAY (user_data);
    DoryDesktopOverlayPrivate *priv = dory_desktop_overlay_get_instance_private (overlay);
...
static void
queue_changed_signal (DoryDesktopOverlay *overlay)
{
    DoryDesktopOverlayPrivate *priv = dory_desktop_overlay_get_instance_private (overlay);
...
on_close_window (GtkWidget *overlay_window,
                 ...
                 gpointer   user_data)
{
    g_return_val_if_fail (DORY_IS_DESKTOP_OVERLAY (user_data), GDK_EVENT_PROPAGATE);

    /* When the window is destroyed, kill the overlay instance also.
     * This will end up clearing the weak pointer made in dory-desktop-icon-grid-view.c. */
...
static void
dory_desktop_overlay_init (DoryDesktopOverlay *overlay)
{
    DoryDesktopOverlayPrivate *priv = dory_desktop_overlay_get_instance_private (overlay);
    GtkWindow *window;
    GtkWidget *widget;
    ...
    overlay->priv = priv;
    priv->manager = dory_desktop_manager_get ();
    priv->is_cinnamon = dory_desktop_manager_get_is_cinnamon (priv->manager);
    priv->syncing = FALSE;

    priv->builder = gtk_builder_new ();
    gtk_builder_set_translation_domain (priv->builder, GETTEXT_PACKAGE);
    gtk_builder_add_from_resource (priv->builder, "/org/dory/dory-desktop-overlay.glade", NULL);
    ...
    if (!priv->is_cinnamon) {
        widget = GTK_WIDGET (dory_desktop_preferences_new ());
...
static void
show_overlay (DoryDesktopOverlay *overlay,
              gint                monitor)
{
    DoryDesktopOverlayPrivate *priv = dory_desktop_overlay_get_instance_private (overlay);
    ...
    priv->monitor = monitor;
    priv->dory_window = dory_desktop_manager_get_window_for_monitor (priv->manager,
                                                                     monitor);
    ...
    if (old_monitor != monitor) {
        ...
        gtk_window_get_position (GTK_WINDOW (priv->dory_window),
                                 &root_x,
                                 &root_y);

        gtk_window_get_size (GTK_WINDOW (priv->dory_window),
                             &parent_width,
                             &parent_height);
...
static void
dory_desktop_overlay_dispose (GObject *object)
{
    DoryDesktopOverlay *overlay = DORY_DESKTOP_OVERLAY (object);
    ...
    G_OBJECT_CLASS (dory_desktop_overlay_parent_class)->dispose (object);
}

static void
dory_desktop_overlay_class_init (DoryDesktopOverlayClass *klass)
{
    signals[ADJUSTS_CHANGED] =
        g_signal_new ("adjusts-changed",
                      ...
                      NULL, NULL, NULL,
                      G_TYPE_NONE, 3,
                      DORY_TYPE_WINDOW, G_TYPE_INT, G_TYPE_INT);

    G_OBJECT_CLASS (klass)->dispose = dory_desktop_overlay_dispose;
}

DoryDesktopOverlay *
dory_desktop_overlay_new (void)
{
    return g_object_new (DORY_TYPE_DESKTOP_OVERLAY, NULL);
}

void
dory_desktop_overlay_show (DoryDesktopOverlay *overlay,
                           gint                monitor)
{
    g_return_if_fail (DORY_IS_DESKTOP_OVERLAY (overlay));
    show_overlay (overlay, monitor);
}

void
dory_desktop_overlay_update_in_place (DoryDesktopOverlay *overlay)
{
    g_return_if_fail (DORY_IS_DESKTOP_OVERLAY (overlay));
