...
#include "dory-desktop-item-properties.h"
...
#include <libdory-private/dory-undo-signal-handlers.h>
...
struct DoryPropertiesWindowDetails {
...
#define DORY_RESPONSE_REVERT 10
...
static gboolean is_computer_directory             (DoryFile *file);
static gboolean is_network_directory              (DoryFile *file);
static gboolean is_burn_directory                 (DoryFile *file);
static void directory_contents_value_field_update (DoryPropertiesWindow *window);
static void file_changed_callback                 (DoryFile       *file,
						   gpointer            user_data);
static void permission_button_update              (DoryPropertiesWindow *window,
						   GtkToggleButton    *button);
static void permission_combo_update               (DoryPropertiesWindow *window,
						   GtkComboBox        *combo);
static void value_field_update                    (DoryPropertiesWindow *window,
						   GtkLabel           *field);
static void properties_window_update              (DoryPropertiesWindow *window,
						   GList              *files);
static void size_field_update                     (DoryPropertiesWindow *window);
static void is_directory_ready_callback           (DoryFile       *file,
						   gpointer            data);
static void cancel_group_change_callback          (DoryPropertiesWindow *window);
static void cancel_owner_change_callback          (DoryPropertiesWindow *window);
static void parent_widget_destroyed_callback      (GtkWidget          *widget,
						   gpointer            callback_data);
static void select_image_button_callback          (GtkWidget          *widget,
						   DoryPropertiesWindow *properties_window);
...
static void append_extension_pages                (DoryPropertiesWindow *window);

static gboolean name_field_focus_out              (DoryEntry *name_field,
						   GdkEventFocus *event,
						   gpointer callback_data);
static void name_field_activate                   (DoryEntry *name_field,
						   gpointer callback_data);
...
static GtkWidget* create_pie_widget 		  (DoryPropertiesWindow *window);

G_DEFINE_TYPE (DoryPropertiesWindow, dory_properties_window, GTK_TYPE_DIALOG);

static gboolean
is_multi_file_window (DoryPropertiesWindow *window)
{
	GList *l;
	guint count;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		if (!dory_file_is_gone (DORY_FILE (l->data))) {
			count++;
			if (count > 1) {
				return TRUE;
			}
...
static int
get_not_gone_original_file_count (DoryPropertiesWindow *window)
{
	GList *l;
	int count;
...
static DoryFile *
get_original_file (DoryPropertiesWindow *window)
{
	...
	return DORY_FILE (window->details->original_files->data);
}

static DoryFile *
get_target_file_for_original_file (DoryFile *file)
{
	...
	if (DORY_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			...
			if (location) {
				target_file = dory_file_get (location);
		...
		uri_to_display = dory_file_get_activation_uri (file);
		if (uri_to_display != NULL) {
			target_file = dory_file_get_by_uri (uri_to_display);
...
static DoryFile *
get_target_file (DoryPropertiesWindow *window)
{
	return DORY_FILE (window->details->target_files->data);
}
...
static void
get_image_for_properties_window (DoryPropertiesWindow *window,
				 char **icon_name,
				 GdkPixbuf **icon_pixbuf)
{
	DoryIconInfo *icon, *new_icon;
	GList *l;
	gint icon_scale;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		...
		if (!icon) {
			icon = dory_file_get_icon (file, DORY_ICON_SIZE_STANDARD, 0, icon_scale,
                                       DORY_FILE_ICON_FLAGS_USE_THUMBNAILS |
                                       DORY_FILE_ICON_FLAGS_IGNORE_VISITING);
		} else {
			new_icon = dory_file_get_icon (file, DORY_ICON_SIZE_STANDARD, 0, icon_scale,
                                           DORY_FILE_ICON_FLAGS_USE_THUMBNAILS |
                                           DORY_FILE_ICON_FLAGS_IGNORE_VISITING);
			if (!new_icon || new_icon != icon) {
				dory_icon_info_unref (icon);
				dory_icon_info_unref (new_icon);
				icon = NULL;
				break;
			}
			dory_icon_info_unref (new_icon);
	...
	if (!icon) {
		icon = dory_icon_info_lookup_from_name ("text-x-generic", DORY_ICON_SIZE_STANDARD, icon_scale);
	}

	if (icon_name != NULL) {
		*icon_name = g_strdup (dory_icon_info_get_used_name (icon));
	}

	if (icon_pixbuf != NULL) {
		*icon_pixbuf = dory_icon_info_get_pixbuf_at_size (icon, DORY_ICON_SIZE_STANDARD);
	}
...
static void
update_properties_window_icon (DoryPropertiesWindow *window)
{
	GdkPixbuf *pixbuf;
...
static void
reset_icon (DoryPropertiesWindow *properties_window)
{
	GList *l;
	...
	for (l = properties_window->details->original_files; l != NULL; l = l->next) {
		...
		dory_file_set_metadata (file,
					    DORY_METADATA_KEY_ICON_SCALE,
					    NULL, NULL);
		dory_file_set_metadata (file,
					    DORY_METADATA_KEY_CUSTOM_ICON,
					    NULL, NULL);
        dory_file_set_metadata (file,
                        DORY_METADATA_KEY_CUSTOM_ICON_NAME,
                        NULL, NULL);
	}
}
...
static void
dory_properties_window_drag_data_received (GtkWidget *widget, GdkDragContext *context,
					       int x, int y,
					       ...
					       guint info, guint time)
{
	...
	if (!exactly_one) {
		...
		if (uri_is_local_image (uris[0])) {
			set_icon (uris[0], DORY_PROPERTIES_WINDOW (window));
		} else {
			GFile *f;
...
static GtkWidget *
create_image_widget (DoryPropertiesWindow *window,
		     gboolean is_customizable)
{
	...
	if (is_customizable) {
		...
		g_signal_connect (image, "drag_data_received",
				  G_CALLBACK (dory_properties_window_drag_data_received), NULL);
...
static void
set_name_field (DoryPropertiesWindow *window,
		...
		const gchar *name)
{
	 ...
	 */
	use_label = is_multi_file_window (window) || !dory_file_can_rename (get_original_file (window));
	new_widget = !window->details->name_field || (use_label ? DORY_IS_ENTRY (window->details->name_field) : GTK_IS_LABEL (window->details->name_field));
	...
	if (new_widget) {
		...
		if (use_label) {
			...
			window->details->name_field = dory_entry_new ();
			 ...
			 * we've removed Undo from Dory for now, I'm just ifdeffing out this
			...
			dory_undo_set_up_dory_entry_for_undo ( DORY_ENTRY (window->details->name_field));
			dory_undo_editable_set_undo_key (GTK_EDITABLE (window->details->name_field), TRUE);
#endif
...
static void
update_name_field (DoryPropertiesWindow *window)
{
	DoryFile *file;
	...
	if (is_multi_file_window (window)) {
		...
		if (file == NULL || dory_file_is_gone (file)) {
			...
			current_name = dory_file_get_display_name (file);
		}
...
static void
name_field_restore_original_name (DoryEntry *name_field)
{
	...
	dory_entry_select_all (name_field);

	g_free (displayed_name);
}

static void
rename_callback (DoryFile *file, GFile *res_loc, GError *error, gpointer callback_data)
{
	DoryPropertiesWindow *window;

	window = DORY_PROPERTIES_WINDOW (callback_data);

	/* Complain to user if rename failed. */
	if (error != NULL) {
		dory_report_error_renaming_file (file,
						     window->details->pending_name,
						     error,
						     GTK_WINDOW (window));
		if (window->details->name_field != NULL) {
			name_field_restore_original_name (DORY_ENTRY (window->details->name_field));
		}
	}
...
static void
set_pending_name (DoryPropertiesWindow *window, const char *name)
{
...
static void
name_field_done_editing (DoryEntry *name_field, DoryPropertiesWindow *window)
{
	DoryFile *file;
	char *new_name;
	const char *original_name;

	g_return_if_fail (DORY_IS_ENTRY (name_field));
	...
	if (strlen (new_name) == 0) {
		name_field_restore_original_name (DORY_ENTRY (name_field));
	} else {
		...
		if (strcmp (new_name, original_name) != 0) {
			...
			g_object_ref (window);
			dory_file_rename (file, new_name,
...
static gboolean
name_field_focus_out (DoryEntry *name_field,
		      GdkEventFocus *event,
		      gpointer callback_data)
{
	g_assert (DORY_IS_PROPERTIES_WINDOW (callback_data));

	if (gtk_widget_get_sensitive (GTK_WIDGET (name_field))) {
		name_field_done_editing (name_field, DORY_PROPERTIES_WINDOW (callback_data));
	}
...
static void
name_field_activate (DoryEntry *name_field, gpointer callback_data)
{
	g_assert (DORY_IS_ENTRY (name_field));
	g_assert (DORY_IS_PROPERTIES_WINDOW (callback_data));

	/* Accept changes. */
	name_field_done_editing (name_field, DORY_PROPERTIES_WINDOW (callback_data));

	dory_entry_select_all_at_idle (name_field);
}

static void
update_properties_window_title (DoryPropertiesWindow *window)
{
	char *name, *title;
	DoryFile *file;
...
static void
clear_extension_pages (DoryPropertiesWindow *window)
{
...
static void
refresh_extension_pages (DoryPropertiesWindow *window)
{
...
static void
remove_from_dialog (DoryPropertiesWindow *window,
		    DoryFile *file)
{
	int index;
	GList *original_link;
	...
	original_file = DORY_FILE (original_link->data);
	target_file = DORY_FILE (target_link->data);
	...
	dory_file_monitor_remove (original_file, &window->details->original_files);
	dory_file_monitor_remove (target_file, &window->details->target_files);
...
static GList *
get_mime_list (DoryPropertiesWindow *window)
{
	GList *ret;
	GList *l;
	...
	ret = NULL;
	for (l = window->details->target_files; l != NULL; l = l->next) {
		ret = g_list_append (ret, dory_file_get_mime_type (DORY_FILE (l->data)));
	}
...
static void
properties_window_update (DoryPropertiesWindow *window,
			  GList *files)
{
	GList *l;
	GList *mime_list;
	GList *tmp;
	DoryFile *changed_file;
	...
	for (tmp = files; tmp != NULL; tmp = tmp->next) {
		changed_file = DORY_FILE (tmp->data);

		if (changed_file && dory_file_is_gone (changed_file)) {
...
update_files_callback (gpointer data)
{
 	...
 	window = DORY_PROPERTIES_WINDOW (data);
	...
	if (window->details->original_files == NULL) {
		...
		dory_file_list_free (window->details->changed_files);
...
static void
schedule_files_update (DoryPropertiesWindow *window)
 {
 	g_assert (DORY_IS_PROPERTIES_WINDOW (window));
...
file_list_attributes_identical (GList *file_list, const char *attribute_name)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (first_attr == NULL) {
			first_attr = dory_file_get_string_attribute_with_default (file, attribute_name);
		} else {
			char *attr;
			attr = dory_file_get_string_attribute_with_default (file, attribute_name);
				...
				const char *inconsistent_value)
{
	if (file_list_attributes_identical (file_list, attribute_name)) {
		...
		for (l = file_list; l != NULL; l = l->next) {
			...
			file = DORY_FILE (l->data);
			if (!dory_file_is_gone (file)) {
				return dory_file_get_string_attribute_with_default
					(file,
					 attribute_name);
			}
		}
...
file_list_all_directories (GList *file_list)
{
	GList *l;
	for (l = file_list; l != NULL; l = l->next) {
		if (!dory_file_is_directory (DORY_FILE (l->data))) {
			return FALSE;
		}
	}
	return TRUE;
...
static void
value_field_update (DoryPropertiesWindow *window, GtkLabel *label)
{
...
static GtkWidget*
attach_value_field_ellipsizing (DoryPropertiesWindow *window,
				GtkGrid *grid,
				GtkWidget *sibling,
...
static GtkWidget*
attach_value_field (DoryPropertiesWindow *window,
		    GtkGrid *grid,
		    GtkWidget *sibling,
		    const char *file_attribute_name,
		    const char *inconsistent_string,
...
static void
group_change_callback (DoryFile *file,
		       GFile *res_loc,
		       GError *error,
		       DoryPropertiesWindow *window)
{
	...
	dory_report_error_setting_group (file, error, GTK_WINDOW (window));
...
static void
cancel_group_change_callback (DoryPropertiesWindow *window)
{
	DoryFile *file;
	char *group;
	...
	dory_file_cancel (file, (DoryFileOperationCallback) group_change_callback, window);
...
static gboolean
schedule_group_change_timeout (DoryPropertiesWindow *window)
{
	DoryFile *file;
	char *group;
	...
	dory_file_set_group
		(file,  group,
		 (DoryFileOperationCallback) group_change_callback, window);
...
static void
schedule_group_change (DoryPropertiesWindow *window,
		       DoryFile       *file,
		       const char         *group)
{
	g_assert (DORY_IS_PROPERTIES_WINDOW (window));
	...
	window->details->group_change_file = dory_file_ref (file);
...
static void
unschedule_or_cancel_group_change (DoryPropertiesWindow *window)
{
	DoryFile *file;
	char *group;
...
static void
changed_group_callback (GtkComboBox *combo_box, DoryFile *file)
{
	DoryPropertiesWindow *window;
	char *group;
	char *cur_group;
	...
	cur_group = dory_file_get_group_name (file);

	if (group != NULL && strcmp (group, cur_group) != 0) {
		/* Try to change file group. If this fails, complain to user. */
		window = DORY_PROPERTIES_WINDOW (gtk_widget_get_ancestor (GTK_WIDGET (combo_box), GTK_TYPE_WINDOW));
...
static void
synch_groups_combo_box (GtkComboBox *combo_box, DoryFile *file)
{
	GList *groups;
	GList *node;
	GtkTreeModel *model;
	...
	groups = dory_file_get_settable_group_names (file);
	...
	current_group_name = dory_file_get_group_name (file);
		        ...
		        DoryFile *file)
{
	...
	g_signal_connect_data (combo_box, "changed",
			       ...
			       dory_file_ref (file),
			       (GClosureNotify)dory_file_unref, 0);

	return combo_box;
}

static void
owner_change_callback (DoryFile *file,
		       ...
		       DoryPropertiesWindow *window)
{
	...
	dory_report_error_setting_owner (file, error, GTK_WINDOW (window));
...
static void
cancel_owner_change_callback (DoryPropertiesWindow *window)
{
	DoryFile *file;
	char *owner;
	...
	dory_file_cancel (file, (DoryFileOperationCallback) owner_change_callback, window);
...
static gboolean
schedule_owner_change_timeout (DoryPropertiesWindow *window)
{
	DoryFile *file;
	char *owner;
	...
	dory_file_set_owner
		(file,  owner,
		 (DoryFileOperationCallback) owner_change_callback, window);
...
static void
schedule_owner_change (DoryPropertiesWindow *window,
		       DoryFile       *file,
		       const char         *owner)
{
	g_assert (DORY_IS_PROPERTIES_WINDOW (window));
	...
	window->details->owner_change_file = dory_file_ref (file);
...
static void
unschedule_or_cancel_owner_change (DoryPropertiesWindow *window)
{
	DoryFile *file;
	char *owner;
...
static void
changed_owner_callback (GtkComboBox *combo_box, DoryFile* file)
{
	DoryPropertiesWindow *window;
	char *owner_text;
	char **name_array;
	...
	g_free (owner_text);
	cur_owner = dory_file_get_owner_name (file);
...
static void
synch_user_menu (GtkComboBox *combo_box, DoryFile *file)
{
	GList *users;
	GList *node;
	GtkTreeModel *model;
	...
	owner_name = dory_file_get_string_attribute (file, "owner");
		 ...
		 GList *prefix_candidates)
{
	...
	for (p = prefix_candidates; p != NULL; p = p->next) {
		...
		candidate_location = dory_file_get_location (DORY_FILE (p->data));
...
file_size_for_display (goffset size)
{
	...
	size_prefix = dory_global_preferences_get_size_prefix_preference ();
...
static void
directory_contents_value_field_update (DoryPropertiesWindow *window)
{
	DoryRequestStatus file_status, status;
	gchar *text, *temp;
	...
	for (l = window->details->target_files; l; l = l->next) {
		...
		if (dory_file_is_directory (file)) {
			file_status = dory_file_get_deep_counts (file,
					 &directory_count,
					 &file_count,
					 &file_unreadable,
                     &hidden_count,
			...
			if (file_status != DORY_REQUEST_DONE) {
				status = file_status;
			}
		} else {
			++total_count;
			total_size += dory_file_get_size (file);
		}
	}
...
static void
schedule_directory_contents_update (DoryPropertiesWindow *window)
{
	g_assert (DORY_IS_PROPERTIES_WINDOW (window));
...
static void
append_title_value_pair (DoryPropertiesWindow *window,
			 GtkGrid *grid,
			 const char *title,
 			 const char *file_attribute_name,
...
static void
append_title_and_ellipsizing_value (DoryPropertiesWindow *window,
				    GtkGrid *grid,
				    const char *title,
				    const char *file_attribute_name,
				    const char *inconsistent_state,
...
static void
append_directory_contents_fields (DoryPropertiesWindow *window,
				  GtkGrid *grid)
{
 ...
 */
static void
append_size_field (DoryPropertiesWindow *window,
		   GtkGrid *grid)
{
	GtkLabel *title_field, *value_field;
...
static gboolean
is_computer_directory (DoryFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_network_directory (DoryFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_burn_directory (DoryFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
should_show_custom_icon_buttons (DoryPropertiesWindow *window)
{
	if (is_multi_file_window (window)) {
		return FALSE;
	}
...
static gboolean
should_show_file_type (DoryPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && (dory_file_is_in_trash (get_target_file (window)) ||
...
static gboolean
should_show_location_info (DoryPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && (dory_file_is_in_trash (get_target_file (window)) ||
		is_computer_directory (get_target_file (window)) ||
...
static gboolean
should_show_accessed_date (DoryPropertiesWindow *window)
{
...
static gboolean
should_show_modified_date (DoryPropertiesWindow *window)
{
    return !is_multi_file_window (window);
}

static gboolean
should_show_link_target (DoryPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && dory_file_is_symbolic_link (get_target_file (window))) {
		return TRUE;
	}
...
static gboolean
location_show_original (DoryPropertiesWindow *window)
{
   DoryFile *file;

   /* there is no way a recent item will be mixed with
      other items so just pick the first file to check */
   file = DORY_FILE (g_list_nth_data (window->details->original_files, 0));
   return (file != NULL && !dory_file_is_in_recent (file));
}

static gboolean
should_show_free_space (DoryPropertiesWindow *window)
{
	...
	if (!is_multi_file_window (window)
		...
		is_computer_directory (get_target_file (window)) ||
		is_network_directory (get_target_file (window)) ||
        dory_file_is_in_recent (get_target_file (window)) ||
		is_burn_directory (get_target_file (window)))) {
		return FALSE;
	}
...
static gboolean
should_show_volume_usage (DoryPropertiesWindow *window)
{
	DoryFile 		*file;
...
static GtkWidget*
create_pie_widget (DoryPropertiesWindow *window)
{
	DoryFile		*file;
	GtkGrid                 *grid;
...
static GtkWidget*
create_volume_usage_widget (DoryPropertiesWindow *window)
{
	GtkWidget *piewidget;
	gchar *uri;
	DoryFile *file;
...
static void
create_basic_page (DoryPropertiesWindow *window)
{
	GtkGrid *grid;
	...
	hbox = create_page_with_hbox (window->details->stack, "basic", _("Basic"),
				      "help:gnome-help/dory-file-properties-basic");
	...
	if (DORY_IS_ENTRY (window->details->name_field)) {
		dory_entry_select_all (DORY_ENTRY (window->details->name_field));
		gtk_widget_grab_focus (GTK_WIDGET (window->details->name_field));
	}

	if (dory_desktop_item_properties_should_show (window->details->target_files)) {
		...
		gtk_size_group_add_widget (label_size_group,
					   GTK_WIDGET (window->details->name_label));
		box = dory_desktop_item_properties_make_box (label_size_group,
								 window->details->target_files);
	...
	if (is_multi_file_window (window) ||
	    dory_file_is_directory (get_target_file (window))) {
...
static gboolean
files_has_directory (DoryPropertiesWindow *window)
{
	GList *l;
...
static gboolean
files_has_changable_permissions_directory (DoryPropertiesWindow *window)
{
	GList *l;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (dory_file_is_directory (file) &&
		    ...
		    dory_file_can_set_permissions (file)) {
			return TRUE;
		}
...
static gboolean
files_has_file (DoryPropertiesWindow *window)
{
	GList *l;
...
static void
start_long_operation (DoryPropertiesWindow *window)
{
...
static void
end_long_operation (DoryPropertiesWindow *window)
{
...
static void
size_field_update (DoryPropertiesWindow *window)
{
	DoryFile *file;
	goffset size;
	 ...
	 */
	if (is_multi_file_window (window) || dory_file_is_directory (file)) {
		return;
	}

	size = dory_file_get_size (file);
...
static void
permission_change_callback (DoryFile *file,
			    GFile *res_loc,
			    GError *error,
			    gpointer callback_data)
{
	...
	/* Report the error if it's an error. */
	dory_report_error_setting_permissions (file, error, NULL);

	g_object_unref (window);
}

static void
update_permissions (DoryPropertiesWindow *window,
		    ...
		    gboolean use_original)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (!apply_to_both_folder_and_dir &&
		    ((dory_file_is_directory (file) && !is_folder) ||
		     (!dory_file_is_directory (file) && is_folder))) {
			continue;
		}

		permissions = dory_file_get_permissions (file);
		if (use_original) {
			gpointer ptr;
...
static gboolean
initial_permission_state_consistent (DoryPropertiesWindow *window,
				     guint32 mask,
				     gboolean is_folder,
...
static void
permission_button_update (DoryPropertiesWindow *window,
			  GtkToggleButton *button)
{
	GList *l;
	gboolean all_set;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		file_permissions = dory_file_get_permissions (file);
...
static void
set_up_permissions_checkbox (DoryPropertiesWindow *window,
...
static GtkWidget *
add_permissions_checkbox_with_label (DoryPropertiesWindow *window,
				     GtkGrid *grid,
				     GtkWidget *sibling,
				     const char *label,
...
static GtkWidget *
add_permissions_checkbox (DoryPropertiesWindow *window,
			  GtkGrid *grid,
			  GtkWidget *sibling,
			  CheckboxType type,
			  guint32 permission_to_check,
...
static void
permission_combo_changed (GtkWidget *combo, DoryPropertiesWindow *window)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static void
permission_combo_update (DoryPropertiesWindow *window,
			 GtkComboBox *combo)
{
	PermissionType type;
...
static void
add_permissions_combo_box (DoryPropertiesWindow *window, GtkGrid *grid,
...
static GtkWidget *
append_special_execution_checkbox (DoryPropertiesWindow *window,
				   GtkGrid *grid,
				   GtkWidget *sibling,
...
static void
append_special_execution_flags (DoryPropertiesWindow *window, GtkGrid *grid)
{
	GtkWidget *title;
...
static void
create_simple_permissions (DoryPropertiesWindow *window, GtkGrid *page_grid)
{
	...
	if (!is_multi_file_window (window) && dory_file_can_set_owner (get_target_file (window))) {
	...
	if (!is_multi_file_window (window) && dory_file_can_set_group (get_target_file (window))) {
...
static void
create_permission_checkboxes (DoryPropertiesWindow *window,
			      GtkGrid *page_grid,
			      gboolean is_folder)
{
...
static void
create_advanced_permissions (DoryPropertiesWindow *window, GtkGrid *page_grid)
{
	GtkLabel *group_label;
	GtkLabel *owner_label;
	GtkComboBox *group_combo_box;
			 ...
			 DoryPropertiesWindow *window)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (dory_file_is_directory (file) &&
			...
			start_long_operation (window);
			g_object_ref (window);
			dory_file_set_permissions_recursive (uri,
								 file_permission,
...
static void
create_permissions_page (DoryPropertiesWindow *window)
{
	...
	vbox = create_page_with_vbox (window->details->stack, "permissions",
				      _("Permissions"),
				      "help:gnome-help/dory-file-properties-permissions");
	...
	if (all_can_get_permissions (file_list) && all_can_get_permissions (window->details->target_files)) {
		...
		if (g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_ADVANCED_PERMISSIONS)) {
		...
		if (!is_multi_file_window (window)) {
			file_name = dory_file_get_display_name (get_target_file (window));
...
static void
append_extension_pages (DoryPropertiesWindow *window)
{
	GList *providers;
	GList *p;

 	providers = dory_module_get_extensions_for_type (DORY_TYPE_PROPERTY_PAGE_PROVIDER);

	for (p = providers; p != NULL; p = p->next) {
		DoryPropertyPageProvider *provider;
		GList *pages;
		GList *l;

		provider = DORY_PROPERTY_PAGE_PROVIDER (p->data);

		pages = dory_property_page_provider_get_pages
		...
		for (l = pages; l != NULL; l = l->next) {
			...
			page = DORY_PROPERTY_PAGE (l->data);
...
static gboolean
should_show_permissions (DoryPropertiesWindow *window)
{
	DoryFile *file;
...
get_pending_key (GList *file_list)
{
	...
	uris = NULL;
	for (l = file_list; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, dory_file_get_uri (DORY_FILE (l->data)));
	}
		  ...
		  const char *startup_id)
{
	...
	data->original_files = dory_file_list_copy (original_files);
	data->target_files = dory_file_list_copy (target_files);
...
{
	dory_file_list_free (data->original_files);
	dory_file_list_free (data->target_files);
...
static void
file_changed_callback (DoryFile *file, gpointer user_data)
{
	DoryPropertiesWindow *window = DORY_PROPERTIES_WINDOW (user_data);
...
static gboolean
is_a_special_file (DoryFile *file)
{
	if (file == NULL ||
...
static gboolean
should_show_open_with (DoryPropertiesWindow *window)
{
	DoryFile *file;
...
static void
create_open_with_page (DoryPropertiesWindow *window)
{
	GtkWidget *vbox;
	char *mime_type;
	char *uri = NULL;
	...
	mime_type = dory_file_get_mime_type (get_target_file (window));

	if (!is_multi_file_window (window)) {
		uri = dory_file_get_uri (get_target_file (window));
		if (uri == NULL) {
			return;
		}
	} else {
	...
	vbox = dory_mime_application_chooser_new (uri, uris, mime_type, NULL);
    g_free (uri);
...
static DoryPropertiesWindow *
create_properties_window (StartupData *startup_data)
{
	DoryPropertiesWindow *window;
	GList *l;

	window = DORY_PROPERTIES_WINDOW (gtk_widget_new (DORY_TYPE_PROPERTIES_WINDOW, NULL));

	window->details->original_files = dory_file_list_copy (startup_data->original_files);

	window->details->target_files = dory_file_list_copy (startup_data->target_files);

	gtk_window_set_wmclass (GTK_WINDOW (window), "file_properties", "Dory");
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		attributes = 0;
		if (dory_file_is_directory (file)) {
			attributes |= DORY_FILE_ATTRIBUTE_DEEP_COUNTS;
		}

		attributes |= DORY_FILE_ATTRIBUTE_INFO;
		dory_file_monitor_add (file, &window->details->target_files, attributes);
	}

	for (l = window->details->target_files; l != NULL; l = l->next) {
		g_signal_connect_object (DORY_FILE (l->data),
					 "changed",
					 G_CALLBACK (file_changed_callback),
					 G_OBJECT (window),
					 0);
...
get_target_file_list (GList *original_files)
{
	...
	for (l = original_files; l != NULL; l = l->next) {
		...
		target = get_target_file_for_original_file (DORY_FILE (l->data));
...
static void
add_window (DoryPropertiesWindow *window)
{
	if (!is_multi_file_window (window)) {
...
static void
remove_window (DoryPropertiesWindow *window)
{
	gpointer key;
...
cancel_call_when_ready_callback (gpointer key,
				 ...
				 gpointer user_data)
{
	dory_file_cancel_call_when_ready
		(DORY_FILE (key),
...
static void
is_directory_ready_callback (DoryFile *file,
			     gpointer data)
{
	...
	if (g_hash_table_size (startup_data->pending_files) == 0) {
		DoryPropertiesWindow *new_window;
...
void
dory_properties_window_present (GList       *original_files,
				    GtkWidget   *parent_widget,
				    const gchar *startup_id)
{
	...
	dory_file_list_free (target_files);
	g_free(pending_key);
...
{
	GError *error = NULL;
	DoryPropertiesWindow *window = DORY_PROPERTIES_WINDOW (dialog);
	GtkWidget *curpage;
	const char *helpuri;
...
real_destroy (GtkWidget *object)
{
	...
	window = DORY_PROPERTIES_WINDOW (object);

	remove_window (window);

	for (l = window->details->original_files; l != NULL; l = l->next) {
		dory_file_monitor_remove (DORY_FILE (l->data), &window->details->original_files);
	}
	dory_file_list_free (window->details->original_files);
	window->details->original_files = NULL;

	for (l = window->details->target_files; l != NULL; l = l->next) {
		dory_file_monitor_remove (DORY_FILE (l->data), &window->details->target_files);
	}
	dory_file_list_free (window->details->target_files);
	...
	GTK_WIDGET_CLASS (dory_properties_window_parent_class)->destroy (object);
}
...
real_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (dory_properties_window_parent_class)->finalize (object);
}
...
static void
set_icon (const char* icon_string, DoryPropertiesWindow *properties_window)
{
    DoryFile *file;
    char *file_uri;
    GList *l;
	...
	g_assert (DORY_IS_PROPERTIES_WINDOW (properties_window));
    ...
    if (g_path_is_absolute (icon_string)) {
		...
		for (l = properties_window->details->original_files; l != NULL; l = l->next) {
			...
			if (dory_file_is_mime_type (file, "application/x-desktop")) {
				if (dory_link_local_set_icon (file_uri, icon_path)) {
                ...
                dory_file_set_metadata (file, DORY_METADATA_KEY_CUSTOM_ICON_NAME, NULL, NULL);
				dory_file_set_metadata (file, DORY_METADATA_KEY_CUSTOM_ICON, NULL, real_icon_uri);
				dory_file_set_metadata (file, DORY_METADATA_KEY_ICON_SCALE, NULL, NULL);
        ...
        for (l = properties_window->details->original_files; l != NULL; l = l->next) {
            ...
            if (dory_file_is_mime_type (file, "application/x-desktop")) {
                if (dory_link_local_set_icon (file_uri, icon_string)) {
                    dory_file_invalidate_attributes (file,
                                         DORY_FILE_ATTRIBUTE_INFO |
                                         DORY_FILE_ATTRIBUTE_LINK_INFO);
                }
            } else {
                dory_file_set_metadata (file, DORY_METADATA_KEY_CUSTOM_ICON, NULL, NULL);
                dory_file_set_metadata (file, DORY_METADATA_KEY_CUSTOM_ICON_NAME, NULL, icon_string);
                dory_file_set_metadata (file, DORY_METADATA_KEY_ICON_SCALE, NULL, NULL);
            }
			      ...
			      DoryPropertiesWindow *window)
{
	...
	if (dialog == NULL) {
        ...
        for (l = window->details->original_files; l != NULL; l = l->next) {
            ...
            image_uri = dory_file_get_metadata (file, DORY_METADATA_KEY_CUSTOM_ICON, NULL);
            icon_name = dory_file_get_metadata (file, DORY_METADATA_KEY_CUSTOM_ICON_NAME, NULL);
...
static void
dory_properties_window_class_init (DoryPropertiesWindowClass *class)
{
	...
	g_type_class_add_private (class, sizeof (DoryPropertiesWindowDetails));
}

static void
dory_properties_window_init (DoryPropertiesWindow *window)
{
	window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, DORY_TYPE_PROPERTIES_WINDOW,
						       DoryPropertiesWindowDetails);
}
