...
G_DEFINE_TYPE (DoryToolbar, dory_toolbar, GTK_TYPE_BOX);

static void
dory_toolbar_update_root_state (DoryToolbar *self)
{
    ...
    if ((is_admin_uri ||
         (dory_user_is_root () && !dory_treating_root_as_normal())) &&
         g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_ROOT_WARNING)) {
...
static void
toolbar_update_appearance (DoryToolbar *self)
{
	GtkWidget *widgetitem;
	gboolean icon_toolbar;
	gboolean show_location_entry;

    dory_toolbar_update_root_state (self);
    ...
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_PREVIOUS_ICON_TOOLBAR);
    ...
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_NEXT_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->up_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_UP_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->refresh_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_RELOAD_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->home_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_HOME_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->computer_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_COMPUTER_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->search_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_SEARCH_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->new_folder_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_NEW_FOLDER_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->open_terminal_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_OPEN_IN_TERMINAL_TOOLBAR);
    if (icon_toolbar == FALSE ) {gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->toggle_location_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_EDIT_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->icon_view_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_ICON_VIEW_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->list_view_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_LIST_VIEW_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->compact_view_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_COMPACT_VIEW_ICON_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->show_thumbnails_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_SHOW_THUMBNAILS_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}

    widgetitem = self->priv->show_extra_pane_button;
    icon_toolbar = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_TOGGLE_EXTRA_PANE_TOOLBAR);
    if ( icon_toolbar == FALSE ) { gtk_widget_hide (widgetitem); }
    else {gtk_widget_show (GTK_WIDGET(widgetitem));}
}

static void
setup_root_info_bar (DoryToolbar *self) {
...
static GtkWidget *
toolbar_create_toolbutton (DoryToolbar *self,
...
static void
dory_toolbar_constructed (GObject *obj)
{
	DoryToolbar *self = DORY_TOOLBAR (obj);
	GtkWidget *toolbar;
    GtkWidget *hbox;
	...
	G_OBJECT_CLASS (dory_toolbar_parent_class)->constructed (obj);

	gtk_style_context_set_junction_sides (gtk_widget_get_style_context (GTK_WIDGET (self)),
					      GTK_JUNCTION_BOTTOM);

    self->priv->show_location_entry = g_settings_get_boolean (dory_preferences, DORY_PREFERENCES_SHOW_LOCATION_ENTRY);
    ...
    self->priv->previous_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_BACK);
    gtk_container_add (GTK_CONTAINER (box), self->priv->previous_button);

    self->priv->next_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_FORWARD);
    gtk_container_add (GTK_CONTAINER (box), self->priv->next_button);

    self->priv->up_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_UP);
    gtk_container_add (GTK_CONTAINER (box), self->priv->up_button);

    self->priv->refresh_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_RELOAD);
    gtk_container_add (GTK_CONTAINER (box), self->priv->refresh_button);

    self->priv->home_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_HOME);
    gtk_container_add (GTK_CONTAINER (box), self->priv->home_button);

    self->priv->computer_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_COMPUTER);
    ...
    self->priv->path_bar = g_object_new (DORY_TYPE_PATH_BAR, NULL);
    gtk_stack_add_named(GTK_STACK (self->priv->stack), GTK_WIDGET (self->priv->path_bar), "path_bar");

    /* Entry-Like Location Bar */
    self->priv->location_bar = dory_location_bar_new ();
    ...
    self->priv->toggle_location_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_TOGGLE_LOCATION);
    gtk_container_add (GTK_CONTAINER (box), self->priv->toggle_location_button);

    self->priv->open_terminal_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_OPEN_IN_TERMINAL);
    gtk_container_add (GTK_CONTAINER (box), self->priv->open_terminal_button);

    self->priv->new_folder_button = toolbar_create_toolbutton (self, FALSE, DORY_ACTION_NEW_FOLDER);
    gtk_container_add (GTK_CONTAINER (box), self->priv->new_folder_button);

    self->priv->search_button = toolbar_create_toolbutton (self, TRUE, DORY_ACTION_SEARCH);
    gtk_container_add (GTK_CONTAINER (box), self->priv->search_button);

    self->priv->show_thumbnails_button = toolbar_create_toolbutton (self, TRUE, DORY_ACTION_SHOW_THUMBNAILS);
    gtk_container_add (GTK_CONTAINER (box), self->priv->show_thumbnails_button);

    setup_root_info_bar (self);

    self->priv->show_extra_pane_button = toolbar_create_toolbutton (self, TRUE, DORY_ACTION_SHOW_HIDE_EXTRA_PANE);
    gtk_container_add (GTK_CONTAINER (box), self->priv->show_extra_pane_button);

    self->priv->icon_view_button = toolbar_create_toolbutton (self, TRUE, DORY_ACTION_ICON_VIEW);
    gtk_container_add (GTK_CONTAINER (box), self->priv->icon_view_button);

    self->priv->list_view_button = toolbar_create_toolbutton (self, TRUE, DORY_ACTION_LIST_VIEW);
    gtk_container_add (GTK_CONTAINER (box), self->priv->list_view_button);

    self->priv->compact_view_button = toolbar_create_toolbutton (self, TRUE, DORY_ACTION_COMPACT_VIEW);
...
static void
dory_toolbar_init (DoryToolbar *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, DORY_TYPE_TOOLBAR,
						  DoryToolbarPriv);
	self->priv->show_main_bar = TRUE;	
}

static void
dory_toolbar_get_property (GObject *object,
			       guint property_id,
			       GValue *value,
			       GParamSpec *pspec)
{
	DoryToolbar *self = DORY_TOOLBAR (object);
...
static void
dory_toolbar_set_property (GObject *object,
			       guint property_id,
			       const GValue *value,
			       GParamSpec *pspec)
{
	...
	case PROP_ACTION_GROUP:
		...
		break;
	case PROP_SHOW_LOCATION_ENTRY:
		dory_toolbar_set_show_location_entry (self, g_value_get_boolean (value));
		break;
	case PROP_SHOW_MAIN_BAR:
		dory_toolbar_set_show_main_bar (self, g_value_get_boolean (value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
		break;
...
static void
dory_toolbar_dispose (GObject *obj)
{
	DoryToolbar *self = DORY_TOOLBAR (obj);
	...
	G_OBJECT_CLASS (dory_toolbar_parent_class)->dispose (obj);
}

static void
dory_toolbar_class_init (DoryToolbarClass *klass)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (klass);
	oclass->get_property = dory_toolbar_get_property;
	oclass->set_property = dory_toolbar_set_property;
	oclass->constructed = dory_toolbar_constructed;
	oclass->dispose = dory_toolbar_dispose;
	...
	g_type_class_add_private (klass, sizeof (DoryToolbarClass));
	g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
}

GtkWidget *
dory_toolbar_new (GtkActionGroup *action_group)
{
	return g_object_new (DORY_TYPE_TOOLBAR,
...
GtkWidget *
dory_toolbar_get_path_bar (DoryToolbar *self)
{
	return self->priv->path_bar;
}

GtkWidget *
dory_toolbar_get_location_bar (DoryToolbar *self)
{
	return self->priv->location_bar;
}

gboolean
dory_toolbar_get_show_location_entry (DoryToolbar *self)
{
	return self->priv->show_location_entry;
}

void
dory_toolbar_set_show_main_bar (DoryToolbar *self,
				    gboolean show_main_bar)
{
...
void
dory_toolbar_set_show_location_entry (DoryToolbar *self,
					  gboolean show_location_entry)
{
...
void
dory_toolbar_update_for_location (DoryToolbar *self)
{
    toolbar_update_appearance (self);
}