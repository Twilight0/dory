...
G_DEFINE_TYPE (DoryFilterBar, dory_filter_bar, GTK_TYPE_BOX)

static void
clear_button_clicked_cb (GtkButton *button, DoryFilterBar *bar)
{
    g_signal_emit (bar, signals[CANCEL], 0);
}

static void
dory_filter_bar_init (DoryFilterBar *bar)
{
...
static void
dory_filter_bar_class_init (DoryFilterBarClass *klass)
{
...
GtkWidget *
dory_filter_bar_new (void)
{
    return g_object_new (DORY_TYPE_FILTER_BAR, NULL);
}

const char *
dory_filter_bar_get_text (DoryFilterBar *bar)
{
    g_return_val_if_fail (DORY_IS_FILTER_BAR (bar), NULL);
    return gtk_entry_get_text (GTK_ENTRY (bar->entry));
}

void
dory_filter_bar_set_text (DoryFilterBar *bar, const char *text)
{
    g_return_if_fail (DORY_IS_FILTER_BAR (bar));
