...
struct DoryBlankDesktopWindowDetails {
    gint monitor;
    GtkWidget *event_box;
...
G_DEFINE_TYPE (DoryBlankDesktopWindow, dory_blank_desktop_window, 
               GTK_TYPE_WINDOW);

static void update_actions_visibility (DoryBlankDesktopWindow *window);
static void update_actions_menu (DoryBlankDesktopWindow *window);
static void actions_changed (gpointer user_data);

static void
action_show_overlay (GtkAction *action, gpointer user_data)
{
    g_return_if_fail (DORY_IS_BLANK_DESKTOP_WINDOW (user_data));
    DoryBlankDesktopWindow *window = DORY_BLANK_DESKTOP_WINDOW (user_data);
...
static void
reset_popup_menu (DoryBlankDesktopWindow *window)
{
...
static void
do_popup_menu (DoryBlankDesktopWindow *window, GdkEventButton *event)
{
...
static gboolean
on_popup_menu (GtkWidget *widget, DoryBlankDesktopWindow *window)
{
...
static gboolean
on_button_press (GtkWidget *widget, GdkEventButton *event, DoryBlankDesktopWindow *window)
{
...
static void
run_action_callback (DoryAction *action, gpointer user_data)
{
    dory_action_activate (action, NULL, NULL, GTK_WINDOW (user_data));
}

static void
update_actions_visibility (DoryBlankDesktopWindow *window)
{
    dory_action_manager_update_action_states (window->details->action_manager,
...
static void
clear_ui (DoryBlankDesktopWindow *window)
{
...
static void
update_actions_menu (DoryBlankDesktopWindow *window)
{
    ...
    window->details->action_group = gtk_action_group_new ("DoryBlankDesktopActions");
    ...
    window->details->actions_merge_id =
            gtk_ui_manager_add_ui_from_resource (window->details->ui_manager, "/org/dory/dory-blank-desktop-window-ui.xml", NULL);

    dory_action_manager_iterate_actions (window->details->action_manager,
...
static void
dory_blank_desktop_window_dispose (GObject *obj)
{
    DoryBlankDesktopWindow *window = DORY_BLANK_DESKTOP_WINDOW (obj);
    ...
    g_signal_handlers_disconnect_by_func (dory_menu_config_preferences, reset_popup_menu, window);

    G_OBJECT_CLASS (dory_blank_desktop_window_parent_class)->dispose (obj);
}

static void
dory_blank_desktop_window_finalize (GObject *obj)
{
    DoryBlankDesktopWindow *window = DORY_BLANK_DESKTOP_WINDOW (obj);

    g_object_unref (window->details->ui_manager);

    G_OBJECT_CLASS (dory_blank_desktop_window_parent_class)->finalize (obj);
}
...
{
    DoryBlankDesktopWindow *window;

    window = DORY_BLANK_DESKTOP_WINDOW (widget);

    GTK_WIDGET_CLASS (dory_blank_desktop_window_parent_class)->show (widget);
    gtk_ui_manager_ensure_update (window->details->ui_manager);
    actions_changed (window);
}

static void
dory_blank_desktop_window_constructed (GObject *obj)
{
	AtkObject *accessible;
    DoryApplication *application;
	DoryBlankDesktopWindow *window = DORY_BLANK_DESKTOP_WINDOW (obj);

	G_OBJECT_CLASS (dory_blank_desktop_window_parent_class)->constructed (obj);
    ...
    window->details->action_manager = dory_action_manager_new ();
    ...
    dory_blank_desktop_window_update_geometry (window);
...
static void
dory_blank_desktop_window_get_property (GObject *object,
                                           guint property_id,
                                         GValue *value,
                                     GParamSpec *pspec)
{
    DoryBlankDesktopWindow *window = DORY_BLANK_DESKTOP_WINDOW (object);
...
static void
dory_blank_desktop_window_set_property (GObject *object,
                                           guint property_id,
                                   const GValue *value,
                                     GParamSpec *pspec)
{
...
static void
dory_blank_desktop_window_init (DoryBlankDesktopWindow *window)
{
	window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, DORY_TYPE_BLANK_DESKTOP_WINDOW,
						       DoryBlankDesktopWindowDetails);
    ...
    gtk_style_context_add_class (gtk_widget_get_style_context (GTK_WIDGET (window)), "dory-desktop-window");
}

DoryBlankDesktopWindow *
dory_blank_desktop_window_new (gint monitor)
{
	DoryBlankDesktopWindow *window;

	window = g_object_new (DORY_TYPE_BLANK_DESKTOP_WINDOW,
                           "monitor", monitor,
                           NULL);
...
static gboolean
dory_blank_desktop_window_delete_event (GtkWidget *widget,
                                        GdkEventAny *event)
{
...
map (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (dory_blank_desktop_window_parent_class)->map (widget);
...
{
	GTK_WIDGET_CLASS (dory_blank_desktop_window_parent_class)->unrealize (widget);
}
...
realize (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (dory_blank_desktop_window_parent_class)->realize (widget);
}

static void
dory_blank_desktop_window_class_init (DoryBlankDesktopWindowClass *klass)
{
	GtkWidgetClass *wclass = GTK_WIDGET_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->constructed = dory_blank_desktop_window_constructed;
    oclass->dispose = dory_blank_desktop_window_dispose;
	oclass->finalize = dory_blank_desktop_window_finalize;
    oclass->set_property = dory_blank_desktop_window_set_property;
    oclass->get_property = dory_blank_desktop_window_get_property;

	wclass->realize = realize;
	wclass->unrealize = unrealize;
	wclass->map = map;
    wclass->show = show;
	wclass->delete_event = dory_blank_desktop_window_delete_event;
	...
	g_type_class_add_private (klass, sizeof (DoryBlankDesktopWindowDetails));

    g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
}

void
dory_blank_desktop_window_update_geometry (DoryBlankDesktopWindow *window)
{
    GdkRectangle rect;

    if (dory_desktop_utils_configure_layer_shell (GTK_WINDOW (window),
                                                    window->details->monitor,
                                                    FALSE)) {
        DEBUG ("DoryBlankDesktopWindow: using layer-shell for monitor %d",
               window->details->monitor);
        return;
    }

    dory_desktop_manager_get_window_rect_for_monitor (dory_desktop_manager_get (),
                                                      window->details->monitor,
                                                      &rect);

    DEBUG ("DoryBlankDesktopWindow monitor:%d: x:%d, y:%d, w:%d, h:%d",
           window->details->monitor,
           rect.x, rect.y,
