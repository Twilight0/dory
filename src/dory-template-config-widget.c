...
G_DEFINE_TYPE (DoryTemplateConfigWidget, dory_template_config_widget, DORY_TYPE_CONFIG_BASE_WIDGET);

typedef struct {
    DoryTemplateConfigWidget *widget;
...
static void refresh_widget (DoryTemplateConfigWidget *widget);
static void start_renaming_selected_row (DoryTemplateConfigWidget *widget, GtkWidget *row);
...
static void
cancel_file_monitors (DoryTemplateConfigWidget *widget)
{
    GList *l;
...
on_dir_changed (GFileMonitor     *monitor,
                ...
                gpointer          user_data)
{
    DoryTemplateConfigWidget *widget = DORY_TEMPLATE_CONFIG_WIDGET (user_data);

    refresh_widget (widget);
}

static void
monitor_path (DoryTemplateConfigWidget *widget, const gchar *path)
{
...
static void
populate_from_directory (DoryTemplateConfigWidget *widget, const gchar *path)
{
    GDir *dir;
...
static void
refresh_widget (DoryTemplateConfigWidget *widget)
{
    ...
    dory_config_base_widget_clear_list (DORY_CONFIG_BASE_WIDGET (widget));
    ...
    if (widget->templates == NULL) {
        ...
        gtk_container_add (GTK_CONTAINER (DORY_CONFIG_BASE_WIDGET (widget)->listbox), empty_row);
        ...
        for (l = widget->templates; l != NULL; l=l->next) {
            ...
            gtk_container_add (GTK_CONTAINER (DORY_CONFIG_BASE_WIDGET (widget)->listbox), row);
        }

        gtk_widget_set_sensitive (GTK_WIDGET (DORY_CONFIG_BASE_WIDGET (widget)->listbox), TRUE);
    }

    dory_config_base_widget_set_default_buttons_sensitive (DORY_CONFIG_BASE_WIDGET (widget), widget->templates != NULL);
}
...
{
    DoryTemplateConfigWidget *widget = DORY_TEMPLATE_CONFIG_WIDGET (user_data);
    GtkListBoxRow *row = gtk_list_box_get_selected_row (GTK_LIST_BOX (DORY_CONFIG_BASE_WIDGET (widget)->listbox));
...
static void
start_renaming_selected_row (DoryTemplateConfigWidget *widget, GtkWidget *row)
{
    TemplateInfo *info = g_object_get_data (G_OBJECT (row), "template-info");
...
static void
import_template (DoryTemplateConfigWidget *widget,
                 const gchar              *filename)
{
    ...
    template_dir = dory_get_templates_directory ();
...
{
    DoryTemplateConfigWidget *widget = DORY_TEMPLATE_CONFIG_WIDGET (user_data);
    GtkWidget *row = GTK_WIDGET (gtk_list_box_get_selected_row (GTK_LIST_BOX (DORY_CONFIG_BASE_WIDGET (widget)->listbox)));
...
static void
on_open_folder_clicked (GtkWidget *button, DoryTemplateConfigWidget *widget)
{
    gchar *path = NULL;
    dory_ensure_valid_templates_directory ();
    path = dory_get_templates_directory ();
    ...
    dory_application_open_location (dory_application_get_singleton (),
                                    location,
                                    NULL,
                                    "dory",
                                    FALSE);
...
on_drag_data_received(GtkWidget *widget,
                      ...
                      gpointer user_data)
{
    DoryTemplateConfigWidget *self = DORY_TEMPLATE_CONFIG_WIDGET (user_data);
...
static void
dory_template_config_widget_finalize (GObject *object)
{
    DoryTemplateConfigWidget *widget = DORY_TEMPLATE_CONFIG_WIDGET (object);
    ...
    G_OBJECT_CLASS (dory_template_config_widget_parent_class)->finalize (object);
}

static void
dory_template_config_widget_class_init (DoryTemplateConfigWidgetClass *klass)
{
    GObjectClass *oclass;
    oclass = G_OBJECT_CLASS (klass);

    oclass->finalize = dory_template_config_widget_finalize;
}

static void
dory_template_config_widget_init (DoryTemplateConfigWidget *self)
{
    self->templates = NULL;

    GtkWidget *label = dory_config_base_widget_get_label (DORY_CONFIG_BASE_WIDGET (self));
    gtk_list_box_set_selection_mode (GTK_LIST_BOX (DORY_CONFIG_BASE_WIDGET (self)->listbox), GTK_SELECTION_SINGLE);
    gtk_list_box_set_activate_on_single_click (GTK_LIST_BOX (DORY_CONFIG_BASE_WIDGET (self)->listbox), FALSE);
    ...
    GtkWidget *bb = DORY_CONFIG_BASE_WIDGET (self)->rbuttonbox;
    ...
    bb = DORY_CONFIG_BASE_WIDGET (self)->lbuttonbox;
    ...
    gtk_widget_hide (dory_config_base_widget_get_enable_button (DORY_CONFIG_BASE_WIDGET (self)));
    gtk_widget_hide (dory_config_base_widget_get_disable_button (DORY_CONFIG_BASE_WIDGET (self)));

    g_signal_connect (DORY_CONFIG_BASE_WIDGET (self)->listbox, "row-selected", G_CALLBACK (on_row_selected), self);
    g_signal_connect (DORY_CONFIG_BASE_WIDGET (self)->listbox, "row-activated", G_CALLBACK (on_row_activated), self);

    gtk_drag_dest_set(DORY_CONFIG_BASE_WIDGET (self)->listbox,
                      GTK_DEST_DEFAULT_ALL,
                      drop_targets,
                      G_N_ELEMENTS(drop_targets),
                      GDK_ACTION_COPY);

    g_signal_connect(DORY_CONFIG_BASE_WIDGET (self)->listbox, "drag-data-received",
                    G_CALLBACK(on_drag_data_received), self);
    g_signal_connect(DORY_CONFIG_BASE_WIDGET (self)->listbox, "drag-motion",
                    G_CALLBACK(on_drag_motion), NULL);

    refresh_widget (self);
    on_row_selected (DORY_CONFIG_BASE_WIDGET (self)->listbox, NULL, self);
}

GtkWidget *
dory_template_config_widget_new (void)
{
  return g_object_new (DORY_TYPE_TEMPLATE_CONFIG_WIDGET, NULL);
}
