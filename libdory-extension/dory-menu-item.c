/*
 *  dory-menu-item.c - Menu items exported by DoryMenuProvider
 *                         objects.
 *
...
struct _DoryMenuItem
{
    GObject parent_class;

    DoryMenuItemPrivate *details;
};

G_DEFINE_TYPE_WITH_PRIVATE (DoryMenuItem, dory_menu_item, G_TYPE_OBJECT)
 ...
 *
 * Returns: a newly create #DoryMenuItem
 */
DoryMenuItem *
dory_menu_item_new (const char *name,
			const char *label,
			const char *tip,
			const char *icon)
{
	...
	item = g_object_new (DORY_TYPE_MENU_ITEM, 
			     "name", name,
			     "label", label,
			     "tip", tip,
			     "icon", icon,
 ...
 *
 * Returns: a newly created #DoryMenuItem that will pass widgets to an eventual DoryWidgetAction
 */
DoryMenuItem *
dory_menu_item_new_widget (const char *name,
                           GtkWidget  *widget_a,
                           GtkWidget  *widget_b)
{
    DoryMenuItem *item;
...
/**
 * dory_menu_item_new_separator:
 * @name: the identifier for the menu item
 *
 * Creates a new menu item that represents a menu separator.
 * 
 * Returns: a newly created #DoryMenuItem
 */
DoryMenuItem *
dory_menu_item_new_separator (const char *name)
{
    DoryMenuItem *item;
...
/**
 * dory_menu_item_activate:
 * @item: pointer to a #DoryMenuItem
 *
 * emits the activate signal.
 */
void
dory_menu_item_activate (DoryMenuItem *item)
{
	g_signal_emit (item, signals[ACTIVATE], 0);
}

/**
 * dory_menu_item_set_submenu:
 * @item: pointer to a #DoryMenuItem
 * @menu: pointer to a #DoryMenu to attach to the button
 *
 * Attachs a menu to the given #DoryMenuItem.
 */
void
dory_menu_item_set_submenu (DoryMenuItem *item, DoryMenu *menu)
{
	g_object_set (item, "menu", menu, NULL);
}

/**
 * dory_menu_item_set_widget_a:
 * @item: pointer to a #DoryMenuItem
 * @widget: pointer to a #GtkWidget use in place of text
 *
 * Sets the custom widget A for this #DoryMenuItem
 */
void
dory_menu_item_set_widget_a (DoryMenuItem *item, GtkWidget *widget)
{
...
/**
 * dory_menu_item_set_widget_b:
 * @item: pointer to a #DoryMenuItem
 * @widget: pointer to a #GtkWidget use in place of text
 *
 * Sets the custom widget B for this #DoryMenuItem
 */
void
dory_menu_item_set_widget_b (DoryMenuItem *item, GtkWidget *widget)
{
...
static void
dory_menu_item_get_property (GObject *object,
				 guint param_id,
				 GValue *value,
				 GParamSpec *pspec)
{
...
static void
dory_menu_item_set_property (GObject *object,
				 guint param_id,
				 const GValue *value,
				 GParamSpec *pspec)
{
...
static void
dory_menu_item_finalize (GObject *object)
{
	DoryMenuItem *item;
	...
	G_OBJECT_CLASS (dory_menu_item_parent_class)->finalize (object);
}

static void
dory_menu_item_init (DoryMenuItem *item)
{
    item->details = G_TYPE_INSTANCE_GET_PRIVATE (item, DORY_TYPE_MENU_ITEM, DoryMenuItemPrivate);
...
static void
dory_menu_item_class_init (DoryMenuItemClass *class)
{
	G_OBJECT_CLASS (class)->finalize = dory_menu_item_finalize;
	G_OBJECT_CLASS (class)->get_property = dory_menu_item_get_property;
	G_OBJECT_CLASS (class)->set_property = dory_menu_item_set_property;
